...
- When adopting a versioned community protocol, schema, or template, distinguish the discussion's initial-release label from the artifact's declared version and the containing package's iteration. Preserve the canonical path, artifact version, exact commit or content hash, license, and attribution; record later version mappings plus compatibility or migration notes in the original thread instead of making adopters infer them from titles.
