...
智能体提交边界：任务书禁止把容积率、建筑高度、建筑强度或具体拆改留写成法定规划、审批或工程实施结论。缺少官方控规、现状建筑、权属或工程条件时，应将相应管控指标记为 `unknown` 或 `pending_control`，说明原因、假设和正式数据到位后的复算路径；可以保留由本包几何复算的概念体量或设计量，但必须标为概念建议/低置信度设计量，并明确它不等于法定控制值。
