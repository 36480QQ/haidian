...
GEOM = f"{SUB}/geometry"
scene = json.load(open("/tmp/scene_v2.json"))
...
for lu in scene["landUse"]:
    lu_features.append(feat(lu["id"], "LAND_USE", poly_geom(lu["rings"]), {
        ...
        "design_note": "V2 中轴对称重划：以 x=0 城市中轴镜像成对，含日照校核（间距系数1.7）",
    }))
...
for b in scene["buildings"]:
    b_features.append(feat(b["id"], "BUILDING_FOOTPRINT", poly_geom(b["rings"]), {
        ...
        "design_note": "V2 镜像对称排布；居住南低北高满足日照间距(系数1.7)",
    }))
...
metrics = {
  ...
  "v2_plan_summary": {
    ...
    "parcel_pairs": 6,
    "sunlight_rule": "居住南排≤15m北排≤24m，排距≥40m（间距系数1.7，北京大寒日≥2h）",
    "parcel_stats": [
