...
def build_prompt(review_input: dict) -> str:
    return "\n".join(
        [
            ...
            "Participant-collected sources are not required to appear in the maintainer-owned central source_registry. Absence from the central registry is neither approval nor prohibition and must not by itself reduce risk_compliance; judge participant sources using the package's own sources.json.",
            "Organizer-owned missing geometry or official data is a data gap, not a participant repair. Record it in data_gaps_zh only; required_next_actions_zh must contain participant-controlled repairs and must not block featured-candidate solely because organizer data still needs recalculation.",
