...
def fig_site_overview():
    ...
    # ── Satellite underlay DISABLED (copyright compliance: no Esri/Maxar imagery) ──
    sat_img, sat_extent = None, None
    if False:  # sat_img is not None:
    ...
    # PROVISIONAL banner is added by _add_banners.py, no need to draw in figure

    fig.savefig(FIG_DIR / "site-overview.png", dpi=90,
...
def fig_land_use():
    ...
    nx, ny, nw, nh = 0.645, 0.028, 0.35, 0.205
    ...
    # description area spans from x=nx+0.155 to panel right edge (nx+nw)
    desc_x0 = nx + 0.155
    desc_x1 = nx + nw - 0.01
    desc_w = desc_x1 - desc_x0
    for i, (label, desc) in enumerate(notes):
        ny_i = ny + nh - 0.045 - i * 0.030
        ...
        # wrap desc to panel width
        wrapped = _tw.wrap(desc, width=22) or [desc]
        for j, line in enumerate(wrapped):
            ax.text(desc_x0, ny_i + (0.011 if j == 0 else -0.012) - j*0.013, line,
                    fontsize=7.2, color="#44403c", ha="left", va="center", transform=T, zorder=25)

    # Disclaimer banner is added by _add_banners.py, no need to draw in figure
...
def fig_key_areas():
    ...
    for idx, (kid, ax) in enumerate(zip(["KEY-001", "KEY-002", "KEY-003"], axes)):
        ...
        # ── Satellite base map DISABLED (copyright compliance) ──
        half_lon = 0.0125
        half_lat = 0.010
        if False:
        ...
        # ── 概念干预类型示意 (CONCEPTUAL INTERVENTION TYPE ONLY) ──
        # 注意：本图为概念方案，不依据测绘/权属/控规做真实宗地级
        # 保留/改造/新建判定。以下色块仅为"概念干预类型示意"，
        # 实际分类须待测绘、权属核查、控规确认后方可确定。
        # 移除原确定性拆改留分区，仅以统一淡色示意"概念更新片区"，
        # 不标注具体保留/改造/新建结论。
        concept_zone = box(rail_lon - 0.0018, miny + h*0.10,
                           rail_lon + 0.0018, maxy - h*0.10)
        czc = concept_zone.intersection(k)
        if not czc.is_empty:
            plot_polygon(ax, czc, facecolor="#a8a29e", edgecolor="none",
                        alpha=0.12, zorder=7)
        ...
        # ── 概念干预类型示意标注 (NO deterministic 保留/改造/新建 conclusion) ──
        # 统一以"概念干预类型示意"说明，不标注具体宗地级保留/改造/新建结论。
        concept_note_pos = {
            "KEY-001": (minx + w*0.06, miny + h*0.06, "left", "bottom"),
            "KEY-002": (minx + w*0.06, miny + h*0.06, "left", "bottom"),
            "KEY-003": (maxx - w*0.06, miny + h*0.06, "right", "bottom"),
        }
        cnp = concept_note_pos[kid]
        ax.text(cnp[0], cnp[1], "概念干预类型示意",
                fontsize=6.5, color="#57534e", ha=cnp[2], va=cnp[3],
                zorder=15, fontweight="bold", alpha=0.85,
                bbox=dict(boxstyle="round,pad=0.12", facecolor=BG_DARK,
                          edgecolor="#94a3b8", alpha=0.75, linewidth=0.5))
        ...
        # ── Renewal data annotation (concept-level only, no specific numbers) ──
        if kid == "KEY-002":
            data_text = "更新策略：保留·改造·新建（具体规模待可研）"
            ...
            data_text = "更新方向：国际交流中心（概念建议，待可研）"
            ax.text(cx_k, miny - h*0.06, data_text, fontsize=5.8, color="#57534e",
                    ha="center", va="top", zorder=20,
                    bbox=dict(boxstyle="round,pad=0.2", facecolor=BG_DARK,
                              edgecolor=MUTED, alpha=0.8, linewidth=0.5))
        elif kid == "KEY-001":
            data_text = "现状基础：已建成园区·科研机构入驻（概念示意）"
            ax.text(cx_k, miny - h*0.06, data_text, fontsize=5.8, color="#57534e",
                    ha="center", va="top", zorder=20,
                    bbox=dict(boxstyle="round,pad=0.2", facecolor=BG_DARK,
                              edgecolor=MUTED, alpha=0.8, linewidth=0.5))
    ...
    legend_elements = [
        ...
        mpatches.Patch(facecolor="#94a3b8", alpha=0.40, edgecolor="#64748b",
                       linewidth=0.5, label="保留（概念干预示意·非现状建筑判定）"),
        mpatches.Patch(facecolor="#ea580c", alpha=0.28, edgecolor="#c2410c",
                       linewidth=0.5, label="改造更新（概念干预示意·非现状判定）"),
        mpatches.Patch(facecolor="#dc2626", alpha=0.22, edgecolor="#dc2626",
                       linewidth=0.7, linestyle="--", label="拆除新建（概念干预示意·非现状判定）"),
        # 空间结构
    ...
    fig.savefig(out, dpi=100, bbox_inches="tight", facecolor=BG_DARK)
    plt.close(fig)
    print("  key-areas.png")
...
def fig_mobility():
    ...
    ax.text(0.025, 0.888, "约9 km 铁路绿廊 · 15 处 AI 场景节点",
    ...
    ax.add_patch(FancyBboxPatch((nx, ny), nw, nh,
        boxstyle="round,pad=0.008", facecolor="#ffffff", edgecolor="#d6d3d1",
        linewidth=0.8, alpha=0.95, transform=T, zorder=28))
    ax.text(nx + 0.012, ny + nh - 0.015, "AI 场景节点（15 处：4 处测试验证 + 11 处概念建议）",
            fontsize=10, fontweight="bold", color="#1c1917",
            ha="left", va="top", transform=T, zorder=29)
    TEST_IDS = {"SC-03", "SC-04", "SC-08", "SC-09"}
    # Two columns
    col1 = scenarios[:8]
    col2 = scenarios[8:]
    for i, (slon, slat, sid, sname, color, lm, ldy) in enumerate(col1):
        ...
        ax.plot(nx + 0.018, sy_i, "o", markersize=5, color=color,
                transform=T, zorder=29)
        star = " ★" if lm else ""
        test = "〔测试〕" if sid in TEST_IDS else ""
        ax.text(nx + 0.030, sy_i, f"{sid} {sname}{test}{star}", fontsize=7.5,
    ...
    for i, (slon, slat, sid, sname, color, lm, ldy) in enumerate(col2):
        ...
        star = " ★" if lm else ""
        test = "〔测试〕" if sid in TEST_IDS else ""
        ax.text(nx + 0.187, sy_i, f"{sid} {sname}{test}{star}", fontsize=7.5,
                color="#374151", ha="left", va="center", transform=T, zorder=29)

    # ── Disclaimer is added by _add_banners.py (top PROVISIONAL + bottom OSM),
    #    so we do NOT draw an in-figure bottom bar here (avoids duplicate banners). ──
...
def fig_metrics():
    ...
    # ── 提交者概念自评 显著标注 ──
    fig.text(0.5, 0.905, "★ 提交者概念自评 · SUBMITTER CONCEPT SELF-EVALUATION（非第三方专业评审）",
    ...
    # ── Footnote ──
    fig.text(0.5, 0.018,
             "注：本图为【提交者概念自评】，非第三方专业评审。评分为概念性对照（1-5分），基于公开资料整理；"
             "评分规则：1=基础缺失 2=有概念无落地 3=有策略待验证 4=有落地路径和KPI 5=有实施机制和验证框架；"
             "全球案例数据来自公开报道，可能存在统计口径差异，逐项评分规则与证据附表见 proposal.md 正文。"
             "空间指标依据 GeoJSON 在 EPSG:4548 投影下复算。所有空间建议均为概念方案，不构成政府审定结论。",
...
def fig_section():
    ...
    ax.text(22, 3.8, "中低层研发街区 · 191.9ha", ha="center", va="top",
    ...
    ax.text(49, 6.0, "低多层混合+概念地标 · 104.3ha", ha="center", va="top",
            fontsize=6.5, color="#78716c", zorder=11)
    ...
    ax.text(76, 4.0, "中高层商务集群 · 72.4ha", ha="center", va="top",
            fontsize=6.5, color="#78716c", zorder=11)
    ...
    fig.savefig(FIG_DIR / "concept-section.png", dpi=140, bbox_inches="tight",
                facecolor=BG_DARK)
    plt.close(fig)
...
def fig_site_readiness():
    ...
    fig.savefig(FIG_DIR / "site-readiness.png", dpi=120, bbox_inches="tight",
                facecolor=BG_DARK)
    plt.close(fig)
