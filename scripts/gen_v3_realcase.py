...
# ---- 3.3 居住地块：板式住宅南低北高均匀分布（案例：万柳书院/使馆壹号院/融创北京壹号院）+ 教育 + 底商 ----
# 排布逻辑：北京在北半球，阳光自南射入——南排矮、北排高，保证每排不被前排遮挡
# 高度带 30-45m（10-15层）＝北京高端住宅主流高度（万柳书院7-15层、融创北京壹号院12-18层、使馆壹号院12-18层）
# 6 排错落 zigzag + 楼栋长度不齐（45/55/65m）+ 山墙距24-26m（高端社区大间距）+ 点式塔楼散布
def gen_residential(pid, x0, x1, y0, y1, tag):
    ...
    # 6 排南低北高均匀分布整个地块：高度从南到北 30→45m 每排+3m（利于日照）
    # 排距 260m ≈ 5.8×最高楼高，远超北京板楼日照间距系数 1.5-1.7
    rows = [
        # (y 位置比例, 起点偏移, 楼栋长度序列, 高度/层数, 颜色, 案例名)  ← y 增=向北，高度递增=南低北高
        (0.06,  30, [45,55,45,65], 30, 10, "#C9952F", "万柳书院(10层/30m)"),
        (0.22,  85, [55,45,65,45], 33, 11, "#D9A23F", "使馆壹号院(11层/33m)"),
        (0.38,  48, [45,65,45,55], 36, 12, "#E8B84C", "融创北京壹号院(12层/36m)"),
        (0.54,  80, [65,45,55,45], 39, 13, "#F2CD66", "融创北京壹号院(13层/39m)"),
        (0.70,  35, [45,55,45,65], 42, 14, "#F7DA7E", "使馆壹号院(14层/42m)"),
        (0.86,  90, [55,45,65,45], 45, 15, "#FCE796", "金茂府(15层/45m)"),
    ]
    ...
    # 点式高层塔楼散布（2 栋，参考北京高端社区点式楼——万柳书院/使馆壹号院点板结合）
    # 位置选在中部安全区（避开西翼斜边界：x<-530 处场地内缩）
    towers = [(0.30, 0.52, 30, 30), (0.68, 0.62, 30, 30)]  # (x比例, y比例, 见方, 高度)
    for tx, ty, sz, h in towers:
        bx = x0 + w*tx; by = y0 + hgt_span*ty
        bld_box(pid, bx, by, bx+sz, by+sz, h, 10, "residential", "点式住宅", "#D9A23F",
                f"点式高层住宅·参考北京万柳书院式点式楼(10层/30m)")
    # 配建小学（教学楼 4层/16m — 参考北京中关村三小教学楼）
    bx = x0 + 40; by = y0 + hgt_span*0.95
