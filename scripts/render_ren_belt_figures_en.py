...
def add_footer(fig, text: str):
    ...
    fig.text(0.95, 0.025, "REN BELT / 2026 · CONCEPT PROPOSAL", fontsize=8, color=COLORS["ink"], ha="right", fontweight="bold")
...
def overview(root: Path, out: Path, layers: dict[str, list], context: Path):
    ...
    ax = fig.add_subplot(gs[0, 0]); setup(ax, "Overview | One Spine · Three Stations · Ren Wings", "Public map is a location reference; dashed lines are provisional, not official redlines")
    plot_base(ax, layers, context)
    for feature, name, col in zip(layers["key_areas"], ["Zhongzhiyuan", "AI Origin Community", "Dazhongsi"], [COLORS["commerce"], COLORS["spine"], COLORS["public"]]):
    ...
    label(ax, 116.3500, 39.977, "Jing-Zhang Heritage Park · 9 km spine", color=COLORS["spine"], size=10, weight="bold", ha="left", box=True)
    ax.text(0.03, 0.05, "Base map: OpenStreetMap contributors (location reference)", transform=ax.transAxes, fontsize=7, color=COLORS["muted"], bbox=dict(facecolor="white", alpha=.8, edgecolor="none"))
    panel = fig.add_subplot(gs[0, 1]); style_axis(panel); panel.set_xlim(0,1); panel.set_ylim(0,1)
    panel.text(.08,.92,"Design decisions",fontsize=16,fontweight="bold",color=COLORS["ink"])
    points=[("01", "Heritage spine", "Translate railway heritage into a continuous, walkable public-space armature."), ("02", "Three stations", "Each key area hosts a distinct industry, community or cultural exchange role."), ("03", "Ren wings", "Technology-services wing × scenario-enablement wing, linked by shared interfaces."), ("04", "Test before scale", "Start with reversible pilots; phase projects that depend on official conditions.")]
    ...
    panel.add_patch(FancyBboxPatch((.06,.08),.88,.13,boxstyle="round,pad=.02",facecolor="#F8EBDD",edgecolor="#E6B783")); panel.text(.1,.16,"Boundary status",fontsize=9,fontweight="bold",color=COLORS["rail"]); panel.text(.1,.11,"provisional / replace with official geometry",fontsize=9,color=COLORS["ink"])
    met = fig.add_subplot(gs[0, 2]); style_axis(met); met.set_xlim(0,1); met.set_ylim(0,1); met.text(.08,.92,"Auditable metrics",fontsize=16,fontweight="bold",color=COLORS["ink"])
    metric_card(met,.08,.78,"11.41", "km² design area (provisional)", COLORS["rail"]); metric_card(met,.08,.60,"24.0%", "Green ratio", COLORS["spine"]); metric_card(met,.08,.42,"10.4%", "Public-space ratio", COLORS["public"]); metric_card(met,.08,.24,"13.6", "km slow-mobility corridor", COLORS["ink"])
    add_footer(fig, "Source: geometry/*.geojson · metrics.json · OSM location reference; design layers are conceptual")
    fig.savefig(out / "site-overview.en.png", dpi=160, bbox_inches="tight"); plt.close(fig)
...
def land_use(root: Path, out: Path, layers: dict[str, list], context: Path):
    ...
    ax=fig.add_subplot(gs[0,0]); setup(ax,"Land-use structure | From a strip to readable spatial layers","Nine land-use bands are read as three vertical districts around a central heritage greenway")
    plot_base(ax,layers,context)
    for i, feature in enumerate(layers["land_use"]):
        g=geom(feature); c=g.centroid; name=feature["properties"].get("name_en","") or {"0701":"Residential / community","1401":"Park / heritage greenway","05":"Industry / commerce","0802":"AI research / innovation","0804":"Education / talent services"}.get(feature["properties"].get("land_use_code"),"Design district")
        short=name.replace(" / ","·")
        label(ax,c.x,c.y,short,size=8,color=COLORS["ink"],box=True)
    label(ax,116.3502,40.026,"North | Zhongzhiyuan",color=COLORS["ink"],size=10,weight="bold",ha="left",box=True)
    label(ax,116.3502,39.988,"Middle | AI Origin Community",color=COLORS["ink"],size=10,weight="bold",ha="left",box=True)
    label(ax,116.3502,39.946,"South | Dazhongsi",color=COLORS["ink"],size=10,weight="bold",ha="left",box=True)
    panel=fig.add_subplot(gs[0,1]); style_axis(panel); panel.set_xlim(0,1); panel.set_ylim(0,1)
    panel.text(.07,.92,"Three questions answered by one map",fontsize=16,fontweight="bold",color=COLORS["ink"])
    rows=[("Relationship","The central greenway connects public space, slow mobility and the heritage narrative."),("Difference","North: R&D and industry; middle: community and transfer; south: exchange and consumption."),("Boundary","Colour blocks are design districts; orange dashes show provisional limits, never statutory redlines.")]
    ...
    handles=[Rectangle((0,0),1,1,facecolor=COLORS["residential"],label="Residential / community"),Rectangle((0,0),1,1,facecolor=COLORS["green"],label="Park / green"),Rectangle((0,0),1,1,facecolor=COLORS["research"],label="Education / research"),Rectangle((0,0),1,1,facecolor=COLORS["commerce"],label="Industry / commerce")]
    panel.legend(handles=handles,loc="lower left",bbox_to_anchor=(.06,.07),frameon=False,ncol=2,fontsize=9)
    add_footer(fig,"Source: geometry/land_use.geojson · site_boundary.geojson · OSM location reference")
    fig.savefig(out/"land-use-structure.en.png",dpi=160,bbox_inches="tight"); plt.close(fig)


def key_areas(root: Path, out: Path, layers: dict[str, list], context: Path):
    fig=plt.figure(figsize=(18,8.8),facecolor=COLORS["paper"]); fig.text(.05,.94,"Key areas | Three locatable operating units",fontsize=22,fontweight="bold",color=COLORS["ink"]); fig.text(.05,.905,"Rectangles are not redlines: each area shows public-map context, links, nodes and the next professional review step.",fontsize=10,color=COLORS["muted"])
    gs=fig.add_gridspec(1,3,left=.04,right=.96,top=.86,bottom=.1,wspace=.035)
    names=["Zhongzhiyuan AI Innovation Acceleration Area","Beijing AI Origin Community","Dazhongsi AI Industry Cluster"]; colors=[COLORS["commerce"],COLORS["spine"],COLORS["public"]]; areas=["Approx. 192.1 ha","Approx. 104.3 ha","Approx. 72.0 ha"]
    for i,(feature,name,col,area) in enumerate(zip(layers["key_areas"],names,colors,areas)):
        ...
        notes=["Dependencies: official boundary, tenure, regulatory controls","Action: innovation plaza + controlled test scenarios","Links: Qinghe / heritage spine / slow mobility"] if i==0 else ["Dependencies: community participation / service capacity","Action: Ren Plaza + transfer street","Links: universities / Origin station / greenway"] if i==1 else ["Dependencies: rail interface, heritage and municipal conditions","Action: intelligent exchange plaza + pitch lounge","Links: Dazhongsi station / southern gateway"]
        for j,n in enumerate(notes): ax.text(.05,.22-j*.065,n,transform=ax.transAxes,fontsize=8.5,color=COLORS["muted"])
        ax.text(.05,.04,"provisional / conceptual siting",transform=ax.transAxes,fontsize=8,color=COLORS["provisional"],fontweight="bold")
    add_footer(fig,"Source: geometry/key_areas.geojson · roads.geojson · OSM location reference; rectangle edges are not statutory redlines")
    fig.savefig(out/"key-areas.en.png",dpi=160,bbox_inches="tight"); plt.close(fig)
...
def mobility(root: Path, out: Path, layers: dict[str, list], context: Path):
    ...
    ax=fig.add_subplot(gs[0,0]); setup(ax,"Transport, slow mobility × blue-green public space","One map combines the 9 km green spine, east-west links, five green spaces and public nodes")
    ...
    for feature in layers["roads"]:
        ...
        g=geom(feature); c=g.centroid; label(ax,c.x+.001,c.y,feature["properties"].get("name_en","") or {"0701":"Residential / community","1401":"Park / heritage greenway","05":"Industry / commerce","0802":"AI research / innovation","0804":"Education / talent services"}.get(feature["properties"].get("land_use_code"),"Design district").replace("Jing-Zhang Heritage Park","Jing-Zhang Park"),size=8,color=COLORS["ink"],ha="left",box=True)
    ...
    panel=fig.add_subplot(gs[0,1]); style_axis(panel); panel.set_xlim(0,1); panel.set_ylim(0,1)
    panel.text(.08,.92,"System moves",fontsize=16,fontweight="bold",color=COLORS["ink"])
    actions=[("Spine","Greenway-first walking · 9 km continuous narrative"),("Cross-links","Three east-west links · stitch communities"),("Nodes","Five public spaces · 10-minute station walks"),("Blue-green","Qinghe, Xiaoyue River and heritage park connected"),("Safety","Delivery robots / governance sandboxes on controlled segments only")]
    ...
    panel.text(.08,.12,wrap_copy("Roads are schematic centerlines; official redlines, fire, municipal, rail and flood conditions require professional review.", 32),fontsize=9,color=COLORS["muted"],va="top")
    add_footer(fig,"Source: geometry/roads.geojson · green_space.geojson · public_space.geojson · OSM location reference")
    fig.savefig(out/"mobility-bluegreen.en.png",dpi=160,bbox_inches="tight"); plt.close(fig)
...
def metrics(root: Path, out: Path, layers: dict[str, list], context: Path):
    ...
    ax=fig.add_subplot(gs[0,0]); setup(ax,"Metrics evidence | Design decisions and recalculation basis","All values are recomputed from geometry / metrics.json; provisional limits support concept discussion only")
    ...
    panel=fig.add_subplot(gs[0,1]); style_axis(panel); panel.set_xlim(0,1); panel.set_ylim(0,1); panel.text(.08,.92,"Core metrics",fontsize=16,fontweight="bold",color=COLORS["ink"])
    data=[("11.41 km²","site_area_sqm","Overall design area (provisional)",COLORS["rail"]),("24.0%","green_ratio","Green ratio",COLORS["spine"]),("10.4%","public_space_ratio","Public-space ratio",COLORS["public"]),("723,000 m²","building_footprint_area_sqm","Schematic building footprint",COLORS["commerce"]),("13.6 km","road_greenway_length_km","Slow-mobility / link corridor",COLORS["ink"])]
    ...
    panel.add_patch(FancyBboxPatch((.06,.06),.88,.12,boxstyle="round,pad=.02",facecolor="#F8EBDD",edgecolor="#E6B783")); panel.text(.1,.135,"Evidence chain",fontsize=9,fontweight="bold",color=COLORS["rail"]); panel.text(.1,.095,"geometry → EPSG:4548 → metrics.json → this figure",fontsize=8.5,color=COLORS["ink"])
    add_footer(fig,"Source: metrics.json · scripts/spatial_review.py · geometry/*.geojson; FAR / height remain unknown")
    fig.savefig(out/"metrics-evidence.en.png",dpi=160,bbox_inches="tight"); plt.close(fig)
