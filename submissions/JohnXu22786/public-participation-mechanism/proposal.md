...
| 字体 | Noto Sans SC / Noto Sans | Google Noto 字体（OFL） | HTML/PDF 嵌入使用 | 已登记 sources.json 字体许可（OFL） |
| 图标 / 图像素材 | matplotlib / scikit-image / shapely / pyproj 默认符号 | 项目自带默认符号 | 概念图件 | 未单独声明来源（属工具默认） |
| 商标（不在先） | 任何与官方或他人商标雷同之处 | 参与者声明不主张在先 | 概念展示 | 不涉及商标（清权完成前） |
| 案例 | 六项全球 AI 创新生态案例（SRC-GLOBAL-CASE-01..06） | 各案例发布方公开主页 | 概念级摘要引用、不作逐字复用 | research_hypothesis（链接核验中） |
