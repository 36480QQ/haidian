...
title: "Dual-Use Peacetime Emergency (Concept)"
...
The concept ecosystem connects eight support conditions: land, space, industry, finance, talent, compute, data and test scenarios. Its proposed chain is university research -> enterprise transfer -> community service -> public governance. Six station/space precedents are kept as a separate reference group: Japanese disaster-prevention parks; Tokyo Metropolitan Area Outer Underground Discharge Channel; Singapore Civil Defence shelters; China's dual-use policy pilots; Beijing Pinggu dual-use practice exchange; and the Netherlands Room for the River programme. They are not global AI ecosystem cases. In this bounded verification, only C02 (Dutch Government Algorithm Register) and C04 (UK AI Security Institute) have a verified first-party landing-page fact and count toward `global_case_count=2`; C01, C03, C05 and C06 remain non-counted background references because their current fact/licence status was not independently verified. Each record in `sources.json` retains publisher, URL, access date, publication-date status, rights boundary, fact status, translation mechanism and non-copy condition. None implies local partnership, funding, procurement or deployment.
...
`metrics.json` records provisional geometry values and formulas, including `green_ratio=0.110044`, `public_space_ratio=0.003167`, `landmark_count=3`, `scenario_node_count=8`, `global_case_count=2`, `scenario_card_count=12`, `industry_test_scenario_count=3`, `persona_count=5`, and `phase_count=3`. Geometry-derived values are low or medium confidence; content counts describe this package, not an official inventory. The five operating measures remain `to_verify` until records exist. Official geometry and control data must trigger a versioned whole-package recalculation.

## 11. Risk, rights and public compliance

The package uses public or cleared references for attribution only and a separate asset ledger in `report/copyright_statement.md`. The local CJK font is Noto Sans SC under SIL OFL 1.1; no remote assets are required. Diagrams, geometry, text and concept names are participant assets for `COMMUNITY-DISPLAY-ONLY`; third-party images, logos, maps and marks are not reproduced. Rights, brand searches, data governance, fire, rail, heritage, structure, ownership and accessibility checks remain pending where stated.
