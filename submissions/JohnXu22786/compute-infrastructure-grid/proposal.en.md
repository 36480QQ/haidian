---
title: "CMP·JZ Compute-Energy Loop: AI Compute Infrastructure & Compute-Energy Synergy Loop (Concept)"
...
![Overall design scope overview (provisional geometry, with legend and scale bar)](assets/figures/site-overview.png)

![Conceptual land-use structure (recomputed shares, low-precision display)](assets/figures/land-use-structure.png)
...
![Three-node index and design-task figure (with legend, scale bar and north arrow)](assets/figures/key-areas.png)
...
- **Logo mark**: `logo-cmpjz.png` (assets/figures/logo-cmpjz.png) — a three-node ring emblem: three dots (Hub, Edge Station, Synergy Park) connected by double ring flows; the inner ring implies compute and data loops, the outer ring green power and waste-heat loops, with the "算力智环 CMP·JZ" wordmark; language-neutral, shared by zh/en layouts.
...
![CMP·JZ brand logo (language-neutral)](assets/figures/logo-cmpjz.png)
...
Land use concept: on the temporary overall boundary, a five-category conceptual zoning completely covers the area — industrial R&D, energy and municipal, public services, green and public space, and residential and supporting uses (recomputed shares: research land about 35%, park green about 21%, business finance about 19%, commercial about 15%, urban residential about 10%; rounded, low-precision display). This is a conceptual land-use suggestion that does not change statutory land uses; the same logic will be re-cut and recomputed when the official regulatory plan arrives.
...
![Slow-mobility and blue-green space analysis (with legend, scale bar and north arrow)](assets/figures/mobility-bluegreen.png)
...
Three-level wayfinding system (concept): ① belt level — ring motifs and the bilingual "算力智环 CMP·JZ" master motif along the green belt; ② node level — bilingual entrance signage and direction at the three nodes; ③ facility level — functional labels of disclosure boards and science exhibits, with accessible large-type versions. All wayfinding content is human-reviewed; English terms prefer the competition glossary recommendations of docs/terminology-glossary.md.

International communication copy (examples only, conceptual): "Compute in a loop, energy in a loop, heat and data in a loop — every calculation becomes a visible city cycle", mirroring the Chinese master copy "让算力成环、让电力成环、让热力与数据亦成环" as a substantively equivalent zh/en concept layer.
...
| Responsible actors | Haidian data & compute coordination authority (proposed) leading; professional compute operator via open selection | Sub-district community service center + community volunteers | District-heating network owner + responsible energy operator (via open procedure) |
...
![Core-metric evidence figure (ratio and count on separate axes, low-precision display)](assets/figures/metrics-evidence.png)
