...
| 案例 | 可借鉴机制 | 京张转译 | 使用边界 |
| --- | --- | --- | --- |
| Singapore one-north [source:CASE-JTC-ONE-NORTH] | 科研、产业、生活混合与长期园区运营 | 三港共享服务与日常生活嵌入 | background-only，不移植指标 |
| Paris-Saclay [source:CASE-PARIS-SACLAY] | 科研网络、公共空间与区域协同 | 三区两翼的知识与交通协同 | 不推断本项目机构承诺 |
| Waterfront Toronto [source:CASE-WATERFRONT-TORONTO] | 公共利益、数字治理、分期问责 | 场景试验前置数据与退出规则 | 不复制技术或采购安排 |
| Barcelona 22@ [source:CASE-22-BARCELONA] | 创新街区、产业空间与公共服务耦合 | 研发、服务、社区混合的空间类型 | 不替代北京控规 |
| Cambridge Kendall Square [source:CASE-CAMBRIDGE-KENDALL] | 知识机构、步行公共空间、社区收益 | 近校转化与公共客厅 | 不推断地块或企业事实 |
| Mila Montréal [source:CASE-MILA] | 开放科研、人才社区、负责任 AI 文化 | 开源荣誉、治理沙盒与人才网络 | 机构公开页仅作文化背景 |
