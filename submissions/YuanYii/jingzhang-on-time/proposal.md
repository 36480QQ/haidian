...
summary: "城市 AI 公共服务应当像列车一样有公开时刻表：正点率可查、变更需公告、停运有预告、晚点有补偿、跨服务换乘有衔接。以京张遗址公园为运行图正线，构建一图九站一脊空间语法（三主站+六区间乘降所（社区服务点））与城市运行图治理协议。"
...
>
> 版本：v3.0 | 2026-08-14 | YuanYii | 第三次提交（命名双层校准 + 治理断言并入设计章节 + 实证基线补强）
>
> 一图九站一脊：以京张铁路遗址公园为运行图正线（9.5km 主脊），沿线设 9 座站——3 座功能主站（发车场站/零公里站/到发场站）+ 6 座区间乘降所（社区服务点），构建「统筹研究—总体设计—重点区域」三级空间控制体系；以城市运行图为城市 AI 治理协议内核——把 AI 服务的每一次承诺画在运行图上。
...
**版本演进说明**：v2 对命名体系做功能性校准（乘降所与社区服务点双层表述），并将图层治理属性断言、待复算事项显性化为可核验清单；机制主线（运行图/三显示/天窗/KPI）自 v1 起保持不变，全部改动保持双语同步与合规门禁一致。
## 三层范围工作框架
...
**总体概念**：以京张铁路遗址公园为运行图正线（9.5km 主脊），构建「一图九站一脊」空间语法——主脊承载全部 AI 公共服务时空轨迹，9 座站沿正线均布（站间距约 1.3km）：3 座功能主站（发车场站/零公里站/到发场站）承担验证发车、首班转译、到发采用的换乘职能；6 座区间乘降所（社区服务点）（MS-01~06）承担高频日常停靠（社区服务/问询/导览/接驳/便民），是线路利用率的实体支撑——每座乘降所（社区服务点）登记停靠时刻，服务按运行图时刻到站/离站，正点率考核覆盖停靠；以城市运行图（Urban Operating Diagram）为治理协议内核：所有 AI 公共服务像列车一样有公开时刻表，正点率可查、变更需公告、停运有预告、晚点有补偿、跨服务换乘有衔接。
...
**区间乘降所（MS-01~06，社区服务点）**：沿主脊均布的轻量停靠站（每座间距约 1.3km），概念落位为清河/北五环段、学院路/学清路段、北太平庄段、学院南路段、大钟寺北段、南端收尾段——优先依托高校接口、社区节点与断点缝合单元（15 处断点中 6 处升级）。每座乘降所（社区服务点）配置 4 类轻量设施：时刻表信息点（T-02 延伸）、信号柱（与 SP 合设）、慢行驿站（座椅/饮水/遮阳/共享单车接驳）、人工服务窗口（问询/导览/便民——无 AI 等价路径）。功能：高频日常停靠、区间服务换乘交接点（T-04 延伸，凭证人工签署）、天窗检修停靠点（与天窗广场候选 6 处合设）。准入 L1 备案制，责任角色 R-STATION-OPS / R-COMMUNITY-CARE。
...
**乘降所场景延伸**：乘降所（社区服务点）不新增场景卡（保持 8 张克制原则），作为 T-02（正点率公告牌延伸为时刻表信息点）、T-03（天窗检修停靠点）、T-04（区间换乘交接点）、T-05（人工窗口受理补偿）的空间延伸。
