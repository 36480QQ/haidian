...
license: "CC-BY-4.0"
summary: "把京张铁路遗址走廊读作城市的'道岔'：詹天佑一百年前用'人'字形展线换轨上坡，今天AI创新带在同一片场地上换轨转向——科研换轨产业、实验室换轨城市、文化换轨体验。方案以'一岔三场两翼'组织空间：岔心（京张公园换轨枢纽）、始发场（AI原点社区）、加速轨道（众智园）、编组场（大钟寺）、东侧线（中关村科技服务翼）、西侧线（小月河场景赋能翼）；以铁路语汇治理12个AI场景（信号灯红牌暂停、扳道房人工接管、联锁协议可退役）；用五项慢变量和四道复核门把公共利益、空间与年度运营串起来；4处AI朝圣地标对接纪念机制。概念方案，基于临时边界，供专业团队深化。"
...
| agent.2 生态体系 | 六案例机制转译、八类生态要素闭环、五个区域最小接口 | 第三章生态与协同表 | 产业/运营团队核对资源、权属和可用性 |
| agent.3 AI+场景 | 12张场景卡、3个产业测试、6类画像、人工兜底与退出 | 第六章；`visual/assets/switch-protocol.json` | 场景运营方逐卡试点、审查、复盘 |
| agent.4 公共空间 | 东西缝合、南北贯通、4处地标、10项可拆卸组件及生命周期 | 第八、九章；公共空间图层 | 交通、园林、文保与无障碍专业联合深化 |
...
| 慢变量 | 第一年度样点 | 测量/对照方法 | 季度公共产物 | 进入下一步的条件 |
|---|---|---|---|---|
| SV-01 树荫与热舒适 | 南/中/北各2个绿廊样点，另设1个未介入对照点 | 树冠与遮荫时长现场复测；前后季节对照，异常点人工复核 | 遮荫时段图、座椅/雨天可用性清单 | 连续两季不恶化，且补救责任明确 |
| SV-02 雨后渗透与恢复 | 低洼点、雨水花园、小月河界面各选样点 | 每场明显降雨记录积水和消退时间；月度人工剖面与读数并列 | 雨后恢复曲线、冲突读数清单 | 人工与传感口径可解释，异常点不被用于扩点 |
| SV-03 无障碍连续性 | 北三环、知春路、站点接驳各1段 | 轮椅/婴儿车/步行者共同复测；修复前后与相邻未修段对照 | 断点图、通行耗时、参与者复测记录 | 断点有责任人和限期，未修复不推荐新路线 |
| SV-04 街区业态与小商户存续 | 大钟寺和站前自愿商户样本 | 季度自愿填报、年度同比；影响记录可撤回，不作个体信用判断 | 存续/退出汇总、需求与调整台账 | 商户能查看、纠正、撤回数据，且无强制接入 |
| SV-05 儿童独立出行与照护安全 | 学校—社区短段，分成3个可随时收回的路段 | 学期内陪测与逐段放手；只记录事件类型，不记录可识别轨迹 | 分段放手记录、照护者复盘、停止记录 | 照护者和儿童可退出；安全事件零容忍 |

第一年节奏是“春季定样点与授权、夏季做热舒适和通行共测、秋季做商户与人工服务协商、冬季开年度总账和公众质询”。统一分析规则是：按样点和参与者类型分层，做同点前后、相邻未介入点和异常点比较；异常不删除，阈值在首轮基线与参与者共同确认后冻结，当前不预填改善百分比；年度账至少保留样点/窗口、版本、方法与责任、原始记录索引、聚合结果、异常与异议、处置和下次复核日期。该状态型指标目前为待测 `slow_variable_baseline`，不是现状得分[metric:slow_variable_baseline]。机器可读任务单见 `visual/assets/validation-ledger.json`；任何一项没有获得第一份可复核证据，都只能停留在黄灯试点或概念接口层[source:AGENT-TASKBOOK][depth:phasing_implementation][depth:risk_missing_data]。
