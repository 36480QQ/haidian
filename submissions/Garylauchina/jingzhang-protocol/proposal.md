...
summary: "以 BUILD—TEST—LIVE—LEARN/UPDATE 城市生命周期与七类公共接口为骨架，把百年京张 AI 创新带设计为可验证、可申诉、可更新的人机共生城市协议；V0.3 已形成拓扑闭合的空间底稿、12 个场景节点和依赖式实施包，仍使用临时边界。"
...
本方案以北京市规划和自然资源委员会海淀分局发布的《百年京张AI创新带城市设计国际方案征集资格预审公告》为第一依据，并以 `brief/site-package/` 中经维护者登记的任务、范围、枚举、指标和来源清单为机器可读依据。所有判断均拆分为可追溯来源、可复算指标、可校验图层和可人工复核假设。公告要求达到控制性详细规划的城市设计深度和规划综合实施方案的城市设计深度；V0.3 已完成概念空间与证据结构，但 A3 文册、A0 展板、最终 HTML、官方控制条件和专业签核仍是正式投稿前置项。
