...
| 编号 | 场景卡 | 类型 | 空间载体 | 设计说明 | 人工复核边界 |
| --- | --- | --- | --- | --- | --- |
| 01 | 量子真机测试场 | **产业测试验证 T1** | 叠加场 | 面向科研与产业开放的量子计算真机/云测试场地，对齐 Quafu 杯量子计算真机挑战赛机制 | 测试资格审核；结果可追溯、可复核 |
| 02 | 量子安全政务通道 | **产业测试验证 T2** | 观测翼/政务节点 | 量子直接通信技术用于公共服务敏感信息的原型试点 | 试点范围与数据分级须主管部门确认 |
| 03 | 真随机数公共基准站 | **产业测试验证 T3** | 观测广场 | 城市级量子随机数生成器为摇号、抽签等公共随机性事件提供公开基准 | 流程公开审计；不涉及个人数据 |
| 04 | 叠加站台 | 公共空间 | 相干时间轴北端 | 铁路遗产、AI 体验与公共生活叠加的复合空间 | 文保约束优先 |
| 05 | 纠缠实验室 | 产业协作 | 纠缠社区 | 高校-企业联合实验室开放日与成果对接 | 科研保密边界 |
| 06 | 隧穿商业街 | 智能原生消费 | 隧穿坊 | 智能终端、内容消费与无人零售的复合商业街 | 隐私与未成年人保护 |
| 07 | 观测广场 | 公众参与治理 | 纠缠社区 | 城市议题公示、公众投票与反馈的"测量"节点 | 投票规则公开；防刷票 |
| 08 | 量子科普体验馆 | 科普朝圣 | 纠缠社区/原点 | 量子计算、量子通信与量子测量的公众科普与研学 | 内容科学审查 |
| 09 | 相干时间轴步道 | 文化体验 | 京张遗址公园 | 1909-2013-2026 三线相干叙事的文化步道与导视系统 | 历史事实审定 |
| 10 | 开源发布厅 | 开发者社区 | 纠缠社区 | 成果发布、代码贡献展示与小型路演 | 版权与授权核查 |
| 11 | AI 慢行导航 | AI+交通 | 全域慢行网络 | 可解释导视与低侵入传感辅助识别慢行断点与无障碍需求 | 不采集个人轨迹 |
| 12 | 数据要素会客厅 | 数据要素 | 隧穿坊 | 以合规、授权、可审计为前提的数据要素流通服务界面 | 合规审查与审计 |

场景-空间-运营映射、隐私边界、人工复核与运营主体完整登记于 `compliance_matrix.json`，正文表格为可读摘要 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:ai_scenario_operation]。
...
**版权与合规**：本方案仅使用公开或已核实资料，量子事实来自北京量子信息科学研究院官网公开页面（访问日期 2026-08-11，登记于 `sources.json`）；未使用非公开规划资料、未授权图片、字体、商标与人物肖像；全部 AI 生成内容由作者负责事实、引用与表达 [source:BAQIS-2026]。HTML 页面离线可用，无远程资源与跟踪代码。本方案不声称官方批准、审定控规、最终土地权属、建设规模或保证实施。
