...
- **未来科学城（科研放大）**：承担"从 1 到 N"的科研放大——工程化、中试、国家重大平台，是策源成果走向规模化的"放大站"；
...
**实施指标（阶段目标）**：近期 3 个试点场景落地（可立即启动的软设施）。慢行断点缝合数、端侧算力节点数、治理知识库沉淀记录数、公众反馈回流次数等量化指标，当前 status=unknown——需在官方红线、市政交通权属条件与正式运营基线到位后，设定有来源的阶段目标与计算方法，并以正式几何替换后触发全包重算 [metric:key_area_count] [data:geometry/phasing.geojson#PHASE-001]。
