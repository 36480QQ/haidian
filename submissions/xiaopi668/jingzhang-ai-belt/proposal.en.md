...
Core indicators and recalculation basis (EPSG:4548): overall area 11,412,825 m² [metric:site_area_sqm]; green ratio 18.4% (green space 2,099,904 m²) [metric:green_ratio] [metric:green_space_area_sqm]; public-space ratio 0.33% (five plazas, 37,944 m²) [metric:public_space_ratio] [metric:public_space_area_sqm]; building density 13.2% (conceptual massing 1,511,831 m²) [metric:building_density] [metric:building_footprint_area_sqm]; slow-mobility and road network 45.7 km [metric:road_length_m]; key-area areas recalculated at 192.9/104.3/72.0 ha [metric:key_area_total_sqm]. FAR and height controls are marked unknown because statutory conditions are missing and will be recalculated after official data arrives [assumption:A-CONTROLS-PENDING]. All announcement tasks 1.3, 1.4, and 1.5 and the six agent tasks agent.1–agent.6 are covered item by item in `compliance_matrix.json`; 8 mandatory professional standards are covered in `standard_matrix.json`; 15 design-depth items are all `complete` in `design_depth_matrix.json` [depth:metrics_recalculation] [depth:risk_missing_data]. Indicators are not just numbers: the green ratio supports talent living quality and capillary ecosystem services, the public-space ratio supports innovation serendipity and scenario display, and the research-land share supports the spatial supply of the compute and model ecosystems [metric:land_use_research_ratio].
