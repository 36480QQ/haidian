...
summary: "以詹天佑折返展线的工程机制——以空间换能力——为原型，把 9.7 公里京张遗址公园走廊组织为「一轴一支、三处折返、两翼配置」的创新回路：上行轴为南北贯通的遗址公园主轴，验证轴为自北京AI原点社区分出的东南向支轴，三处折返台完成东西缝合；以「道钉」为贡献单元与荣誉体系，把「贡献可记忆」做成可进入的公共空间。方案提交 42 个用地地块、34 处建筑基底、19 条道路与 19 处公共空间的可复算几何，16 项已知指标与 4 项写明缺口的待补指标，并完整回应公告 1.3–1.5 与面向智能体任务书 agent.1–agent.6。全部空间落地建议均为概念建议，边界为临时粗略边界。"
...
- 总体设计范围｜面积：11.41 km²（临时边界复算值）｜回答：结构、用地、交通、蓝绿、风貌如何落图｜落点：42 个用地地块与全部设计图层｜精度：临时粗略边界，需复算
...
![图 F2 空间结构与用地布局：从结构判断传导到 42 个用地地块](assets/figures/land-use-structure.png)
...
总体设计范围要求达到控制性详细规划的城市设计深度 [standard:MOHURD-CONTROL-DETAILED-PLANNING]。本方案提交的用地图层 [data:geometry/land_use.geojson#LU-002] 共 [metric:land_use_parcel_count] 42 个地块，完整覆盖提交边界且互不重叠（覆盖差值小于场地面积的 0.01%），用地代码依 [standard:MNR-LAND-USE-CLASSIFICATION-GUIDE] 的项目子集表达 [depth:land_use_layout]。
...
关于开发强度 [depth:development_intensity_controls]：本方案不提出容积率数值。[metric:floor_area_ratio] 在 `metrics.json` 中的状态为 `unknown`，原因写明为「公开资料中没有本片区经批准的容积率控制条件」。同理，[depth:height_massing_character] 中的建筑高度控制在 `metrics.json` 中为 `unknown`，缺少控规高度条件与文物保护建设控制地带条件。本方案只提出形态原则而非数值：主轴两侧 100 米范围内宜低强度、开放界面、避免连续高墙；折返台周边宜形成高度上的节点性起伏；沿五环与铁路一侧宜以防护绿带过渡。这些原则由 [data:geometry/buildings.geojson#BLDG-001] 表达的 34 处建筑基底给出尺度示意，[metric:building_footprint_area_sqm] 复算为 41.85 公顷，仅表达更新单元的尺度与分布，不是实测基底，也不构成建筑规模结论。
...
建筑基底图层 [data:geometry/buildings.geojson#BLDG-014] 共 34 处，按拆改留分为四类 [depth:retain_renovate_demolish]：

- 保留｜处数：5｜基底面积：59 900｜依据：现状功能仍适配、结构与权属稳定
- 改造｜处数：9｜基底面积：106 300｜依据：结构可利用、功能需转换（含遗存活化）
- 拆除重建｜处数：3｜基底面积：40 500｜依据：与主轴或折返台缝合关系冲突且不具改造价值
- 新建｜处数：17｜基底面积：211 800｜依据：落在留白、低效空间或更新地块上
- 合计｜处数：34｜基底面积：418 500｜依据：[metric:building_footprint_area_sqm]

拆改留分类是本方案中最需要克制的部分。 建筑基底为示意性更新单元，不是实测基底；权属、结构安全、租约、居民意愿与法定程序全部未知。因此本方案把「拆除重建」严格控制在 3 处、占基底面积不足 10%，且全部位于与折返台缝合关系直接冲突的位置；其余以保留和改造为主，改造占比明显高于拆除。这一比例本身就是一个设计判断：在一条以存量为主的走廊上，更新的默认动作应当是改造而不是拆除。 任何一处的分类结论，都必须在权属核实与法定程序之后才能成立。
