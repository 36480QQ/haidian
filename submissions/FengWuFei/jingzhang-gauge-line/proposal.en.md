...
**Boundary status:** The official SITE_BOUNDARY and three KEY_AREA polygons are not yet available, so this proposal uses `provisional_boundaries.geojson` to generate a temporary formal package. `geometry/site_boundary.geojson` and `geometry/key_areas.geojson` are both marked `provisional_constraint`, `official_boundary=false`. This organizer data gap does not block content scoring; after official polygons are supplied, site boundary, key areas, land use, roads, green space, public space, buildings, phasing and metrics must all be recalculated [metric:site_area_sqm]. Boundary evidence returns to the overall-scope layer [data:geometry/site_boundary.geojson#SITE-001].
