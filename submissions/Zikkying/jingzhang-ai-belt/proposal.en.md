---
title: "JZ-AI Origin Belt — Conceptual Urban Design for the Centennial Jing-Zhang AI Innovation Belt"
language: "en"
...
version: "v1.0"
summary: "A conceptual proposal structured as one axis, three cores, two loops and multiple nodes: the Jing-Zhang heritage cultural axis runs north-south, anchored by the Zhongzhiyuan, AI Origin Community and Dazhongsi cores, with a slow-mobility loop, a blue-green ecological loop, 12 AI scenario nodes and 3 AI pilgrimage landmark concept nodes. Generated on the provisional substitute boundary; all spatial proposals are conceptual material for professional teams to deepen."
...
**Logo direction (concept only)**: a motif in which railway gauge lines evolve into twin data streams — two parallel lines derived from sleeper units converge northward into a single origin point, combined with the herringbone switchback symbol of the historic Jing-Zhang Railway, signifying the relay from historical rail to intelligent rail. Suggested palette: railway indigo plus computing teal; all lettering must be original with properly licensed fonts.
...
![Site evidence chain and proposal overview](assets/figures/site-overview.png)
...
**Global AI ecosystem benchmarking** (agent.2, six cases; qualitative public-knowledge comparison, no unverified figures):

| Case | Transferable mechanism | Translation for the belt |
| --- | --- | --- |
| Silicon Valley–Stanford | University sourcing + venture capital + open-source culture | Near-campus incubation and open-source release in the Origin Community |
| Kendall Square, Boston | Walking-distance campus–industry stitching | The campus–park–block slow-mobility ring |
| King's Cross Knowledge Quarter, London | Rail-heritage renewal + tech HQs + public realm | Heritage-plus-innovation renewal of the railway corridor |
| Munich Werksviertel-type districts | Industrial relics converted into creative and testing space | Converting inefficient parcels into testing grounds |
| Tel Aviv innovation districts | Dense startup community and international events | Global AI Week and developer-community operations |
| Shenzhen Bay tech ecosystem | Open scenarios and supply-chain support | Scenario Open Days and industry testing scenarios |
...
The transport strategy is "rail as skeleton, slow mobility as veins, micro-circulation as capillaries" [depth:traffic_rail_slow_parking]. North-south, the heritage cultural-axis greenway runs the full belt (about 9.7 km conceptual alignment); east-west, five slow-mobility connectors stitch blocks severed by rail and ring roads; each key district has one micro-circulation ring; the conceptual slow network totals about 28.1 km (recalculated, not road redlines) [data:geometry/roads.geojson#ROAD-001] [metric:slow_mobility_network_length_m]. Key nodes include the Dazhongsi four-quadrant pedestrian connection, the North Fifth Ring crossing, and rail feeder at Wudaokou and Qinghua East Road West Entrance; parking and cycle parking concentrate at stations and district entrances, with exact scale awaiting transport studies.

![Composite slow-mobility and blue-green public-space system](assets/figures/mobility-bluegreen.png)
...
**Copyright and bilingual**: the primary file of this package is Chinese; `proposal.en.md` (this file) is the complete English counterpart. Drawings, figures and HTML are provided in both languages. All text, geometry and figures are originally generated by the agent from public site-package materials under the COMMUNITY-DISPLAY-ONLY license; the scope and licenses of cited materials are recorded in `sources.json` and `report/copyright_statement.md`. The HTML pages are fully offline, load no remote resources and track no visitor behavior.
