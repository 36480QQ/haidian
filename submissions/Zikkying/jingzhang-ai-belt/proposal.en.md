...
version: "v1.0"
summary: "A conceptual proposal structured as one axis, three cores, two loops and multiple nodes: the Jing-Zhang heritage cultural axis runs north-south, anchored by the Zhongzhiyuan, AI Origin Community and Dazhongsi cores, with a slow-mobility loop, a blue-green ecological loop, 10 AI scenario nodes and 3 AI pilgrimage landmark concept nodes. Generated on the provisional substitute boundary; all spatial proposals are conceptual material for professional teams to deepen."
...
![Original JZ-AI Origin Belt mark and visual-identity direction](assets/brand/jz-ai-origin-mark.svg)
...
![Site evidence chain and proposal overview](assets/figures/site-overview.en.png)
...
**Copyright and bilingual**: the primary file of this package is Chinese; `proposal.en.md` (this file) is the complete English counterpart. Drawings, figures and HTML are provided in both languages. All text, geometry and figures are originally generated by the agent from public site-package materials under the COMMUNITY-DISPLAY-ONLY license; the scope and licenses of cited materials are recorded in `sources.json` and `report/copyright_statement.md`. The three concept scenes were generated with the OpenAI image-generation tool on 2026-08-12; the host exposed no specific underlying model, so it is recorded truthfully as `unknown`. The original SVG has not undergone a trademark search and carries no registrability or exclusivity claim. PDFs are built locally with ReportLab and Pillow; the copyright ledger records each font name, embedding state, licence, redistribution status and viewer-compatibility boundary. No font program with unverified redistribution rights may be embedded in the submitted PDFs. The HTML pages are fully offline, load no remote resources and track no visitor behavior.
