...
本稿为简体中文本地准备稿，由 `cactus-agent` 编制。它以未来世代影响审查、四尺度代理试验、五维资产—负担审查和逐场景未来继承条款为方法，把百年京张从历史叙事转化为可以被今天验证、修订和停止的公共责任。当前包保持 `package_state=scaffold`，未进入最终化、远程协作或真实投稿流程。

## 设计依据与资料清单

### 核心判断

《百年京张·未来继承人计划》不是从技术清单出发，而是从征集任务、真实城市使用与长期公共责任三者的交集出发。方案把“让今天的创新，不透支明天的选择权”作为总判断：任何 AI 场景都必须先说明今天解决什么问题、谁获得便利、谁可能承担额外负担，再检查未来是否仍能更换技术、改变用途、恢复普通服务并读懂当年的决定。编制模型为 `cactus-agent`；当前成果是本地准备稿，`package_state=scaffold`，没有对外投稿，也不表示已通过维护者、专家或政府部门审查。
...
![现有总体范围图基线；下一轮重绘无 AI 底图与三处继承压力场](assets/figures/site-overview.png)

> 图面状态：当前 PNG 尚未同步新术语，只作为临时几何与既有节点关系的基线。下一轮重绘后，方可用图面证明“无 AI 底图—三处继承压力场—两翼长期支撑”。
...
![现有概念用地图基线；下一轮重绘无 AI 底图](assets/figures/land-use-structure.png)
...
![现有重点区图基线；下一轮重绘三处继承压力场](assets/figures/key-areas.png)
...
本轮统一使用中文主名 **百年京张·未来继承人计划**，不制作英文显示名，以保持简体中文准备稿的边界。正式双语阶段如确有需要，再由专业译者完成翻译、回译、法律边界和商标近似审查；当前中文主名不是注册商标或已批准国际传播口径。
...
当前仅完成简体中文准备内容，Agent: cactus-agent，`package_state=scaffold`。英文对照稿、图中文字对照、版权最终确认、正式空间资料替换、外部专业复核以及人工投稿决定仍是后续门禁；不得复制中文冒充英文，也不得因本地校验通过就声称具备正式评审资格。本轮不调用最终化流程，不创建提交、推送、拉取请求或真实投稿。后续每次准备动作都应记录输入版本、自检结果、未关闭问题与人工责任，确保“未完成”能够被看见。
