...
![Actual orientation, verified site skeleton, and limits of the reversible design overlay](assets/figures/site-grounding.en.png)

The grounding figure keeps three things separate. Panel A uses an OSM snapshot only for the orientation among Xueyuan South Road, North Third Ring, Zhichun Road, and the heritage-park direction; the orange band is not a redline. Panel B extracts only publicly verifiable elements: restored rails, blue acoustic barrier, east-side red slow path, and neighborhood/office skyline. Panel C alone adds the staffed kiosk, physical emergency stop, and short service spur. The robot cannot enter the main slow path or cross the heritage rails. OSM is open geographic context—not a substitute for survey, tenure, or planning approval.[source:OSM-CONTEXT-20260829] [source:OFFICIAL-JZR-PHOTO-REF-20260730]

![Site-constrained concept overlay for the representative Xueyuan South Road–Zhichun Road segment: rails, barrier, and red slow path retained; robot stopped behind the side-spur stop line](assets/media/slow-line-site-grounded.jpg)

The image above is a **site-constrained conceptual overlay for a representative segment**. It retains the north–south restored rails, blue acoustic barrier, east-side red slow path, planting, and neighborhood skyline. Every new element is a small reversible addition. The robot waits behind the line on a paved spur perpendicular to the main slow path; it neither occupies the slow path nor crosses the rails. The bicycle travels on the main path. This is not a site photograph, official rendering, exact camera reconstruction, approved plan, or engineering evidence. Actual siting still requires survey, tenure, traffic, heritage, and operating approval.
