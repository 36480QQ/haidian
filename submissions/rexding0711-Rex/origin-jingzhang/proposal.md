...
**权重设定逻辑（概念建议）**：D_dev 与 P_compute 权重相同（各 0.4），因为开发者密度与算力负荷是「硬需求」，直接决定服务前台的物理容量；F_git 权重较低（0.2），开源活跃度是「软信号」，反映社区活力但不直接决定空间需求。权重可由运营主体按阶段调整——孵化期提高 D_dev 权重，成熟期提高 F_git 权重，这正是「可编程」的含义。算法已提供可运行演示脚本 `scripts/origin_node_demo.py`，评审可复现节点分级结果。

**节点分级示例（概念推演）**：

| 地块 | D_dev | P_compute | F_git | S_node | 等级 | 判定理由 |
| --- | --- | --- | --- | --- | --- | --- |
| LU-008 众智园核心研发 | 0.8 | 0.9 | 0.9 | 0.86 | LEVEL_3 | 全栈创新，三指标均高 |
| LU-011 原点核心孵化 | 0.9 | 0.7 | 0.8 | 0.78 | LEVEL_2 | 开发者密集，算力中等 |
| LU-014 大钟寺商业 | 0.5 | 0.4 | 0.3 | 0.40 | LEVEL_1 | 商业为主，创新密度较低 |
