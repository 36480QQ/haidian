...
| 失败怎么办？ | 原点·未提交区：公开记录被证据门拦截/停止/否决的方案 | 未提交区设计（见朝圣地标章节） |
...
| 原点·未提交区 Origin Uncommitted Zone | 原点碑南侧 | 公开记录被证据门拦截、被停止条件触发、被公众否决、被人类评审认为不合适的方案——每块展板写着「这个方案没有建成，这是正确的决定」。呼应京张铁路历史（詹天佑原始方案中也有被放弃的备选线路）与 git 文化（uncommitted）。原点碑记录「建成的」，未提交区记录「被正确拒绝的」，两者合在一起才是完整的城市记忆 |
...
### 行动包治理套件（可实施性证据链，RACI+KPI+SLO+停止条件）
