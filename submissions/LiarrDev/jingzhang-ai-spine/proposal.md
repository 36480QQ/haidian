...
**贯穿全文的运行机制——公共回路**：方案给每一次AI试点设定同一条可复核的公共回路——提问、获准、小测、裁决、回执、退出。没有普通服务、人工责任、停止条件或清权证据，就不进入下一阶段。这一机制不是一张概念图，而是近期试点、中期建设与远期运营共用的运行章程：它让"可验证"从口号变成每个项目都能逐条勾对的清单，也让"可撤回"从原则变成明确的退出条件 [data:risk.json] [data:simulation.json]。
...
方案逐项映射公告三大定位 [source:OFFICIAL-ANNOUNCEMENT] 与任务书五大功能 [source:AGENT-TASKBOOK]，每项定位/功能均给出本方案的空间或运营回答与可审阅证据；"不是的东西"一列用于明确方案边界，避免把概念建议表述为已确定安排：

| 任务书定位 | 本方案的空间/运营回答 | 可审阅证据 |
| --- | --- | --- |
| 百年京张文化带 | 遗址公园文化主轴 + 三重文化叙事 + 京张智脊纪念碑廊 | 文化融合叙事章节 [data:geometry/green_space.geojson] |
| 都市AI生活体验带 | 12张AI场景卡落到日常公共空间 + 蓝绿慢行复合环 | AI场景卡章节 [data:geometry/public_space.geojson] |
| AI融合创新带 | "一脊三椎、两翼展开"产业骨架 + 三区两翼协同回路 | 空间结构章节 [data:geometry/site_boundary.geojson#SITE-001] |

| 任务书五大功能 | 本方案落地接口 | 不是的东西 |
| --- | --- | --- |
| AI全栈自主创新体系 | 众智园智源椎：芯片/框架/算法→评测/安全→应用转化→标准/生态的全链条空间 | 不是未经安全审查的生产系统 |
| 世界级AI创新生态 | AI原点社区：开源发布厅、成果转化街、人才特区 | 不是已签署的招商或合作协议 |
| AI+场景赋能新范式 | 12张场景卡 + 4个测试验证场景 + 场景数据-治理审计表 | 不是"装置越多越智能" |
| 智能化AI活力城市 | 遗址公园活力带、站点TOD、蓝绿慢行复合环、公共状态板 | 不是以算法取代公共服务 |
| AI治理全球话语权 | 安全治理沙盒、标准制定工作坊、数据要素会客厅 | 不是国际组织背书或政策承诺 |

| 三区两翼 | 角色与接口 | 主要空间证据 |
| --- | --- | --- |
| 众智园智源椎 | 全栈自主创新、标准治理、低碳算力 | [data:geometry/key_areas.geojson#PROV-KEY-001] |
| AI原点社区智脉椎 | 开源协作、成果转化、人才生活 | [data:geometry/key_areas.geojson#PROV-KEY-002] |
| 大钟寺智汇椎 | 产业汇聚、国际交往、数据要素 | [data:geometry/key_areas.geojson#PROV-KEY-003] |
| 中关村科技服务翼 | 资本、知识产权、技术交易赋能 | 概念建议，需相关主体另行协商 |
| 小月河场景赋能翼 | AI场景试验、公共体验、生活服务 | [data:geometry/green_space.geojson] |

上述映射的逐项证据由 `compliance_matrix.json` 的 requirement_id、report_sections、geojson_layers、metrics、drawings 和 visual_sections 字段结构化记录，评审可逐条追溯到正文、图纸、图层和指标。国家、市、区三级公开政策仅作为方向合理性的参照（完整登记见 `sources.json`），不构成任何形式的官方背书或审批结论。
...
**外部区域协同**：本方案并非孤立片区，而是嵌入北京全域和京津冀的创新网络 [source:AGENT-TASKBOOK]。区域协同不是把区域名词排列在地图上，而是建立"输入—输出—责任/证据"三要素的可回流责任链——每个外部区域有明确输入、京张带输出和需确认的责任边界：

| 区域接口 | 输入 | 京张带输出 | 需确认的责任/证据 |
| --- | --- | --- | --- |
| 北纬社区 | 大模型应用与场景落地需求 | 场景题、参与反馈、可复现实验 | 社区授权、参与补偿、隐私边界 |
| 未来科学城 | 能源与生命科学交叉能力 | 低碳构件、端侧算力、机器人接口小试 | 工程安全、生态和市政校核 |
| 怀柔科学城 | 大科学装置与基础研究成果 | 面向公众的解释性展示与验证问题 | 科研设施运营方同意、数据清权 |
| 北京经开区 | 智能制造与高端制造能力 | 经过安全门的公共验证证据 | 企业/园区合作、采购与责任边界 |
| 京津冀协同网络 | 多样城市场景与比较反馈 | 方案扩散、复用或退出记录 | 区域主管部门和运营方确认 |

每一条区域关系均为概念性方向建议 [source:AGENT-TASKBOOK]，不声称供地、投资、招商、数据共享或政府承诺；其可实施单位是"一个问题、一个责任主体、一份证据、一次公开复盘、一次扩散或退出"，便于运营团队和后续深化团队接续，具体合作机制需由相关主体另行协商。
...
方案关注多元使用者的不同需求，确保AI创新带不是精英专属空间 [depth:public_interest_inclusion]。公共利益不是一句承诺，而是"先测再承诺"的可核验基线——在试点前取得证据，不把"待测"写成"已达标"：

| 审计项 | 当前状态 | 试点前证据 | 冲突处置 |
| --- | --- | --- | --- |
| 无障碍连续与夜间可达 | 未知，不填造假基线 | 日/夜配对步行审计、照度/声音记录 | 暂停场景，给人工路线并公开修复 |
| 无App获得服务 | 未知，列入服务盘点 | 纸面、人工、语音、多语种路径测试 | 保留人工窗口，数字层不得成为前提 |
| 分组影响差异 | 未知，分组调查 | 老人、残障、照护、夜班、儿童、访客、小商户、维护人员参与 | 公开分歧，不以平均数掩盖差异 |
| 投诉与回应 | 未知，建立公共状态板 | 每案确认/修复时钟与责任人 | 7-14日回应；无法修复则撤回 |
| 隐私与清权 | 仅最小数据、无个体轨迹 | 来源、授权、留存、哈希逐项核验 | 授权撤回/越权访问即熔断 |

参与原则为"公开问题—给时间和渠道—返回决定及理由—记录异议—补救或退出"。在此基础上，方案明确以下保障：
...
**场景最低运营合同**：每个场景组均设定可核验的最低运营承诺——空间载体、触发/最小数据、责任与SLA、非AI等价/退出条件。SLA是设计合同草案而非法定标准；任何场景在授权、数据、安全或公共接受条件不满足时按列明的退出条件停用，并归还普通公共使用 [data:compliance_matrix.json]：

| 场景组 | 空间载体 | 触发/最小数据 | 责任与SLA | 非AI等价/退出 |
| --- | --- | --- | --- | --- |
| 慢行导航与无障碍辅助 | 京张遗址公园 | 公开慢行断面/路口资料、环境传感（不采集个人轨迹） | 园区管理负责；季检，投诉7-14日响应 | 人工引导与实体导视；断点数据失效即停 |
| 安全治理沙盒与标准工作坊 | 众智园 | 公开安全评测方法与模型行为数据（脱敏） | 评测机构+园区联合审查；窗口结束发布摘要 | 纸面评测/专家会；来源不明即拒绝令牌 |
| 端侧算力与低碳算力驿站 | 总体设计范围节点 | 公开算力需求与公共服务资料（仅聚合） | 政府+运营商联合监管；授权到期即停 | 人工柜台；授权撤回即熔断 |
| 开源发布与成果转化 | AI原点社区 | 公开代码贡献记录与活动数据（仅聚合统计） | 开源社区运营；活动7日记录、材料2工作日更正 | 人工柜台与公告栏；贡献记录可署名可撤回 |
| 国际路演与数据要素会客厅 | 大钟寺片区 | 公开商务活动与合规授权数据（脱敏可审计） | 企业+物业、数据交易所+法律合规审查 | 普通会议与人工柜台；越权访问即熔断 |
| 机器人配送试验区 | 小月河翼 | 公开道路与聚合物流数据（不采集个人收件信息） | 物流企业+政府联合监管；每次运行24小时复盘 | 人工配送/手推车；急停失败即退出 |
| AI公共艺术与文化导视 | 遗址公园沿线 | 公开工业遗存与AI生成艺术资料（标注版权） | 文化机构审核版权；导视损坏48小时替换 | 人工讲解/纸本/语音；版权不明即下架 |

场景运营记录进入公共知识库 [source:AGENT-TASKBOOK]，所有窗口结束后归还普通公共使用。
...
### 评审可见证据层：13个任务书评审维度一页回读

本节把最影响内容评分的证据直接写回正文，让不打开附加JSON的评审者也能逐项复核；只复述任务书与本方案的概念接口，不把设计建议升级为政府决策或法定规划条件 [source:AGENT-TASKBOOK]。

| 任务书评审维度 | 本方案的一句可核验回答 | 证据落点 |
| --- | --- | --- |
| 目标契合度 | 三大定位逐项映射到空间/运营回答与可审阅证据 | 任务书硬映射表、compliance_matrix.json |
| 功能匹配度 | 五大功能均有落地接口并明确"不是的东西" | 任务书硬映射表 |
| 品牌识别度 | "智脊·京张"Logo三变体、色彩/字体系统、国际传播语汇 | 品牌识别系统章节、assets/identity/spine-mark.svg |
| 区域协同性 | 五区域"输入—输出—责任/证据"可回流责任链 | 外部区域协同表 |
| 规划创新性 | "一脊三椎、两翼展开"空间结构 + 三层范围工作框架 | 空间结构章节、land_use.geojson |
| 产业支撑度 | 全栈自主创新体系四层链条 + 端侧算力/测试/标准空间 | 众智园详细设计章节 |
| 场景可感知度 | 12张场景卡落到具体空间载体 + 最低运营合同 | AI场景卡章节、场景最低运营合同表 |
| 空间明确性 | 三处重点区域节点级计划 + 空间证据引用 | key_areas.geojson#PROV-KEY-001/002/003 |
| 可转化性 | 概念/空间/活动/技术四类可转化交付物接口 | 可转化交付物段落 |
| 表达完整性 | proposal/图纸/HTML/指标/图层/证据引用闭环 + 双语 | manifest.json、self_check.json |
| 公开合规性 | 全程"概念建议"措辞 + 版权清权声明 + 无远程资源 | 风险合规章节 |
| 国际传播力 | 双语交付物 + 英文命名体系 + 全球可检索品牌资产 | proposal.en.md、visual/index.en.html |
| 长期运营价值 | 四季活动 + 运营联盟 + 90天验证 + 三道评估门 + 品牌沉淀 | 长期运营章节 |

本节验收不是"方案写得更长"，而是13个任务书评审维度均能回到一句可核验的话；官方边界、控规、权属、市政、文保等数据到位后，全部空间结果须重新复算 [assumption:A-CONTROLS-001]。
