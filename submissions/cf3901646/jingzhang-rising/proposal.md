...
**其中「AI治理全球话语权」是本方案投入最重的一项。** 理由写在正文后段：芯片与模型的追赶需要时间，而**治理规则的空位是现在就存在的**。全球城市都还没回答「AI 进入公共服务后出错了谁负责」。本方案给出的答案不是宣言，是一套六项公开答卷加五项人工兜底硬要求的**可照抄清单**——这是最有可能被国际引用、也最符合「话语权」本义的路径 [depth:governance_mechanism]。
...
**这个回路的价值在于它可以被检验。** 任一环断裂都有可观察的征兆：造出的能力无人验证（1→3 断）、开源成果无法商业化（2→4 断）、有营收却不反哺研发（4→5→1 断）。本方案因此把「回路是否闭合」列为年度公开复算的观察项之一，而不只是画一张箭头图 [depth:implementation_roadmap]。

**两翼为什么是翼而不是区。** 中关村科技服务翼与小月河场景赋能翼不承担独立的空间主体地位，它们的作用是**向三区双向输血**：一个输入要素与资本，一个输入场景与验证。翼的绩效不看自身产值，而看**它让三区多完成了多少次闭环**。

### 四、区域创新协同关系

本带不是孤岛，需说明其在北京乃至京津冀创新格局中的位置 [depth:regional_coordination]：
...
任务书 `agent.1` 要求回答综合规划、空间产业融合与国土空间规划的创新思路 [source:AGENT-TASKBOOK]。本方案给出三条，均立足于「**这条带与常规产业园区到底有什么不同**」这一问题 [depth:planning_innovation]。
...
**时间公地是本方案的核心差异价值主张。** 与其说京张向上是一条"地理空间"，不如说它是一条**时间轴上的数据开放基础设施** [depth:planning_innovation] [depth:long_term_operation]。
...
- 承载空间：智源台"AI 数据开放中心"[data:geometry/key_areas.geojson#PROV-KEY-PUBLIC-DATA]
...
- 承载空间：原点坊"创新成果转化中心" [data:geometry/key_areas.geojson#PROV-KEY-TRANSFER]

**层级 3：治理透明数据层（系统生成，实时可查）**
- 内容：AI 服务的三色状态卡、失败记录、人工通道响应时间、公众投诉与反馈汇总
- 可视化：三维实时仪表板（集中展示于"算力之钟"等公共构筑物）
- 访问权：全社会可查（不含个人身份信息），实时更新不延迟
- 承载空间：智汇钟"治理可视化中心" [data:geometry/key_areas.geojson#PROV-KEY-GOVERNANCE]

**层级 4：学术与社区数据层（协议公开，研究反馈）**
- 流程：高校与社区组织可申请访问脱敏数据集，用于学术研究与社会评估
- 责任：发表成果时须致谢数据来源，向带内社区反馈研究发现
- 成果物：年度"京张数据故事集"，汇总学术发现、社区反馈、政策建议
- 承载空间：北大、清华、中科院在带内的研究中心 [standard:RESEARCH-DATA-GOVERNANCE]

#### 5.3 时间递进的共创框架

**过去—现在—未来三代人的对话** [depth:design_mechanism_innovation]：
...
京张向上提出的"时间公地 + 数据透明 + 失败记录"模式，与以下国际实践相呼应但各有差异 [source:INTERNATIONAL-GOVERNANCE] [metric:governance_innovation_count]：

| 参考案例 | 地点 | 核心机制 | 京张的创新点 |
|---------|------|---------|-----------|
| Open Data Movement | 全球 | 公共数据开放 | 京张加入"应用数据回流"（企业必须贡献） + "失败记录透明"（不只报成功） |
| Regulatory Sandbox | 新加坡、英国 | 管制沙箱、有限试验 | 京张加入"时间递进"框架（沙箱不只是短期试验，而是长期生态） |
| AI Ethics Board | 欧盟、加州 | 伦理审查 | 京张加入"人工兜底"（审查结果要落实到人工通道设计） |
| Data Commons | 印度、肯尼亚 | 社区数据共有 | 京张加入"三色状态"可视化（让数据治理成为公共景观） |

#### 5.5 时间公地的风险与边界

**必须声明**的限制与假设 [assumption:A-TIMELANDS-001]：
...
一条创新带真正的门槛不在于有没有好想法，而在于**想法在哪一环会断掉**。本方案针对四类关键要素给出保障机制建议 [depth:industrial_support]：
...
一份方案是否可实施，不取决于它画了几期，而取决于**它敢不敢指定第一个试点、并预先写下失败判据**。本方案因此提出三个**首启试点区**，均位于已建成、权属相对单一、无需大规模拆迁的地段，可在不等待官方边界发布的前提下先行启动 [depth:implementation_roadmap]。

| 试点 | 位置 | 首年动作 | 牵头主体建议 | 一年后验收指标 | 失败判据（触发退出或重做） |
| --- | --- | --- | --- | --- | --- |
| **试点一：人工兜底样板窗口** | 原点坊范围内一处既有政务或社区服务窗口 | 按五项硬性要求改造 1 处，同址设人工通道并公示公开答卷六项 | 属地街道 + 服务窗口运营单位 | 人工通道达标率 100%；答卷六项全公示；老年办事完成率不低于改造前 | 出现"AI 是唯一入口"情形，或红色降级时无人工承接 |
| **试点二：一条缝合链贯通** | 7 条跨廊道缝合链中最短、障碍最少的 1 条 | 打通东西向慢行 1 条，配夜间照明与无障碍坡道 | 属地园林／交通主管部门 + 铁路产权单位 | 步行绕行距离下降；日均通行人次持续三个月增长 | 因权属协调无法开工，或贯通后日均通行人次低于预期一半 |
| **试点三：场景开放清单首发** | 小月河场景赋能翼 | 发布首批 3 个可申请的真实测试场景，含申请、评估、退出全流程 | 区级科技主管部门 + 场景权属单位 | 首批场景全部有团队进入并产出验证报告（含失败记录） | 清单发布后无团队申请，或申请后无法实际进场 |
...
**关于失败判据的说明。** 上表最后一列不是免责条款，而是本方案的方法论立场：**一个不肯预先写下失败判据的实施计划，事后就无法被追责**。三项试点若未达标，应公开记录原因并调整方案，而非默默扩大规模。这与"公开答卷"中的"失败记录"与"退出条件"两项是同一套逻辑 [depth:risk_register]。
...
**一条创新带最终留下什么？** 不是建筑，建筑会老；也不是某一届活动，活动会散。真正能沉淀下来的只有三样东西 [depth:long_term_operation]：
