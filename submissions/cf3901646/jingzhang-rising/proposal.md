...
**随包可复演证据。** `visual/assets/answer-sheet/` 将同一套规则编译为 12 份机器可读场景合同与离线校验器；`results.json` 绑定合同、案例和校验器 SHA-256。当前合成回执为 **84/84 符合预期**：12 份完整合同只被识别为 `protocol_ready_not_authorized`，并未被误写成“已开放”；对每份合同分别删除责任、F2 数据来源、F4 失败记录、F6 退出条件、H1 同址或 H5 可切换，共 72 条负例，全部被判为 `blocked`。任一负例被放行，整套答卷机即失败。该结果只证明**协议校验逻辑闭合**，不证明现场安全、有效、合规或获批；可在 `visual/index.html` 的“AI 赶考答卷”标签页主动删项复演。
