...
This ledger covers **every one of the 54 files** in `submissions/cf3901646/jingzhang-rising/`. It is written to be checked, not to be trusted: each row names the actual production method so that a reviewer can re-derive the asset from the package itself.
...
| `report/proposal.html`, `report/proposal.en.html`, `visual/index.html`, `visual/index.en.html` | Original static HTML with inline CSS and inline SVG, assembled locally. The visual pages load only the two package-local answer-sheet JavaScript files disclosed in the next row. | **Zero remote dependencies**: no CDN, no remote script, no web font, no remote map tile, no iframe, no analytics or tracking, no network request of any kind. Opens correctly from a local filesystem with networking disabled. | Same display-only license. |
| `visual/assets/answer-sheet/*` (5 files) and the answer-sheet tab in `visual/index*.html` | Original offline interaction, twelve scenario contracts, eighty-four synthetic cases, machine receipts, a browser data bundle and a dependency-free validator authored for this submission. The validator checks declared F1-F6, H1-H5 and G0-G5 field completeness; it does not execute participant code from contracts, access a network, collect data or make planning decisions. | Derived only from this proposal's own twelve scenario cards and governance rules. `results.json` binds contracts, cases and validator by SHA-256. The 84/84 receipt is explicitly limited to synthetic protocol classification and is not site-performance, safety, compliance or approval evidence. | Same display-only license. May be reused only with the conceptual/not-authorised limitation preserved. |
