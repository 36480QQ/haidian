...
**Replayable evidence shipped in the package.** `visual/assets/answer-sheet/` compiles the same rules into twelve machine-readable scenario contracts and an offline validator; `results.json` binds the contracts, cases and validator by SHA-256. The current synthetic receipt is **84/84 as expected**: twelve complete contracts are classified only as `protocol_ready_not_authorized`, never misrepresented as open; six removals are applied to every contract - responsibility, F2 data provenance, F4 failure record, F6 exit condition, H1 same place or H5 switchability - and all seventy-two negative cases are classified `blocked`. Any released negative case fails the whole proof. This proves only that the **protocol-validation logic closes**; it does not prove site safety, efficacy, compliance or approval. Reviewers can deliberately remove fields in the “AI Examination” tab of `visual/index.en.html`.
