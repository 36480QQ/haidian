...
由此形成五段生态链：基础研究与开源工具在原点社区被解释；模型、芯片、算力和安全能力在众智园被测试；企业服务与智能原生业态在大钟寺被体验；中关村科技服务翼提供法务、知识产权、资本和国际连接；小月河场景翼把城市问题与使用反馈送回研发端。每一次场景开放均配“问题、数据、责任人、人工复核、退出条件、公共回报”六栏护照 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:overall_spatial_structure]。

命名使用“京张共智环 / Jing-Zhang CoMind Loop”。标志方向为一枚未闭合双环：外环象征城市日常，内环象征研发系统，缺口形成交换门；禁止使用企业商标和未经授权字体。主色“铁路炭灰+公园青绿”，橙色只标识需要公众注意和人工确认的接口。
...
| 编号 | 概念项目 | 先决条件 | 公共验收问题 |
| --- | --- | --- | --- |
| C01 | 共智翻译主线 | 现状慢行与权属核查 | 是否连续、无障碍、夜间可辨？ |
| C02 | 众智园责任测试庭 | 河道、交通、能耗复核 | 失败和暂停是否同样可见？ |
| C03 | 原点开源译站 | 校园与园区协商 | 贡献是否可署名、匿名、撤回？ |
| C04 | 大钟寺城市体验厅 | 轨道与路口工程评估 | 测试是否被误读为采购承诺？ |
| C05 | 三座公共信息地标 | 文保与景观审查 | 内容是否比造型更重要？ |
| C06 | 十二场景接口 | 数据、安全、运营评估 | 是否保留人工和非数字路径？ |
| C07 | 共智项目护照平台 | 公开规则与维护主体 | 问题、版本、责任、退出是否齐全？ |
| C08 | 年度共智交接礼 | 活动许可与社区共创 | 是否公开失败、反馈和下一步？ |
...
内部复算得到：临时总体边界约 11.413 平方公里，概念绿地比例 6.4%，概念公共空间比例 0.4%，场景节点 12 个、概念更新项目 8 项。数值均与 `metrics.json` 和 GeoJSON 同源，不能替代官方面积与控规指标 [metric:site_area_sqm] [metric:green_ratio] [depth:metrics_recalculation]。
