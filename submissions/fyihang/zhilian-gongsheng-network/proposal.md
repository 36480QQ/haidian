...
本方案以高校知识生产、产业转化、社区日常服务和公共治理的协同为目标，形成“一链、三核、两翼、多点”的概念性网络。一链依托既有慢行、轨道接驳与蓝绿空间；三核分别为众智园低碳验证庭、AI原点社区共学生态院和大钟寺公共采用客厅；两翼为科技服务与场景赋能；多点优先利用既有公共设施、首层界面、闲置空间和绿地。所有空间动作均为概念建议，不新增法定边界或工程线位。[source:AGENT-TASKBOOK] [depth:overall_spatial_structure]

更新以少拆少建、透水铺装、雨水花园、树荫补植、可替换街道构件和低扰动施工为优先。每项设施均对应维护责任、巡检、降级、替换和反馈机制。控规强度、道路红线、产权、市政、消防、文保和能源条件待正式资料核实，不从临时边界或概念几何推定。[standard:MOHURD-URBAN-DESIGN-MEASURES] [source:SOURCE-REGISTRY]
