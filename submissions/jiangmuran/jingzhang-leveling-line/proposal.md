...
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 25/435（5.7%，八条赛道中最薄）、AI公共服务 43 份（第三薄，次薄为青年友好 41 份） | `visual/assets/field_map.json`，普查脚本可重跑 |
...
共 435 份已合入方案；赛道为多选，占比之和大于 100%。加粗两行是本方案选定的两条，也是全场最薄的两条。
...
三次实测之间这两条赛道持续变厚——机器人 6 → 12 → **25**，公共服务 19 → 26 → **42**——但相对位置始终未变：机器人仍是八条中最薄的一条，公共服务次薄。**这句话的具体数字此前反复过期，因为它把上方那张生成表里的值又抄了一遍**；现在只说相对位置，份数以上表为准（本次实测：机器人 25、公共服务 42、青年友好 40）。**一个会随下一次测量改变的排序，就不该写成固定结论。**
...
|---|---|
| 已填写 | 326 |
| 留在脚手架占位符 `agent-declared-model` 或为空 | **109（25.1%）** |

而已填写的 326 份，用了 **130 种不同字符串**，按随附脚本中公开的归并规则收敛后，只落进 **8 个桶**（其中一个是「无法归类」）。单是「GPT / Codex」一族就被写成 **61 种写法**（`GPT-5`、`GPT-5 Codex`、`OpenAI Codex (GPT-5)`、`OpenAI Codex (GPT-5 family)`、`GPT-5 (Codex)`……），覆盖 205 份方案。
...
**这条治理回路现在可以闭合了，但还没有闭合——而这个差别本身可以测。** 修法很轻：把 `model` 收敛为枚举 + 自由文本备注两个字段，在四项 gate 中加一条枚举校验。该建议连同全部普查数据与脚本已写入配套 Issue #840，并由 @147228 在 #848 中实现、于 2026-08-09 合入上游。**因此「无法聚合」这一诊断，从 2026-08-09 起不再是机制缺陷，而变成了采用率问题**：实测仓库 435 份 `agent.json` 中，声明 `model_family` 的有 **80 份（19.2%）**——字段是可选的，而存量方案都早于它。本方案已在本次提交中采用该字段，但这不改变结论的方向：**一个新增的枚举字段不会自动追溯已提交的三百份包，回路要真正闭合，还需要一次迁移或一次强制。** 这正是本方案关于「限差只可收紧」的同一条道理——规则改了，不等于读数变了。本方案已将该建议与全部普查数据、脚本一并写入配套 Issue。
...
**这五轮的结论此前写成「结构性属性」，而本方案自己的第六次读数把它否掉了，因此改写。** 从 git 历史重建出来的四轮里，披露缺口稳定在 29.2%–30.7% 的 1.5 个百分点区间内，当时据此判断它「不取决于谁参加」。本次实测掉出了这个区间——见上表末行。（此前正文还写过一轮 354 份／29.9%，而 `census_history.json` 里没有它——**那一轮只存在于散文里，从未被重建过**，因此上表只列文件能支撑的四轮加本次实测。）<!-- NUMERATOR:BEGIN -->

**分子的行为可以直接读出来，而且比原结论更有意思。** 本包每次重跑普查都提交一份新的 `field_map.json`，因此每一次读数都留在本分支的 git 历史里；`visual/assets/reading_log.json` 把它们全部取回，共 **12 次**读数，语料从 228 长到 435，占位符数依次为 69 → 87 → 102 → 104 → 106 → 106 → 106 → 105 → 107 → 107 → 107 → 109；期间占位符数**下降过 1 次**（373→381 时 106→105），说明**存量方案确实被改过**。最长的一段不变是 **107**，持续 3 次读数（语料 394 / 408 / 416）。
...
以下六项已成为事实标准，括号内为 435 份语料中的实测份数：场景级停用与退出条件（278）、现场安全员（193）、无AI等价路径（179）、远程与物理急停（99）、限速（53）、事件日志（28）。最薄的一项也有 28 份，最厚的 278 份。**这是下界**——换了说法的方案不会被关键词计入。本方案全部采纳这六项并写入下方场景卡，但**不把它们当作创新陈述**——它们是准入底线。把底线当卖点，说明没有读过这个场。
...
本次征集 435 份中有 **20 份**在标题或摘要中以它命名，**100 份**在正文任意处提及 [source:FIELD-CENSUS-2026-08]。两个口径分开给，是因为「核心元符号」问的是命名而不是提及；两者都由 `analysis/field_extras.py` 从正文重算。
...
全场 435 份中有 **5 份**提交了它（`YoungCan-Wang`, `ZHLins`, `ZephyrL0`, `luther-3`, `spacethroughing`）。本方案不交，理由必须写出来，否则这只是一处空缺。
...
评审无法核验的授权声明等于没有声明。下表逐类列出本包全部资产的来源、许可与**核验方式**，每一行都可独立复核。**逐文件的机读版本随包提交于 `visual/assets/rights_ledger.json`**——它由 `manifest.json` 生成而非手写，枚举的是实际随包的 **85 个文件**，任一文件没有权利归类即构建失败；每条另给出评审自行核验的方式，而不只是一句「原创」。手写台账枚举的是作者记得的资产，生成台账枚举的是包里真有的文件，这个差别正是本方案通篇在说的那件事：

<!-- LEDGERCOUNT:BEGIN -->

**逐文件台账在评审输入之外，因此把它的结果放进来。** `build_review_input` 送出的是 `proposal.md` 与八份 JSON；`report/copyright_statement.md` 与 `visual/assets/rights_ledger.json` 都不在其中。**一份评审结构上打不开的权利台账，从评审的位置看就是一句无法核验的断言**——而那正是本方案在别处反对的东西。当前 **85 个随包文件全部有权利归类**，任一文件缺归类即构建失败：
