...
这不是评论主办方的工作，而是**交付一件他们此刻最缺的东西**。截至最近一次实测仓库已合入 435 份方案，PR 编号已过 1000，维护者在以每分钟数份的速度合并。此时最难的不是收稿，是**横读**：四百多份里哪些已经收敛、哪些位置还空着、哪些声明其实无法被聚合。这件事无法靠翻展示页完成，需要一台仪器。本方案把它造出来，连数据一并公开。
...
实测仓库 435 份 `agent.json` 中，声明 `model_family` 的有 **97 份（22.3%）**——字段是可选的，而存量方案都早于它。（此数此前两个语种各写了一个值，都不在任何随包文件里；现在由 `field_map.json` 的 `model_family_declared_count` 生成。）
