...
**这一段现在是历史，因为它已经被修掉了。** 该发现写入配套 Issue #1058，并于 2026-08-11 关闭：修法不是扩充枚举，而是把 `role` 从封闭枚举改成开放 pattern （`^[a-z][a-z0-9_]{1,63}$`），规范角色集写进 description。本包因此从声明四条 schema 例外变为声明零条 role 例外（仅余一条 `data_confidence` 根属性）。**该次实测时**语料为 727 份，其中 **81 份**不满足，且这份 schema **依然没有任何 gate 执行**——被修掉的是枚举太窄，没有被修掉的是没人检查。而 `scripts/validate_submission.py` 校验的是「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**——仓库里没有任何一处做这件事。这就是一份公开契约可以被三分之一的语料长期违反而无人察觉的原因。
...
以下六项已成为事实标准，括号内为 793 份语料中的实测份数：场景级停用与退出条件（478）、无AI等价路径（361）、现场安全员（325）、远程与物理急停（171）、限速（97）、事件日志（66）。最薄的一项也有 66 份，最厚的 478 份。**这是下界**——换了说法的方案不会被关键词计入。本方案全部采纳这六项并写入下方场景卡，但**不把它们当作创新陈述**——它们是准入底线。把底线当卖点，说明没有读过这个场。
...
本次征集 793 份中有 **55 份**在标题或摘要中以它命名，**202 份**在正文任意处提及 [source:FIELD-CENSUS-2026-08]。两个口径分开给，是因为「核心元符号」问的是命名而不是提及；两者都由 `analysis/field_extras.py` 从正文重算。
...
**同一条标准指向本包的闸门本身。** 十五道闸门此前只被观察到通过，而一道从不失败的闸门与一道不会失败的闸门，产生的证据完全一样。因此本方案人为损坏自己的包，每次损坏都要求对应的闸门报错并**指名**所损坏的东西：结果随包提交于 `visual/assets/gate_mutation.json`，十五道中十二道已被证明会失败，另三道连同理由一并写明。同一目录下的 `visual/assets/claim_audit.json` 记录两版正文里带单位的量是否逐条对齐，`visual/assets/agent_declarations.json` 记录全场语料的机器可读声明状况。**须说准：`scripts/review_submission.py` 不读 `visual/assets/`**——这三份文件不进评审模型的上下文，因此这里指名它们，是拿到仓库的人唯一能知道它们存在的途径。。**
...
当前 793 份中，**8 份**随包提交了独立的勘误文件——本方案，以及 `147228`, `LShengYi`, `dingle2001`, `felix-lhz`, `lqqk7`, `wocaonimaworinixi-collab`, `youngkermit8-coder`；另有 **12 份**在正文里出现「自查发现并修正」一类表述。这个数字在本方案首次写下那句话时是 1（只有本方案），现在不是了。
