...
几何水准不因为仪器报了一个高程就相信它。它走一圈回到起点，把同一个点读两次，两次之差叫**闭合差**——那是关于这台仪器可信到什么程度的唯一诚实说法。本方案通篇主张一座城市应当公开这个数。

**本包的打分过程意外地构成了这样一个闭合环，因此本节把仪器对准打分本身。**

PR #1122 得 **94/100**。十七分钟后，PR #1125 得 **91/100**。两次之间，评审能读到的东西没有变。

`scripts/review_submission.py` 的 `build_review_input` 逐条写明哪些文件进入模型：`proposal.md`、八份 JSON、五张固定文件名的图、每份 PDF 的前三页、两个页面的首屏截图，以及 `self_check_submission.py` 子进程的输出。按这份清单逐文件比对 git blob 哈希：`proposal.md` **逐字节相同**，`metrics.json`、`assumptions.json`、`sources.json`、`self_check.json`、三份矩阵、五张固定图、四份 PDF、两个被截屏的页面及其引用的全部图片，**全部相同**；`self_check_submission.py` 的输出实测多次运行逐字节相同。

**说精确一点：不是全套逐字节相同。** 唯一有差异的评审可见文件是 `manifest.json`，它差在 **2 个 `sha256` 字符串**上——`changelog.md` 与 `visual/assets/review_gate_survey.json` 的摘要，而这两份文件都不在评审输入里。manifest 的其余部分（每条 `path`、`role`、`required`、`language` 与全部顶层字段）逐项相同。把这一步做成检查而不是一句「差不多一样」，是因为省掉它正是本包勘误册记了八次的那个形状。

**于是：仪器把同一个对象读了两次，返回 94 和 91。差 3 分。**

**这里有一个对本方案有利的读法，本节明确不采用。** 这一对不说明那个 91 判错了；它说明两次读数相差 3，而那个 94 同样可能是宽松的一次。**本方案不申请重评、不申请任何调整**——一份要求保留自己两次读数里较高那次的包，做的正是本方案通篇反对的事。

它支持的结论比这窄，而且这句话对本方案同样不利：**两份方案之间 3 分的差距落在这台仪器自身的重复性以内，把它当成质量差别是过度解读读数。** 本包目前正处在若干个这样的差距的有利一侧。

**这不是对评审的批评。** 一台会重复的仪器不存在；水准仪也不存在，所以才要走闭合环。可批评的从来不是读数有偏差，而是**读数的偏差没有被公开**——这正是本方案对这条创新带提出的全部主张，此刻恰好落回它自己身上。

限制写在随包 JSON 的 `limits_zh` 里：两次都走 approve 路径，因此只有加权总分可观测，七维中哪一维动了无从得知；n 很小，这里测的是一次不一致而不是一个分布；也无法从外部证明两次用的是同一个模型版本与同一份 prompt。全部比对随包提交于 `visual/assets/review_repeatability.json`，`report` 完全离线，任何人可以自己重跑这次比对。
