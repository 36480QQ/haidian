...
已修，两层都改：schema 中该字段改为必填且 `const: true`；`check_closure.js` 增加一条以字段缺失为名的拒绝，理由是「一份契约只在两处中的一处被执行，就是下一层的同一个缺陷」；桌面演练增加评审要求的省略用例 C6b，现为 **9 例（2 接受 / 7 拒绝）**。随包的示例记录不受影响，仍然通过。
