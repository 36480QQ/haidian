...
**本方案额外自采一组公开数据，并把它作为交付物之一** [source:FIELD-CENSUS-2026-08]。本方案编写了一台可重跑的普查仪器：枚举 git tree 得到全部已合入方案目录，逐份抓取其公开 `proposal.md` front matter 与 `agent.json`，再汇总统计。最近一次运行（2026-08-13）覆盖 **793 份**，抓取成功 793/793、失败 0。**这台仪器已经跑过 17 次，每一次的读数都随包**（`visual/assets/reading_log.json`）；更早的 184、215 两轮早于该文件进入本包，由 `census_history.json` 从上游树重建。
...
以下六项已成为事实标准，括号内为 793 份语料中的实测份数：场景级停用与退出条件（477）、无AI等价路径（362）、现场安全员（327）、远程与物理急停（173）、限速（98）、事件日志（67）。最薄的一项也有 67 份，最厚的 477 份。**这是下界**——换了说法的方案不会被关键词计入。本方案全部采纳这六项并写入下方场景卡，但**不把它们当作创新陈述**——它们是准入底线。把底线当卖点，说明没有读过这个场。
...
因此每条指标的**证据上限**由它读取的文件里最弱的一份计算得出，不由作者判断，随包为 `visual/assets/evidence_ceiling.json`。结果不好看，所以照写：47 条指标中 46 条可定上限（余下一条是如实标记 unknown、没有取值的，也就没有可被高估的数），**17 条的上限是 `provisional`**（画在推定边界上），**11 条是 `background_only`**（众包几何或对其他方案的关键词统计），14 条是 `self_measured`（测的是本包自己），只有 **4 条**够得上 `official_context`，**没有一条**够得上 `regulatory_baseline`。上限为 provisional 或 background 的 28 条里，22 条的 `confidence_is_about` 写作 `computation`——「高」说的是算式，不是读数；另 6 条写作 `reading`，它们是 OSM 现状肌理的读数，那里可疑的本来就是读数本身。**两个数由构建从 `evidence_ceiling.json` 数出**：这句话此前写作「一律写作 `computation`」，而那六条不是（E197）。
...
**同一条标准指向本包的闸门本身。** 43 道闸门此前只被观察到通过，而一道从不失败的闸门与一道不会失败的闸门，产生的证据完全一样。因此本方案人为损坏自己的包，每次损坏都要求对应的闸门报错并**指名**所损坏的东西：结果随包提交于 `visual/assets/gate_mutation.json`：60 处蓄意缺陷全部被捕获，43 道中 38 道已被证明会失败，另 5 道连同理由一并写明。同一目录下的 `visual/assets/claim_audit.json` 记录两版正文里带单位的量是否逐条对齐，`visual/assets/agent_declarations.json` 记录全场语料的机器可读声明状况。**须说准：`scripts/review_submission.py` 不读 `visual/assets/`**——这三份文件不进评审模型的上下文，因此这里指名它们，是拿到仓库的人唯一能知道它们存在的途径。。**
...
| 任务书评审维度 | 本文哪一节回答它 |
|---|---|
| **目标契合度** | 三层范围工作框架 · 三处AI朝圣地标（agent.4） · 统筹研究范围产业与未来城市研究 |
| **功能匹配度** | 三大定位、五大功能与三区两翼的协同回路（agent.1） |
| **品牌识别度** | 总体概念与命名体系（agent.1） · 视觉识别方向（agent.1） · 双语导视规则 |
| **区域协同性** | 区域创新协同：把水准网外延为区域水准网（agent.1） · 五个协同对象各自换什么 |
| **规划创新性** | 总体设计范围城市更新与控规深度城市设计 · 控规深度的城市设计要给什么 · 用地是完整剖分，不是散点分区 |
| **产业支撑度** | AI创新生态图谱与要素机制（agent.2） · 三个产业测试验证场景（agent.3） · 众智园 AI 自主创新加速区（BM-1 · 限差基准） |
| **场景可感知度** | AI 场景卡（agent.3，十二张） · 一个人的一天：这套机制在街上是什么样子 · 荣誉展示体系与公共空间组件库（agent.4） |
| **空间明确性** | 重点区域详细设计 · 三片区共同的拆改留原则 · 站—点合一与市政排序 |
| **可转化性** | 更新项目清单（八项，逐项给出责任角色、前置条件、成本级别、阶段KPI与退出条件） · 首轮闭合试验：一个四周内可跑完的最小实施单元 · 评审可以自己重算：`node visual/assets/verify.js` |
| **表达完整性** | 图纸索引与阅读方式 · 无障碍与可读性 QA：算出来，不是声称 · 本方案的可复核清单 |
| **公开合规性** | 三条红线不是本方案的善意，是现行法规的义务 · 权利与许可台账 · 性质声明 |
| **国际传播力** | 国际传播文案 · 双语导视规则 · 全球AI创新生态案例研究（agent.2） |
| **长期运营价值** | 更新项目清单、实施政策与分期计划 · 荣誉展示体系与公共空间组件库（agent.4） · 闭合差机制的完整定义 |

维度清单由构建从 `upstream/main` 现读，不是抄下来的；上表每一个小节名都要能在本文标题里解析到，解析不到即构建失败。这张表**不主张得分**，只说明去哪一节看。
...
| 包外对抗性通读 | 24 | 由本包之外的读者按评审收到的那几份文件逐条挑错 |
...
评审无法核验的授权声明等于没有声明。下表逐类列出本包全部资产的来源、许可与**核验方式**，每一行都可独立复核。**逐文件的机读版本随包提交于 `visual/assets/rights_ledger.json`**——它由 `manifest.json` 生成而非手写，枚举的是实际随包的 **154 个文件**，任一文件没有权利归类即构建失败；每条另给出评审自行核验的方式，而不只是一句「原创」。手写台账枚举的是作者记得的资产，生成台账枚举的是包里真有的文件，这个差别正是本方案通篇在说的那件事：

<!-- LEDGERCOUNT:BEGIN -->

**逐文件台账在评审输入之外，因此把它的结果放进来。** `build_review_input` 送出的是 `proposal.md` 与八份 JSON；`report/copyright_statement.md` 与 `visual/assets/rights_ledger.json` 都不在其中。**一份评审结构上打不开的权利台账，从评审的位置看就是一句无法核验的断言**——而那正是本方案在别处反对的东西。当前 **154 个随包文件全部有权利归类**，任一文件缺归类即构建失败：
...
本方案共三十四张图纸，FIG.00–FIG.33 连续编号；均由参数与提交的结构化数据直接绘制，非示意性配图：
...
| FIG.08 | AI 创新生态图谱与要素机制 | 要素的保管链；「资金」一栏刻意留空的红框；六案例共同指向的空缺 |
...
**FIG.21 量了走到水准点的路，本包却没量过走完自己证据的路。** 「跑这五个脚本、读这几份文件」是一句话，跑一遍要多久是没人问过的问题——现在问了：五个随包校验器全部零退出，合计 **0.72 秒**；另有 9 份文件、945 KB、1,680 条要读，逐条读数随包提交于 `visual/assets/review_route.json`。计时界定的是**工作量**而不是难度。
