...
<!-- OPSVOCAB:BEGIN -->
**再往外一层：把这件事**运行**起来所用的词。** 公告 1.3「征集目的」要「全生命周期的空间供给与运营策略」，任务书要场景-空间-运营映射、运营团队、招引转化、活动品牌与传播视觉系统——**这七个词在本方案里出现过零次，而它们指的事本方案大都做了，只是用自己的词说的**。左列逐词读自公告或任务书 @ `upstream/main` 并每次构建对照；做不到的，写明做不到和为什么。

| 用词 | 出处 | 本方案的回答 |
|---|---|---|
| **全生命周期** | 公告 1.3 | 公告 1.3 要「全生命周期的空间供给与运营策略」。本方案的全生命周期是四条规则：F1 准入试验决定能不能开；运行期按等级复测；**闭合差超限，整条路线退回，服务降为它的非 AI 等价物**；恢复须连续两个周期合格、原因公布、四方同意。生命周期的终点在本方案里是写死的，不是留白的。 |
| **运营策略** | 公告 1.3 | 同一句话里的第二个词。本方案的运营策略不是节庆日历，是复测日历：月度社区复测日（三等点）、季度场景开放日（二等点）、半年度路线复测（一等路线）、年度归零仪式。**每一次活动同时是一次读数**，因此活动办不下去时读数会先停，而不是读数停了活动照办。 |
| **场景-空间-运营** | 任务书 | 任务书要「场景-空间-运营映射」。本方案的十二张场景卡逐张绑定一个测点等级（空间）与一个复测周期加主导岗位（运营）：三等点—月度—P4/P5/P7，二等点—季度—P2/P3，一等路线—半年度—具资质机构。**三列是同一张表的三列，不是三节文字。** |
| **招引转化** | 任务书 | 任务书要「国际传播和招引转化机制」。本方案给出的转化路径是参与复测 → 提出场景 → 进入测试场 → 取得水准合格 → 落地运营，每一步的通过条件都是读数而不是评议。**同一条任务书禁止把招商写成确定承诺**，因此这里给的是通道，不是名单，也不是投资额。 |
| **运营团队** | 任务书 | 任务书的可转化性一条问的是能否被专业团队、运营团队或传播团队继续深化。本方案对此的答复是岗位规格那一节：**谁可以决定什么，以及缺岗时什么不许开**。接手的团队不需要读完全文，读岗位规格就知道自己被授权到哪里为止。 |
| **活动品牌** | 任务书 | 任务书要活动品牌，同一条又禁止「只写宣传口号而没有运营机制」。**本方案不取品牌名**，理由写在这里：一个背后没有可重复动作的品牌就是口号。承担品牌的是四个可重复的动作本身——社区复测日、场景开放日、路线复测、归零仪式；取名属责任主体的决定，本方案不代拟。 |
| **传播视觉** | 任务书 | 任务书要传播视觉系统。本方案能负责的那一部分是可复算的那一部分：导视编号语法、**编号永不翻译**（`BM-0` 在任何语言里都是 `BM-0`）、以及空间不足时先压说明后压英文、绝不压编号与读数的取舍次序。标志、色彩与字体属需要授权的资产，本方案不出具——**任务书同一条禁止未经授权使用字体与图片**。 |
<!-- OPSVOCAB:END -->
