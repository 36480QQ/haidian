...
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 21/371（5.1%，八条赛道中最薄）、AI公共服务 32 份（与青年友好并列次薄） | `visual/assets/field_map.json`，普查脚本可重跑 |
...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（最近一次实测：371 份已合入方案中 241 份申报治理赛道，「证据链/可复算」类表述出现在 31.1% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在覆盖最薄的赛道上：`robotics-autonomous-mobility` 按标签计 354 份中仅 **21 份（5.7%）**，是八条赛道里最薄的一条，而次薄位由青年友好公共空间与 AI 公共服务并列占据，各 32 份；`ai-public-services` **30 份（8.9%）**，仅次于它。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
`ai-traffic-walkability` 251 份（67.7%）、`civic-agent-governance` 230 份（65.0%）、`enterprise-services-ecosystem` 217 份（58.5%）；而 `robotics-autonomous-mobility` 仅 **21 份（5.7%）**，`youth-friendly-public-space` 与 `ai-public-services` 并列次薄，各 **34 份（9.2%）**。
...
|---|---|
| 已填写 | 265 |
| 留在脚手架占位符 `agent-declared-model` 或为空 | **106（28.6%）** |
| 合计 | 354 |

而已填写的 265 份，用了 **117 种不同字符串**，按随附脚本中公开的归并规则收敛后，只落进 **8 个桶**（其中一个是「无法归类」）。单是 GPT/Codex 一族就被写成 **55 种写法**（`GPT-5`、`GPT-5 Codex`、`OpenAI Codex (GPT-5)`、`OpenAI Codex (GPT-5 family)`、`GPT-5 (Codex)`……），覆盖 166 份方案。
...
**这条治理回路现在可以闭合了，但还没有闭合——而这个差别本身可以测。** 修法很轻：把 `model` 收敛为枚举 + 自由文本备注两个字段，在四项 gate 中加一条枚举校验。该建议连同全部普查数据与脚本已写入配套 Issue #840，并由 @147228 在 #848 中实现、于 2026-08-09 合入上游。**因此「无法聚合」这一诊断，从 2026-08-09 起不再是机制缺陷，而变成了采用率问题**：实测仓库 371 份 `agent.json` 中，声明 `model_family` 的有 **23 份（6.6%）**——字段是可选的，而存量方案都早于它。本方案已在本次提交中采用该字段，但这不改变结论的方向：**一个新增的枚举字段不会自动追溯已提交的三百份包，回路要真正闭合，还需要一次迁移或一次强制。** 这正是本方案关于「限差只可收紧」的同一条道理——规则改了，不等于读数变了。本方案已将该建议与全部普查数据、脚本一并写入配套 Issue。

#### 第二个可测的机制缺口：一份没人执行的清单契约

`model_family` 那条是「字段有了、采用率低」。用同一台仪器再测一遍，找到的是另一种形态：**一份已发布的契约，没有任何 gate 执行它。**

仓库在 `brief/site-package/schemas/manifest.schema.json` 发布了清单 schema，每份方案都提交 `manifest.json`。把二者对上，结果是 **371 份已合入方案中有 117 份（34.6%，测于 338 份时）不满足这份 schema**。而 `scripts/validate_submission.py` 校验的是「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**——仓库里没有任何一处做这件事。这就是一份公开契约可以被三分之一的语料长期违反而无人察觉的原因。
...
按本方案自己的规矩，在为这个差别写下任何一句话之前先去量。结果是：全场 **366 份**方案中有 **20 份**提交了 `spatial.json`，合计 **231 个**概念对象，三档分布为 **public 56 / cleared 2 / provisional 173**。**13 份**包里每一条都是 provisional，**6 份**至少有一条 public。本包 14 条全部 provisional，按警告条数排在 **第 3 位／共 20 份**。
...
在全部 **839 条**已发布评分中，**820 个**不同的 head commit 里有 **12 个**被评审了不止一次，涉及 **31 条**读数。
...
限制逐条写在随包 JSON 的 `limits_zh` 里，其中最重要的一条对本节结论不利：同一 commit 的重复评审有可能命中队列缓存而根本没有调用模型，那会返回完全相同的分数——因此「完全一致」的比例是上界，上面这个标准差是**下界**。全部分组、每次读数与统计口径随包提交于 `visual/assets/review_repeatability.json`，`report` 完全离线，任何人可以自己重跑。
...
评审无法核验的授权声明等于没有声明。下表逐类列出本包全部资产的来源、许可与**核验方式**，每一行都可独立复核。**逐文件的机读版本随包提交于 `visual/assets/rights_ledger.json`**——它由 `manifest.json` 生成而非手写，枚举的是实际随包的 **83 个文件**，任一文件没有权利归类即构建失败；每条另给出评审自行核验的方式，而不只是一句「原创」。手写台账枚举的是作者记得的资产，生成台账枚举的是包里真有的文件，这个差别正是本方案通篇在说的那件事：

<!-- LEDGERCOUNT:BEGIN -->

**逐文件台账在评审输入之外，因此把它的结果放进来。** `build_review_input` 送出的是 `proposal.md` 与八份 JSON；`report/copyright_statement.md` 与 `visual/assets/rights_ledger.json` 都不在其中。**一份评审结构上打不开的权利台账，从评审的位置看就是一句无法核验的断言**——而那正是本方案在别处反对的东西。当前 **83 个随包文件全部有权利归类**，任一文件缺归类即构建失败：
