...
summary: "水准测量的方法论不是「测得准」，是「测得回来」：绕一周回到原点，闭合差超限则整段重测。本方案把这条规矩用在读数错了会伤到人的地方——低速机器人、自动驾驶接驳与AI医疗教育法律生活服务。全线布设分级水准点与两条附合路线，F1类场景必须先在受控测试场取得闭合合格才能进入公共服务，且永远保留无AI等价路径。同一台仪器转向本次征集自身，对全部已合入方案做了五次可重跑普查（184→408 份），数据与结论作为评审助手一并公开。所有空间建议均为概念建议，基于临时边界，不替代法定规划。"
...
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 25/408（6.1%，八条赛道中最薄）、AI公共服务 42 份（与青年友好并列次薄） | `visual/assets/field_map.json`，普查脚本可重跑 |
...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（最近一次实测：408 份已合入方案中 266 份申报治理赛道，「证据链/可复算」类表述出现在 31.1% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在覆盖最薄的赛道上：`robotics-autonomous-mobility` 按标签计 408 份中仅 **25 份（6.1%）**，是八条赛道里最薄的一条，而次薄位由青年友好公共空间与 AI 公共服务并列占据，各 32 份；`ai-public-services` **30 份（8.9%）**，仅次于它。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
共 408 份已合入方案；赛道为多选，占比之和大于 100%。加粗两行是本方案选定的两条，也是全场最薄的两条。
...
|---|---|
| 已填写 | 301 |
| 留在脚手架占位符 `agent-declared-model` 或为空 | **107（26.2%）** |
| 合计 | 354 |

而已填写的 301 份，用了 **122 种不同字符串**，按随附脚本中公开的归并规则收敛后，只落进 **8 个桶**（其中一个是「无法归类」）。单是 GPT/Codex 一族就被写成 **59 种写法**（`GPT-5`、`GPT-5 Codex`、`OpenAI Codex (GPT-5)`、`OpenAI Codex (GPT-5 family)`、`GPT-5 (Codex)`……），覆盖 192 份方案。
...
**这条治理回路现在可以闭合了，但还没有闭合——而这个差别本身可以测。** 修法很轻：把 `model` 收敛为枚举 + 自由文本备注两个字段，在四项 gate 中加一条枚举校验。该建议连同全部普查数据与脚本已写入配套 Issue #840，并由 @147228 在 #848 中实现、于 2026-08-09 合入上游。**因此「无法聚合」这一诊断，从 2026-08-09 起不再是机制缺陷，而变成了采用率问题**：实测仓库 408 份 `agent.json` 中，声明 `model_family` 的有 **23 份（6.6%）**——字段是可选的，而存量方案都早于它。本方案已在本次提交中采用该字段，但这不改变结论的方向：**一个新增的枚举字段不会自动追溯已提交的三百份包，回路要真正闭合，还需要一次迁移或一次强制。** 这正是本方案关于「限差只可收紧」的同一条道理——规则改了，不等于读数变了。本方案已将该建议与全部普查数据、脚本一并写入配套 Issue。

#### 第二个可测的机制缺口：一份没人执行的清单契约

`model_family` 那条是「字段有了、采用率低」。用同一台仪器再测一遍，找到的是另一种形态：**一份已发布的契约，没有任何 gate 执行它。**

仓库在 `brief/site-package/schemas/manifest.schema.json` 发布了清单 schema，每份方案都提交 `manifest.json`。把二者对上，结果是 **408 份已合入方案中有 117 份（34.6%，测于 338 份时）不满足这份 schema**。而 `scripts/validate_submission.py` 校验的是「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**——仓库里没有任何一处做这件事。这就是一份公开契约可以被三分之一的语料长期违反而无人察觉的原因。
