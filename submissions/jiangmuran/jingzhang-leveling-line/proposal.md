...
**而「京张遗址公园」这个名字，此前在本文里出现零次。** 本方案称它「实测公园」「遗址公园」，并且只在一个位置上处理过它：作为与本方案推定主脊相差 **1,116.7 m** 的那个误差对象。公告第五项设计要求用的也是这套词——「结合公园已实施区域和原设计方案……塑造更具创新活力和特色魅力的**京张遗址公园活力带**」——这一整项的关键词同样是零（E-PEND3）。
...
**同一条标准指向本包的闸门本身。** 60 道闸门此前只被观察到通过，而一道从不失败的闸门与一道不会失败的闸门，产生的证据完全一样。因此本方案人为损坏自己的包，每次损坏都要求对应的闸门报错并**指名**所损坏的东西：结果随包提交于 `visual/assets/gate_mutation.json`：87 处蓄意缺陷全部被捕获，60 道中 57 道已被证明会失败，另 3 道连同理由一并写明。同一目录下的 `visual/assets/claim_audit.json` 记录两版正文里带单位的量是否逐条对齐，`visual/assets/agent_declarations.json` 记录全场语料的机器可读声明状况。**须说准：`scripts/review_submission.py` 不读 `visual/assets/`**——这三份文件不进评审模型的上下文，因此这里指名它们，是拿到仓库的人唯一能知道它们存在的途径。。**
...
任务书规定了「三区两翼」，因此半数以上方案画出同一骨架：不是共识，是题面所致。**把同一结构再画一遍没有增量。** 有增量的是说清这些单元之间靠什么机制传递责任——这正是水准网回答的问题。
...
**这一段现在是历史，因为它已经被修掉了。** 该发现写入配套 Issue #1058，2026-08-11 关闭：修法不是扩充枚举，而是把 `role` 从封闭枚举改成开放 pattern（`^[a-z][a-z0-9_]{1,63}$`）。本包因此从四条 schema 例外变为零条 role 例外（仅余 `data_confidence` 根属性）。**该次实测时**语料 825 份，其中 **79 份**不满足，且这份 schema **依然没有任何 gate 执行**——被修掉的是枚举太窄，没被修掉的是没人检查。`scripts/validate_submission.py` 只校验「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**。这就是一份公开契约可以被三分之一的语料长期违反而无人察觉的原因。
...
**本方案当时也在这 117 份里，不作豁免。** 本包当时有 21 处违规、5 种类型，同一原因：changelog、校验脚本、证据数据在枚举里都没有对应项。把 changelog 写成 `narrative` 只为通过一份没人执行的校验，是对机器说假话。做法是**把违规声明出来并加一道自己的闸**：`visual/assets/manifest_schema_survey.json` 逐条列出本包的 5 种违规，`analysis/build_all.py` 每次构建重新校验（`analysis/` 下的脚本只用于说明出处，**均不在提交包内**），出现清单以外的新违规即构建失败。**已声明的例外不能给新的意外提供藏身处。**
...
必须把话说准：占位符未改**不等于**作者隐瞒模型——相当多作者在 `authorName` 或正文里写明了；写法离散也不是谁的错，是字段没给枚举。母题识别基于中文关键词正则，占比均为**下界**。**这条规矩用在本方案自己身上，产出了比任何单次快照都更强的结论。** 五次独立复测（前四次此前只存在于 `changelog.md`，即只存在于作者的说法里；现已从 git 历史逐轮重建并随包提交为 `visual/assets/census_history.json`——**本方案自己的规矩不给作者留例外**）：
...
**重建结果值得单说，因为它不是四行全对。** 按「树内目录数恰好相等的最新提交」逐轮重算：215、228、298 三轮与留档值**完全一致**（0.307 / 0.303 / 0.292）；最早的 184 一轮重建为 **0.304**，留档 0.299，差**一份包**。差值不作调和，原因写在 `census_history.json` 的 `method` 里：重建取的是目录数相等的最新提交，与当初取数的那一刻不必然同时。**三轮精确重合、一轮差一份**，比四行都对上更能说明这套重建是在算，不是在抄。

**这五轮的结论此前写成「结构性属性」，而本方案自己的下一次读数把它否掉了，因此改写。** 从 git 历史重建出来的四轮里，披露缺口稳定在 29.2%–30.7% 的 1.5 个百分点区间内，当时据此判断它「不取决于谁参加」；本次实测掉出了这个区间——见上表末行。（此前正文还写过一轮 354 份／29.9%，而 `census_history.json` 里没有它——**那一轮只存在于散文里，从未被重建过**。）<!-- NUMERATOR:BEGIN -->
...
需要把话说准的三点。**第一，上述 PR 均由他人实现，本方案只提供了测量、脚本与问题定位。** **第二，#848、#850 与 #861 已于 2026-08-09 合入上游**：`model_family`（枚举 8 值）与 `model_detail` 现已是仓库规则；#850 把本方案的 OSM 交叉核验写入仓库自己的 `provisional_boundaries_basis.md`，载入 17.49 ha 与 412.5 m 两个读数；#861 修复了本方案在 Issue #858 报告的校验时序竞态，**并因此解除了本方案自己「有运行排队时不得推送」的操作纪律**。仅 #807 尚未合入。#850 明确写明 OSM 结果「未作为中央可重放来源」，因此 `background_only` 定级**保持不变**——被记录不等于被登记。上游枚举定为 8 值，本方案独立归并实测得到 9 个桶，该次修复后为 **8 值对 9 桶**，此前这句写成「一致」，是把相互印证说得比数据更漂亮。**第三，展示索引滞后的起伏，本方案不归功于自己**——无法证明因果。
...
OSM 原始坐标与全部口径随包提交于 `visual/assets/osm_reference.json`，并由随包的 `node visual/assets/check_osm.js` 从这些坐标独立复算（抓取脚本无法进包，入库格式白名单不含 .py） [source:OSM-REFERENCE-2026-08]。这一节的全部内容画在下图里——**推定边界、实测公园、本方案落位与那 412.5 m 的红线在同一张图上按比例呈现**。文字里的差异是断言，按比例画在自己的后果旁边才是证据。
...
**一、这条已撤回，而撤回本身是这一节最该读的部分。** 本方案最初写的是：#957 的匹配器要求带冒号前缀，而 1,203 条历史 action 匹配数为 0，因此它是空集。发表两小时后，#950 的作者 @Sonike 指出这个推断不成立——**那 1,203 条全部产生于旧 prompt，而旧 prompt 从未要求该前缀；在一个不要求它的语料里统计它的出现率，结果必然是 0。** #957 同时改了 prompt，他在新 prompt 下实测 5 次采样中 4 次输出前缀。

**这与本方案刚刚指出他的错误是同一个形状**：拿一个测量去回答它回答不了的问题。他的八份样本证明「这八份里组织方项都在」，证不出「组织方项是原因」；本方案的 1,203 条证明「旧 prompt 下没人写这个前缀」，证不出「新 prompt 下匹配器不会命中」。**本方案用了不到两小时犯了同一个错。**
...
**这处发现的形状正是本方案通篇在讲的那件事**：#950 与 #957 都是认真的工作，八份样本的诊断在八份里都成立——**差别不在认真程度，在 n**。一次读数看不出系统性偏差，全量能。本方案主张的因此不是「更谨慎地判断」，而是「把判断建立在可被别人重跑的读数上」：全部计数、归类规则与匹配字符串随包提交于 `visual/assets/review_gate_survey.json`，任何人可以改了规则重跑，包括改到与本方案结论相反。
...
**还有一件在图表上说不清的。** `assets/media/leveling-year.mp3` 是 48 秒的数据发声（无语音、无音乐素材、非现场录音）：全年 214 小时复测工时按月排开，每一次脉冲代表 2.5 小时。前半段所有周期同月起算，峰值 83.5 小时；后半段把三个年度点分开，峰值降到 31.5 小时——**工作量一次没变**。FIG.30 把这件事画成两组柱子，而柱子要求你比较高度；**声音不要求你判断任何东西——第一个月一到，你就知道数不过来**。**还有一件只有页面能做的。** `visual/index.html` 里有一块可交互画布：把三个年度点搬到别的月份，峰值月立刻重算。其中的 `year_explorer.js` 是 `fig_year.py` 排布函数在另一种语言里的**第二次实现**，模型随包提交为 `year_model.json`，闸门在 10 个排布上逐一比对。**图纸只能被相信，这块画布可以被反驳**——11,880 种排布已在 Python 里搜过，读者能到达的每一个都不优于已发布的 31.5 h。**列出它们不是清单癖：一件没有被正文指名的成品，等于没有人被告知它存在。**
...
**FIG.21 量了走到水准点的路，本包却没量过走完自己证据的路。** 「跑这五个脚本、读这几份文件」是一句话，跑一遍要多久是没人问过的问题——现在问了：五个随包校验器全部零退出，合计 **0.72 秒**；另有 9 份文件、1023 KB、1,732 条要读，逐条读数随包提交于 `visual/assets/review_route.json`。计时界定的是**工作量**而不是难度。
