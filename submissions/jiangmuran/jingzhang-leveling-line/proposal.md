...
**分子的行为可以直接读出来，而且比原结论更有意思。** 本包每次重跑普查都提交一份新的 `field_map.json`，因此每一次读数都留在本分支的 git 历史里；`visual/assets/reading_log.json` 把它们全部取回，共 **11 次**读数，语料从 228 长到 416，占位符数依次为 69 → 87 → 102 → 104 → 106 → 106 → 106 → 105 → 107 → 107 → 107；期间占位符数**下降过 1 次**（373→381 时 106→105），说明**存量方案确实被改过**。最长的一段不变是 **107**，持续 3 次读数（语料 394 / 408 / 416）。
