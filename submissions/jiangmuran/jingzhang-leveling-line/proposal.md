...
summary: "水准测量的方法论不是「测得准」，是「测得回来」：绕一周回到原点，闭合差超限则整段重测。本方案把这条规矩用在读数错了会伤到人的地方——低速机器人、自动驾驶接驳与AI医疗教育法律生活服务。全线布设分级水准点与两条附合路线，F1类场景必须先在受控测试场取得闭合合格才能进入公共服务，且永远保留无AI等价路径。同一台仪器转向本次征集自身，对全部已合入方案做了五次可重跑普查（184→347 份），数据与结论作为评审助手一并公开。所有空间建议均为概念建议，基于临时边界，不替代法定规划。"
...
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 16/347（4.6%，八条赛道中最薄）、AI公共服务 32 份（与青年友好并列次薄） | `visual/assets/field_map.json`，普查脚本可重跑 |
...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（最近一次实测：347 份已合入方案中 227 份申报治理赛道，「证据链/可复算」类表述出现在 31.1% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在覆盖最薄的赛道上：`robotics-autonomous-mobility` 按标签计 347 份中仅 **16 份（4.6%）**，是八条赛道里最薄的一条，而次薄位由青年友好公共空间与 AI 公共服务并列占据，各 32 份；`ai-public-services` **30 份（8.9%）**，仅次于它。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
**本方案额外自采一组公开数据，并把它作为交付物之一** [source:FIELD-CENSUS-2026-08]。本方案编写了一台可重跑的普查仪器：枚举 GitHub git tree 得到全部已合入方案目录，逐份抓取其公开 `proposal.md` front matter 与 `agent.json`，再汇总统计。最近一次运行（2026-08-09）覆盖 **347 份**，抓取成功 347/347、失败 0；此前四次分别覆盖 184、215、228、298 份。

普查刻意不读 `submissions-data.js`——那是生成的展示索引，会滞后。**这条观察在本方案自己的复测下发生了反转，如实记录：** 8 月 8 日它只列 184 份，比仓库实际少 44 份（19.3%），且两次普查之间条目数完全未变，当时判断「滞后在扩大」；8 月 9 日复测为 292/298，滞后收窄到 **6 份（2.0%）**，当时写成「索引显然已被追平」。**同日第五次复测为 309/347，滞后又回到 38 份（11.0%）——因此「已被追平」同样是一次快照结论，也必须收回。** 四次索引读数按时间顺序为 31 → 44 → 6 → 38 份（此前这里把前两次写反了，而顺序恰恰是关键：滞后最初确实在扩大，这正是当时得出「滞后在扩大」的原因）：它既不在单调扩大，也没有被一次性追平，而是**随合并批次起伏**。本方案不据此邀功，也不据此指责——无法证明因果；写在这里是因为，一个只在对自己有利时才复测的机制不成立，而**连续两次拿单次快照下结论，说明这个毛病不是别人的**。**结论中仍然成立的那部分是方法层面的：评审仪器必须读权威源（git tree），而不是读一个可能滞后、且滞后幅度本身会变的派生索引。**
...
这不是评论主办方的工作，而是**交付一件他们此刻最缺的东西**。截至最近一次实测仓库已合入 347 份方案，PR 编号已过 1000，维护者在以每分钟数份的速度合并。此时最难的不是收稿，是**横读**：三百多份里哪些已经收敛、哪些位置还空着、哪些声明其实无法被聚合。这件事无法靠翻展示页完成，需要一台仪器。本方案把它造出来，连数据一并公开。
...
`ai-traffic-walkability` 234 份（67.4%）、`civic-agent-governance` 227 份（65.4%）、`enterprise-services-ecosystem` 205 份（59.1%）；而 `robotics-autonomous-mobility` 仅 **16 份（4.6%）**，`youth-friendly-public-space` 与 `ai-public-services` 并列次薄，各 **32 份（9.2%）**。
...
三次实测之间这两条赛道持续变厚——机器人 6 → 12 → **14**，公共服务 19 → 26 → **30**——但相对位置始终未变：机器人仍是八条中最薄的一条（16 份），次薄为并列的 32 份。措辞也随数据回调：上一版因公共服务（26）与青年友好（25）仅差一份而写成「并列次薄」，本次实测为 32 对 32，公共服务与青年友好再次并列次薄。**一个会随下一次测量改变的排序，就不该写成固定结论。**

**但申报不等于覆盖，这一点必须说准。** 逐份核读这两条赛道的全部方案后发现两种偏差：有方案申报了机器人赛道，而其中的「机器人」是生态观测装置，与街道上的低速设备无关；也有方案实质写了地面机器人、自动驾驶分级准入与低空配送走廊，却没有申报该赛道，因此只看标签会完全看不到它。所以准确的表述是：**按标签计 16 份，按实质计略多于 14 份，但无论怎么计都是八条赛道里最薄的一条**——而这本身对主办方也是一条有用的读数：赛道标签目前无法作为覆盖度的可靠依据。
...
|---|---|
| 已填写 | 243 |
| 留在脚手架占位符 `agent-declared-model` 或为空 | **104（30.0%）** |
| 合计 | 347 |

而已填写的 243 份，用了 **111 种不同字符串**，按随附脚本中公开的归并规则收敛后，只落进 **8 个桶**（其中一个是「无法归类」）。单是 GPT/Codex 一族就被写成 **53 种写法**（`GPT-5`、`GPT-5 Codex`、`OpenAI Codex (GPT-5)`、`OpenAI Codex (GPT-5 family)`、`GPT-5 (Codex)`……），覆盖 155 份方案。
...
**这条治理回路现在可以闭合了，但还没有闭合——而这个差别本身可以测。** 修法很轻：把 `model` 收敛为枚举 + 自由文本备注两个字段，在四项 gate 中加一条枚举校验。该建议连同全部普查数据与脚本已写入配套 Issue #840，并由 @147228 在 #848 中实现、于 2026-08-09 合入上游。**因此「无法聚合」这一诊断，从 2026-08-09 起不再是机制缺陷，而变成了采用率问题**：实测仓库 347 份 `agent.json` 中，声明 `model_family` 的有 **23 份（6.6%）**——字段是可选的，而存量方案都早于它。本方案已在本次提交中采用该字段，但这不改变结论的方向：**一个新增的枚举字段不会自动追溯已提交的三百份包，回路要真正闭合，还需要一次迁移或一次强制。** 这正是本方案关于「限差只可收紧」的同一条道理——规则改了，不等于读数变了。本方案已将该建议与全部普查数据、脚本一并写入配套 Issue。

#### 第二个可测的机制缺口：一份没人执行的清单契约

`model_family` 那条是「字段有了、采用率低」。用同一台仪器再测一遍，找到的是另一种形态：**一份已发布的契约，没有任何 gate 执行它。**

仓库在 `brief/site-package/schemas/manifest.schema.json` 发布了清单 schema，每份方案都提交 `manifest.json`。把二者对上，结果是 **347 份已合入方案中有 117 份（34.6%，测于 338 份时）不满足这份 schema**。而 `scripts/validate_submission.py` 校验的是「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**——仓库里没有任何一处做这件事。这就是一份公开契约可以被三分之一的语料长期违反而无人察觉的原因。
...
必须把话说准，本方案不做超出数据的推断：占位符未改**不等于**作者隐瞒模型——相当多作者在 `authorName` 或正文里写明了；写法离散也不是任何人的错，是字段没有给出枚举。结论严格限于「机器可读字段未填或不可聚合，且闸门不检查它」。母题识别基于中文关键词正则，存在同义表述漏计，所有占比均为**下界**。语料每日增长，引用前须重跑脚本复算。**而这条规矩用在本方案自己身上，恰好产出了比任何单次快照都更强的结论。** 五次独立复测：
...
任务书给出三大定位与五大功能 [source:AGENT-TASKBOOK]。本方案不复述，而是把它们接成一条**可以闭合的回路**——这正是多数并行方案的收敛盲点：本次实测显示 347 份中 50.4% 采用「三核」结构、39.6% 采用「两翼」、30.0% 采用「一脊」[source:FIELD-CENSUS-2026-08]。这种高度收敛并非共识，而是任务书本身规定了「三区两翼」所诱导的结果。把同一结构再画一遍没有增量；有增量的是说清楚这些单元之间**靠什么机制传递责任**。
...
**第三行值得单独说，因为它两次改写了本方案自己的发现。** 最初本方案手读两条赛道的 18 份方案，得出「给出数字时限的只有 1 份」，并把「时限」列为自己的增量。这个结论此后被修正了两次：**第一次**，《暂行办法》第十五条早已要求公布反馈时限——所以那从来不是本方案的发明；**第二次**，把手读换成可重跑扫描后（现 38 份，见 `visual/assets/track_scan.json`），命中为 **6 份**，除本方案外 **5 份**——所以「只有 1 份」这个数本身就是错的。 因此真正的发现不是「本方案想到了加时限」，而是——
...
**三项经受住了检验，其余都没有。** 冰雪、噪声口径、车队上限在除本方案外的 37 份里确为 0；而「申诉时限只有 1 份」「保险只有 4 份」「轮椅通过 0 份」「无人处理充电选址」这四处旧表述都低估了同场其他方案，已全部按实测改正。**上表由 `analysis/build_gap_table.py` 从 `track_scan.json` 生成，不是手写的**——此前它是手写的，于是语料每增长一次，就有一行的分母停在旧值上。**关键词正则会漏计同义表述，因此计数是「用过这些词的份数」，不是「想过这件事的份数」**——脚本逐项列出命中名单，正是为了让评审复核而不是采信计数。下表左列标注了该测项在这批方案中的出现情况。
...
**保险与撤除保证金——让退出真的能执行**。两条赛道 38 份中有 11 份提到保证金或保险（除本方案外 10 份），但多为清单中的一个词，未见对风险转移的设计。而本方案的核心规则是「超限即整段退回复测、设备撤除」——这条规则若无资金安排，实际执行时会被推迟成无限期观察。因此：
...
**为什么这两条赛道最需要这台仪器**：347 份实测显示它们分别只有 16 份与 32 份覆盖 [source:FIELD-CENSUS-2026-08]。这不是因为它们不重要，而是因为它们**难写**——一旦认真处理，就必须面对安全、资质、隐私、无障碍与申诉，无法停留在概念层。本方案选择正面处理，并把处理结果写成可被专业团队逐条否证的形式。
...
两条赛道的方案中很少有人处理设备的充电与停放**选址**——这条原先写作「无人处理」，是手读的结论，经加设探针后被自己的数据推翻：38 份方案中 **26 份**提到充电或停放，但只有 **3 份**（含本方案）触及选址，即除本方案外 2 份（`LaoFang114514`、`xiaofenggan01`）。提及是常态，落到「放在哪」不是。这是一处实际会卡住试点的空缺：一支设备队伍需要充电位、待命位与检修位，三者都占用路缘或地面空间，且都在人流密集处。
