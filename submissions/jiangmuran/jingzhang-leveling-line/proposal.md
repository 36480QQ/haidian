...
**一条以 AI 产业为主题的创新带，此前没有说过算力落在哪里。** 「算力」在本文里出现三次，全在一张表的一行；散热、变电、智算、训练、推理各零次。评分维度要的是把 AI 能力与产业、空间、交通、公共服务、文化和治理**结合**——本方案与治理结合最深，产业×空间这一格几乎是空的（E-PEND8）。
...
以下六项已成为事实标准，括号内为 793 份语料中的实测份数：场景级停用与退出条件（477）、无AI等价路径（363）、现场安全员（327）、远程与物理急停（174）、限速（98）、事件日志（67）。最薄的一项也有 67 份，最厚的 477 份。**这是下界**——换了说法的方案不会被关键词计入。本方案全部采纳这六项并写入下方场景卡，但**不把它们当作创新陈述**——它们是准入底线。把底线当卖点，说明没有读过这个场。
...
**同一条标准指向本包的闸门本身。** 62 道闸门此前只被观察到通过，而一道从不失败的闸门与一道不会失败的闸门，产生的证据完全一样。因此本方案人为损坏自己的包，每次损坏都要求对应的闸门报错并**指名**所损坏的东西：结果随包提交于 `visual/assets/gate_mutation.json`：89 处蓄意缺陷全部被捕获，62 道中 59 道已被证明会失败，另 3 道连同理由一并写明。同一目录下的 `visual/assets/claim_audit.json` 记录两版正文里带单位的量是否逐条对齐，`visual/assets/agent_declarations.json` 记录全场语料的机器可读声明状况。**须说准：`scripts/review_submission.py` 不读 `visual/assets/`**——这三份文件不进评审模型的上下文，因此这里指名它们，是拿到仓库的人唯一能知道它们存在的途径。。**
...
| 包外对抗性通读 | 28 | 由本包之外的读者按评审收到的那几份文件逐条挑错 |
...
| 交付物在提交前没被看过 | 28 |
| 声明活得比包的内容久 | 19 |
| 这个说法在这种载体上原理上无法被核对 | 17 |
| 写成硬约束，却判不了不合格 | 14 |
| 数字活得比装它的那句话久 | 9 |
| 记在了发现它的地方，没记在别人会去查的地方 | 9 |
| 规则对别人成立，对自己没有执行 | 9 |
| 引用解析不到 | 8 |
| 把举不出编号的出处当作出处 | 6 |
| 数字背后没有文件 | 6 |
...
任务书规定了「三区两翼」，半数以上方案画出同一骨架：不是共识，是题面所致。**把同一结构再画一遍没有增量。** 有增量的是说清这些单元之间靠什么机制传递责任——这正是水准网回答的问题。
...
四份方案用了后一种写法，它们申报的赛道**从未被任何一次计数看见**，八条赛道里六条被低估。包内的 `field_map.json` 其实一直在报警：`proposals_declaring_no_known_track` 曾为 14，而真值是 10，多出来的四份正是这四个文件——**读数不对时，数据里往往已经有一条对不上的线索，只是没人去追。** 解析器已修（`- item` 行归并入上一个空值键），并把普查的数据源从 GitHub API 改为本地 git tree：API 会限流，而一次因 403 跳过的普查，正是数字变陈旧的起点。
...
**但申报不等于覆盖。** 逐份核读发现两种偏差：有方案申报了机器人赛道，而其中的「机器人」是生态观测装置；也有方案实质写了地面机器人与低空配送走廊却没有申报。准确的表述是：**按标签计 25 份，按实质计略多于 14 份，无论怎么计都是八条赛道里最薄的一条**——赛道标签目前无法作为覆盖度的可靠依据。
...
**结论：无法从这个字段聚合出「这次征集由哪些模型生成」。** 字段填了但不可机读——这比「有人留空」更值得修，而且不涉及任何作者的过失。这就是闭合差：出发时声明的规则（结构化、可机读、可被筛选排序），与回到原点时的实测状态（见上表），对不上。
...
仓库在 `brief/site-package/schemas/manifest.schema.json` 发布了清单 schema，每份方案都提交 `manifest.json`。把二者对上，**测于 338 份语料时有 117 份（34.6%）不满足这份 schema**，违规集中在四个 role 上——`changelog`、`evidence_data`、`verification_script`、`risk_matrix`——本包当时也用了这四个，把自己算在那 117 份里，不作豁免。

**这一段现在是历史。** 该发现写入配套 Issue #1058，2026-08-11 关闭：修法是把 `role` 从封闭枚举改成开放 pattern。本包因此从四条 schema 例外变为零条 role 例外。**该次实测时**语料 825 份，其中 **79 份**不满足，且这份 schema **依然没有任何 gate 执行**——被修掉的是枚举太窄，没被修掉的是没人检查。`scripts/validate_submission.py` 只校验「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**。
...
`changelog` 一行值得单看：**38 位互不相识的作者，各自伸手去够同一个不存在的槽位。** 一份随包提交变更记录的方案，在枚举里找不到能诚实描述它的 role，于是写下 `changelog` 并静默失败。这不是三十八次疏忽，这是一次规格缺项。

**本方案当时也在这 117 份里，不作豁免。** 本包当时有 21 处违规、5 种类型，同一原因：changelog、校验脚本、证据数据在枚举里都没有对应项。把 changelog 写成 `narrative` 只为通过一份没人执行的校验，是对机器说假话。做法是**把违规声明出来并加一道自己的闸**：`visual/assets/manifest_schema_survey.json` 逐条列出本包的 5 种违规，`analysis/build_all.py` 每次构建重新校验（`analysis/` 下的脚本只说明出处，**不在提交包内**），出现清单以外的新违规即构建失败。**已声明的例外不能藏新的意外。**

修法同样很轻，二选一：扩充枚举以覆盖语料实际携带的角色，或在入库校验中执行这份 schema，使缺口在提交当时可见。两条都不做，等于长期维持一份三分之一语料都不满足的已发布契约——上游已采纳第一条的变体，该次修复后为 **75 份**不满足（855 份中），本包 0 处。全部计数与方法说明已写入配套 Issue [#1058](https://github.com/open-city-ai/haidian/issues/1058)，并随包提交于 `visual/assets/manifest_schema_survey.json`，任何人可自行重跑复核。


必须把话说准：占位符未改**不等于**作者隐瞒模型——相当多作者在 `authorName` 或正文里写明了；写法离散也不是谁的错，是字段没给枚举。母题识别基于中文关键词正则，占比均为**下界**。**这条规矩用在本方案自己身上，产出了比任何单次快照都更强的结论。** 五次独立复测（前四次此前只存在于 `changelog.md`；现已从 git 历史逐轮重建并随包提交为 `visual/assets/census_history.json`——**本方案自己的规矩不给作者留例外**）：
...
**重建结果值得单说，因为它不是四行全对。** 按「树内目录数恰好相等的最新提交」逐轮重算：215、228、298 三轮与留档值**完全一致**（0.307 / 0.303 / 0.292）；最早的 184 一轮重建为 **0.304**，留档 0.299，差**一份包**。差值不作调和，原因写在 `census_history.json` 的 `method` 里：重建取的是目录数相等的最新提交，与当初取数的那一刻不必然同时。**三轮精确重合、一轮差一份**，比四行都对上更能说明这套重建是在算。

**这五轮的结论此前写成「结构性属性」，而本方案自己的下一次读数把它否掉了，改写。** 从 git 历史重建的四轮里，披露缺口稳定在 29.2%–30.7% 的区间内，当时据此判断它「不取决于谁参加」；本次实测掉出了这个区间。（此前正文还写过一轮 354 份／29.9%，而 `census_history.json` 里没有它——**那一轮只存在于散文里**。）<!-- NUMERATOR:BEGIN -->

**分子的行为可以直接读出来，比原结论更有意思。** 本包每次重跑普查都提交一份新的 `field_map.json`，读数都留在本分支的 git 历史里；`visual/assets/reading_log.json` 把它们取回，共 **17 次**读数，语料从 228 长到 793，占位符数依次为 69 → 87 → 102 → 104 → 106 → 106 → 106 → 105 → 107 → 107 → 107 → 109 → 109 → 129 → 131 → 143 → 144；期间占位符数**下降过 1 次**（373→381 时 106→105），说明**存量方案确实被改过**。最长的一段不变是 **107**，持续 3 次读数（语料 394 / 408 / 416）。

**存量占比与增量比例回答两个不同的问题，只有后者支持这一段的论点。** 日志里最大的一次跳变是语料 440→678（新增 238 份），其间占位符只增加 **20 份**：新合入的方案里约 **8.4%** 留空，全部存量的比例是 **18.2%**。占比在降，主要不是有人回头改了旧包，而是**新来的人本来就填**。

**此前这一段是凭记忆写的**——四次写成三次，方向也反了。没有文件支撑的数字，即使是自己测的，也会在复述中走形。完整更正见勘误册。

<!-- NUMERATOR:END -->

这些发现不削弱这次征集，恰恰相反：它们证明这套开源征集机制**真的在产生可被检验的公共证据**——全部投稿、评审记录与合入历史都是公开的，上面每一条读数都可以被任何人重跑。缺的只是最后一步：把闭合差算出来，并让它有后果。
...
需要把话说准的三点。**第一，上述 PR 均由他人实现，本方案只提供了测量、脚本与问题定位。** **第二，#848、#850 与 #861 已于 2026-08-09 合入上游**：`model_family`（枚举 8 值）与 `model_detail` 现已是仓库规则；#850 把本方案的 OSM 交叉核验写入仓库自己的 `provisional_boundaries_basis.md`，载入 17.49 ha 与 412.5 m 两个读数；#861 修复了本方案在 Issue #858 报告的校验时序竞态，**并因此解除了本方案自己「有运行排队时不得推送」的操作纪律**。仅 #807 尚未合入。#850 明确写明 OSM 结果「未作为中央可重放来源」， `background_only` 定级**保持不变**——被记录不等于被登记。上游枚举定为 8 值，本方案独立归并实测得到 9 个桶，该次修复后为 **8 值对 9 桶**，此前这句写成「一致」，是把相互印证说得比数据更漂亮。**第三，展示索引滞后的起伏，本方案不归功于自己**——无法证明因果。

写出这一节不是记功，而是它构成对本方案核心主张的一次外部检验：一个可被独立重跑的读数，确实被另一方复现并采纳了。**这正是闭合差机制要求的那种事——读数由不同主体独立取得，并因此产生后果。** 本方案自己也接受同一条规矩：若上游最终判定这些测量有误，本方案须整体重算，而不是保留有利部分。
...
**一、这条已撤回，而撤回本身是这一节最该读的部分。** 本方案最初写的是：#957 的匹配器要求带冒号前缀，而 1,203 条历史 action 匹配数为 0，它是空集。发表两小时后，#950 的作者 @Sonike 指出这个推断不成立——**那 1,203 条全部产生于旧 prompt，而旧 prompt 从未要求该前缀；在一个不要求它的语料里统计它的出现率，结果必然是 0。** #957 同时改了 prompt，他在新 prompt 下实测 5 次采样中 4 次输出前缀。

**这与本方案刚刚指出他的错误是同一个形状**：拿一个测量去回答它回答不了的问题。他的八份样本证明「这八份里组织方项都在」，证不出它是原因；本方案的 1,203 条证明「旧 prompt 下没人写这个前缀」，证不出新 prompt 下匹配器不会命中。**本方案用了不到两小时犯了同一个错。**

**二、即使匹配器完美也不通。** 汇总项 `完成七维评分中列出的 N 项详细 required repairs` 在组织方拆分**之前**被 append，且不带组织方前缀，必然落进参与者一侧——146 份 verdict 全部携带它。

**三、组织方项从来不是唯一阻塞项。** 在最宽松的剥离之后，剩余参与者 action **全部在 3 条以上**——随包文件把它分桶为「3+」，最小值本身没有被保留下来，这里不给出一个它支撑不了的数；「仅剩组织方项」的 PR 数在四种规则下都是 0。

组织方依赖当然真实（`官方` 122 次、`重算` 59、`组织方` 45）。测量只说明一件事：**移除它们不改变任何一个结果。**

**这处发现的形状正是本方案通篇在讲的那件事**：#950 与 #957 都是认真的工作，八份样本的诊断在八份里都成立——**差别不在认真程度，在 n**。本方案主张的不是「更谨慎地判断」，而是「把判断建立在可被别人重跑的读数上」：全部计数与匹配字符串随包提交于 `visual/assets/review_gate_survey.json`，任何人可以改了规则重跑。
...
本方案用全量把这个估计换成了观测——**汇总项本身写着 repair 总数**， 146 份历史 verdict 的 `repair_count` 可以直接读出：
...
**后记：#957 已于 2026-08-10 合入，上面这段可以对着已合入的代码重说一遍，而不再是对着一份提案。** 组织方项分流已生效：带 `组织方：`／`主办方：` 前缀的 action 现在被移入 `data_gaps_zh`，不再计入 `required_next_actions_zh`。**而汇总项不受影响**——`ai_review_submission.py` 里那段在 `repair_count > 0` 时无条件追加的句子，不带任何前缀，因此仍然进入 participant actions，仍然使列表非空。
...
评审无法核验的授权声明等于没有声明。下表逐类列出本包全部资产的来源、许可与**核验方式**，每一行都可独立复核。**逐文件的机读版本随包提交于 `visual/assets/rights_ledger.json`**——它由 `manifest.json` 生成而非手写，枚举的是实际随包的 **156 个文件**，任一文件没有权利归类即构建失败；每条另给出评审自行核验的方式，而不只是一句「原创」。手写台账枚举的是作者记得的资产，生成台账枚举的是包里真有的文件，这个差别正是本方案通篇在说的那件事：

<!-- LEDGERCOUNT:BEGIN -->

**逐文件台账在评审输入之外，因此把它的结果放进来。** `build_review_input` 送出的是 `proposal.md` 与八份 JSON；`report/copyright_statement.md` 与 `visual/assets/rights_ledger.json` 都不在其中。**一份评审结构上打不开的权利台账，从评审的位置看就是一句无法核验的断言**——而那正是本方案在别处反对的东西。当前 **156 个随包文件全部有权利归类**，任一文件缺归类即构建失败：
...
**FIG.21 量了走到水准点的路，本包却没量过走完自己证据的路。** 「跑这五个脚本、读这几份文件」是一句话，跑一遍要多久是没人问过的问题——现在问了：五个随包校验器全部零退出，合计 **0.72 秒**；另有 9 份文件、1035 KB、1,739 条要读，逐条读数随包提交于 `visual/assets/review_route.json`。计时界定的是**工作量**而不是难度。
