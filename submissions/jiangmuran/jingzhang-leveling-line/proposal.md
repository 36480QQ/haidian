...
以下六项已成为事实标准，括号内为 793 份语料中的实测份数：场景级停用与退出条件（478）、无AI等价路径（362）、现场安全员（327）、远程与物理急停（173）、限速（98）、事件日志（67）。最薄的一项也有 67 份，最厚的 478 份。**这是下界**——换了说法的方案不会被关键词计入。本方案全部采纳这六项并写入下方场景卡，但**不把它们当作创新陈述**——它们是准入底线。把底线当卖点，说明没有读过这个场。
...
**这条脊今天走起来是多少米。** 本方案画出的主脊 9,443 m 是一条设计线；在**现状已测绘的可步行路网**上，按测序顺序把八个水准点走一遍是 **11,472 m**——**多走 2,029 m，倍率 1.21**（`analysis/route_check.py`，同一份 OSM 缓存建图，可步行等级逐条写明，端点 12 m 内视为同一节点，快速路辅路未单独标注因此该数为下界）。**这个差就是这条带要做的事**：一条不减少绕行的带子，什么也没有改变。最差的一段是 BM-303→BM-1，倍率 2.28。

两处更硬的读数一并登出：**BM-21 在这张图上根本走不到任何其他水准点**——它落在已测绘可步行网络的一个孤立片段里；BM-22 距 BM-0 步行 1,658 m，而两点直线相距不到 1.2 km。本包自己的 `roads.geojson` 给出的接入距离是 0 m，因为那条线是本方案画的。**两个网络，两个答案，差额就是待建的部分。**
...
| 作者自查 | 140 | 本轮工作中发现 |
| 本包自己的闸门 | 15 | 构建时被抓出——这是闸门本该做的事 |
| 包外对抗性通读 | 19 | 由本包之外的读者按评审收到的那几份文件逐条挑错 |
...
**FIG.21 量了走到水准点的路，本包却没量过走完自己证据的路。** 「跑这五个脚本、读这几份文件」是一句话，跑一遍要多久是没人问过的问题——现在问了：五个随包校验器全部零退出，合计 **0.74 秒**；另有 9 份文件、923 KB、1,667 条要读，逐条读数随包提交于 `visual/assets/review_route.json`。计时界定的是**工作量**而不是难度。
