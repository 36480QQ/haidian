...
三次实测之间这两条赛道持续变厚——机器人 6 → 12 → **14**，公共服务 19 → 26 → **30**——但相对位置始终未变：机器人仍是八条中最薄的一条（14 份，恰为次薄者 28 份的一半）。措辞也随数据回调：上一版因公共服务（26）与青年友好（25）仅差一份而写成「并列次薄」，本次实测为 30 对 28，公共服务重新单独居于次薄位。**一个会随下一次测量改变的排序，就不该写成固定结论。**

**但申报不等于覆盖，这一点必须说准。** 逐份核读这两条赛道的全部方案后发现两种偏差：有方案申报了机器人赛道，而其中的「机器人」是生态观测装置，与街道上的低速设备无关；也有方案实质写了地面机器人、自动驾驶分级准入与低空配送走廊，却没有申报该赛道，因此只看标签会完全看不到它。所以准确的表述是：**按标签计 14 份，按实质计略多于 14 份，但无论怎么计都是八条赛道里最薄的一条**——而这本身对主办方也是一条有用的读数：赛道标签目前无法作为覆盖度的可靠依据。
...
**这条治理回路现在可以闭合了，但还没有闭合——而这个差别本身可以测。** 修法很轻：把 `model` 收敛为枚举 + 自由文本备注两个字段，在四项 gate 中加一条枚举校验。该建议连同全部普查数据与脚本已写入配套 Issue #840，并由 @147228 在 #848 中实现、于 2026-08-09 合入上游。**因此「无法聚合」这一诊断，从 2026-08-09 起不再是机制缺陷，而变成了采用率问题**：实测仓库 338 份 `agent.json` 中，声明 `model_family` 的有 **16 份（4.7%）**——字段是可选的，而存量方案都早于它。本方案已在本次提交中采用该字段，但这不改变结论的方向：**一个新增的枚举字段不会自动追溯已提交的三百份包，回路要真正闭合，还需要一次迁移或一次强制。** 这正是本方案关于「限差只可收紧」的同一条道理——规则改了，不等于读数变了。本方案已将该建议与全部普查数据、脚本一并写入配套 Issue。

#### 第二个可测的机制缺口：一份没人执行的清单契约

`model_family` 那条是「字段有了、采用率低」。用同一台仪器再测一遍，找到的是另一种形态：**一份已发布的契约，没有任何 gate 执行它。**

仓库在 `brief/site-package/schemas/manifest.schema.json` 发布了清单 schema，每份方案都提交 `manifest.json`。把二者对上，结果是 **338 份已合入方案中有 117 份（34.6%）不满足这份 schema**。而 `scripts/validate_submission.py` 校验的是「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**——仓库里没有任何一处做这件事。这就是一份公开契约可以被三分之一的语料长期违反而无人察觉的原因。

**但违规的形状说明这不是作者不认真。** 逐条归并后，绝大多数违规是同一件事：`role` 枚举里没有方案实际需要的角色。
...
**标 `*` 的限差尚未设定值。** F3 的初值 0.20 已公布，F1、F2 由 BM-1 的限差议事厅设定，本方案不代为拟定——在这里写一个看起来合理的数字，正是本方案通篇反对的那种替换。因此这些卡在 `scenario_cards.json` 里被标为 `operational: false` 并写明原因，而不是让缺口藏在「超限」两个字里。
...
| [metric:green_ratio] | 0.1966 | 120 m 主脊绿廊（已扣除建筑基底）/ 总体设计范围 |
...
**先说一处对本方案不利的纠正：此前的 0.2025 把 66,925 m² 本方案自己的建筑基底算作了绿地。** 绿廊是主脊 120 m 缓冲，而六个建筑基底就在这条缓冲里，此前没有从绿地图层中扣除。这个偏差的方向是对作者有利的——一个偏向自己的读数，比一个随机的错更该被单独指出来。现已扣除，`green_ratio` 由 **0.2025 降为 0.1966**，并加了一道闸门：绿地图层与建筑图层的相交面积必须为 0（容差 1 m²），超出即构建失败，报错逐个点名是哪几栋建筑。
