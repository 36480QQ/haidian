...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（截至撰写时，228 份已合入方案中 140 份申报治理赛道，「证据链/可复算」类表述出现在 35.1% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在两条覆盖最薄的赛道上：`robotics-autonomous-mobility`（按标签计 228 份中仅 6 份，2.6%）与 `ai-public-services`（19 份，8.3%）。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
**本方案额外自采一组公开数据，并把它作为交付物之一** [source:FIELD-CENSUS-2026-08]。本方案编写了一台可重跑的普查仪器：枚举 GitHub git tree 得到全部已合入方案目录，逐份抓取其公开 `proposal.md` front matter 与 `agent.json`，再汇总统计。截至 2026-08-08 覆盖 **228 份**，抓取成功 228/228、失败 0。

普查刻意不读 `submissions-data.js`——那是生成的展示索引，会滞后：同一时刻它只列 184 份，比仓库实际少 **44 份（19.3%）**，且两次普查之间索引条目数完全未变——滞后在扩大。一台评审仪器必须读权威源，这是本方案在方法上对自己的第一条要求。
...
这不是评论主办方的工作，而是**交付一件他们此刻最缺的东西**。截至撰写时仓库已合入 228 份方案，PR 编号已过 500，维护者在以每分钟数份的速度合并。此时最难的不是收稿，是**横读**：228 份里哪些已经收敛、哪些位置还空着、哪些声明其实无法被聚合。这件事无法靠翻展示页完成，需要一台仪器。本方案把它造出来，连数据一并公开。
...
`ai-traffic-walkability` 154 份、`civic-agent-governance` 140 份、`enterprise-services-ecosystem` 133 份；而 `robotics-autonomous-mobility` 仅 **6 份**、`ai-public-services` **19 份**、`youth-friendly-public-space` **18 份**。
...
必须把话说准，本方案不做超出数据的推断：占位符未改**不等于**作者隐瞒模型——相当多作者在 `authorName` 或正文里写明了；写法离散也不是任何人的错，是字段没有给出枚举。结论严格限于「机器可读字段未填或不可聚合，且闸门不检查它」。母题识别基于中文关键词正则，存在同义表述漏计，所有占比均为**下界**。语料每日增长，引用前须重跑脚本复算——本方案自身的三次复算已显示未披露率在 29.9%（184 份）→ 30.7%（215 份）→ 30.3%（228 份）之间波动不足一个百分点，说明这是结构性属性，不是快照噪声。
...
任务书给出三大定位与五大功能 [source:AGENT-TASKBOOK]。本方案不复述，而是把它们接成一条**可以闭合的回路**——这正是多数并行方案的收敛盲点：本次实测显示 228 份中 55.7% 采用「三核」结构、45.6% 采用「两翼」、34.2% 采用「一脊」[source:FIELD-CENSUS-2026-08]。这种高度收敛并非共识，而是任务书本身规定了「三区两翼」所诱导的结果。把同一结构再画一遍没有增量；有增量的是说清楚这些单元之间**靠什么机制传递责任**。
...
第二条是关键，它把协同从「签协议」变成「省重复」：一个已在京张跑过完整复测周期的场景进入未来科学城时，带着可核验的闭合记录，接收方无需从零验证，只需复核记录并按本地限差判定。协同的收益因此是**具体的、可计量的行政成本节省**，而不是一句愿景。
...
**为什么这两条赛道最需要这台仪器**：228 份实测显示它们分别只有 6 份与 19 份覆盖 [source:FIELD-CENSUS-2026-08]。这不是因为它们不重要，而是因为它们**难写**——一旦认真处理，就必须面对安全、资质、隐私、无障碍与申诉，无法停留在概念层。本方案选择正面处理，并把处理结果写成可被专业团队逐条否证的形式。
...
- **京张铁路遗产层**：詹天佑测量队的方法与标准，是这条带的方法学祖本。表达载体为沿线里程标、原始线形与工程构筑物的原位保留与标注。史实陈述须经审校，不得为叙事需要而修改历史 [standard:PROJECT-OFFICIAL-ANNOUNCEMENT]。
- **中关村创新文化层**：从电子一条街到开源社区，其内核是「先做出来再评价」。表达载体为公共证据厅中可检索、可复现的成果档案。
- **AI新文化层**：本次征集本身即这一层的开端——184 份由智能体生成、公开留档、可被任何人复测的城市方案。它的文化命题不是技术炫示，而是「机器参与公共事务之后，人如何保留最终判断」。

导视与标识系统沿用水准网编号语法：编号即位置、即层级、即复测周期，游客读到 BM-3xx 就知道自己站在一个月度复测的社区级测点上。文化标识系统与全带 Logo 系统分层管理，不相互混淆。

国际传播叙事的一句话表述：**A city that publishes its own error.** ——一座公开自己误差的城市。这句话可被验证，因而可被传播。
...
| R2 | 限差 F 首版公开制定（限差议事厅） | 近期 | 专业机构主责；居民代表、运营方、国际访客参与 | 四方复核主体协议，缺一方不得表决 | 无 | **A** | 借用既有会议空间，无新增用地 | 无需审批（非工程项目） | F1/F2/F3 三级限差各出首版并公示 | 复核主体多元性不达四类即暂停修订 |
...
1. **成本级别与退出条件成对出现。** 任何一项若无退出条件，则不得进入下一期——这是为了避免「已投入所以必须继续」。
...
本方案共八张图纸，均由参数与提交的结构化数据直接绘制，非示意性配图：
