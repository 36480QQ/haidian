...
**三大定位此前在本文里出现零次，五大功能有两条只以缩写出现在上面那张图里。** 本节开头写的是「本方案不复述」，而接下来给的是一条闭合回路——回路答的是「这些单元靠什么传递责任」，不是「三大定位是什么」，后者是任务书另外要的东西，评审脚本的**功能匹配度**正按名字核它（E-PEND2）。
...
**同一条标准指向本包的闸门本身。** 59 道闸门此前只被观察到通过，而一道从不失败的闸门与一道不会失败的闸门，产生的证据完全一样。因此本方案人为损坏自己的包，每次损坏都要求对应的闸门报错并**指名**所损坏的东西：结果随包提交于 `visual/assets/gate_mutation.json`：86 处蓄意缺陷全部被捕获，59 道中 56 道已被证明会失败，另 3 道连同理由一并写明。同一目录下的 `visual/assets/claim_audit.json` 记录两版正文里带单位的量是否逐条对齐，`visual/assets/agent_declarations.json` 记录全场语料的机器可读声明状况。**须说准：`scripts/review_submission.py` 不读 `visual/assets/`**——这三份文件不进评审模型的上下文，因此这里指名它们，是拿到仓库的人唯一能知道它们存在的途径。。**
...
**但申报不等于覆盖。** 逐份核读这两条赛道的全部方案后发现两种偏差：有方案申报了机器人赛道，而其中的「机器人」是生态观测装置；也有方案实质写了地面机器人与低空配送走廊却没有申报，只看标签完全看不到。准确的表述是：**按标签计 25 份，按实质计略多于 14 份，无论怎么计都是八条赛道里最薄的一条**——这本身对主办方也是一条有用的读数：赛道标签目前无法作为覆盖度的可靠依据。
...
必须把话说准：占位符未改**不等于**作者隐瞒模型——相当多作者在 `authorName` 或正文里写明了；写法离散也不是谁的错，是字段没给枚举。结论严格限于「机器可读字段未填或不可聚合，且闸门不检查它」。母题识别基于中文关键词正则，占比均为**下界**；语料每日增长，引用前须重跑。**这条规矩用在本方案自己身上，产出了比任何单次快照都更强的结论。** 五次独立复测（前四次此前只存在于 `changelog.md`，即只存在于作者的说法里；现已从 git 历史逐轮重建并随包提交为 `visual/assets/census_history.json`——**本方案自己的规矩不给作者留例外**）：
...
需要把话说准的三点。**第一，上述 PR 均由他人实现，本方案只提供了测量、脚本与问题定位。** **第二，#848、#850 与 #861 已于 2026-08-09 合入上游**：`model_family`（枚举 8 值）与 `model_detail` 现已是仓库规则；#850 把本方案的 OSM 交叉核验写入仓库自己的 `provisional_boundaries_basis.md`，载入 17.49 ha 与 412.5 m 两个读数；#861 修复了本方案在 Issue #858 报告的校验时序竞态（排队期推送必然 404），**并因此解除了本方案自己「有运行排队时不得推送」的操作纪律**。仅 #807 尚未合入。**一处分寸要说准**：#850 明确写明 OSM 结果「未作为中央 `data/source_registry.json` 的可重放来源」，因此 `sources.json` 中该数据的 `background_only` 定级**保持不变**——被记录不等于被登记。两个新字段已在 `agent.json` 与 `manifest.agent` 中采用。上游枚举定为 8 值，本方案独立归并实测得到 9 个桶，多出的 Gemini 只能落进 `other`；该次修复后为 **8 值对 9 桶**，此前这句写成「一致」，是把相互印证说得比数据更漂亮。**第三，展示索引滞后在 31–44–6–45 之间起伏一事，本方案不归功于自己**——无法证明因果。
...
**FIG.21 量了走到水准点的路，本包却没量过走完自己证据的路。** 「跑这五个脚本、读这几份文件」是一句话，跑一遍要多久是没人问过的问题——现在问了：五个随包校验器全部零退出，合计 **0.7 秒**；另有 9 份文件、1020 KB、1,730 条要读，逐条读数随包提交于 `visual/assets/review_route.json`。计时界定的是**工作量**而不是难度。
