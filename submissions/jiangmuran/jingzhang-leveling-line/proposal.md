...
需要把话说准的三点。**第一，上述 PR 均由他人实现，本方案只提供了测量、脚本与问题定位**；#848 的措辞是「实现 #840 中低风险的结构化披露部分」，#850 的措辞是「记录 #846 提出的可复现交叉核验」。**第二，四个 PR 中 #848 已于 2026-08-09 合入上游**，`model_family`（枚举 8 值：gpt / claude / deepseek / qwen / glm / kimi / grok / other）与 `model_detail` 现已是仓库规则的一部分；其余三个截至撰写时尚未合入，本方案不把它们表述为已生效规则。**本方案已在 `agent.json` 与 `manifest.agent` 中采用这两个新字段**——既然本方案主张该字段应当可聚合，就不能在它可用之后仍然不用。顺带一提，上游枚举定为 8 个值，与本方案独立归并实测得到的 8 个桶一致，这是一次意外的相互印证。**第三，展示索引滞后从 44 份收窄到 6 份一事，本方案不归功于自己**——无法证明因果。
