...
| **包外评审** | **1** | [@anselasimov-web](https://github.com/anselasimov-web) 在 PR #1002 中发现 |
...
**第一行出现五次。** 翻译覆盖率数的是「交给翻译器的字符串」而不是「页面上的中文」；CJK 泄漏闸门扫文本而扫不到 PDF；构建判定表达式漏掉两条打印 PASS 的闸门；`verify.js` 末尾三条结论只打印不拒绝；闭合差检查器只拦显式 `false` 而不拦字段缺失。五次都是同一件事：**检查问的是「有没有说错」，而不是「有没有说」。**
