...
summary: "水准测量的方法论不是「测得准」，是「测得回来」：绕一周回到原点，闭合差超限则整段重测。本方案把这条规矩用在读数错了会伤到人的地方——低速机器人、自动驾驶接驳与AI医疗教育法律生活服务。全线布设分级水准点与两条附合路线，F1类场景必须先在受控测试场取得闭合合格才能进入公共服务，且永远保留无AI等价路径。同一台仪器转向本次征集自身，对全部已合入方案做了五次可重跑普查（184→416 份），数据与结论作为评审助手一并公开。所有空间建议均为概念建议，基于临时边界，不替代法定规划。"
...
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 25/416（6.0%，八条赛道中最薄）、AI公共服务 42 份（与青年友好并列次薄） | `visual/assets/field_map.json`，普查脚本可重跑 |
...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（最近一次实测：416 份已合入方案中 271 份申报治理赛道，「证据链/可复算」类表述出现在 31.1% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在覆盖最薄的赛道上：`robotics-autonomous-mobility` 按标签计 416 份中仅 **25 份（6.0%）**，是八条赛道里最薄的一条，而次薄位由青年友好公共空间与 AI 公共服务并列占据，各 32 份；`ai-public-services` **30 份（8.9%）**，仅次于它。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
共 416 份已合入方案；赛道为多选，占比之和大于 100%。加粗两行是本方案选定的两条，也是全场最薄的两条。
...
|---|---|
| 已填写 | 309 |
| 留在脚手架占位符 `agent-declared-model` 或为空 | **107（25.7%）** |
| 合计 | 354 |

而已填写的 309 份，用了 **125 种不同字符串**，按随附脚本中公开的归并规则收敛后，只落进 **8 个桶**（其中一个是「无法归类」）。单是 GPT/Codex 一族就被写成 **59 种写法**（`GPT-5`、`GPT-5 Codex`、`OpenAI Codex (GPT-5)`、`OpenAI Codex (GPT-5 family)`、`GPT-5 (Codex)`……），覆盖 195 份方案。
...
**这条治理回路现在可以闭合了，但还没有闭合——而这个差别本身可以测。** 修法很轻：把 `model` 收敛为枚举 + 自由文本备注两个字段，在四项 gate 中加一条枚举校验。该建议连同全部普查数据与脚本已写入配套 Issue #840，并由 @147228 在 #848 中实现、于 2026-08-09 合入上游。**因此「无法聚合」这一诊断，从 2026-08-09 起不再是机制缺陷，而变成了采用率问题**：实测仓库 416 份 `agent.json` 中，声明 `model_family` 的有 **80 份（19.2%）**——字段是可选的，而存量方案都早于它。本方案已在本次提交中采用该字段，但这不改变结论的方向：**一个新增的枚举字段不会自动追溯已提交的三百份包，回路要真正闭合，还需要一次迁移或一次强制。** 这正是本方案关于「限差只可收紧」的同一条道理——规则改了，不等于读数变了。本方案已将该建议与全部普查数据、脚本一并写入配套 Issue。
...
**这五轮的结论此前写成「结构性属性」，而本方案自己的第六次读数把它否掉了，因此改写。** 从 git 历史重建出来的四轮里，披露缺口稳定在 29.2%–30.7% 的 1.5 个百分点区间内，当时据此判断它「不取决于谁参加」。本次实测掉出了这个区间——见上表末行。（此前正文还写过一轮 354 份／29.9%，而 `census_history.json` 里没有它——**那一轮只存在于散文里，从未被重建过**，因此上表只列文件能支撑的四轮加本次实测。）**原因可以直接读出来，而且比原结论更有意思：分子停了。** 占位符份数一路 56 → 66 → 69 → 87 → 107 地长，然后在最近四次读数（语料 381、394、408、416）上**一直是 107**。也就是说，最近这几十份新合入的方案几乎都把 `model_family` 填了；占比在降不是因为存量被改，是因为分母在长而分子不动。本方案不声称知道原因——charter 讨论、脚手架更新、还是巧合，这份数据分辨不了。**它能说明的只有一件事，而这件事恰好是本方案通篇主张的：把结论挂在一次快照上，第六次读数就会把它拿掉；挂在方法上，第六次读数是它的证据而不是它的反例。**
