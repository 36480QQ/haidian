...
| FIG.15 | Phasing: advanced by closure results, not by dates | `phasing.geojson` shipped long ago, was repaired once, entering recompute path — **and no drawing showed it**. The three increments, the benchmarks each adds, the condition opening it (a number, not a year), and **what each phase costs to run**: the near term holds BM-0 alone, one session a year |
...
8. `visual/assets/accessibility_qa.json` and `parity_qa.json` — computed contrast, A0 type height, HTML checks, bilingual parity per section; `label_collision_qa.json` finds one label over another (51 pairs, 19 sheets); `i18n_order_qa.json` finds a template filling number slots out of order — it printed "the 27 m distance FIG.3.0 fixes"
...
**FIG.21 measured the walk to a benchmark; nothing measured the walk through this package's own evidence.** The five verifiers exit 0 in **0.76 s**; 9 files, 1024 KB, 1,628 entries left to read, per item in `visual/assets/review_route.json`. Timings bound effort, not difficulty.
