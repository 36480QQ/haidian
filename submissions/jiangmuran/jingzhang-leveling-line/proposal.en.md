...
| FIG.18 | Maintenance: putting the wear on the parts that can be swapped | A monthly re-survey is a maintenance problem first. One asymmetry drives it: **the stone is the only part that must never be replaced and the only one a person cannot carry** (about 230 kg) — a benchmark that moves is not the same one — so the wear goes on the face, post, seat and guidance strip; the last block states what happens when a stone is lost anyway |
...
9. `risk.json` — the eight-dimension self-assessment, with mitigations and human review
10. `changelog.md` — **including the errors this proposal found in itself**
...
**FIG.21 measured the walk to a benchmark; nothing had measured the walk through this package's own evidence.** The five verifiers exit 0 in **0.75 s**; 9 files, 974 KB and 1,613 entries remain to read, per item in `visual/assets/review_route.json`. Timings bound effort, not difficulty.
