...
Three things must be stated precisely. **First, all of those PRs were written by someone else; this proposal contributed the measurement, the scripts and the diagnosis** — #848 describes itself as implementing "the low-risk structured disclosure portion of #840", and #850 as recording "the reproducible OSM cross-check raised in #846". **Second, three of the four — #848, #850 and #861 — merged upstream on 2026-08-09.** `model_family` (an eight-value enum: gpt / claude / deepseek / qwen / glm / kimi / grok / other) and `model_detail` are now repository rules; #850 records this proposal's OSM cross-check in the repository's own `provisional_boundaries_basis.md`, citing Issue #846 and carrying the 17.49 ha and 412.5 m readings; #861 fixes the validation timing race of Issue #858, where a push during a queued run guaranteed a 404, **which also lifted this proposal's own rule of never pushing while a run was queued**. Only #807 remains open, and it is not presented as an effective rule. **One distinction stays exact:** #850 states that the OSM result is *not* registered as a replayable source in `data/source_registry.json`, so this package's `background_only` grading **stays as it is** — being recorded is not being registered. **Both new fields are adopted in `agent.json` and `manifest.agent`**: a proposal arguing the field should be aggregable cannot decline to use it once it exists. The upstream enum settled on eight values, matching the eight buckets this proposal's independent mapping measured. **Third, this proposal claims no credit for the gallery index lag moving 31–44–6–45**; causation cannot be shown.
...
**The cases locate a gap; they do not endorse this proposal.** All six register and assess, and none institutionalises returning to the origin and computing. That judgement is itself refutable, and a reviewer who knows a counter-example is invited to name it.

### Index to the drawings, and what to read in each

**The forms these sheets ship in.** The thirty-four sheets are also composed into `drawings/a3-booklet.en.pdf`, read page by page, and `drawings/a0-boards.en.pdf`, read standing up. Two renderings open directly: `report/proposal.en.html` is this document as a web page and `visual/index.en.html` a visual index threading the shipped structured data; `report/narrative.md` is the argument alone, without tables or gate records. Five files ship behind them: `visual/assets/personas.json` (the nine personas, making every `P` number assertable), `score_correlates.json` (what correlates with score across the field), `gate_mutation.json` (deliberate damage proving 25 of 29 gates can fail), `claim_audit.json` (unit-carrying measurements across both editions) and `agent_declarations.json` (the field's disclosure state). **Listing them is not housekeeping: a finished product the prose never names is one nobody has been told exists.**
