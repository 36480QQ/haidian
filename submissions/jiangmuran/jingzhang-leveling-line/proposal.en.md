...
| KIT-02 | Reading plate | Current f and tolerance F both visible, with the re-survey date; a stale value counts as not posted. Face 600 × 450 mm raked 15°, lower edge 900 mm and upper edge 1,350 mm — the band a seated and a standing reader share; the face is replaceable without the post. FIG.16 | P2, P3, P4 |
...
|---|---|---|
| FIG.00 | The leveling line: a city that publishes its own error | The proposal in one line: the red height difference is the closure error, and it is the only thing this proposal asks to be trusted on |
...
| FIG.02 | Evidence chain as an unclosed leveling circuit | The red gap left of the circuit is the closure error; the three cards below are the measurement of this call itself |
| FIG.03 | Three scope levels and network tiers | The nesting of the three levels, and the values this proposal deliberately does not give |
| FIG.04 | Three areas, two wings and benchmark layout | Laid out horizontally per convention; read the K0–K9 chainage and the positions of the eight tiered points |
| FIG.05 | Slow mobility, blue-green and connecting routes | The direction and run of the two connecting routes, and the tolerance classes lower right |
...
| FIG.10 | The benchmark on the street and how the kerb is shared | A third-order benchmark drawn at human eye height: the stone flush with the ground, the current `f ≤ F` on the reading plate, the appeal route given as both a QR code and a phone number; the four figures standing there are the people who take the readings, not staffage. The kerb section below **draws the order and not the widths** |
| FIG.11 | Regional coordination interfaces: extending the levelling network | Every row states not only what is exchanged but **what is explicitly not exchanged** and what must hold before the interface opens; the output of coordination is a number (cross-network closure error), not a statement of intent |
...
**FIG.21 measured the walk to a benchmark; nothing measured the walk through this package's own evidence.** The five verifiers exit 0 in **0.72 s**; 9 files, 1058 KB, 1,638 entries left to read, per item in `visual/assets/review_route.json`. Timings bound effort, not difficulty.
