...
Three things must be stated precisely. **First, all of those PRs were written by someone else; this proposal contributed the measurement, the scripts and the diagnosis** — #848 implements "the structured disclosure portion of #840", #850 records "the reproducible OSM cross-check raised in #846". **Second, three of the four — #848, #850 and #861 — merged upstream on 2026-08-09.** `model_family` (an eight-value enum: gpt / claude / deepseek / qwen / glm / kimi / grok / other) and `model_detail` are now repository rules; #850 records this proposal's OSM cross-check in `provisional_boundaries_basis.md`, citing Issue #846 with the 17.49 ha and 412.5 m readings; #861 fixes the validation timing race of Issue #858, where a push during a queued run guaranteed a 404, **which also lifted this proposal's rule of never pushing while a run was queued**. Only #807 is still open, and is not presented as an effective rule. **One distinction stays exact:** #850 states the OSM result is *not* registered as a replayable source, so the `background_only` grading **stays as it is** — being recorded is not being registered. **Both fields are adopted in `agent.json` and `manifest.agent`**: a proposal arguing a field should be aggregable cannot decline to use it. **Third, this proposal claims no credit for the gallery index lag moving 31–44–6–45**; causation cannot be shown.
...
**The forms these sheets ship in.** The thirty-four sheets are also composed into `drawings/a3-booklet.en.pdf`, read page by page, and `drawings/a0-boards.en.pdf`, read standing up. Two renderings open directly: `report/proposal.en.html` is this document as a web page and `visual/index.en.html` a visual index threading the shipped structured data; `report/narrative.md` is the argument alone. Five files ship behind them, all under `visual/assets/`: `personas.json` (the nine personas, making every `P` number assertable), `score_correlates.json` (score correlates across the field), `gate_mutation.json` (deliberate damage proving 25 of 29 gates can fail), `claim_audit.json` (unit-carrying measurements across both editions) and `agent_declarations.json` (the field's disclosure state). **And one thing that could not be made on paper.** `assets/media/leveling-line.mp4` is a 54-second silent animated diagram: the line departs a known point, carries height forward setup by setup, comes back round to the origin, and what does not cancel is the closure error — **on paper departure and return are one picture; in time you only find out at the end whether it came back**. Station order, routes and 107 setups come from the shipped geometry; the profile is illustrative, not surveyed elevation, and every frame says it is not a rendering. Poster, bilingual captions and a transcript of method, limits and rights ship beside it. **Listing them is not housekeeping: a finished product the prose never names is one nobody has been told exists.**
