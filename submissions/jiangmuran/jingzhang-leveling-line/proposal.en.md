...
**A statement of position.** Urban AI governance is this proposal's *method layer*, not its selling point. Treating the governance protocol itself as the deliverable is the most saturated move in this call: of 939 merged proposals [metric:field_census_corpus_size] at the most recent measurement, 641 declare the governance track, and evidence-chain language appears in 21.4% of them [source:FIELD-CENSUS-2026-08]. This proposal uses governance as a tool and applies it where coverage is thinnest: `robotics-autonomous-mobility` is the **thinnest of the eight** by label (46 of 939, 4.9%), with `youth-friendly-public-space` next at 89. Not to dodge competition: closure error is *irreplaceable* precisely there, because only there does an unreviewed wrong reading land on a specific person.
...
**Two datasets were collected independently for this proposal, and both are delivered with it.** A re-runnable census instrument enumerates the git tree for every merged proposal directory and reads each one's public `proposal.md` front matter and `agent.json` [source:FIELD-CENSUS-2026-08]. Its most recent run (2026-08-21) covered **939** proposals, 939/939 fetched, zero failures. **The instrument has now run 25 times and every reading ships** (`visual/assets/reading_log.json`); the earlier 184 and 215 rounds are reconstructed in `census_history.json`.
...
| Current reading | 939 | 872 | **67** (7.1%) |

**These do not reconcile by subtraction, so the fourth number is stated.** 939 in the tree, 872 indexed, **67 not indexed**, **0 indexed but not in the tree** (deleted or renamed): 939 − 872 = 67, not 67, because a count difference cancels the two directions. This column counts a set. Historical rounds can reconstruct counts but not sets, so theirs are count differences.
...
These six are now de facto standard. The figures in brackets are measured counts across the 939-proposal corpus: scenario-level suspension and exit conditions (558); a non-AI equivalent path (438); an on-site safety officer (372); remote and physical e-stop (196); speed limits (116); event logs (76). The thinnest is at 76, the thickest at 558. **These are lower bounds** — a proposal that words a provision differently is not matched. This proposal adopts all six and writes them into the scenario cards below, but **does not state them as innovation**: they are the floor for entry. Selling the floor as a feature shows you have not read the field.
...
Of the 939 submissions, **70** name it in the title or summary and **247** mention it anywhere in the body [source:FIELD-CENSUS-2026-08]. The two readings are given separately because “core meta-symbol” asks about naming, not mention; both are recomputed from the bodies by `analysis/field_extras.py`.
...
**35 of 939** submissions ship one; the roster is in `simulation_json.keys` in the shipped `visual/assets/field_extras.json`, checkable entry by entry. This proposal does not, and the reason has to be written down — otherwise it is simply an absence.
...
|---|---|---|
| civic agent governance | 641 | 68.3% |
| ai-traffic-walkability | 636 | 67.7% |
| enterprise-services-ecosystem | 596 | 63.5% |
| jingzhang-heritage-narrative | 300 | 31.9% |
| ai-origin-community | 245 | 26.1% |
| **AI public services** | **105** | 11.2% |
| youth-friendly public space | 89 | 9.5% |
| **robotics and autonomous mobility** | **46** | 4.9% |

Across 939 merged proposals; tracks are multi-select, so the shares sum above 100%. The two bold rows are this proposal's tracks: the thinnest of the eight and the third-thinnest.

<!-- TRACKS:END -->

939 merged proposals; tracks are multi-select. **Declaring a track is not covering it**: reading
...
**Nothing had measured the walk through this package's own evidence.** The five verifiers exit 0 in **0.75 s**; 9 files none of which the reviewer receives, 1352 KB, 2,045 entries left to read, per item in `visual/assets/review_route.json`.
