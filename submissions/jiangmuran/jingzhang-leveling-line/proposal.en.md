...
8. `visual/assets/accessibility_qa.json` and `parity_qa.json` — computed contrast, A0 type height, HTML checks, bilingual parity per section; `label_collision_qa.json` one label over another; `i18n_order_qa.json` a template filling number slots out of order
...
**FIG.21 measured the walk to a benchmark; nothing measured the walk through this package's own evidence.** The five verifiers exit 0 in **0.97 s**; 9 files, 1034 KB, 1,631 entries left to read, per item in `visual/assets/review_route.json`. Timings bound effort, not difficulty.
