...
方案在 11.4 km² 总体设计范围内识别五处门户节点，每处门户承担从一种城市状态到另一种城市状态的转换职能。门户不是物理入口，而是功能转换的界面——人和智能体在门户处完成角色切换、速度切换或服务切换 [depth:overall_spatial_structure]。

| 门户编号 | 名称 | 位置 | 转换职能 | 触发条件 | 空间动作 |
| --- | --- | --- | --- | --- | --- |
| G1 | 清河门户 | 众智园北侧，清河与京张走廊交汇 | 从城市机动车速度到步行速度 | 机动车 ≤30 km/h → 步行优先 | 下沉广场 + 自行车停靠 + 算力参观厅入口 |
| G2 | 五道口门户 | AI 原点社区中部，五道口站周边 | 从轨道交通到街区生活 | 轨道到站 → 5 分钟生活圈 | 站前广场 + AI 导览终端 + 孵化器首层通透 |
| G3 | 清华园门户 | AI 原点社区南部，清华园火车站遗址 | 从历史遗产到 AI 文化 | 遗址参观 → AI 文化体验 | 遗址保护 + 开源成果展示节点 + 文化叙事墙 |
| G4 | 大钟寺门户 | 大钟寺站周边 | 从传统商业到智能原生业态 | 批发市场拆除 → 智能终端展示带 | 站前公共空间 + 四象限步行连通 + 数字资产交易入口 |
| G5 | 西直门门户 | 总体设计范围南端 | 从城市级到区域级 | 11.4 km² → 43.6 km² 统筹研究 | 标识系统 + 慢行接驳 + 信息导览 |

每处门户的空间动作均为概念建议，待 official 道路红线、文保范围与轨道保护范围补齐后调整。门户的转换职能不替代法定规划控制，而是为专业团队提供深化方向 [depth:overall_spatial_structure] [depth:risk_missing_data]。
...
本方案明确区分概念建议与法定规划，并建立与海淀区分区规划、控制性详细规划、城市设计导则三层法定体系的接口 [standard:MOHURD-CONTROL-DETAILED-PLANNING] [standard:MOHURD-URBAN-DESIGN-MEASURES]。接口不是替代，而是为后续法定深化提供结构化输入。

| 法定层级 | 本方案的角色 | 提供的输入 | 不提供的输出 | 接口文件 |
| --- | --- | --- | --- | --- |
| 分区规划 | 概念建议 | 产业功能分区、重点区定位、廊道结构 | 用地性质变更、人口规模调整 | compliance_matrix.json |
| 控制性详细规划 | 待深化方向 | 用地编码、开发强度概念建议、高度分区 | 容积率确定值、退线距离、建筑密度 | metrics.json (status=unknown) |
| 城市设计导则 | 概念图则 | 风貌四色基调、建筑体量分级、街墙贴线概念 | 景观视廊精确控制、文保建设控制地带 | standard_matrix.json |

**控规条件的待补清单**：容积率（floor_area_ratio）、建筑高度（building_height_m）、建筑密度（building_density）、绿地率（green_ratio）、退线距离（setback_m）五项控规指标均为 `status=unknown`，登记在 `metrics.json` 与 `assumptions.json#ASM-002` 中。取得 official 控规条件后，方案需按以下流程重算：

1. 替换 `geometry/site_boundary.geojson` 为 official polygon
2. 重新运行 `scripts/generate_geometry.py` 生成全部 9 个 GeoJSON
3. 重新运行 `scripts/generate_metrics.py` 复算全部 21 个核心指标
4. 重新运行 `scripts/generate_figures.py` 渲染全部 5 张 PNG
5. 重新运行 `scripts/render_proposal_html.py` + `scripts/embed_fonts_in_html.py`
6. 重新运行 `scripts/self_check_submission.py --mark-self-checked --json`
7. 检查 `compliance_matrix.json` 中 23 项任务的证据映射是否仍然有效

这一流程确保 official 数据发布后，方案能在一次重算中完成全部更新，正文无需逐条修改 [depth:risk_missing_data]。
...
方案为三处重点区分别编写"日常剧本"，描述一个典型工作日中人与智能体在空间中的交互序列。剧本不是实施方案，而是帮助评审者理解空间设计如何支撑日常活动 [depth:three_key_area_detailed_design]。

**众智园日常剧本（工作日 09:00-18:00）**：

| 时段 | 人群 | 空间动作 | 智能体角色 | 数据流 |
| --- | --- | --- | --- | --- |
| 09:00 | AI 研究者从清河站步行至众智园 | 经过 G1 清河门户，速度从机动车切换为步行 | 算力参观厅显示当日算力占用 | 匿名停留计数 |
| 10:00 | 研究团队在基础大模型研发中心工作 | 内核区封闭研发 | SC-09 芯片测试验证场景运行 | 测试数据本地化 |
| 12:00 | 午休，步行至清河滨水公园 | 走廊绿道休息 | AI 导览终端推荐附近餐厅 | 无个人数据采集 |
| 14:00 | 安全治理实验室红队测试 | SC-10 安全治理场景运行 | 攻击样本物理隔离存储 | 无外部数据流出 |
| 16:00 | 国际会议中心学术交流 | 外环布局 | AI 翻译辅助 | 公开会议文本 |
| 18:00 | 下班，步行至清河站 | 经 G1 门户返回 | 算力参观厅切换为夜间展示 | 匿名停留计数归零 |

**AI 原点社区日常剧本（工作日 07:00-22:00）**：

| 时段 | 人群 | 空间动作 | 智能体角色 | 数据流 |
| --- | --- | --- | --- | --- |
| 07:00 | 学生从五道口站到校 | 经过 G2 五道口门户 | SC-06 AI+教育场景启动 | 学习行为≤6个月 |
| 09:00 | 创业者在孵化器工作 | 近校孵化带首层通透 | SC-03 企业服务智能体 | 企业数据本地化 |
| 12:00 | 居民在社区中心就医 | 5 分钟生活圈 | SC-07 AI+医疗辅助诊疗 | 医疗数据加密传输 |
| 14:00 | 开发者在 AI 原点广场交流 | 开源成果展示节点 | SC-01 AI 文化导览 | 匿名停留计数 |
| 18:00 | 居民在口袋公园休憩 | GREEN-003 学院路口袋公园 | 智能座椅充电 + 环境传感 | PM2.5/噪声公开 API |
| 22:00 | 夜间照明自动降低 | 居民区光污染控制 | 智能照明人感+光感 | 无个人数据 |

**大钟寺日常剧本（工作日 10:00-20:00）**：

| 时段 | 人群 | 空间动作 | 智能体角色 | 数据流 |
| --- | --- | --- | --- | --- |
| 10:00 | 消费者从大钟寺站到智能终端展示带 | 经过 G4 大钟寺门户 | SC-12 AI+商业推荐 | 购物行为 90 天 |
| 12:00 | 午餐，企业总部塔楼底层 | 首层架空通透 | SC-03 企业服务 | 企业数据本地化 |
| 14:00 | 数据要素交易中心参观 | 站东南象限监管沙盒 | SC-11 标准化验证 | 公开标准草案 |
| 16:00 | AI 里程碑公园参观 | PUBLIC-005 里程碑节点 | 12 个 AI 历史事件交互 | 无个人数据采集 |
| 18:00 | 下班高峰，四象限步行连通 | 地下通道 + 二层连廊 | SC-02 AI+交通慢行 | 聚合交通指标 30 天 |
| 20:00 | 夜间智能终端展示带灯光秀 | 品牌色灯光 | SC-12 推荐策略复盘 | 推荐偏见 7 天复盘 |

所有剧本中的时段、人群与空间动作均为概念建议，不构成具体运营计划；智能体角色按 `assumptions.json#ASM-006` 数据治理台账落实隐私边界与人工复核 [source:AGENT-TASKBOOK]。
...
12 张 AI 场景卡共享一套贯穿性治理约束，确保智能体行为在所有场景中保持一致的可审计性 [standard:GENERATIVE-AI-INTERIM-MEASURES]。这些约束不替代每个场景的单独数据治理台账（见 `assumptions.json#ASM-006`），而是提供跨场景的统一底线。

| 约束编号 | 约束内容 | 验证方式 | 适用场景 |
| --- | --- | --- | --- |
| X01 | 每个 AI 触点必须可被公众否决 | 检查否决通道是否存在且有效 | SC-01 至 SC-12 |
| X02 | 每个 AI 关键判断须人工复核 ≥1 人 | 检查复核记录与排班表 | SC-03/04/06/07/08/09/10/11 |
| X03 | 个人信息采集须单独同意（PIPL 第 28 条） | 检查同意书与授权链 | SC-06/07/12 |
| X04 | 非个人数据须脱敏后公开 | 检查脱敏算法与公开 API | SC-01/02/05 |
| X05 | 数据保存期限须公示 | 检查公示页面与实际执行 | SC-01 至 SC-12 |
| X06 | 数据泄露须 1h 启动响应、24h 通报 | 检查事件响应预案与演练记录 | SC-03/04/07/09/10/12 |
| X07 | 智能体决策须可追溯 | 检查决策日志与审计接口 | SC-03/04/06/07/09/10/11 |
| X08 | 公众申诉渠道须 7 天内回复 | 检查申诉记录与回复时效 | SC-01/04/06/07/08/12 |
| X09 | AI 生成内容须标注「AI 生成」 | 检查内容标注与用户告知 | SC-01/06/08/12 |
| X10 | 无障碍通道须与 AI 服务并行提供 | 检查物理通道与人工替代 | SC-01/02/04/06/07/08 |
| X11 | 老年人须有非智能体替代路径 | 检查人工窗口与电话服务 | SC-01/03/06/07/08/12 |
| X12 | 未成年人须有监护人授权机制 | 检查授权流程与年龄验证 | SC-06 |
| X13 | 商业数据须可携带、可删除 | 检查数据导出与删除接口 | SC-03/12 |
| X14 | 公共安全数据须受公安审批 | 检查审批文件与操作权限 | SC-04 |
| X15 | 攻击样本须物理隔离存储 | 检查物理隔离措施与访问日志 | SC-10 |
| X16 | 标准草案须公开版本控制 | 检查版本历史与公开仓库 | SC-11 |

这 16 项约束（X01-X16）构成本方案的「智能体行为底线」，任何场景的实施前评审都须逐项检查。约束本身不是法律，而是基于 PIPL、数据安全法、网络安全法、生成式 AI 服务管理办法的法律框架下的操作性细化 [standard:GENERATIVE-AI-INTERIM-MEASURES] [standard:BARRIER-FREE-ENVIRONMENT-LAW]。
...
方案不直接给出拆改留名单，而是建立四步判定流程，待 official 现状建筑普查数据补齐后由专业团队逐栋执行 [depth:retain_renovate_demolish]。这一流程本身就是方案成果——它把不确定的结论转化为可操作的判定方法。

| 步骤 | 判定内容 | 所需数据 | 当前状态 | 判定后的空间动作 |
| --- | --- | --- | --- | --- |
| 步骤 1 | 建筑年代与结构鉴定 | 现状建筑普查（年代、结构类型、层数、基底面积） | 缺失 → 待 official 数据 | 年代 ≥50 年或结构特殊 → 进入步骤 2；否则 → 进入步骤 3 |
| 步骤 2 | 历史价值与文保评估 | 文物普查、历史建筑名录、文保范围 polygon | 缺失 → 待 official 数据 | 有文保价值 → 保留 + 改造为文化展示；无文保价值 → 进入步骤 3 |
| 步骤 3 | 功能适配性评估 | 现状使用功能、建筑容量、改造可行性 | 缺失 → 待 official 数据 | 可改造适配新功能 → 改造；不可改造 → 进入步骤 4 |
| 步骤 4 | 拆除重建必要性与可行性 | 权属、拆除许可、投资条件、环境影响 | 缺失 → 待 official 数据 | 拆除重建条件满足 → 拆除 + 新建；条件不满足 → 保留现状 + 待下一轮更新 |

**在 official 数据补齐前，方案给出的替代性处置原则**（非名单，仅方向性）：

- 清华园火车站遗址 → 保留 + 改造为 AI 文化展示节点（历史价值明确）
- 大钟寺批发市场 → 拆除重建为 AI 企业总部（功能已失效，权属待确认）
- 高校周边老旧科研建筑 → 改造为孵化器（结构可适配，年代中等）
- 清河南岸工业仓储 → 待步骤 1-4 完整执行后判定（年代与结构待普查）

这一流程确保在任何数据缺口下，方案都不给出不负责任的具体名单，而是提供可被专业团队直接使用的判定方法 [depth:risk_missing_data] [depth:retain_renovate_demolish]。
...
方案在 11.4 km² 总体设计范围内为四类通行主体分别建立路权原则，确保智能体时代的街道不再只服务机动车 [depth:traffic_rail_slow_parking]。

| 通行主体 | 优先级 | 路权原则 | 设计速度 | 空间保障 |
| --- | --- | --- | --- | --- |
| 步行者 | 1（最高） | 全路段步行优先，交叉口步行过街零等待 | 0-5 km/h | 人行道 ≥3m，无障碍坡道 100% 覆盖 |
| 自行车/低速接驳 | 2 | 独立非机动车道，与机动车物理分离 | 5-20 km/h | 非机动车道 ≥2.5m，每 500m 设停靠点 |
| 机器人/低速配送 | 3 | 专用机器人通道（可与自行车道共线），限速 15 km/h | 5-15 km/h | 机器人通道 ≥1.5m，每 200m 设充电桩 |
| 机动车 | 4（最低） | 让行步行与自行车，限速 30 km/h（重点区内） | ≤30 km/h | 道路红线内最窄车道，不设地面停车场 |

**路权冲突解决方案**：

- 步行 vs 机动车：交叉口采用抬高式人行横道，机动车须减速至 5 km/h 以下
- 自行车 vs 机器人：共线时机器人限速 10 km/h，自行车优先通过
- 机器人 vs 步行：机器人遇行人自动停止，行人 3 秒内未通过则机器人绕行
- 应急车辆：全路段应急车辆优先，智能体交通系统自动切换为应急模式

**慢行系统连续性保障**：京张遗址公园走廊（~7 km）和蓝绿复合慢行环（~15 km）全程无机动车交叉，仅在轨道站点处设地下通道或天桥跨越 [data:geometry/roads.geojson#ROAD-GW-01] [data:geometry/roads.geojson#ROAD-RING-01]。
