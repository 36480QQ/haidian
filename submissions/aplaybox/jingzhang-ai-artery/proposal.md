...
| SC-10 | AI 安全治理与红队测试 ★ | 众智园安全治理实验室 | 攻击样本、防御指标（《网络安全法》；国家 AI 安全中心授权为**未来启动前提/候选条件**，待正式授权证据，当前不作为已具备的合法性依据） | 候选控制者：国家 AI 安全中心（待主管部门确认）；候选处理者：受认可实验室 | **攻击样本物理隔离存储**；防御指标脱敏 3 年 | 无个人信息主体权利请求 | 安全专家复核所有结论 | 样本外泄 1h 物理隔离（运营设计目标）；4h 通报（运营设计目标） | 攻击样本外泄 |
...
| 审计台账 | AUDIT LEDGER | 触点 ID、发起时间、核验方式、委员会组成、决议、公示异议、恢复时间与条件全字段留痕；季度街道办公示，年度区审计局审计 | 否决流程第 (7) 条 |
