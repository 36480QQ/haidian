...
**用户画像（六类）**：深研的 AI 研究员（玄武智库带）、开源社区的开发者（原点社区）、五道口的高校学生、场地原住的老居民、远道而来的"朝圣"访客、以及高频穿行走廊的外卖骑手与配送员。每一类画像都对应明确的空间界面与服务层级，映射关系直接落在下方场景卡表的"空间载体"与"无 AI 通道"两列 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]。具体而言：研究员需要安静的深研界面与可控的实验载体（玄武带、#8）；开发者需要可展示、可协作的公共界面（原点社区、#1 与节气市集）；高校学生需要低价高频的第三空间（青龙得水带与藏风小筑）；老居民需要熟悉的日常服务与不依赖智能手机的通道（#5 健康导航、人工服务台）；朝圣访客需要可带走的文化叙事（#1 导览、#9 AR 步道）；骑手需要安全高效的穿行与停靠（#3 慢行护航、#4 配送港）。
