...
- **北纬社区**：输出共地人工兜底与无手机路径的运行手册；回流社区治理与老年参与经验，进入四季公共生活日历。
- **未来科学城**：输出验证院受控试验的空间与流程模板；回流能源与材料中试需求，作为验证院排期的外部来源。
- **怀柔科学城**：输出可复算状态任务的校核方法；回流大装置计量与标定资源，支撑首年基线测量的仪器校准。
- **北京经济技术开发区**：输出通过验证院与交换厅采用门的场景包（低速配送、无障碍导航）；回流规模化运行数据，用于修订退出阈值。
- **京津冀走廊**：沿京张遗产线组织人才与活动互访，交换厅承担走廊侧展示与转化窗口。
...
六个全球机制案例在本方案中的落位依赖三类要素，逐类写明谁提供、如何计价、如何退出（概念建议）：[assumption:A-OPERATIONS-001]

- **算力**：验证院阶段由入驻主体自带或采购公共算力配额，按试验时长计价；退出时配额释放回公共池，不留专用锁定。
- **资金**：第一跨与公共界面由更新项目资金承担；场景运行费用由运营者承担并在年度公开账本登记；任一场景退出时，其空间部件按可换填充周期回收再用。
- **数据**：场景数据字段、保存期限与人工复核点逐卡登记；退出时数据按登记规则归档或删除，不得转作它用。

### 场景验证与退出

十二张场景卡在运营字段之外补充统一验证框架。基线值为首年实测（当前未测），退出条件为开测前与运营者共同确认的约定线，不是效果承诺：[assumption:A-BASELINE-001]

| 卡 | 效果指标与观测 | 人工接管触发 | 对照设计 | 退出条件 |
|---|---|---|---|---|
| ①无障碍导航 | 同任务完成率，季度人工抽样 | 连续两次求助未响应 | 与无手机路径前后对照 | 完成率低于人工基线 |
| ②低速机器人配送 | 冲突事件率，日志加人工复核 | 任一碰撞或有效投诉 | 试点/对照街区 | 事件率高于约定线 |
| ③公共服务导航 | 柜台兜底使用率 | 队列超时 | 前后对照 | 兜底率持续上升 |
| ④铁路文化导览 | 内容溯源抽检合格率 | 溯源缺失 | 抽样对照 | 抽检不合格 |
| ⑤气候庇护提示 | 提示准确率对照气象实测 | 极端天气误报 | 前后对照 | 误报超约定线 |
| ⑥共享实验室匹配 | 匹配后实际使用率 | 纠纷或安全事件 | 前后对照 | 使用率低于约定线 |
| ⑦企业合规入口 | 咨询转人工率与错误引用 | 法规更新滞后 | 前后对照 | 出现错误引用 |
| ⑧夜间安静区 | 噪声投诉与实测 | 投诉聚集 | 试点/对照时段 | 实测超标 |
| ⑨维护分诊 | 工单响应时长 | 积压超限 | 前后对照 | 响应持续劣化 |
| ⑩活动安全复核 | 人工复核覆盖率（无人脸识别） | 覆盖缺口 | 抽样对照 | 覆盖率下降 |
| ⑪开放基准实验室 | 复算成功率 | 结果不可复算 | 重复运行对照 | 复算失败 |
| ⑫多语种帮助台 | 转人工满意度抽样 | 语种缺口 | 前后对照 | 满意度低于约定线 |

任一退出条件触发时，对应开关保持关闭，场景回到人工或低技术路径；恢复验收通过后方可重启。
...
在三处核心地标之外，公共组件库再命名两处扩展地标，构成五点朝圣序列（三处核心与指标口径一致，两处扩展不计入 landmark_count）：[metric:landmark_count]

L1 大钟寺第一跨——8×8 米可拆支撑与可逆地面，采用与退出都留下可见痕迹；L2 京张证据线——沿走廊布置的来源与假设公示带；L3 验证院透明门槛——公众可见的受控试验边界；L4 原点廊院安静环（扩展）——不被打扰的公共内环；L5 交换厅共享檐与人工窗口（扩展）——全天候人工兜底的城市界面。荣誉展示沿 L2 证据线组织，贡献者姓名与方案记录可持续保存。
...
五个慢变量构成验收基线。基线值为首年实测（当前未测），劣化阈值为开测前共同确认的约定线；任一触发即执行对应开关关闭并转人工复核：[assumption:A-BASELINE-001]

- **BL-01 冬至正午公共日照占比**：C03 室外公共空间固定网格实测（简化代理值 68.0% 仅作设计参考，不作基线）；每年冬至前后各测一次，固定点摄影加人工核对；实测值较上年下降超过约定幅度即触发遮挡审查。[metric:shared_floor_winter_public_sun_proxy]
- **BL-02 公共路线连通完成率**：五个公共空间全程步行审计，季度一次、每次不少于三条路径；出现不可达即触发即时整改。
- **BL-03 人工接管演练**：十二条编码恢复路径每季度实地演练一次；任一路径未达安全终态即冻结对应场景。[metric:handoff_recovery_pass_count]
- **BL-04 无手机同任务完成率**：与①卡对照设计共用样本；连续两个季度低于人工基线即扩充人工窗口。
- **BL-05 首跨可逆性演练**：8×8 米支撑年度拆装演练，记录工时与地面恢复状态；不可完全恢复即停止后续跨复制。

测量记录进入年度公开账本，字段包含观测点位、样本量、方法、数值与异常处理；账本字段格式随首年首次发布固定。
