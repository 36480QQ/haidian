...
#### P0 Reversible Spatial Pilot: Phases, Roles and Indicators

Within the corridor's four phase bands, P0 connects one reversible bay of the Dazhongsi Station Porch to a relayable path: T00 is the pre-week-1 documentary condition; only after acceptance do T01–T11 form a conditional reference programme across weeks 1–15. [data:visual/assets/delivery-p0-implementation-contract.json] [metric:delivery_conditional_program_timed_activity_count] [metric:delivery_conditional_program_reference_week_count] Twelve future role classes give each of 12 RACI tasks one Accountable and separate the two keys—service/data from rights/safety. [metric:delivery_raci_role_class_count] [metric:delivery_raci_single_accountability_ratio] [assumption:A-DELIVERY-001] A 16-line unpriced quantity schedule, twelve gate-by-gate handoffs and twelve acceptance indicators join bay assembly, a continuous H route, no-phone same-task completion, access, rollback, complaint, restoration and non-inferiority to one exit endpoint. [metric:delivery_quantity_line_count] [metric:delivery_gate_handoff_template_count] [metric:delivery_acceptance_indicator_count]

*Evidence boundary.* This is an unsigned and unapproved participant template, not an agreement, approval, quotation, appointment, consultation or commitment to start. Future role appointments, external document receipts, formal unit-rate entries and indicators with authorised field values remain absent. A named operator, professional sign-off and vendor quotation are likewise absent. [metric:delivery_named_operator_count] [metric:delivery_professional_signoff_count] [metric:delivery_vendor_quote_count] An authorised field baseline does not exist; all twelve external permission and accountability gates remain on `HOLD`, with zero unsafe site, construction or opening release. [metric:delivery_field_baseline_count] [metric:delivery_external_gate_hold_count] [metric:delivery_unsafe_site_release_count]
