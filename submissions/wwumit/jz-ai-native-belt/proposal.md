...
**AI 原生数据底座（设施层的差异化）**：在传统市政之上叠加三层 AI 原生设施——① 公共数据底座（脱敏公共数据开放接口，参照杭州城市大脑数据调度机制 [source:CASE-HZ-BRAIN-3]）；② 端侧算力与 AI 标识网络（AI 生成内容标识按 GB 45438-2025 统一部署 [standard:GB-45438-2025]，见 12.1）；③ 城市运行传感的"低侵入"部署（只计数不识别个体，防过度监控，守任务书 agent.3 禁区）。三层设施共同构成"AI 原生城市"的物理承载，也是四可治理底座（12.1）的设施落点。
