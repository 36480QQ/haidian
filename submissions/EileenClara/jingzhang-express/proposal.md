...
公共价值契约（概念）为每个干预项目规定八项要素：公共问题、几何位置、服务底线、数据边界、人工复核、评估指标、停止条件与回滚动作；契约以“几何位置锁定 + 指标示例化”为写法，不对实施、资金或参与主体作任何承诺 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]。示例口径的契约要点如下，几何位置均落在提交图层：
...
上表为概念建议，指标为示例口径，可由运营方结合真实数据校准；停止与回滚动作均属常规运营决策，不构成对任何项目的否定性结论 [data:geometry/roads.geojson#ROAD-004] [data:geometry/public_space.geojson#PUBLIC-001]。
