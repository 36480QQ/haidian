...
“轨迹织城 / RailWeave Open City”以百年京张连续公共空间为**城市经线**，以清河、小月河与高校、转河三条区域协同廊道为**城市纬线**，以可准入、可暂停、可解编的 AI 应用为**技术线程**，形成“一轴三廊三核、三链协同”的总体结构。先织牢公共空间、普通服务和区域协同，再有序织入智能应用；即使 AI 暂停，城市基本服务仍可连续运行。
...
| 实施准备 | 一个场地、一个 S06、100 日；四个工作阶段、六个决策窗口、六包 BoQ、E0—E6 | G0 场地、角色、报价和参与授权 |
| 公共价值 | 普通服务不断线，无账号、可拒绝、可申诉、可恢复 | 六类使用群体同任务走查和现场基线 |

### 首幅实施样布：一个场地、一个 S06、100 日

首期不同时启动三处工程，而是在 AI 原点选择一个经权利核验的共享首层候选界面，围绕一条约 100—150 米决策路线、3—5 个决策点和一个 1:1 无障碍服务湾，开展 S06“普通服务 + 可逆 AI 辅助”限定样布。它检验的不是“AI 能否演示”，而是同一名使用者在不注册账号、断网或拒绝 AI 时，能否不增加费用、不产生不合理等待和异地行程地完成同一任务。
...
| **61—90 日｜授权共测** | 仅在授权成立后，限时限人比较同一任务的完成、等待、距离、费用、求助、误导、投诉与员工负担 | E3 同任务对照；D90 只签发强制收尾交接单 | 普通服务劣化或发生严重安全、隐私、公平事件 |
| **91—100 日｜强制收尾** | 撤权、删数、拆机、恢复饰面、纠正固定导视并续接人工服务 | E5 六项退出签收；D100 以 E6 作唯一的保留/修改/撤除/不推进决定 | 任一退出项未签收，不得延长或扩点 |

本次提交已形成 S06 百日实施准备包；现场启动仍须由有权主体完成 G0 前置核验。当前仅处于未启动现场运行的准备阶段，下一步为 E0 场地与授权核验，不以合成测试替代现场授权，也不影响对方案完整度和实施准备度的评审。[data:visual/assets/readiness_decision_receipt.json]

| 使用群体 | 进入方式 | 普通服务底座 | AI 增益与退出后留存 |
|---|---|---|---|
| 居民 | 社区、现场、电话 | 有人受理、纸质指引 | 问题预填；退出后保留问题闭环 |
| 青年人才 | 人才服务与共享首层 | 人工咨询、线下活动 | 机会匹配；退出后保留开放工具 |
| 企业 | 场景需求与验证申请 | 人工清单、专业咨询 | 方案比选；退出后保留成本与验证证据 |
| 高校院所 | 开放问题与复现任务 | 线下工坊、文档接口 | 复现辅助；退出后保留可引用方法 |
| 游客 | 双语导视与文化路径 | 固定导视、人工讲解 | 个性化解释；退出后保留双语内容 |
| 老年人与残障人士等必要支持群体 | 无账号同任务通道 | 触觉、易读、电话、人工 | 自愿辅助；拒用不加价、不绕行 |
...
| 61—90日：授权成立后的限量共测 | 在场地、数据、参与和停止权限依法成立后，由应用责任角色组织限定空间、时段和服务对象共测 | 普通与数字路径对照、求助/误导/申诉台账；D90只形成强制收尾交接单 | 任一授权缺失或撤回、普通服务中断、严重安全/隐私/公平事件、退出责任或资源不可用时立即退出 |
| 91—100日：强制收尾与唯一决策 | 撤销权限、删除或合法移交数据、拆除设备、恢复饰面、校正固定导视并续接人工服务 | E5退出六项回执；D100以E6作唯一的保留/修改/撤除/不推进决定 | 任一退出项未签收、退出预算或维护主体缺失时不得扩展 |
