...
license: "CC-BY-4.0"
summary: "如果今晚关闭 AI，明天城市仍应照常运行。方案以百年京张公共空间和普通服务为经，以三条区域协同廊道为纬，把 AI 作为可准入、可暂停、可解编的技术线程织入城市；由 AI 原点组织问题、众智园独立验证、大钟寺首用转化，并以一个 S06、一个场地、100 日限定样布验证后再决定。"
...
## 如果今晚关闭 AI，明天城市还能不能照常运行？

这是“轨迹织城 / RailWeave Open City”对百年京张提出的第一道问题，也是所有项目的底线。京张铁路把一条工程线路变成了城市发展的经线；今天的任务，不是再铺一层与城市生活脱节的技术，而是先把公共空间、普通服务和区域协同织牢，再让 AI 以可准入、可暂停、可解编的方式进入。

方案由三类线共同组成：**百年京张公共空间和普通服务是不断线的“城市经线”**；连接高校、园区、社区、站点和河道的三条横向廊道是“协同纬线”；每一项 AI 服务只是附有期限和责任人的“技术线程”。城市经纬决定公共价值，AI 线程接受双门审核。空间尚未成熟，不能以算法测试通过代替实施条件；AI 尚未通过数据、人工接管和退出审查，也不能以场地成熟为由先行部署。**先把城市织牢，再把 AI 织入；普通服务不断线，智能应用按需接入。**

![轨迹织城总览｜不断线的城市经纬与可退出AI线程](assets/figures/site-overview.png)

> **这张图怎么看。** 先看纵向京张公共经线，再看清河、小月河与高校、转河三条协同纬线；三处重点区不是三个并列园区，而是同一项目从“提出问题—独立验证—首用转化”的三个节点。图中边界和组件均为研究阶段建议，不是法定红线或建设承诺。

### 一条主线、三个节点、一次有期限的验证

约 43.6 平方公里统筹研究范围负责**区域定链**，约 11.4 平方公里总体设计范围负责**走廊织网**，约 368.4 公顷三处重点区域负责**项目落地验证**。AI 原点把居民、服务单位和企业的真实问题转成可测试任务；众智园复现版本、核验安全和人工接管；大钟寺连接应用单位、合规采购、运行维护和 90/180 日后评估。中关村科技服务翼提供专业服务，小月河场景赋能翼组织公共问题反馈。[source:OFFICIAL-ANNOUNCEMENT] [source:AGENT-TASKBOOK] [depth:three_level_scope_framework]

| 任务书要求 | RailWeave 的空间回答 | 可交付结果 |
|---|---|---|
| 三大定位 | 京张文化经线承载百年叙事，普通城市服务承载 AI 生活体验，三核项目链承载融合创新 | 一条可步行的责任叙事线、一套可拒绝的服务界面、一条可复核的创新转化链 |
| 五大功能 | 协同研发、测试验证、公共服务、成果转化、交流展示分别进入三核和两翼 | 每项功能均对应场地、角色、证据、停止和退出条件 |
| 三区两翼 | AI 原点组织问题，众智园独立验证，大钟寺首用转化；两翼提供专业与场景输入 | 问题单、验证结论、首用回执和年度知识回流 |
| 六项专项任务 | 总体统筹、全栈生态、AI+场景、公共空间、文化融合、长期运营共同指向一个城市织体 | 五张主图、三处原型、十二类场景、100 日样布和 G0—G6 项目门 |

### 首个可执行抓手：一个场地、一个 S06、100 日

方案不同时启动三处工程，先在 AI 原点选择一个经权利核验的共享首层候选界面，围绕一条约 100—150 米决策路线、3—5 个决策点和一个 1:1 无障碍服务湾，开展 S06“普通服务 + 可逆 AI 辅助”限定样布。它检验的不是“AI 能不能演示”，而是同一名使用者在不注册账号、断网或拒绝 AI 时，是否仍能以不增加费用、不产生不合理等待和异地行程的方式完成同一任务。

| 100 日窗口 | 现场动作 | 必须形成的证据 | 不能转段的情形 |
|---|---|---|---|
| **0—10 日｜定界** | 核验权属、路线、开放时段、服务主体与普通服务基线协议 | E0 场地与授权；只允许支出基线和退出准备费用 | 场地、责任、普通服务或退出条件不清 |
| **11—30 日｜共走** | 完成现场基线、1:1 被动样机和受影响用户走查 | E1 基线与无障碍走查 | 关键无障碍阻断未关闭或无等效服务 |
| **31—45 日｜算清** | 编制六包工程量清单、三价对比、值班和单点 RACI | E4 报价可比；人工、法务、保险和退出全价 | 关键报价覆盖不足 80% 或退出未计价 |
| **46—60 日｜拆编** | 在封闭环境测试断网、过期信息、错误建议、撤权与人工接管 | E2 版本、数据与失败关闭记录 | 任一高风险接管分支未走通 |
| **61—90 日｜共测** | 仅在授权成立后，限时限人比较同一任务的完成、等待、距离、费用、求助、误导、投诉与员工负担 | E3 同任务对照 | 普通服务劣化或发生严重安全、隐私、公平事件 |
| **91—100 日｜复原再决定** | 撤权、删数、拆机、恢复饰面、纠正固定导视并续接人工服务 | E5 六项退出签收；E6 保留/修改/撤除/不推进决定 | 任一退出项未签收，不得延长或扩点 |

当前状态为 **G0 / not authorized / not run**。场地权利、角色任命、现场数量、有效报价和参与授权均未完成，现场绩效保持空值，结论为 `do_not_advance`。这不是项目失败，而是依法依规进入现场前应有的诚实状态。下一步只核验 E0，不以更多合成测试替代现场授权。[data:visual/assets/readiness_decision_receipt.json]

### 一名不注册账号的访客，怎样使用这条创新带

一名需要无障碍支持的访客，沿固定导视、触觉地图和连续普通路径到达服务湾，通过纸质资料、电话或有人柜台完成咨询；自愿启用 AI 时，系统只提供建议，例外、决定和申诉仍由人工处理。他可以在 AI 原点提出问题，在众智园看到经脱敏的验证过程，在大钟寺比较普通服务与智能辅助后决定是否采用。断网、误导或项目退出时，人工服务继续运行，设备可以拆除，固定导视、开放工具和失败档案留在城市里。
...
| `agent.3` AI+场景与活力城市 | 十二类场景契约、三项实施准备度原型、S06 0—100 日最小实施包 | `ai-service-blueprints.png`；`weave_contracts.json`；`s06_minimum_delivery_package.json` | `S01—S12`、`S06-MDP-P1—P4`；受控取证框架已形成，现场与专项审查待开展 |
