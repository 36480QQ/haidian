...
summary: "把京张遗址公园接到大钟寺的日常：锈轨引导慢行，公园里设共用林下工位；短驳只作概念。不另造绿廊，不办科技秀场。"
...
京张铁路遗址公园已经把铁轨留在了城里：有人跑步，有人推童车，有人沿轨散步。本方案不重新发明这条公园，也不把它做成科技秀场。问的是另一件事——**大钟寺这一端怎么接进公园，人出站之后往哪走、在哪停、能不能坐下来安静做事** [source:AGENT-TASKBOOK]。

回答叫「林轨」：

- **林**：大钟寺段按公园做——修剪草坪、疏林、树冠压住楼顶；望出去是树，不是对面幕墙。
- **轨**：锈轨只做引导（眼睛跟着走），轨间铺木栈或石粉；重要节点之间，概念上保留低速短驳，与步行分区。

> 一句话：楼在树下，人在公园里，铁轨生锈给人看方向——望出去是自然。

**本方案不做的事**：全区楼宇总控、网红打卡装置、灯光秀、把绿化条当成公园、把共用工位做成私人舱。

动线只加一条轨导走廊。走廊以外，路网怎么走还怎么走。
...
同一条走廊分段处理：多数路段只做引导锈轨；少数路段概念上保留短驳能力；交叉处慢行优先。短驳失败或未获深化，引导轨与公园仍可独立成立。
...
| **轨径** | 从大钟寺站出站后，锈轨嵌入地面引导人进入公园 | 不架高做景观桥；不抛光；不喷漆；不加灯带 |
...
![引导锈轨穿过公园](assets/concepts/guiding-rails.png)

![概念短驳与步行分区](assets/concepts/ai-rail-transport.png)
...
| # | 场景 | 空间载体 | 做什么 | 谁用 |
|---|------|---------|--------|------|
| 01 | 出站入园 | 大钟寺站各出口 | 出站后锈轨引导进入公园。树荫遮阳避雨，不靠灯光秀 | 所有到达者 |
