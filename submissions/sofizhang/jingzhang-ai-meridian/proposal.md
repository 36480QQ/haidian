...
summary: "京张铁路的零公里标立在走廊南端——方案由此得名『零公里以北』：以遗址公园贯通绿轴为主动脉的一脉三枢、两翼六环结构，六条缝合慢行环重新连接被走廊分隔的城市，12张AI场景卡、4类测试验证场景、6类用户画像、4处AI朝圣地标与K0-K9十座站点，让创新从零公里原点向北生长，回应百年京张AI创新带的全部公告与智能体任务。"
...
**场景—空间—运营映射。** 每张场景卡对应：一个空间落点（场景节点图层）、一个运营主体建议（零北运营主体/商户/市政专业单位）、一组观测指标（进入AI创新带指数）与一个风险登记项。小月河场景赋能翼作为东侧日常验证界面，承接 S01/S06/S08 类生活场景的外溢延展，属统筹研究层面的概念建议 [source:AGENT-TASKBOOK]。四类测试验证场景（S02/S03/S10/S11）构成产业测试验证矩阵，从交通、物流、能源算力、出行四个方向提供可申请的实测环境 [metric:test_validation_scenario_count]。
...
**K0-K9 零北站点系统。** 导视、地标与场景的空间载体最终落到十座站点上：它们沿用京张铁路里程碑传统自南向北编号，每座站点同时是一个更新项目触媒和一组场景的落位（详见 A3 图册线路图页与 A0 展板一）[data:geometry/public_space.geojson#SN-12]。
