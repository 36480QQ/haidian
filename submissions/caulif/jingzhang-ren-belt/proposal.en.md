...
**Reviewable agent.1–agent.4 outputs.** The master mark, reverse mark, minimum sizes, palette, typography and wayfinding rules are supplied in `assets/brand/ren-belt-brand-sheet.svg`, `ren-belt-logo.svg` and `ren-belt-logo-reverse.svg`. The six public case mechanisms and their spatial translations are recorded in `visual/assets/evidence/case-study-table.json`. `visual/assets/evidence/scenario-operations-matrix.json` expands the twelve scenario names into space, purpose, operations, data guardrails and acceptance measures; `visual/assets/evidence/landmark-system.json` records four landmarks, a separated honour/display system and a public-space component kit. Cases are mechanism references only, not partnerships, endorsements or implementation promises.

**Agent.6 operations.** The operating funnel is “problem call → open prototype → controlled test → human review/fairness audit → conversion or exit”. A Q1 open lab, Q2 blue-green repair week, Q3 global AI belt week and Q4 public audit/memory walk form the annual calendar in `visual/assets/evidence/annual-operations.json`.
...
**Implementation cards.** `visual/assets/evidence/renewal-implementation-cards.json` expands all twelve renewal projects with phase, owner, dependencies/approvals, safety prerequisites, investment level, quantified acceptance, operations resources and stop conditions. Projects that depend on tenure, regulatory controls, rail, municipal, heritage or flood conditions remain light, reversible pilots rather than approved works.

**Inclusion and fairness.** `visual/assets/evidence/inclusion-framework.json` adds spatial standards and acceptance measures for disabled people, low-income households, carers and children/older people, night workers and digitally excluded users. Paper maps, phone, staffed, audio and non-digital booking channels run alongside digital access. Quarterly resident forums, scenario open days and an annual third-party fairness audit create a participation–complaint–remediation loop; every automated scenario retains human takeover, exit and appeal.
...
This PR revision adds the public locational reference raster [source:OPENSTREETMAP-CONTEXT], Qinglongqiao station and Jing-Zhang Railway Park context photographs [source:WIKIMEDIA-QINGLONGQIAO][source:WIKIMEDIA-JINGZHANG-LOCOMOTIVE], original identity assets [source:SUBMISSION-BRAND-ASSETS], and case/scenario/delivery/inclusion evidence packs [source:SUBMISSION-EVIDENCE-PACKS]. All are bundled offline; copyright and usage boundaries are recorded in `report/copyright_statement.md`.
