...
- **对应旁白：** 第 4 句。
- **重摄记录：** 本镜头于 v5.9 按当时现行的 `assets/figures/mobility-bluegreen.png` 重摄。该图自本片首次渲染（v5.0）以来先后重造为《十节点服务可达域与蓝绿慢行系统图》并于 v5.7 再度重排为《AI 关闭后可达域收缩：差集是审计靶区》，图内主张与构图均已改变，本镜头此前沿用的 v5.0 图件副本随之作废。该图在 v5.9 之后又经 v6.4 几何挪移重绘、v7.7 与 v8.0 两次读数口径改写，现行图题为《按本图自设口径：AI 关闭后可达域收缩 49.3%》；成片未随之重摄，片中本镜头呈现的仍是 v5.9 时点的图件版本，此处如实登记，读者应以 `assets/figures/` 下的现行图版为准。重摄同时按现行图件口径改写本镜头的章节头与要点行：章节头由「骨架与等价服务网络 / FRAMEWORK AND EQUIVALENT SERVICE NETWORK」改为「骨架与可达域审计 / FRAMEWORK AND REACHABILITY AUDIT」，第二条要点由「直接多模态线 + 低刺激安静替代线」改为现行图件图例的原文「通用可达底线（主脊）· 低刺激安静替代路线」，以免片中出现权威图已不再主张的「等价」表述。旁白、字幕文本与全片时间轴不受影响。
...
| 交互场景录制 | 用同一浏览器打开本包 `visual/index.html` 第 16 章交互场景，把 `requestAnimationFrame` 替换为可手动推进的队列，使动画相位每帧严格递增固定量，再逐帧截取「AI 开关条 + 场景舞台」区域（1344×595 CSS 像素，设备像素比 2，即 2688×1190 实际像素）。录制过程中依次执行两次点击：「播放引导流光」与「AI 关闭」。被录制页面的文件内容未作任何修改，运行期间不加载任何远程脚本、字体、瓦片或统计代码。本镜头已于 v5.4 按同一脚本、同一几何与同一点击帧号对当时的页面重新录制，因此画面中的读数面板即现行的声明与实测分列六行版本。**此后的包体修订中场景数据集已重新生成**：`geometry/roads.geojson` 的通用可达底线（ROAD-001）与低刺激安静替代线（ROAD-005）由两点示意直线加密为贴合真实街道走向的 24 顶点折线，`visual/assets/scene-data.js` 随之重出，因此片中第 16 章画面里的两条线位仍是重录当时的直线示意；现行权威画面为 `visual/index.html` 第 16 章的实时渲染与静态后备图 `visual/assets/scene-fallback{,.en}.png`，两者与片中画面不一致时以现行文件为准。此项差异只涉及线位形态，不涉及镜头内的两次点击、节点位置与读数面板的任何一行 |
