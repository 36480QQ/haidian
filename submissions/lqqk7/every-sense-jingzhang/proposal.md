...
**其二，万感服务桌的四个位序不可调换。** 大钟寺包容采用市场的最小物理载体是一张万感服务桌，四个位序依次为人工窗口、多通道信息位、AI 开关、退出回执位。顺序是判据而不是布置建议：人工窗口先于一切 AI 构件设立，退出回执位永远在最后一步之后仍然在场。同一张桌子还承诺「不消费也能停留」——休息、如厕、问询与等候不以消费为前提，停留权先于商业界面成立；该承诺已作为属性登记于两处前厅类公共空间要素，深化与运营阶段按要素逐处核验 [data:geometry/public_space.geojson#PUBLIC-007]。服务桌是这套制度里最容易被别的城市直接带走的一件东西。
