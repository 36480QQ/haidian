...
| 二、产业生态、指标体系、创新网络与产业空间组织 | 统筹研究章的产业重点表、区域协同表与八类要素保障表 | `metrics.json` 全部指标与 `compliance_matrix.json` agent.2 条目 |
