...
交互层在 v7.2 补齐两个维度：第 16 章交互场景增加早高峰、日间、晚间、夜间四时段（与 AI 开关正交成八态，时段只改表现层与描述、不改任何读数——「AI 关闭 × 夜间」是全包最严苛组合，固定导视靠照明可读、人工值守是唯一实时通道）；新增第 17 章离线三维主脊（three.js r160 以 MIT 许可离线入包，几何全部同源于交互场景数据集、无一处手写坐标，AI 开关、昼夜切换与十节点相机漫游全部键盘可达，无 WebGL 时自动退回静态图且空间关系不丢失；一切运动由使用者触发，不自动播放）。
...
**法律接口登记（不做合规结论）。** 本方案当前未获授权、未现场运行、不处理任何真实个人数据，但一旦进入真实运行将触发三条法律谱系的接口：个人信息保护法（参与者数据与分组自报）、数据安全法（运行期数据分级留存与边界）、算法治理谱系（面向公众的算法服务备案义务判定，须在 L3 前完成）。三条接口的触点、触发条件与责任方登记于独立文件，全部状态为「未触发」、全部合规状态保持 unknown——本方案不做任何合规结论，只回答「届时要过哪些门、由谁去过」；清单不穷尽，取得授权后由具备资质的法务逐项复核 [data:visual/assets/legal-interface-register.json]。国家层面的政策语境按来源纪律登记一条：国务院《关于深入实施「人工智能+」行动的意见》（国发〔2025〕11号，2025-08-27）把民生福祉与治理能力列入六大行动领域，本方案的服务等价与人工通道承诺与该方向一致——仅作方向锚点，不据此推断任何审批、配额或项目结论 [source:CN-STATE-COUNCIL-AI-PLUS-2025-11]。
