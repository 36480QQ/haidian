...
空间动作与角色严格匹配。众智园设置验证门槛、状态可视界面和人工接管点，避免未成熟技术直接进入开放空间；原点社区设置低刺激路径、共同设计室、易读服务前厅和真实反馈台，任何参与都须自愿、可理解并可退出；大钟寺设置多通道消费界面、现金与人工替代、公开采用回执和维护入口；其最小物理载体是一张**万感服务桌**——四个位序为人工窗口、多通道信息位、AI 开关、退出回执位，顺序不可调换：人工窗口先于一切 AI 构件设立，退出回执位永远在最后一步之后仍然在场。服务桌是制度里最容易被别的城市直接带走的一件东西。三处通过公园主脊和两翼支持相连，形成技术问题向生活任务转译、生活证据向产品和运营反馈的循环。
...
| 判定规则 | R-01 至 R-12 十二项风险条目各自的停止条件与恢复证据，构成“什么时候必须停、拿什么才能重开”的完整规则集（R-09 至 R-12 为 SEB v0.4 生命周期风险，v7.0 批次新增） | `risk.json` 中十二条 `stop_condition` 与 `resume_condition` [metric:risk_item_count] | 停止条件触发即停止，不以“限期整改”替代停止；恢复须提交证据而不是承诺 |
...
| **该控规已于 2026 年 8 月 11 日经北京市规划和自然资源委员会批复**（据北京日报报道）：范围东至新街口外大街、西至中关村大街、北至成府路、南至西直门外大街，9 个街区约 1668.2 公顷；以绿色公共空间建设引领区域城市更新，发展大模型、智能体、具身智能等新业态，构建生活圈与创新圈融合的一刻钟社区服务圈 | 北京日报 2026-08-11 报道（新闻报道层级；批复成果全文与法定图则尚未经公开渠道取得） | 否 | 报道不含任何地块控制值；本方案不据此推断容积率、高度、密度或绿地率，待批复成果公开后全部图层、指标与图纸整体重算（重算触发条件已入前提登记） |
...
登记邻近已公示地块的官方数值属事实参照，不构成对本方案地块控制值的推断；容积率、高度、密度、绿地率与退线继续标记为待补，触发条件是取得适用的官方控制文件及其计算边界，责任专业为控规实施与建筑测绘人员 [source:HD-LANJINGLIJIA-SCHEME-ADOPTION-20250606] [source:HD-LANJINGLIJIA-RENEWAL-CASE-20260612] [source:HD-URBAN-RENEWAL-GUIDELINE-2025]。
...
**万感定律。** 把上述三件史实读成一条线，京张一百年每一次进步的真正刻度不是速度，而是「谁能用得上」的门槛又降了一格——从工程师才懂的线路，到人人可乘的车站，到本方案要求的「每一种身体都能独立完成」。本方案把这条定律接着写下去：AI 时代的这一格，降给不同身体、感官与认知方式的人。四道证据闸门由此获得一组京张语汇的助记别名——G0 道岔（定线定权）、G1 站台（可上可下）、G2 试车线（封闭成对）、G3 发车（独立复核放行）；别名只助记，判据仍以闸门条文为准。
...
首个 90 天只建立“零阶段证据包”，不承诺开工或开放运营。第 0—30 天进入 **G0 资料与许可闸门**：核对官方范围、现场路线与服务清单，形成伦理、同意和数据处置协议，并把十六个场景改写成可复测任务脚本；R-01、R-02、R-05、R-06 任一责任或资料边界不清即暂停。第 31—60 天进入 **G1 可逆原型闸门** 与 **G2 封闭成对测试闸门**：制作可移除组件，在受控环境完成 AI 开启/关闭成对测试，交付问题日志、维护卡和匿名化测试证据；G2 收尾设 **AI 关闭日演练**——连续一个完整测试日全程保持关闭态运行，全部任务只经固定导视、触觉引导与人工通道完成，演练记录（含未完成任务及原因）作为 G3 复核的必交证据，把「AI ON = AI OFF」从主张钉成一项可执行、可复核的测试设计；R-03、R-04、R-07、R-08 的替代路径、运维和参与条件未闭合时不得升级，不得以内部演示替代参与者证据。第 61—90 天进入 **G3 独立复核闸门**：由未参与制作的专业人员与参与者代表复核，逐项检查 R-01—R-12 全部十二项风险条目的停止条件和恢复证据 [metric:risk_item_count]，结论只能是修正、限定试点或停止；四道闸门缺一不可，未通过 G3 不进入开放空间 [depth:phasing_implementation] [metric:gate_count]。
...
L1 可逆试验阶段，构件安装前须在现场与线上同时公告可撤除范围、试验期限、责任人与撤除方式，公告采用易读中文、图示与语音三种等价形式。L3 限定开放阶段，须公开当期任务脚本的可读摘要、参与者构成的角色组分布与已知未解决障碍。L4a/L4b 采用与常态运行阶段，须公开采用回执、维护责任人、退出方式与复审到期日。三阶段的公告都不以扫码、注册或下载应用为前提 [metric:scenario_exit_path_coverage]。
...
| 展示页与代码（28） | 本方案自研，零第三方依赖 | 手写 HTML/CSS/JS，零外部库 | SEB 规范已授予 CC BY-SA 4.0（随包授予文件） | 离线运行，无外部请求 |
