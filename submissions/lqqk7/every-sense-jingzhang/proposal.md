...
| --- | --- | --- |
| 一、案例参照与发展理念 | 「设计依据与资料清单」的案例转译表与 AI ON = AI OFF 命题 | `sources.json` 六条 CASE-* 来源与转译表的不可移植条件列 |
...
企业旅程图把 L0—L4 五级机制换成企业视角讲一遍：从开放问题库进入，逐级绑定准入、停止、恢复与责任，到大钟寺取得采用回执；企业得到真实问题、测试公信力与可主张的公开凭证，同时必须留下可复核的日志、公开的失败样本、维护责任链与退出预案。成效不以入驻数量或签约额衡量。机制与更新项目章逐句同源，非招商流程 [depth:renewal_projects_policy]。
...
叙事的历史锚点取公有领域通识史实：京张铁路于 1905—1909 年由詹天佑主持设计建造，是首条由中国工程师自行设计施工的干线铁路；青龙桥站以「人」字形折返线闻名，其线路工区自通车起持续运转逾百年 [source:JINGZHANG-RAILWAY-QINGLONGQIAO-NRA]。这三件事实只用于文化叙事定位——「自主设计」对应本方案「自主可核对」的生态判据，「人字形以地形换坡度」对应「以等价通道换包容」的设计转译——不携带保护等级、边界或规划结论，文保判断一律以官方档案为准。
...
| 展示页与代码（18） | 本方案自研，零第三方依赖 | 手写 HTML/CSS/JS，零外部库 | SEB 规范拟议 CC BY-SA 4.0（未定稿） | 离线运行，无外部请求 |
