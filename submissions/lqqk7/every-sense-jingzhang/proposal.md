...
六类画像用于组织需求与测试，不是诊断标签、商业分群或已经完成的用户研究 [metric:persona_count]。每类画像都对应空间动作、设计提出的服务等价目标和隐私边界；真实共同设计须由参与者自愿加入，获得合理便利，理解数据用途并随时退出 [assumption:A-CO-DESIGN-001]，共同设计席位是否覆盖必需角色由角色覆盖率衡量而非由人数衡量 [metric:co_design_role_coverage_ratio]。中国无障碍环境建设法支持平等参与与无障碍信息交流的方向，但不能替代具体产品、建筑或交通的专业符合性评价，也不把本方案的“等价服务”概念自动转化为具体法定义务 [source:ACCESSIBILITY-LAW-CN]。该法关于现场指导与人工办理的要求，严格限于其条文列举的医疗、社会保障、金融、生活缴费等公共服务事项；六类画像与合规矩阵按此边界组织，方案其余的人工兜底均标记为概念设计建议 [standard:BARRIER-FREE-ENVIRONMENT-LAW]。换一句更准确的说法：下表是任务约束维度表而不是人群分类表——第一列的六个维度给出六组任务约束（信息获取通道、沟通方式、行动方式、认知与感官处理、语言与数字门槛、服务提供与维护），不用于识别、标注或统计任何人。表以这六个维度为骨架，逐维展开为十类单列行：十类是六个维度的细分展开而不是新增维度，维度框架不变，`metrics.json` 登记的画像口径仍为六 [metric:persona_count]。细分的唯一目的是让每一行只装一组互不混合的任务约束，便于逐行绑定测试脚本与判据；行动方式与服务提供两维不再细分——前者的四种情形（轮椅、推车、临时受伤、老年步态）指向同一组物理条件即连续净宽、坡度与休息点，拆开不产生不同的设计动作，后者本身只有一类角色。年龄、身份与诊断不构成任何一行，因为它们不是任务约束：儿童、青少年与老年人的任务约束分别落在「认知与学习方式不同者」「无智能手机与低数字素养者」与「轮椅、推车、临时受伤与老年步态者」三行，不另设人群行。
...
诚实边界必须与计划并列：计划已备，执行待定。本计划涉及的一切读数仍为零值或 unknown，一个数字都还没有产出。执行角色如实登记为「提交者本人或其自愿邀请的志愿者」，本方案没有已组建的团队，也不虚构团队；白杖实探项等待白杖使用者本人参与，不以视力正常者的模拟替代。本计划不给日期、不给里程碑：给出排期会让一个尚无授权主体、尚无人员的计划看起来像已排定的工作。回填结构已预留，回填后七项包容性指标不因此改变 [data:visual/assets/spatial-audit-readings.json]。
...
首个 90 天只建立“零阶段证据包”，不承诺开工或开放运营 [assumption:A-IMPLEMENTATION-001]。第 0—30 天进入 **G0 资料与许可闸门**：核对官方范围、现场路线与服务清单，形成伦理、同意和数据处置协议，并把十六个场景改写成可复测任务脚本；R-01、R-02、R-05、R-06 任一责任或资料边界不清即暂停。第 31—60 天进入 **G1 可逆原型闸门** 与 **G2 封闭成对测试闸门**：制作可移除组件，在受控环境完成 AI 开启/关闭成对测试，交付问题日志、维护卡和匿名化测试证据；G2 收尾设 **AI 关闭日演练**——连续一个完整测试日全程保持关闭态运行，全部任务只经固定导视、触觉引导与人工通道尝试并完整记录，演练记录（含未完成任务及原因，不预设全部完成）作为 G3 复核的必交证据，把「AI ON = AI OFF」从主张钉成一项可执行、可复核的测试设计；R-03、R-04、R-07、R-08 的替代路径、运维和参与条件未闭合时不得升级，不得以内部演示替代参与者证据。第 61—90 天进入 **G3 独立复核闸门**：由未参与制作的专业人员与参与者代表复核，逐项检查 R-01—R-12 全部十二项风险条目的停止条件和恢复证据 [metric:risk_item_count]，结论按基准四选一：通过、有条件通过、退回整改，或关闭 AI 层保留服务；四道闸门缺一不可，未通过 G3 不进入开放空间 [depth:phasing_implementation] [metric:gate_count]。
