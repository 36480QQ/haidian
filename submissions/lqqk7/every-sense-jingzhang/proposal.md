...
基准 / Baseline : service-equivalence-baseline v0.3.1 (draft_concept_recommendation)
档案 / Archive  : seb-op04-evidence-chain v0.2.0
...
**听得见的断点。** 连通性档案的两组读数被翻译为声音：一秒等于二十米，AI 开启段三十点三秒行进音走完六百零六米到达；AI 关闭段在四点四四秒（八十八点八米）处戛然而止——**静默的位置就是数据里断点的位置**（静默区实测 RMS 为零）。中英旁白两版随包，映射规则逐项可由档案读数复算 [data:assets/media/audible-break.md]。
...
**动起来的双态。** 三组 AI 生成场景（主脊、缝合区、大钟寺市场）以同一机位先后呈现 AI 开启与关闭——流光导引熄灭后，盲道、铸刻导视、值守与现金人工柜台仍在，通行继续；65 秒短片每帧恒显「概念意象 · AI 生成 · 非建成效果承诺」角标，AI 关闭段垫入「听得见的断点」同源五声音阶数据音，生成档案含两轮人工审改记录 [data:assets/media/dual-state-walk.md]。
