...
**fail-closed默认**：所有AI层在断电、网络中断、系统异常时默认关闭AI功能，空间回退至最安全开放状态（通道常开、照明常亮、无门禁）。不存在AI故障导致公共空间被锁定的可能。
...
| --- | --- | --- | --- | --- | --- | --- |
| 日均穿行人数 | N-01知识渗透 | 当前围墙状态≈0（封闭） | 红外计数器+人工抽样校核 | 周报→月汇总 | 开放后6月内日均≥200人次 | 现场传感器（待部署） |
| 早高峰穿越时间 | N-02潮汐交织 | 待交通工程基线测量 | GPS浮动车+行人计时抽样 | 日报→季度评估 | 较基线缩短≥15% | 交通管理平台（待对接） |
| 入驻创业团队数 | N-03创业孵化 | 当前封闭状态=0 | 入驻协议登记 | 月报 | 运营1年内≥5个团队 | 产业园管理台账 |
...
| 游客停留时长 | N-07记忆锚固 | 待文旅基线调研 | 匿名WiFi探针（仅计时长） | 月报 | 平均停留≥15分钟 | 匿名化统计（待部署） |
...
注：所有基线标注"待"的指标须在项目启动后3个月内完成基线调查；验收阈值为建议值，正式阈值须经业主、运营方和社区代表三方确认。测量方法遵循数据最小化原则，不采集个人身份信息。
