...
下列十二张场景卡同时写明空间、参与者、数据方式和无数据正线；前三张带“产业测试”标记，是开放前必须通过专业与伦理闸门的测试验证场景。

| 场景卡 | 节点与主要使用者 | 合作动作 | 边界与无数据等价正线 |
| --- | --- | --- | --- |
| 01 产业测试：街景感知盲区压力测试 | 众智园；算法团队、轮椅使用者、骑行者 | 用合成、授权片段和现场人工标注比较遮挡、夜间与无障碍识别 | 不持续采集人脸；无数据参与者可提交纸面路况并获得同等整改反馈 |
| 02 产业测试：智能体服务互操作沙盒 | 大钟寺；初创团队、小微商户、测评人员 | 在隔离环境用合成订单测试授权、撤销、异常回退和人工接管 | 不接真实支付；商户可只用模板数据获得同等兼容性报告 |
| 03 产业测试：隐私保护通勤预测 | 两翼与轨道接驳；交通团队、通勤者 | 比较本地聚合、差分统计与无个人轨迹基线的效能 | 拒绝轨迹不影响导引；固定时刻、人工问询和匿名计数构成正线 |
| 04 用途票议事夜 | AI 原点社区；居民、研究者、劳动者 | 逐项修改目的、期限、输出、回报和停止条件 | 可匿名发言或纸面异议，不以同意换取入场资格 |
| 05 数据劳动班前会 | AI 原点社区；采集、清洗、标注和审核人员 | 试做样本、估算工时、确认争议与退出流程 | 拒绝高风险任务不降低进入其他任务的机会，具体劳动关系依法确认 |
| 06 派生链撤回门诊 | 第一张回执亭；贡献者、合规与技术人员 | 查询原始数据、特征、版本、模型和报告的可处理状态 | 提供人工和离线申请；回执区分已删除、停止未来使用、无法技术回滚 |
| 07 社区无障碍路网共绘 | 遗址公园沿线；居民、残障使用者、运维人员 | 志愿标记断点并与现场核查、公开设施数据交叉验证 | 无手机者可在服务台标注；不得公开精确居住或日常轨迹 |
| 08 小微企业联合洞察 | 大钟寺；商户、行业服务团队 | 原始经营数据留在各方控制域，输出最低汇总阈值后的趋势 | 不参与企业仍可获取公共统计与通用培训，不能被排除出公共活动 |
| 09 公园生态观察列车 | 小月河翼与公园；自然教育者、志愿者 | 以季节性观察、环境传感和专家复核形成生态提示 | 传感器不绑定个人；纯人工观察可进入同一成果集 |
| 10 公共回报听证桌 | 大钟寺；参与者、社区组织、项目团队 | 对项目形成的费用、资源或公共成果提出分配选项并留痕 | 回报不是购买同意；无数据参与者同样可讨论普惠公共用途 |
| 11 京张口述史可撤回档案 | 沿线文化节点；铁路家属、居民、学生 | 以分段许可记录音频、文本、展览和研究用途 | 可只提交文字或现场讲述；已公开传播的不可逆边界须事前说明 |
| 12 共数年度开放周 | 一线三区两翼；公众、开发者、专业团队 | 展示失败案例、审计结论、公共回报和下一年度议题 | 不采集可选个人数据也可参会，活动影像设清晰无拍摄区域 |
...
当前图层复算的建筑基底约为 310,807 平方米，仅是临时边界与概念建筑要素下的几何结果 [metric:building_footprint_area_sqm]。总建筑面积、容积率、建筑密度、高度、保留率、改造率和新增量因缺少官方控规、现状普查、权属及工程条件而待正式数据补齐。设计不把该基底值换算成开发承诺，也不设未经证实的投资规模；后续应以单体编码把结构安全、历史价值、使用绩效、能耗、租赁影响和数据设施适配性共同纳入拆改留决策。
...
提交图层给出的绿地比例约 12.34%、公共空间比例约 7.33%，二者是概念几何相对临时计算边界的结果，不是审定绿地率或公共空间控制指标 [metric:green_ratio] [metric:public_space_ratio]。后续取得官方边界和现状绿地、树木、水系、生态及权属资料后，应分别复核数量、连通性、树冠遮阴、透水性、热舒适、洪涝安全和全龄可达性。场景试验需设置季节性关闭与恢复条款，任何数字装置都不得以维护或布线为由长期破坏根系、岸线和遗产铺装。
...
| 概念建筑基底 | 310,807.184 平方米 | 校核建筑图层与空间原型承载 [metric:building_footprint_area_sqm] | 不换算总建筑量、容积率或投资 |
| 概念绿地比例 | 12.3423% | 比较临时方案内部的蓝绿配置 [metric:green_ratio] | 不称法定绿地率 |
| 概念公共空间比例 | 7.3281% | 检查公共节点与合作线的支撑关系 [metric:public_space_ratio] | 不称确权后的公共开放面积 |
