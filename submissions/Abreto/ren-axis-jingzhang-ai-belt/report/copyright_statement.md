...
3. 资产级权利台账（逐类）：
   ...
   - visual/index.html 与内联 SVG（含 REN AXIS 徽标原型、剖面图）：本包原创手写；HTML 使用系统字体栈回退，不加载外部字体；权利状态：原创可展示（徽标为概念方向，未做商标查重）。
   - geometry/*.geojson 设计图层与 metrics.json：本包原创生成（确定性管道：pyproj/shapely，开源许可）。
