...
**包容性核验表（六类画像+残障贯穿群体×过道岔条件）**：任何场景进入试点前须逐画像核验下表；**无法提供同等质量非智能等价路径的场景，不得通过道岔准入**——包容性是准入条件而非事后评估。治理承诺的完备性已固化为可复算指标——量的是设计字段完备率而非运行实测，真实运行覆盖率须试点后另行记录：人工复核字段完备率 1.0[metric:human_review_field_completeness_ratio]、非智能等价路径字段完备率 1.0[metric:no_ai_alternative_field_completeness_ratio]、申诉渠道字段完备率 1.0[metric:appeal_channel_field_completeness_ratio]（公式=非空字段数/12，可由包内机器可读台账 visual/assets/governance-ledger.json 直接复算；"可审查指标"须待试点后真实记录，当前无数值、不虚构）。

| 画像 | 容易被排除的时刻 | 强制底线 | 可审查指标（试点后记录） | 非智能等价路径 |
| --- | --- | --- | --- | --- |
| 前沿研究员 | 算力与数据门槛、评测口径黑箱 | 公开评测口径与申诉复评通道 | 复评申请响应时长 | 线下评审与人工复核 |
| 创业工程师与开发者 | 场景清单信息不对称、准入流程繁复 | 清单公开+道岔评审标准公开 | 申请到答复时长 | 线下服务窗口与人工对接 |
| AI产品运营与设计人才 | 场景试验田排期不透明、数据接口不开放 | 场景开放清单与道岔排期公示 | 申请到进场时长 | 线下对接窗口与人工排期 |
| 高校学生 | 设备与会员门槛、课业时段冲突 | 免费公共时段+无账号即可体验 | 免费时段占比 | 实体课程与线下集市 |
| 社区居民（含老年群体） | 必须扫码、语音过快、界面频繁变化 | 纸质/人工同等入口，大字高对比，座椅遮荫连续 | 非数字入口可用率 | 人工服务台与电话渠道 |
| 残障使用者（贯穿各画像的核验群体） | 坡道断点、错误导航、设备占据通行带 | 无障碍代表参与验收；错误即回退静态导视 | 连续通行率、问题关闭率 | 人工伴行与纸质无障碍图 |
| 国际访问者与客座专家 | 单语界面、支付与认证壁垒 | 双语导视+免注册基础服务 | 双语覆盖率 | 双语人工导览 |
...
**法律与行业接口（登记不替代法定义务）**：每张场景卡在治理台账中登记"法律接口"字段——涉个人信息的场景对应《个人信息保护法》最小必要与删除义务；数据处理对应《数据安全法》分类分级；生成内容场景对应《生成式人工智能服务管理暂行办法》与深度合成内容标识要求；自动驾驶与机器人测试对应国家及北京市智能网联管理规定；公共区域影像依安防法规管理。上述法规名称仅为运营阶段法定义务的类别指针——公开规范文本清单已登记于 sources.json[source:CN-LEGAL-INTERFACE-REFS]（正式文本以国家法律法规数据库为准），不构成本方案的规范论据。登记册仅为运营透明层：不替代法定告知、同意、影响评估、备案与主管部门职责，具体条文适用性与合规结论由运营主体依法定程序评估取得，本方案不自行作出任何合规认定。

**场景-空间-运营映射与机制**：全部场景卡遵循"场景清单公开发布—主体申请—安全与伦理评审—限期测试—人工复核—展示或退出"的开放机制（对应上述状态机的道岔与编组场环节），并默认继承统一治理基线——责任主体为运营平台与场景申请方双列名，许可依场景清单制审批，数据治理执行匿名化最小化并逐场景登记数据类型，KPI 为使用规模、满意度与安全零事件（公式口径：达标率=周期内达标项数/考核项数，基线与阈值在试点前冻结；口径与数据来源逐场景登记，由指标委员会汇总、年度第三方复核构成可审计链条），人工复核按周期强制执行，连续两期未达标或发生安全/隐私事件即触发红牌暂停并公开复盘，恢复须经整改复核、黄牌观察一个周期后重新通过清单制准入，无法整改者转入退役；运营主体建议由区级平台公司联合社区、企业与高校组成，具体机制为概念建议[source:AGENT-TASKBOOK]。八要素保障机制：土地（更新单元统筹与弹性年期建议）、空间（组团化供给）、产业（三站分工）、资金（多元投入建议）、人才（画像驱动配套）、算力（端侧算力节点+区域调度研究）、数据（脱敏开放与数字资产流通研究；深化阶段可在清权前提下引入教育资源分布、公共交通流量与配送热力等可信第三方公开数据，校准用户画像、断点诊断与场景选址）、场景（清单制开放）。以上均不构成政府资金、招商或政策承诺。
...
**公共空间体系**：公共空间合并面积 1,341,759.867 平方米[metric:public_space_area_sqm]，占比 0.117566[metric:public_space_ratio]，统计口径为"站前广场+人字轴漫步公共活动带"，其中漫步活动带与公园绿地空间叠合（公园内开放活动空间），已在指标口径中说明。四处广场——大钟寺交汇站广场、众智站前广场、原点站前广场、南门户广场[data:geometry/public_space.geojson#PS-004]——构成轴上公共活动锚点，叠加科技测试与应用展示功能。
...
**分期计划**[depth:phasing_implementation]：三期滚动实施[data:geometry/phasing.geojson#PH-1]。一期"三站示范启动"（概念时序2026-2028）覆盖 3,692,893.005 平方米[metric:phase1_area_sqm]，聚焦三处重点区的可实施示范项目；二期"人字轴贯通成网"（概念时序2028-2031）覆盖 1,260,687.3 平方米[metric:phase2_area_sqm]，完成主轴公共空间与缝合走廊；三期"两翼融合滚动更新"（概念时序2031-2035）覆盖 6,459,251.9 平方米[metric:phase3_area_sqm]，推动职住社区有机更新与战略留白启动研究。各期推进建议采用条件门机制：完成权属确认、公众参与、安全评估与资料补齐，并以可复现证据与运营责任落实为验收前提（不因已投入而倒逼推进），方进入下一阶段；年份仅为概念参考时序，不构成开发时序或投资承诺。

**更新项目清单（18项）**[metric:renewal_project_count][depth:renewal_project_list]：每项按责任类型标注——【平】政府/平台主导、【市】市场主体主导、【校】校地共建；对应 RACI 口径：标注方为推进责任类型（R），审定责任（A）为依法定程序最终确认的主管或业主责任主体（本概念阶段仅列责任类型、不指认具体机构），高校院所/权属人/属地社区为协商方（C），社会公众经公示知情（I）。18项的九字段完整台账（责任类型/投入档/权属状态/前置条件/需求依据/人工复核/验收证据/暂停退出）与12张场景卡的十一字段治理台账（含权属·许可前置/隐私与期限/KPI基线与阈值/申诉/恢复/退役）以同一数据源呈现于 visual 实施台账面板与 A3 文册台账页，其中权属状态均如实标注待核验、KPI基线均标注待试点前冻结。实体更新项目采用"条件门—验收—停工复核"机制；仅含 AI 运行的场景采用"黄/红牌—人工接管—恢复—退役"机制，二者不混用。项目投入级别粗分三档作相对比较：低（快闪、标识、软性改造类）、中（存量改造与公共空间类）、高（新建组团与接驳设施类），级别仅为概念口径、非造价测算（均为建议分类）。一期十项——1 大钟寺站前广场与四象限步行连通【平】；2 智能终端旗舰体验群载体更新【市】；3 智能体经济总部群载体更新（概念）【市】；4 数据要素与数字资产服务中心【平】；5 原点社区"开源之家"存量改造【平】；6 成果孵化组团有机更新【校】；7 人才公寓组团【平】；8 众智园全栈创新实验组团【平】；9 智算服务与产业展示枢纽【市】；10 "人字门"地标与南门户广场【平】。二期五项——11 人字轴漫步道全线贯通【平】；12 三处缝合走廊慢行改善（北三环、知春路、北四环）【平】；13 "梯度长阶"荣誉空间【平】；14 公共空间组件库首批部署【市】；15 大钟寺站城一体接驳设施【平】。三期三项——16 两翼职住社区有机更新（分单元滚动）【平】；17 战略留白区启动研究【平】[data:geometry/land_use.geojson#LU-C3-E]；18 学院路科教融合区更新【校】。每个项目的空间位置可在 [data:geometry/buildings.geojson#BLD-B1] 等图层中定位，依赖条件（权属、控规、文保、交通评估）在 assumptions.json 中登记。

**首开预可研行动包（概念建议）**：在18项更新项目之外，建议以四个低实施门槛的行动率先建立可感知度：其一，公共空间组件库快闪部署——智能座椅与信息桩原型沿轴三处试点；其二，K0·1909与K8·2026首批K标落位并上线AI导览内容；其三，"开源之家"pop-up——利用存量空间临时改造承接首场开发者活动；其四，知春路智能过街优化试验（信号配时与过街体验改善的临时试点）。四项行动均按"迷你实施卡"口径预填（责任类型/前置程序/最小数据/人工复核/停止条件/验收证据），示例如下：组件快闪——责任类型为运营主体+属地街道，前置为场地权属确认与临时占用许可，最小数据为匿名使用计数，人工复核为每周巡检，停止条件为安全投诉或占用到期即撤，验收证据为使用记录与公众反馈摘要；K标落位——前置为市政杆件与绿化部门会签，停止条件为文物或管线冲突即移位；开源之家 pop-up——前置为消防与临时人流评估，停止条件为容量超限即限流，验收为首场活动复盘；智能过街试验——前置为交管部门批准，最小数据为过街等候时长的聚合统计，停止条件为任何安全事件即恢复原配时。四项行动以不新增永久工程、不涉及控规调整为筛选原则，仍须逐项走完上述前置程序方可试行；试行为期约90天的预可研周期为概念建议，验收以四项行动各自迷你实施卡预填的验收证据为准，并在预可研期内完整执行一次红牌撤回演练——从触发、停止自动输出、人工接管到恢复签认全流程走通并公开记录，把"能撤回"从承诺变成演练过的事实；试行效果作为一期实施方案的输入。

**实施政策建议**：更新单元统筹与成片实施机制、存量空间功能转换与弹性年期通道、校区园区街区融合的协商平台、公众参与与社区共治程序、场景开放清单制度。公众与受影响群体的参与记录（会议纪要、意见与回应台账）应作为各条件门的验收证据之一并公开留存。公开意见入口已开放（本仓库 issue #215，2026-08-08 开通，人类与智能体均可参与），承诺每条意见72小时内给出采纳/部分采纳/不采纳三态回应并附理由，意见—回应台账随后续版本沉淀；截至本版收到外部意见 0 条——开放入口不等于已完成公众参与，本包参与记录仍按"无既成事实"口径，不作为实际共创证据。均为政策研究建议，须经法定程序确认。
...
本方案指标体系分三层：范围与结构指标、用地与空间指标、待确认官方控制指标；面积与长度类指标全部可从 GeoJSON 以 EPSG:4548 投影复算，计数类指标（项目、场景、地标）按正文结构化清单计数，公式逐项写入 metrics.json[depth:metrics_recalculation][source:SITE-PACKAGE]。复算口径与空间复核程序完全同构：自 v2.7 起，绿地与公共空间的并集面积统一在 EPSG:4548 投影后计算（此前在地理坐标下先并集再投影，曾使公共空间面积与空间复核复算值产生 0.070 平方米的边界交点微差），现 metrics.json 与空间复核复算逐位一致。正文保留复算原值以便机器校验对照，阅读时应按约值理解——临时边界下的数字精度不代表真实精度。官方资料到位重算后，将发布新旧指标逐项差异表并记入 changelog，作为"方案即管道"的第一次公开运行记录。
...
**资料合法性与边界风险**[depth:risk_missing_data]：本方案仅使用公开或已清权资料[source:SOURCE-REGISTRY]，不使用任何未获授权或不可追溯的资料。来源实行正式/背景两表隔离：正式事实与规范论据仅取自登记表批准的正式来源；OSM 要素、全球公开案例、活动发布文章与产业报道等 background_only 来源仅承担语境与展示，不支撑红线、精确量测、控制指标或承诺性结论（逐条标注见 sources.json 的 usable_for_formal 字段）。最大数据缺口是官方精确红线与三处重点区多边形缺失，本包以临时粗略边界替代[source:PROVISIONAL-BOUNDARIES][data:geometry/site_boundary.geojson#PROV-SITE-001]；由此产生的全部面积、比例与空间关系仅供参考，替换官方几何后必须整体重算。其余缺口包括：经批准控规条件、现状建筑与权属底数、文保保护范围线、市政容量与交通模型数据，均已写入 assumptions.json 与缺资料清单[source:PROCESSED-FACT-PACK]。
...
**版权与知识产权**：文本与图件为原创生成，未使用未经授权的商标、字体、图片、人物肖像、论文图像或版权材料；引用的公开政策与公告以官方页面为准。本包 constraints.geojson 收录的现状参照要素取自 OpenStreetMap（© OpenStreetMap contributors，ODbL 1.0）[source:OSM-BASE]，仅作 existing_condition 底图语境，设计图层与边界均不使用 OSM 数据。命名与Logo为本包以基础几何原创构造的矢量图形，未进行商标注册与检索、不主张商标权利，正式使用前须完成商标查重与授权审查。方案以 CC-BY-4.0 作为原创内容主许可发布于社区平台（OSM 衍生层依 ODbL 1.0 分许可，CC-BY 总括不适用于该层）。逐文件权利台账见 report/copyright_statement.md，其可独立复核的要点为：字体——两套 PDF 实测 FontFile 计数为 0、文字以 Type 3 矢量轮廓呈现、不含任何可安装字体嵌入（复核方法：检索 PDF 对象流中的 /FontFile 与 /Type3）；OSM 层——ODbL 分许可附 NOTICE，Overpass 提取方法、日期与坐标存档随包交付于 visual/assets/osm-context.json；生成工具的开源库依赖仅用于生成、不随包分发。本社区平台并非征集主办方官方通道，方案与官方征集程序的关系以主办单位解释为准[standard:PROJECT-OFFICIAL-ANNOUNCEMENT]。

**隐私与伦理**：全部AI场景遵循"匿名化、最小化、可拒绝、可复核"原则，不提出侵害隐私或过度监控的场景，测试验证场景在取得法定许可前不投入常态运行；治理机制遵循人本原则，智能体用于增强而非替代人的判断[source:AGENT-TASKBOOK]。

**开放数据资产声明（含许可分层）**：本包以许可分层方式开放——原创内容（proposal、五图、两套PDF、HTML、设计类 GeoJSON 图层、metrics 与三个矩阵）以 CC-BY-4.0 开放；constraints.geojson 中 EX-OSM-* 现状要素与 visual/assets/osm-context.json 为 OSM 衍生数据，依 ODbL 1.0 单独许可、不并入 CC-BY 总括；PROV-* 临时边界要素转录自仓库站点资料包并沿其条款。在此分层下，39个用地单元、指标与矩阵沉淀为可复用的公共数据资产：后续智能体与研究者可直接读取本包 geometry/ 与 metrics.json 作为迭代起点，全部指标的复算口径已在 metrics.json 的 formula 字段逐项声明，替换官方红线后可按 formula 字段逐项复算。这与第三章"方案即管道"机制互为表里，回应智能体共创原则第八条"公共知识沉淀"——本方案不仅是一份成果，也是下一轮共创的输入[source:AGENT-TASKBOOK]。
