...
方案速览：一，这是一片已建成的城区——按公开建筑轮廓的量级参照，现状建筑基底约占17.2%（待官方现状调查核实），没有净地可供平面铺开，**本方案不重新规划这座城市，只为它接通断裂的公共界面，并在约1.3%的触媒点上证明更新的做法**；二，双臂结构源自OSM公开要素可复核的真实分岔，非图面修辞；三，12个AI场景由铁路语汇状态机统一治理，红牌暂停、人工接管、可退役；四，39个用地单元、约36.6公里慢行网络与30项指标由确定性管道生成，官方数据到位即可整包重算；五，首开行动含一次完整的红牌撤回演练，三处朝圣地标对接征集活动公布的纪念机制。**统一声明：本方案为AI生成的开放共创建议，基于临时粗略边界，全部空间内容均为概念建议、参考方案，供专业团队深化研究，不构成政府审定结论；官方红线与控规条件到位后，全部面积、比例与位置关系须整体重算。本声明覆盖全文。**[source:PROVISIONAL-BOUNDARIES][depth:risk_missing_data]
...
**总体结构：一轴双臂三站两翼，三级尺度**。主脊有真实的底盘：按园林绿化系统的公开口径，京张铁路遗址公园全线南起北京北站、北至北五环，约9公里，规划总面积约70公顷，服务海淀9个街镇；一期2.4公里、16.8公顷于2023年建成，二期总用地约53公顷于2026年8月建成开放（南段社区活力段、北段自然休闲段），服务沿线约70个社区与45万居民[source:JZ-PARK-OFFICIAL-FIGURES][source:JZ-PARK-PHASE2-OPEN]。需注意70公顷为全线规划口径、53公顷为二期竣工口径，二者非同一统计范围——本方案在这条已存在的公共空间脊柱上做叠加更新，沿高铁走廊方向定义未来臂[data:geometry/roads.geojson#RD-SPINE]，约2.2公里历史支线沿故线走向斜出西北[data:geometry/roads.geojson#RD-ARM-HIST]，两臂在人字门·道岔广场分岔[data:geometry/public_space.geojson#PS-004]。空间组织分三级：9公里带状整体、三个约800米半径的站区、以及站区内80—150米的街坊原型——39个用地单元是第一二级之间的结构分区，街坊肌理在单元内部由第三级原型控制，深化阶段结合现状路网与权属细分[depth:overall_spatial_structure]。
...
**众智·加速站（众智园，公告口径约192.1公顷；本包临时几何复算约193公顷[metric:key_area_zhongzhiyuan_sqm]）——编组场原型**。这里需要先说明一个现实：该片区在新闻口径中称"学北园"，与公告的"众智园"指同一处，且已进入实施——预计2026年7月具备开园条件，昌平线学知园站出入口与园区楼宇地下连通[source:HD-AI-DISTRICT-PILOTS]。**本站的站城一体化因此不是从零提出的设想，而是对一个已经做出来的接驳做延伸**：本方案关心的是园区之外那一层——公众可走的站台环、绿廊衔接与测试可见性。实验组团如编组股道并列布置[data:geometry/buildings.geojson#BLD-F1]，可分合重组以适配团队生长；外圈是一条公众可走的"站台环"慢行界面——测试在内圈进行，理解在外圈发生，透明规则墙替代实体高墙。绿轴穿园衔接清河与北五环防护绿带[data:geometry/land_use.geojson#LU-G-M]，众智会堂·开源礼堂作轴上锚点，站前广场为全栈创新发布场[data:geometry/public_space.geojson#PS-002]。首层界面强调共享仪器前厅与半室外讨论空间。风险：园区权属协调、生态与强度平衡、国家平台时序。
...
需要先交代一个上位事实：海淀人工智能创新街区已由官方明确以**五道口与大钟寺为两个先导区**[source:HD-AI-DISTRICT-PILOTS]——本方案的原点·策源站与大钟·交汇站正落在这两处，因此三站并非任意选点，而是对既定先导区的空间深化。
...
**现状建成底数（本版新增）**[depth:existing_conditions_diagnosis]：此前本包没有任何现状建筑底数，只能把拆改留写成纯方法框架。本版补入一层概念级现状：按 OpenStreetMap 公开建筑轮廓，总体设计范围内基底不小于400平方米的建筑共1,286处，基底合计约196公顷，约占范围的17.2%——查询语句、过滤规则与全部坐标随包存档[source:OSM-EXISTING-BUILDINGS]。按本包自定的来源纪律，OSM 属背景资料，因此这组数**只用作量级参照，不作为现状事实结论**；它需要官方现状调查交叉核验后才能升级，且未包含建筑面积与层数，故本包不据此推算存量建筑规模。

即便只作量级参照，它也把一个此前无法判断的问题变成了可提问的问题：17.2% 是**建筑基底覆盖率**，不是开发强度；在建成区中这一覆盖率并不高，与高校校园、大院式用地的形态相符。由此可以提出一个待官方现状调查证实的推断——本带可能属于**占满而未建足**：没有净地可供平面铺开，更新容量更可能在立体与界面维度。本方案把重心放在界面缝合与首层开放而非新增用地，与这一推断方向一致；但推断本身依赖背景级数据，且缺少建筑面积与层数，**在官方底数到位前不作为结论使用**。

这个量级也把本包自己的概念规模置于可被质疑的位置，这里如实标出一处需要复核的对照：15处概念组团基底约14.5公顷[metric:building_footprint_area_sqm]（占比约1.3%[metric:building_footprint_ratio]）、概念规模约126万平方米[metric:proposed_total_floor_area_sqm][data:geometry/buildings.geojson#BLD-B1]；公开报道提到创新带"可用更新面积约100万平方米"[source:HD-AI-DISTRICT-PILOTS]。**若两者口径可比，本包概念规模约为其1.26倍**；但"可用更新面积"的统计范围、时点与计容口径均未核实，该公开数值也不是法定容量上限，因此本包不据此判定超量、也不宣称吻合。这处对照的作用是提示：概念组团规模属上限性示意，深化阶段须在取得现状建筑面积底数与官方容量口径后重新校准——这两项资料本包均未取得。
