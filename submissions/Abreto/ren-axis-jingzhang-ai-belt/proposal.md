...
**给评审的五问五答（速览·问答版）**：一问，为什么是"人字"？——1909年人字形线路是中国自主工程的精神原点；本方案中"人"字三重同构：故线与新线在西直门北侧的真实分岔（空间）、品牌字形（标识）、梯度上升（AI方法论），是本带唯一不可移植的在地母题。二问，双臂是画出来的还是真实存在的？——OSM 公开要素显示京张故线走廊（13号线地面段）与高铁走廊真实分岔，历史臂沿真实故线布设（概念级量测、非测绘，坐标存档随包可逐点复核）。三问，AI 究竟在哪里？——在12张有位置、有最低数据、有人工兜底、有申诉与退出按钮的场景卡里，统一受铁路语汇状态机治理（人工复核、非智能等价路径、申诉渠道三项覆盖率均为 1.0 并已固化为可复算指标），不在大屏与口号里。四问，如何避免昙花一现？——四季活动+清单制场景开放+K标纪年+失败档案公开，连续未达标者退出；热度不靠节庆靠机制。五问，如何真实落地？——90天首开包按四项结果验收（含一次完整红牌撤回演练），四道道岔闸门不齐不越级；官方数据每到位一次，"方案即管道"整包重算并记一个版本K标。
...
**包容性硬门表（六类画像×过道岔条件）**：任何场景进入试点前须逐画像核验下表；**无法提供同等质量非智能等价路径的场景，不得通过道岔准入**——包容性是准入条件而非事后评估。治理覆盖已固化为可复算指标：人工复核覆盖率 1.0[metric:human_review_coverage_ratio]、非智能等价路径覆盖率 1.0[metric:no_ai_alternative_ratio]、申诉渠道覆盖率 1.0[metric:appeal_channel_coverage_ratio]（公式=满足项数/12，逐卡证据见场景治理台账；"可审查指标"须待试点后真实记录，当前无数值、不虚构）。
...
| 残障使用者 | 坡道断点、错误导航、设备占据通行带 | 无障碍代表参与验收；错误即回退静态导视 | 连续通行率、问题关闭率 | 人工伴行与纸质无障碍图 |
...
**法律与行业接口（登记不替代法定义务）**：每张场景卡在治理台账中登记"法律接口"字段——涉个人信息的场景对应《个人信息保护法》最小必要与删除义务；数据处理对应《数据安全法》分类分级；生成内容场景对应《生成式人工智能服务管理暂行办法》与深度合成内容标识要求；自动驾驶与机器人测试对应国家及北京市智能网联管理规定；公共区域影像依安防法规管理。登记册仅为运营透明层：不替代法定告知、同意、影响评估、备案与主管部门职责，合规结论由运营主体依法定程序另行取得，本方案不自行作出任何合规认定。
...
**分期计划**[depth:phasing_implementation]：三期滚动实施[data:geometry/phasing.geojson#PH-1]。一期"三站示范启动"（概念时序2026-2028）覆盖 3,692,893.005 平方米[metric:phase1_area_sqm]，聚焦三处重点区的可实施示范项目；二期"人字轴贯通成网"（概念时序2028-2031）覆盖 1,260,687.3 平方米[metric:phase2_area_sqm]，完成主轴公共空间与缝合走廊；三期"两翼融合滚动更新"（概念时序2031-2035）覆盖 6,459,251.9 平方米[metric:phase3_area_sqm]，推动职住社区有机更新与战略留白启动研究。各期推进建议采用条件门机制，显名为四道道岔闸门：公共价值门（拟解决的问题是否由真实使用者确认）、安全权利门（权属/文保/无障碍/数据保护是否过审）、可复现门（效果能否被第三方按包内公式与台账复核）、运营责任门（值守/接管/维护/退出责任是否落实到角色）——四门未齐不越级、不因已投入而倒逼推进；完成后方进入下一阶段，年份仅为概念参考时序，不构成开发时序或投资承诺。
...
**首开预可研行动包（概念建议）**：在18项更新项目之外，建议以四个低实施门槛的行动率先建立可感知度：其一，公共空间组件库快闪部署——智能座椅与信息桩原型沿轴三处试点；其二，K0·1909与K8·2026首批K标落位并上线AI导览内容；其三，"开源之家"pop-up——利用存量空间临时改造承接首场开发者活动；其四，知春路智能过街优化试验（信号配时与过街体验改善的临时试点）。四项行动均按"迷你实施卡"口径预填（责任类型/前置程序/最小数据/人工复核/停止条件/验收证据），示例如下：组件快闪——责任类型为运营主体+属地街道，前置为场地权属确认与临时占用许可，最小数据为匿名使用计数，人工复核为每周巡检，停止条件为安全投诉或占用到期即撤，验收证据为使用记录与公众反馈摘要；K标落位——前置为市政杆件与绿化部门会签，停止条件为文物或管线冲突即移位；开源之家 pop-up——前置为消防与临时人流评估，停止条件为容量超限即限流，验收为首场活动复盘；智能过街试验——前置为交管部门批准，最小数据为过街等候时长的聚合统计，停止条件为任何安全事件即恢复原配时。四项行动以不新增永久工程、不涉及控规调整为筛选原则，仍须逐项走完上述前置程序方可试行；试行为期约90天的预可研周期为概念建议，并以四项结果验收、不以人流量考核：其一，实际关闭至少一处慢行断点（知春路过街试验见效并留档）；其二，完成至少一场可复现的开源发布（开源之家首场活动附复盘）；其三，令至少一类易被排除画像实际受益并留下反馈记录（按包容性硬门表口径）；其四，完整执行一次红牌撤回演练——从触发、停止自动输出、人工接管到恢复签认全流程走通并公开记录（把"能撤回"从承诺变成演练过的事实）。四项有一项不成，一期实施方案相应收缩；试行效果作为一期实施方案的输入。

**实施政策建议**：更新单元统筹与成片实施机制、存量空间功能转换与弹性年期通道、校区园区街区融合的协商平台、公众参与与社区共治程序、场景开放清单制度。公众与受影响群体的参与记录（会议纪要、意见与回应台账）应作为各条件门的验收证据之一并公开留存。该机制已从承诺进入运行：公开意见通道设于本仓库 issue #215（2026-08-08 开通，人类与智能体均可参与，含本次征集其他参赛队），承诺每条意见72小时内给出采纳/部分采纳/不采纳三态回应并附理由，意见—回应台账随每版提交包沉淀、采纳项在 changelog 记名；截至本版（v2.8）台账尚无外部意见——通道运行状态与全部记录可在线核验，不夸大既有参与规模。均为政策研究建议，须经法定程序确认。
