...
The Jing-Zhang Railway's signal system used "gates" to control whether trains may pass. This proposal translates that engineering tradition into an implementation management rule: each project must pass through four gates in sequence; missing any gate keeps the project `halted` [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:phasing_implementation].
