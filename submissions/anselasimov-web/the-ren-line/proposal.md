...
| 评审关切 | 人字形的回答 | 可查证据 |
|---|---|---|
| 可实施性 | 廊道本体不新增建设量,增量只在三处重点区内消化;分期是条件表不是时间表,每期以阶段门启动;12个更新项目逐项标注实施主体与依赖条件 | [depth:renewal_project_list] · [depth:phasing_implementation] · [metric:phase_1_area_sqm] · [data:geometry/phasing.geojson#PH-1] · [source:SRC-BJ-RENEWAL-REG] · [source:SRC-LJLJ-RENEWAL-2026] |
...
| 表达完整度 | 中英双语等义两版;10张图纸(5张总览+5张深度:三区分幅、三断面、地标轴测);四组38项指标一键复算,三张矩阵逐条挂证据 | [depth:metrics_recalculation] · [metric:land_use_total_area_sqm] · [metric:land_use_coverage_gap_sqm] · [standard:MOHURD-ARCH-DESIGN-DEPTH-2016] |
...
用地布局逻辑已在总体章说明;本章交代可复算性与拆改留 [depth:land_use_layout]。全部140个用地单元由记忆线廊道、创新界面缓冲与17条现状道路参照线共同切割生成(轴线±60米绿廊缓冲、±320米创新界面缓冲,叠加大柳树路、皂君庙路、北三环西路、中关村东路、双清路、学清路、学院路—西土城路等沿线现状道路的近似走向),切出的碎屑再按同类相邻单元并入,保证共享边界、无缝无叠;每单元带land_use_code、面积与概念说明,复算公式为EPSG:4548下的多边形面积和 [metric:land_use_total_area_sqm] [depth:metrics_recalculation]。与v1.3的程序化条带相比,单元数由85增至140,中位面积由12.2万平方米降到7.1万平方米,条带型单元占比由25.9%降到13.6%——地块开始像街廓,而不像切片。17条参照线按公开地名常识近似定位、标注为低置信度现状条件,只作切割参照,不是道路红线(A-ROADS-001)。
...
指标体系分四组共38项,全部指标在metrics.json中带公式、来源文件、置信度与假设,可由geometry一键复算(EPSG:4548)[depth:metrics_recalculation]。其中36项状态为known、逐项在本文挂标签可查,2项(官方容积率、官方建筑高度)状态为unknown、列入待补而不臆造。
...
- 本包:`geometry/*.geojson`(9层)、`metrics.json`(38项指标)、`sources.json`(36条来源)、`assumptions.json`(6条假设)、三张矩阵与`self_check.json`
