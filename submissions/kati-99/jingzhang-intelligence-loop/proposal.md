...
为满足面向智能体任务书 agent.3 关于"AI 能力—数据—空间—运营—人工兜底—评估指标"完整链条的要求，10 张场景卡补充空间节点 ID、运营主体、数据输入输出、人工接管机制、绩效指标与场景类型；其中 **02、03、08 明确标识为产业测试验证场景**（满足≥3 个要求）。场景卡的空间落位与运行机制同时进入 `compliance_matrix.json` 与 `metrics.json`，便于评审逐卡回溯。
...
**字体栈。** 标题字采用 PingFang SC / Microsoft YaHei / Noto Sans CJK SC（macOS / Windows / Linux 三平台系统字体回退），正文字采用 Source Han Sans / LXGW WenKai（均为 OFL 协议开源字体，允许商用、允许衍生、无须署名）。所有字体回退顺序在 `visual/index.html` 的 SVG `text` 节点与 PDF 中均已声明，确保评审打开任意设备均能正确显示。

**原创与清权声明。** 本节所有图件、Logo、主色板、字体栈与图件中出现的"京"字构图均为本方案原创或采用开源/公有领域资料；不包含任何企业 LOGO、个人肖像、未授权摄影作品或第三方品牌符号。"全球 AI 创新城市参照"为公开资料参考（图件底部已列出来源 URL：hel.fi、amsterdam.nl、barcelona.cat、data.seoul.go.kr、montrealdeclaration-responsibleai.com、smartnation.gov.sg 等），不构成任何政府承诺或实施安排。"三处 AI 朝圣地标"为概念提议/参考方案，待专业团队深化，非已确定政府建设安排。每一张品牌图件的底部均加注"非政府承诺"或"非已确定政府建设安排"提示，与第 6 项"风险合规与矩阵诚实化"原则一致。完整版权与清权说明见 `copyrights/` 目录与 `copyrights/copyright_statement.md`。
...
| 城市 | 一句定位 | 本方案借鉴（机制/场景） | 公开资料来源 |
| --- | --- | --- | --- |
| 赫尔辛基 Helsinki | 开放数据 + AI 实验城市 | 数据要素治理 → 场景卡 08 数据要素会客厅 | hel.fi |
| 阿姆斯特丹 Amsterdam | 城市 AI 协议 + 公平算法 | 算法公平审计 → AI 慢行导航无障碍与公平性边界 | amsterdam.nl |
| 巴塞罗那 Barcelona | 超级街区 + 公共空间数字孪生 | 蓝绿慢行复合环 + 公共空间数字孪生原型 | barcelona.cat |
| 首尔 Seoul | 智慧交通 + 全球 AI 活动 | AI 慢行导航 + 全球 AI 活动周路线 | data.seoul.go.kr |
| 蒙特利尔 Montreal | 负责任 AI 宣言发源地 | 安全治理沙盒责任边界 + 人工接管机制 | montrealdeclaration-responsibleai.com |
| 新加坡 Singapore | Smart Nation + 端侧算力 | 端侧算力驿站 + 国家/城市级平台对接 | smartnation.gov.sg |

**借鉴方式声明。** 上述 6 例为本方案运营机制与场景设计的概念参考，不复刻任何城市的法定规划、建筑形态或政府品牌。借鉴点全部回到本方案具体的空间节点（蓝绿复合环、京张遗址公园慢行带）、AI 场景卡（01–10）或运营机制（年度活动体系、Q1–Q4 节奏），不外推为整体方案克隆；案例底部资料来源与 `sources.json` 一致，引用前应回到该文件核验可访问性与许可。
...
| 时段 | 范围 | 启动项目 | 启动条件 | 备注 |
| --- | --- | --- | --- | --- |
| 近期 0–12 月 | 轻量试点 | JZ-01 导视/桥下试点、JZ-02 临河标识/雨洪传感、JZ-06 试运行 | 不需控规调整，以运营/导视/传感为主 | 由属地街道 + 公园管理处牵头，可在征集期后立即启动 |
| 中期 1–3 年 | 主体更新 | JZ-03 首层业态、JZ-04 过街设施、JZ-05 端侧算力驿站 | 需官方控规、道路红线、市政、能源条件确认 | 与官方控规编制和市政大修同步推进 |
| 长期 3–10 年 | 治理框架 | 年度活动体系成熟、招引转化链跑通、国际传播机制 | 需年度评估与策略迭代 | 由海淀区牵头建立年度复盘与策略更新机制 |
...
**开发者社区机制。** 由海淀区科委与高校开源联盟双牵头，建立开源许可证核验、贡献墙、数字徽章与 PR/Issue 计数激励；社区活动数据仅做聚合统计，不采集个人行为轨迹，不用于商业推荐。社区运营与场景 01 开源发布厅、场景 02 安全治理沙盒的"红队测试"通道直接对接。
...
**招引转化链。** 由海淀区+高校技转办+持牌投资机构三方共建，路径为"高校成果登记 → IP 评估 → 孵化对接 → 投融资 → 落地签约"，对应场景 07 近校成果转化街的法务/知产/融资服务节点；每步由对应持牌机构操作，融资环节不允许未持牌机构介入。

**国际传播。** 与"全球 AI 创新城市参照"6 例建立信息互换与活动互邀机制，年度活动周的媒体覆盖与多语种导视由海淀区+合作机构统一发布；任何对外发布须回到 `sources.json` 核验可访问性与许可。
...
| 儿童（≤14 岁） | 安全通学、家长可视、慢行优先 | 学校周边限速、AI 慢行导航儿童模式、家长端位置共享（脱敏） | 不得采集儿童精确轨迹；仅聚合统计 | 家长授权；学校+街道双签 |
