...
- **断点修复闭环天数 ≤ 7 日**：测量口径 = 从公众/巡检报修到修复闭环的自然日；基线 = 征集期后实测现状周期（待补）；测量周期 = 季度；建议目标 = 轻量试点段先行达到。
...
| 时段 | 范围 | 启动项目 | 启动条件 | 备注 |
| --- | --- | --- | --- | --- |
| 近期 0–12 月 | 轻量试点 | JZ-01 导视/桥下试点、JZ-02 临河标识/雨洪传感、JZ-06 试运行 | 不需控规调整，以运营/导视/传感为主 | 建议由属地街道 + 公园管理处牵头（待确认），可待征集期后视条件启动（非承诺） |
...
| 儿童（≤14 岁） | 安全通学、家长可视、慢行优先 | 学校周边限速、AI 慢行导航儿童模式、家长端位置共享（脱敏） | 不得采集儿童精确轨迹；位置共享仅做聚合统计；监护人明示授权 + 端侧脱敏 + 保存期 ≤ 30 日 + 到期删除 + 可随时撤回；非智能替代渠道兜底 | 家长授权；学校+街道双签；不满足最小必要与保存期限的个体追踪功能不启用 |
