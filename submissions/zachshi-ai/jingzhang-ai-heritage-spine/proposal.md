...
人字轨道语法解决了"AI 必须回授文化"，使用验证解决了"回授必须被使用"。本方案的元设计层进一步追问六个逻辑与哲学层的问题——每一个都不是外加判断，而是**照亮人字轨道语法中已隐含的逻辑**。六个方向对应 `visual/assets/philosophy/` 下六件制品，每件都是语法自身逻辑的显式化 [E:HERITAGE-PHILOSOPHY-META]。

**价值论层——价值交易表。** 人字轨道语法已隐含价值排序，价值交易表把隐含的价值冲突显式化为"优先方向+代价" [data:visual/assets/philosophy/value-tradeoffs.json#tradeoffs]：

| 价值冲突 | 优先 | 编码在 | 代价 |
| --- | --- | --- | --- |
| 遗产保护 vs AI 介入 | 遗产保护 | 文保三原则 P1 最小干预 | AI 设施只能表面叠加 |
| 回授 vs 效率 | 回授 | 每节点必须回授实体回授物 | 回授物建设运营成本 |
| 体验丰富 vs 遗产原真 | 遗产原真 | 导览用实体回授物非虚拟特效 | 体验不如数字炫目 |

代价被接受，因为优先方向是"文化不被消费"的前提。

**目的论层——在地必然性论证。** 论证文化回授机制为什么只能在京张发生——在地性从背景变成必要条件 [data:visual/assets/philosophy/site-necessity.json#necessity_arguments]：①人字轨道回授精神是京张独有（"人字轨道回馈沿线"正是京张遗产对 AI 的隐喻）；②京张是唯一贯穿海淀核心区的线性铁路遗产（回授链的空间载体）；③京张是"中国自主建造"的精神原点（回授正当性的精神锚点）。脱离京张，回授机制失去精神来源、空间载体与正当性。

**现象学层——第一人称共情脚本。** 用第一人称脚本验证文化回授体验链路每步是否通顺——体验的工程验证，非营销叙事 [data:visual/assets/philosophy/first-person-scripts.json#scripts]。三个脚本（游客触摸回授物/居民投稿记忆/学生看测试报告）各验证一条回授体验链路。

**认识论层——认识论分级。** 每个主张分级描述/预测/规范，注明知识基础——对认知边界诚实 [data:visual/assets/philosophy/epistemic-grading.json#claim_grades]。

**博弈论层——激励相容。** 机制设计核心：让"回授文化"成为占优策略——AI 节点只有在有动机回授时才会真正回授 [data:visual/assets/philosophy/incentive-compatibility.json#incentive_mechanisms]。回授物使用量达标获展示位优先、登记回授获文化合规认证、使用验证公开使不回授可被核查。回授的收益大于不回授的成本，回授成为占优策略。

**时间层——自更新协议。** 方案不是静态；定义当城市变化时回授机制哪部分必须重算的触发协议 [data:visual/assets/philosophy/self-update-protocol.json#triggers]。文化遗产认知变化→重算回授物内容；文保政策变化→重算选址；游客行为变化→重算使用阈值。

**六方向与人字轨道语法的关系。** 六个方向都是语法自身的逻辑延伸：价值表是语法隐含的价值排序显式化，在地必然性是语法的精神正当性，共情脚本是语法可被体验的证明，认识论分级是语法对自身认知边界的诚实，激励相容是语法被遵守的机制保证，自更新是语法对时间的诚实。六者共同把"文化不被消费"从设计原则升级为价值、精神、体验、认知、激励、时间六维自洽的工程承诺 [depth:metrics_recalculation]。
