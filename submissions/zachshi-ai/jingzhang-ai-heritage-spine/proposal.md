...
人字轨道的精神是"让每段路各司其职"。同在京张遗产走廊上的四份方案（同一作者提交）与本方案在空间与机制上互为衔接，共同构成走廊的完整 AI 服务系统；以下为设计衔接建议，具体以各方详细设计为准 [E:HERITAGE-CROSS-INTERLOCK]：

- **与 robot-comobility（断面语法×共行信号）**：机器人网络的文保范围段与本方案的遗产节点空间重合，机器人系统在遗产段只做减法——5km/h 随流、遗产活动日禁入、断面不改铺装。这正是本方案"最小干预、可逆、可识别"三原则在移动设施上的应用。
- **与 silver-accessibility（银龄接力×代际连接站）**：代际连接站的"文化互教台"以遗产记忆为素材——老人讲京张故事、年轻人教数字工具，遗产叙事成为代际连接的内容载体；建议连接站邻近本方案的遗产节点设置。
- **与 civic-data-commons（数据回执×数据时刻表）**：遗产档案与文保监测数据建议纳入公共数据开放清单，每次使用生成可审计回执——数据去向的可见性，是"可识别性"原则在数字层的延伸。
- **与 ai-civic-services（三保一验×认账空间）**：遗产节点的公共服务（导览、票务、咨询）建议纳入"三保一验"保障链，认账台受理遗产段服务投诉——遗产空间里的服务承诺同样需要具名负责。
