...
**临时边界声明：** 本方案使用的场地边界为临时粗略边界（provisional boundary），来源于公告文字四至和面积约束推导 [source:BOUNDARY-SOURCE]。该边界不得作为官方红线、审批依据或精确面积复算依据。当官方精确边界发布后，需重新复算所有图层面积和指标。
...
本方案使用的场地边界为临时粗略边界，不得作为官方红线、审批依据或精确面积复算依据 [source:BOUNDARY-SOURCE]。当官方精确边界发布后，需重新复算所有图层面积和指标。
