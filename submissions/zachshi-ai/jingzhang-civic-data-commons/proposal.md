...
数据公共带的价值在于"让每一次数据使用都被看见"。同在京张遗产走廊上的四份方案（同一作者提交）既是本方案的数据源，也是回执机制的用户；以下为设计衔接建议，具体以各方详细设计为准 [E:DATA-CROSS-INTERLOCK]：

- **与 robot-comobility（断面语法×共行信号）**：机器人信号时刻表的每一次闭锁/降速事件生成调度记录，建议按数据回执规范登记数据去向与保留期；数据时刻表站可与智递驿站共用公示屏，市民在一块屏上同时看到"数据何时开放"与"机器人何时来"。
- **与 heritage-spine（人字轨道×文保三原则）**：遗产档案与文保监测数据建议纳入公共数据开放清单，每次使用生成可审计回执——遗产数据的每一次去向都可追溯，是"可识别性"原则在数字层的延伸。
- **与 silver-accessibility（银龄接力×代际连接站）**：适老服务涉及的健康与出行数据是敏感度最高的公共数据类别之一，建议优先纳入回执规范与异议通道；时刻表站公示屏提供大字版，老人看得见自己的数据去哪。
- **与 ai-civic-services（三保一验×认账空间）**：认账台的办结记录（脱敏后）建议纳入数据时刻表公示——"办成了什么、谁办的、用了什么数据"同时可见，服务问责与数据问责在同一面公示墙上汇合。
