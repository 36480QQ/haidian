...
### 数据对账：复式记账内化（本方案原创，v4）

数据回执解决了"每次使用有记录"，但单式记录只记使用方一侧。本方案从会计学移植**复式记账与期末对账**方法论：每笔数据使用必须成对记账——**借**（使用方获得数据）、**贷**（公共数据池记录这次使用），期末对账核平 [E:DATA-RECONCILIATION]。

**三条对账规则：**
- **R1 成对记账**：每笔数据使用必须借贷成对，借贷不平即标记差异。数据不是"被拿走"而是"被借出且有记录"。
- **R2 期末对账**：季度末核对全部回执的借贷平衡、保留期与删除记录；差异须在下一对账周期前澄清。**不能核平的回执是差异，不是已结记录。**
- **R3 结果公开**：对账结果公开（脱敏），差异记录进入公共台账，接受公众与监管查阅。

**无数据基线（负空间）。** 无任何数据服务时，市民仍能通过人工窗口、纸面表格与社区网络办成基本服务。数据服务是可关闭的叠加层；对账确保其关闭时**不留未结记录**——就像关账前的清账，数据关闭前必须账目核平 [data:visual/assets/reconciliation-ledger.json#negative-baseline]。

**12 个数据集的复式台账**（完整见 `visual/assets/reconciliation-ledger.json`，借贷成对、季度核平）：

| 数据集 | 借（使用方获得） | 贷（公共池记录） | 平衡 |
| --- | --- | --- | --- |
| 公共空间开放度数据 | 服务运营方-线路规划 | 公共池-使用记录（90天/删） | ✓ |
| 文化遗产点位公开信息 | 开发者-导览生成 | 公共池-使用记录（365天/留） | ✓ |
| 公众反馈聚合统计 | 研究-质量改进 | 公共池-使用记录（180天/删） | ✓ |

对账与回执的关系：回执是每一笔的记录，对账是全部记录的总账。单笔回执可能"看似完整"但整体借贷不平——对账就是抓出这种"单笔完整、整体失衡"的漏洞 [source:data-security-law]。
