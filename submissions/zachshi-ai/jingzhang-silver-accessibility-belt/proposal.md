...
summary: "把京张铁路遗址公园重新定义为'适老服务脊梁'，针对京张沿线老龄社区，构建AI辅助老年人就医、办事与无障碍出行的公共服务带。方案提出'京张银龄无障碍服务带'总体概念，以南北约9公里连续无障碍慢行主轴串联三处重点区与沿线社区、医院、政务点、地铁口，布局10张AI适老场景卡（含3个测试验证场景）、5类适老用户画像、3个温暖地标、9处用地图斑（7类适老功能）与7个银龄服务节点，形成可体验、可推广、可迭代的适老无障碍智能服务概念方案。"
...
**三区两翼协同（适老视角）。** 三区：众智园承担适老 AI 技术验证（中试楼+连接站技术原型），AI原点社区承担银龄生活示范（连接站+健康驿站+认账窗口），大钟寺承担适老服务产业转化（服务集市+办事服务厅）。两翼：中关村科技服务翼导入志愿者、社工与政策咨询人力，小月河场景赋能翼承载滨水无障碍休憩与户外疗愈场景的延伸。要素流为"技术（众智园验证）→ 生活（原点示范）→ 产业（大钟寺转化）"，人力沿中关村翼流入，场景沿小月河翼溢出。与北纬社区（适老社区经验互鉴）、未来科学城（健康科技对接）、怀柔科学城（康复工程研究）、经开区（适老设备制造）、京津冀（异地养老协同）的关系均为证据边界清楚的概念协同假设，不虚构已签合作、招商、资金或政府安排 [source:agent-taskbook]。

### 与同走廊方案的互锁关系（v9）

连接的意义在于"不把老人单独留在任何一条路上"。同在京张遗产走廊上的四份方案（同一作者提交）与本方案在空间与机制上互为衔接，以下为设计衔接建议，具体以各方详细设计为准 [E:SILVER-CROSS-INTERLOCK]：

- **与 robot-comobility（断面语法×共行信号）**：建议代际连接站与智递驿站邻近或共址设置——老人在驿站人工窗口取药的同时，连接站的对坐长椅承接等待时的代际交流；机器人承担重物配送，人承接陪伴，各做各自擅长的事。
- **与 heritage-spine（人字轨道×文保三原则）**：连接站的"文化互教台"以遗产记忆为素材——老人讲京张故事、年轻人教数字工具；遗产叙事让"被教"的老人同时成为"在教"的长者，单向的适老服务变成双向的代际连接。
- **与 civic-data-commons（数据回执×数据时刻表）**：适老服务涉及的健康与出行数据建议纳入数据回执规范——老人及其家属有权知道自己的数据去了哪里；数据时刻表站的大字版公示屏适老可读。
- **与 ai-civic-services（三保一验×认账空间）**：适老政务服务与认账台衔接——认账台保留人工窗口与大字版认账卡，不会用手机的老人同样能拿到具名负责的纸质回执。
...
用地布局在公告既定结构基础上叠加适老视角，共 9 个用地图斑（`land_use_zone_count=9` 的计数对象），功能上归纳为 7 类适老用地：适老AI研发与中试用地、社区生活与银龄服务用地、绿地与无障碍开敞空间用地、适老商业与银龄消费用地、道路与无障碍慢行廊道用地、政务与公共服务适老化用地、遗址公园无障碍绿地等 [data:geometry/land_use.geojson#LU-001]。所有用地分类为概念建议，用地代码参照《国土空间调查、规划、用途管制用地用海分类指南》对应，但具体地块边界、容积率与拆改留须在控规条件确认后由专业团队深化 [standard:MNR-LAND-USE-CLASSIFICATION-GUIDE]。
...
用地面积依据临时边界在 EPSG:4548 下复算，共 9 个用地图斑（功能归纳为 7 类适老用地），详见 `geometry/land_use.geojson` [metric:land_use_zone_count]。用地面积为概念建议值，须在官方精确边界和控规条件确认后重算 [data:geometry/land_use.geojson#LU-001] [metric:site_area_sqm]。
...
适老服务的长期运营以"连接不断档"为成功标准，而非活动数量 [source:agent-taskbook]：

- **年度活动体系**：春·代际踏春行（遗产公园无障碍徒步，轮椅可及）；夏·银龄健康夜市（健康驿站延时服务+纳凉）；秋·代际互教节（连接站文化互教台集中开放）；冬·暖阳陪伴周（独居老人结对探访加开）。全部为非承诺性概念建议，须由运营主体依法举办。
- **活动品牌与 IP**："银龄+"品牌延展——银龄市集（老人手作+青年摊主结对）、银龄课堂（老人教手艺、青年教数字工具）；品牌资产归属与授权方式待运营主体确定后依法约定。
- **社区运营**：银龄接力志愿者台账（R1 交接/R2 签收/R3 回访/R4 无 AI 等价/R5 代际通道）作为运营标准动作；每座连接站设拟议"站长"角色（具名人工，概念角色非现职授权），负责开站、回执与投诉受理。
- **场景开放流程**：新适老场景上线前须经连接站试运转+老人代表体验评审（可用性门槛），通过后才进入正式服务目录；评审记录进入连接回执台账。
- **公共体验与地标运营**：3 个温暖地标（银龄温暖广场/代际记忆广场/记忆墙）全年开放，设人工值守时段；记忆墙内容每季由社区老人共议更新。
- **运营 RACI 与资源框架（拟议角色，非现职授权）**：运营统筹（公共空间运营方）、站长（人工值守）、志愿者领队（银龄接力调度）、健康顾问（社区卫生中心对口）、独立评估（季度连接测量复核）；资源假设为公共空间运营收入+数字包容专项+志愿服务时长置换，均为待核验概念估算。
- **国际传播到转化**：适老设计指南（断面/连接站/回执模板）开源中英文发布；接待适老城市交流团；从"展示"走向"标准输出"——把连接可测量性三级量表作为可被其他城市引用的公共知识资产。
...
三处重点区、9 个用地图斑（7 类适老功能）、5 栋概念建筑、5 条道路、3 个分期均与图层对应 [metric:key_area_count] [metric:land_use_zone_count] [metric:building_count]。
