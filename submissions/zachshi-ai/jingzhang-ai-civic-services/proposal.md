...
### 核心机制：认账空间（Accountability Spaces，本方案原创，v5）

三保一验解决了"办不成有人负责"，但还缺一个更根本的追问。本方案提出反直觉事实：**政务服务 AI 越智能，市民越不知道找谁**——AI 挡在市民与办事员之间，事"办成了"但没人认账。三保一验的核心问题不是"办得多规范"，而是**"谁签字、谁负责、谁能看见"**——以"认账"为透镜（正如 human-hours 以时间为透镜，本方案以认账为透镜）[E:CIVIC-ACCOUNTABILITY-SPACES]。

**设计原则：每个服务点长出"认账面"。** 市民能亲眼看到谁在为自己办事、办到哪一步、谁签的字——就像铁路售票窗口显示经办信息。三保一验是制度，认账空间是它的**物理载体** [data:visual/assets/accountability-stands.json#design-principle]。

**5 座认账台（沿走廊布设，完整规格见 `visual/assets/accountability-stands.json`）：**

| 台 | 位置 | 显示内容 | 无AI等价 |
| --- | --- | --- | --- |
| AS-01 原点社区认账台 | 政务服务驿站 | 事项/经办人（姓名+岗位）/阶段/时限/签字/投诉 | 纸面认账卡（窗口发放） |
| AS-02 众智园认账台 | 人才健康服务段 | 事项/经办人/阶段/转介进度/签字 | 纸面认账卡 |
| AS-03 大钟寺认账台 | 法律援助中心 | 援助事项/经办律师（脱敏）/阶段/签字 | 纸面认账卡 |
| AS-04 公园中段认账台 | 中段驿站 | 事项/经办人/阶段/签字 | 纸面认账卡 |
| AS-05 南端认账台 | 南端节点 | 事项/经办人/阶段/签字 | 纸面认账卡 |

**办成公示（市民看到"谁签的字"）。** 完成或受理的办理事项在认账台公示（脱敏）：事项 / 经办人（姓+工号） / 阶段 / 签字 / 时限 / 投诉入口。完整示例见 `visual/assets/completion-display.json` [data:visual/assets/completion-display.json#displays]：

| 事项 | 经办 | 阶段 | 签字 |
| --- | --- | --- | --- |
| 社保卡办理 | 综合窗口 王X（0231） | 已受理 | 王X 2026-10-02 |
| 转诊导航 | 健康服务台 李X（0117） | 已转介 | 李X 2026-10-02 |
| 援助申请 | 法援中心 张X（4521） | 初审中 | 张X 2026-10-02 |

**负空间基线。** 无任何 AI 时，窗口经办人当面告知"谁办、办到哪、找谁问"——认账功能不依赖 AI，纸面认账卡与电子显示同内容 [data:visual/assets/accountability-stands.json#negative-baseline]。

**规则闭合验证。** `run_accountability_tabletop.js` 对 12 服务 × 6 条规则分支（完整/无具名经办/无签字/阶段不可见/缺投诉渠道/时限未知）共 72 个合成案例全部正确分类（36 阻断/12 认账/24 标记），证明认账规则逻辑闭合——但仅证明分类正确，不构成现场服务、合规或授权证据，现场绩效仍为 null、状态 `not_authorized_not_run` [data:visual/assets/accountability-tabletop-evidence.json#blocked]。

**与三保一验的关系。** 三保一验回答"谁负责、多快响应、怎么留痕、谁验证"（制度），认账空间回答"市民在哪里看见负责"（空间）——回执是认账的内容，认账台是回执的展示面。两者共同保证：**市民办的事，不仅能办成，还能看见谁在办** [standard:GENERATIVE-AI-INTERIM-MEASURES]。

### 元设计层：哲学层增强（本方案原创，v6）

认账空间解决了"市民在哪里看见负责"，三保一验解决了"谁负责、怎么留痕、谁验证"。本方案的元设计层进一步追问六个逻辑与哲学层的问题——每一个都不是外加判断，而是**照亮认账透镜中已隐含的逻辑**。六个方向对应 `visual/assets/philosophy/` 下六件制品，每件都是透镜自身逻辑的显式化 [E:CIVIC-PHILOSOPHY-META]。

**价值论层——价值交易表。** 认账透镜已隐含价值排序，价值交易表把隐含的价值冲突显式化为"优先方向+代价" [data:visual/assets/philosophy/value-tradeoffs.json#tradeoffs]：

| 价值冲突 | 优先 | 编码在 | 代价 |
| --- | --- | --- | --- |
| 认账 vs 效率 | 认账 | 每服务点长出认账面，经办人签字可见 | 服务流程变慢 |
| 认账 vs 隐私 | 隐私 | 认账面显示经办人脱敏姓名，不显示市民材料 | 市民侧信息不完全公开 |
| 认账 vs 匿名 | 认账 | 经办人具名签字，服务可追溯到人 | 经办人需承担责任 |

代价被接受，因为优先方向是"公共信任"的前提。

**目的论层——在地必然性论证。** 论证认账机制为什么适合京张——在地性从背景变成必要条件 [data:visual/assets/philosophy/site-necessity.json#necessity_arguments]：①京张走廊公共服务节点密度高（认账需求真实）；②京张铁路售票窗口是"经办信息可见"的传统（认账面的正当性来源）；③京张是"市民服务脊梁"的设计定位。

**现象学层——第一人称共情脚本。** 用第一人称脚本验证认账体验链路每步是否通顺——体验的工程验证，非营销叙事 [data:visual/assets/philosophy/first-person-scripts.json#scripts]。三个脚本（市民看见负责/经办人签字负责/质疑市民投诉被受理）各验证一条认账体验链路。

**认识论层——认识论分级。** 每个主张分级描述/预测/规范，注明知识基础——对认知边界诚实 [data:visual/assets/philosophy/epistemic-grading.json#claim_grades]。

**博弈论层——激励相容。** 机制设计核心：让"认账"成为占优策略——经办人只有在有动机认账时才会真正认账 [data:visual/assets/philosophy/incentive-compatibility.json#incentive_mechanisms]。认账获绩效认可、获"认证服务"标识、认账后可被市民核查使推诿不可能。

**时间层——自更新协议。** 方案不是静态；定义当城市变化时认账机制哪部分必须重算的触发协议 [data:visual/assets/philosophy/self-update-protocol.json#triggers]。公共服务事项变化→重算认账台显示字段；政务服务流程变化→重算认账阶段定义；市民信任水平变化→重算认账台呈现方式与投诉通道设计。

**六方向与认账透镜的关系。** 六个方向都是透镜自身的逻辑延伸：价值表是透镜隐含的价值排序显式化，在地必然性是透镜的在地正当性，共情脚本是透镜可被体验的证明，认识论分级是透镜对自身认知边界的诚实，激励相容是透镜被遵守的机制保证，自更新是透镜对时间的诚实。六者共同把"市民能看见负责"从设计原则升级为价值、在地、体验、认知、激励、时间六维自洽的工程承诺 [depth:metrics_recalculation]。
