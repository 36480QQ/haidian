---
title: "Jingzhang AI Low-Speed Robot Co-Mobility Network v7: Dual Engines — Cross-Section Grammar x Co-Mobility License"
...
# Jingzhang AI Low-Speed Robot Co-Mobility Network v7: Dual Engines
...
**Methodology source.** Traffic engineering's "self-explaining roads" — letting the road's appearance itself convey the rules without extra signage. It is the discipline's own methodology and a natural extension of the cross-section grammar lens [data:visual/assets/self-explaining-grammar.json#methodology-source].
...
**Rule-closure verification.** `run_self_explaining_tabletop.js` correctly classifies all 24 synthetic cases — 4 cross-sections x 6 rule branches (feature-to-rule / rule-to-feature / fingerprint uniqueness / fingerprint duplication / missing feature / missing rule) — and the four cross-sections' visual fingerprints are unique — proving the grammar is **unambiguous and reversible**. This proves only that the mapping logic closes; it is not an on-site pedestrian comprehension test or authorization evidence; field performance remains null, status `not_authorized_not_run` [data:visual/assets/self-explaining-tabletop-evidence.json#blocked].

**Field test protocol (the standard for "self-explaining" holding).** Recruit 30 volunteers unfamiliar with this proposal (including representatives of the elderly, children and the disabled), give them only text-free cross-section drawings, and ask: where robots go / how fast / who yields / whether entry is allowed. Only when the four cross-sections' average correctness is ≥80% and the pedestrian-priority section reaches ≥90% is the cross-section grammar judged "self-explaining"; any cross-section below 70% triggers adjustment of that cross-section's visual features (add bands / change separation / change material), and only after the retest passes does it enter the G0 spatial gate. The test depends on no AI at all — what it verifies is the space's own ability to convey rules; the full protocol is in `visual/assets/self-explaining-test-protocol.json` [data:visual/assets/self-explaining-test-protocol.json#pass-rule].

**Relation to the cross-section grammar.** The grammar is a design promise; the self-explaining test is the verification of that promise — **whether the grammar truly "grows rules from geometry" is proven by whether people unfamiliar with the proposal can read the rules from the drawing**. This verification upgrades "the cross-section is a contract" from a design claim into a testable engineering commitment [depth:metrics_recalculation].

### Meta-Design Layer: Philosophy Layer Enhancement (original to this proposal, v7)

The cross-section grammar solved "rules grow from geometry"; the self-explaining test verified that "the rules can be read out". The meta-design layer of this proposal goes further, asking six questions at the logical and philosophical levels — none of them externally added judgements; each one **illuminates logic already implicit in the proposal's mechanisms**. The six directions correspond to six artifacts under `visual/assets/philosophy/`, each an explicit statement of the proposal's own logic [E:ROBOT-PHILOSOPHY-META].

**Value Theory — Value Trade-offs.** The proposal's mechanisms already imply a value ordering; the value trade-off table makes the implicit value conflicts explicit as "priority direction + cost" [data:visual/assets/philosophy/value-tradeoffs.json#tradeoffs]:

| Value conflict | Priority | Encoded in | Cost |
| --- | --- | --- | --- |
| Safety vs efficiency | Safety | Dedicated section 15 km/h cap | Delivery time +~20% |
| Heritage protection vs development intensity | Heritage protection | Heritage section 5 km/h cap; paving left unchanged | No high-speed dedicated section in heritage sections |
| Visibility vs convenience | Visibility | Public data timetable | Extra record-keeping by operators |
| Efficiency vs equity | Equity | Human fallback retained | Higher operating cost than pure automation |

The costs are accepted, because the priority directions are the precondition of co-mobility; the proposal does not sacrifice safety and equity for efficiency [standard:BARRIER-FREE-ENVIRONMENT-LAW].

**Teleology — Site Necessity.** Argues why this proposal can only happen at Jingzhang rather than being a generic scheme portable to any city — site-specificity moves from background to necessary condition [data:visual/assets/philosophy/site-necessity.json#necessity_arguments]: ① the only linearly superimposed corridor of "railway heritage + AI innovation belt"; ② the "timetable" as a public-memory symbol belongs to Jingzhang alone; ③ the spirit of independently building the herringbone track is a spiritual legacy unique to Jingzhang. Detached from Jingzhang, the cross-section grammar loses its historical source, its public-memory anchor and its spiritual legitimacy.

**Phenomenology — First-Person Empathy Scripts.** First-person scripts verify whether every step of the experience chain runs smooth — this is engineering validation of the experience, not marketing narrative [data:visual/assets/philosophy/first-person-scripts.json#scripts]. Three scripts (an elderly resident picking up medicine / a developer walking three cross-section types / a citizen receiving a delivered lunch) each verify whether every step of one experience chain is reachable.

**Epistemology — Epistemic Grading.** Every claim is graded as descriptive / predictive / normative with its knowledge basis noted — making the proposal honest about its own epistemic boundaries [data:visual/assets/philosophy/epistemic-grading.json#claim_grades]. The three grades stay distinct: description (observable facts), prediction (methodological inference, not field-verified), and norm (design suggestion, conceptual not reviewed) — avoiding treating inference as fact or suggestion as official review.

**Game Theory — Incentive Compatibility.** The core of mechanism design: make "honesty" the dominant strategy — a mechanism is stable only when participants are motivated to comply [data:visual/assets/philosophy/incentive-compatibility.json#incentive_mechanisms]. Honest publication of the data timetable earns traffic priority (concealment caught in audit loses the priority); certified services earn subsidies and trust badges (uncertified ones get neither subsidy nor badge); randomized audits make concealment impossible. When the gains of honest publication exceed the losses of concealment, honesty becomes the dominant strategy.

**Temporal — Self-Update Protocol.** The proposal is not a static design; it defines the trigger protocol for which parts of the proposal must be recomputed when the city changes [data:visual/assets/philosophy/self-update-protocol.json#triggers]. Demographic change → recompute the silver-age nodes; robot technology maturation → recompute the cross-section speed caps; policy and regulation change → recompute the license stages; official boundary release → recompute all areas. The proposal stays honest toward time.

**The six directions and the cross-section grammar.** All six directions are logical extensions of the cross-section grammar itself: the value table makes explicit the grammar's implicit value ordering; site necessity is the grammar's historical legitimacy; the empathy scripts are proof that the grammar can be experienced; epistemic grading is the grammar's honesty about its own epistemic boundaries; incentive compatibility is the mechanism guaranteeing the grammar is complied with; self-update is the grammar's honesty toward time. Together the six upgrade "the cross-section is a contract" from a spatial claim into an engineering commitment self-consistent across six dimensions — value, history, experience, epistemology, incentives and time [depth:metrics_recalculation].
