...
iteration: "v3.0"
summary: "以京张铁路遗址公园慢行主轴为脊梁，提出'共行断面语法（Co-Mobility Cross-Section Grammar）'作为核心组织概念：四种断面原型（专用段2.5m/共享段/行人优先段/测试步道2.0m）× 四级速度分区（5/8/10/15 km/h）× 冲突测试场，构成一套可读写的街道断面原型系统，使低速机器人与行人的共存规则从几何约束中生成，而非从抽象政策中推导。在此空间语法之上，以'共行执照（Co-Mobility License）'作为治理实施层——该合同框架灵感来自 jingzhang-168 的 BASE/BOOST/BLACKOUT/BEQUEST 四阶段合同——将断面原型签署为可运营的空间契约。方案配套 15.4km 专用+3.4km 共享车道、六座智递驿站、两座充电维保基地、12张AI场景卡（含4张测试验证场景）、8类用户画像、3个AI朝圣地标、7个全球生态案例、品牌视觉规范与运营KPI体系，形成可体验、可验证、可推广的低速机器人共行网络概念方案。"
...
低速机器人要安全进入城市开放空间，需要的不是一份许可清单，而是一套**可读写的街道断面语法**。正如铁路轨距决定了什么列车能在哪条线上运行，街道断面原型决定了机器人能在哪里、以什么速度与行人共存。本方案提出**共行断面语法（Co-Mobility Cross-Section Grammar）**：四种断面类型——专用段（2.5m，物理隔离）、共享段（共面，视觉引导）、行人优先段（共面，遗产铺装随流）、测试步道（2.0m，临时隔离）——不仅是车道宽度的差异，它们是**空间契约**，编码了谁让行、以什么速度、在什么条件下、以及什么兜底方案。冲突测试场是新断面原型在部署前必须通过验证的场所。共行执照（Co-Mobility License，灵感来自 jingzhang-168 的 BASE/BOOST/BLACKOUT/BEQUEST 合同框架）是将这些空间契约签署为运营行为的治理实施层 [E:ROBOT-V3-CROSS-SECTION-GRAMMAR]。
...
**迭代说明。** 本方案为第三次迭代：v1（68/100）建立了"一廊四支、六驿两基"空间结构；v2（75/100）引入共行执照协议但组织概念 borrowed from jingzhang-168；v3 将原创的断面语法提升为组织概念，将共行执照降格为治理实施层，并在风险章节记录完整的迭代教训。
...
**公共空间组件库。** 驿站模块、充电桩、接驳站台、信息柱四类组件统一设计语言（银灰+京张绿），可组合、可迁移，供专业团队深化复用。组件库与断面语法的可逆性原则对应：极端天气或公共事件下，组件可快速断电、收储或转为人工服务点，断面可从机器人模式还原为纯慢行模式，保证公共空间在机器人停运时仍完整可用 [depth:blue_green_public_space]。
