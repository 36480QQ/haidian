...
- **AI 场景**：场景卡 02 安全治理沙盒、06 清河低碳创新廊、08 海乌绿色智算调度测试、10 气候数字孪生城市运维测试。
...
#### 节点卡 N4｜校—站缝合桥 / Campus–Station Stitch（形态：跨越阻隔的慢行连接＋两端落地小广场）
...
- **AI 场景**：场景卡 05 大钟寺国际路演客厅、07 数据要素会客厅、03 断网／断电失效安全、10 气候数字孪生城市运维测试（发布端）。
...
| 6. 投诉、争议、暂停与退出机制 | 受理投诉、处理争议、执行暂停与退出 | 任一使用者可就单一场景提出投诉；达到暂停条件时立即降级为人工服务 | 暂停与退出不得以任何理由降低基本公共服务水平 |
...
3. **年度审计与续期／修改／停止决定**：由第 3 类责任方出具公开审计意见，结合第 4 类的使用者意见，作出续期、修改或停止三选一的决定并公开说明理由。停止是常规选项之一，不视为失败。
...
**当前包状态说明**：本包 `package_state` 仍为 `scaffold`，几何、指标、五张图件、HTML 与 A3/A0 PDF 均为占位产物；本轮任务只重写中文主体正文，指标值与图件不随之更新，因此**正文中的指标引用只指向计算口径与设计含义，不指向已完成的复算结果**。
...
**版权与合规**：所有图片、图纸、图标、字体、数据与代码资产的来源、许可与授权状态须在 `sources.json` 与 `report/copyright_statement.md` 中登记；不使用未授权的商标、肖像、论文图像与版权材料；不复制北京中轴线或世界遗产官方视觉。HTML 页面不得加载远程脚本、远程地图瓦片、远程字体、iframe、表单或外部 API，不得跟踪评审者行为。主文件为中文，`proposal.en.md` 译文缺失只产生 non-blocking warning。
