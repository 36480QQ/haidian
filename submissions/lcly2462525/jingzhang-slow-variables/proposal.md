...
四道放行门决定一个场景能否扩大。G1 检查数据和安全，G2 检查现场人工复核，G3 处理公众异议，G4 检查五类慢变量有没有恶化。任一未通过，项目缩小、暂停或退出 [source:AGENT-TASKBOOK] [data:geometry/public_space.geojson#PUBLIC-001] [depth:risk_missing_data]。
...
三处 AI 朝圣节点不做巨型科技雕塑。北端“零号基线碑”公开测量方法和失败记录。中段“异议站台”保存少数意见与修订版本。南端“十年总账墙”展示长期公共收益和已经退出的项目。荣誉只授予可复现方法、公共贡献和及时退出，不按融资额与流量排名 [source:AGENT-TASKBOOK] [depth:blue_green_public_space]。
...
长期运营建议包括 Slow Variables Assembly 国际开放周、开发者修复营和市民陪审团。每场活动必须留下可下载的方法、局限、异议和退出决定。主办、预算、招商与政策都要后续授权，方案不把概念活动写成既定事实。
...
过程指标已经明确。12 张场景卡，3 项产业测试，7 类使用者，3 处朝圣节点，4 道放行门。任务覆盖、专业标准和设计深度分别保存在三份矩阵。正文负责解释为什么，结构化文件负责证明是否完整。
