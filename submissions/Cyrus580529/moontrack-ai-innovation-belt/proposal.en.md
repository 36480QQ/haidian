...
**The primary identity is a wordmark, not a pictorial symbol.** This version turns the direction above into actual assets, and in doing so overturns the previous version. That version's mark was a pictorial symbol - a moon disc with the measured Xiaoyue River centreline - geometrically honest but **unreadable to a viewer**. Three further forms were tried: a moon phase, a chevron notch, a doubled zigzag. All shared one fault: they needed explaining. **A mark that needs explaining has failed**, and that matters far more than the merit of sharing a source with the package's layers, so the pictorial symbol was abandoned.

The identity is now carried by two things. The **wordmark**, MoonTrack, is the primary form and is used everywhere, especially at small sizes - it cannot be misread, which is its whole merit. The moon is carried by the name itself; the two red rules beneath the wordmark take the doubled character of rail track and abstract the chevron, but on the page they function as a divider and ask nothing of the reader.

The **emblem** is a moon gate, real Beijing garden vocabulary. Inside the aperture, from bottom to top: water, track, the chevron switchback and the moon - this corridor compressed into one roundel. It is used on covers, drawing title pages and boards. **It is illegible below 128 px, so the wordmark is used at small sizes instead**, a rule written into the identity system's misuse clauses. The emblem is an AI-generated brand image, not a site photograph, and depicts no existing place.

The chevron returns to its proper place as a **secondary motif**, used on components, wayfinding and event material, never as the identity mark. Colour, variants, lockups and misuse rules are set out below.
