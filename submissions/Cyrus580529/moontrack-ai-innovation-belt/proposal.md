...
summary: "月轨 MoonTrack：把路权规则当成可以标价的对象。对 421 段非机动车道做连通性实测，慢行网碎成 62 个分量，五家适老机构沿法定路权的网络可达数为 0。同一批 1,619 个门再测一次——人走路末端中位数 82.8 米，机器人 249.6 米：这就是'只准走非机动车道'这条规则的价钱。据此给出可比价的政策顺序：先改规则（零工程、可达网络 6.5 倍），再补 596 米连接，最后做末端。30 条调度台账在实测路权图上复算，结论包内可重跑。"
