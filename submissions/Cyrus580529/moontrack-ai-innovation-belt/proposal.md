...
**标识主形是字标，不是图形符号。**这一版把上述方向做成了实际资产，并且推翻了前一版的做法。前一版的标志是一个图形符号——月轮加小月河实测中线，几何上诚实，但**读者看不懂它**。此后又试过月相、人字切口、双轨折线三种形，共同的问题一样：需要解释。**需要解释的标志就是没做成**，这一点比"标志与图层同源"这个优点重要得多，所以放弃了图形符号。

现在的标识由两件东西承担。**字标**「月轨 MoonTrack」是主形，用于一切场合，尤其是小尺寸——它不会被误读，这是它的全部优点。"月"由名字本身承担；字标下那两道红线取自轨道的双线特征，也是人字展线的抽象，但它在版面上的身份是分隔线，不要求读者从中读出含义。

**徽章**是一枚月洞门——北京园林的真实语汇。门洞之内自下而上是水、轨、人字展线与月，把这条走廊压进一枚圆里。它用于封面、图纸扉页与展板；**在 128 像素以下不可辨识，因此小尺寸一律使用字标**，这条写进了识别系统的禁用条款。徽章为 AI 生成的品牌图像，非现场照片，不描绘任何既有场所。

人字展线退回它本来的位置：**次级母题**，用于构件、导视与活动物料，不进标识。色彩、变体、组合应用与禁用条款见下图。
