...
summary: "Proposes The Proving Ground as the overall concept. Zhan Tianyou's real legacy on the Jing-Zhang line was not the zigzag at Qinglongqiao but standardisation: the 1435mm gauge, the automatic coupler, and the Jing-Zhang Standard Drawings. A century ago this line answered how a new technology is uniformly measured and built; today the same line should answer how AI is uniformly measured and accepted. The proposal runs a 9.66 km Baseline along the heritage park as one shared observation section, organises the three key areas as L1 closed proving ground, L2 semi-open pilot and L3 open deployment, promotes systems by published baseline rather than administrative approval, and adds five public-reading metrology nodes, twelve scenario cards with thresholds, and an annual baseline release. All content is an open co-creation suggestion."
...
The zigzag was a clever response to terrain. What actually shaped the following hundred years was a different kind of work. He adopted the 1435 mm standard gauge on the Jing-Zhang line, so that China's rail network could interoperate with the world from the start. He petitioned the Qing government to adopt the automatic coupler, replacing a patchwork of incompatible practices with one common rule. The Guangdong Chinese Engineers' Society published the Jing-Zhang Standard Drawings. In 1913 he merged three societies into the Chinese Institute of Engineers, whose stated purpose began with "to define construction standards". He even directed a Chinese-English engineering glossary to unify technical terminology.
...
Every company tests its own way, every district hangs its own sign, and the measurement definitions do not interoperate. This is precisely the railway's situation a century ago: Germany used standard gauge, Russia broad gauge, France narrow gauge. Each segment ran; together they formed no network.
...
This is the most accurate inheritance of the Jing-Zhang spirit. Zhan Tianyou's contribution was not that he built faster than others, but that he defined the standard everyone afterwards had to follow.
...
2. **AstaZero in Sweden and ALP.Lab in Austria and Germany**: graded closed facilities releasing capability step by step. Lesson: graded environments are proven practice; this proposal's contribution is extending them from dedicated facilities into a **real urban district**.
3. **One-North and CETRAN in Singapore**: a research district and an autonomous vehicle test centre in the same place. Lesson: validation facilities should not sit far from the city, or results lose urban representativeness.
...
No existing example combines all three attributes: city-scale, publicly published baselines, and graded release. That is the differentiated opportunity argued here. Haidian has the university density, the industrial scale, and the real urban scenarios to build **the first city-scale AI proving ground in the world**.
...
The submitted land-use layer is a complete partition of the entire boundary, not a set of concept patches. Fifteen polygons cover the whole boundary with a residual of 93.8 square metres, or 0.00082% of the site, far below the validation tolerance[metric:land_use_coverage_gap_sqm][data:geometry/land_use.geojson#LU-001]. Adjacent polygons share boundary coordinates with no gaps and no overlaps. This is worth stating because many concept schemes draw only key parcels and leave large unlabelled voids, which makes areas impossible to recompute and regulatory-plan depth impossible to certify[standard:MOHURD-CONTROL-DETAILED-PLANNING][depth:land_use_layout].
...
The grading borrows directly from staged release practice in machine learning: a model does not go from training straight to production but passes offline evaluation, shadow mode, gradual rollout, and full deployment. This proposal translates those stages in time into three districts in space, where **promotion depends on passing a published baseline rather than on administrative approval**. That is the essential difference from a conventional pilot zone: pilot zones admit by qualification, a proving ground releases by result.
...
| # | Scenario | Task | Key indicators | Baseline source | Fallback | Level |
|---|---|---|---|---|---|---|
| SC-01 | Adaptive signal timing | Intersection signal control | Delay per person, secondary crossing rate | Four weeks of fixed-time measurement | Revert to fixed timing | L2→L3 |
| SC-02 | Low-speed delivery robot | Footway delivery | Interventions per 1000 km, yield success | L1 closed-site result | Slow down or leave footway | L1→L2 |
| SC-03 | Accessible wayfinding | Guidance for vision and mobility needs | Route success rate, misdirection rate | Human-escort control group | Switch to human operator | L2 |
| SC-04 | Government service assistant | Enquiry and document pre-check | First-pass rate, wrong guidance rate | Existing counter service data | Transfer to counter | L2→L3 |
| SC-05 | Community health navigation | Triage advice and care routing | Triage agreement, miss rate | Independent physician review | Mandatory human review | L2 |
| SC-06 | Cultural guide agent | Heritage park interpretation | Factual error rate, dwell time | Verified professional script | Fixed narration | L2→L3 |
| SC-07 | District crowd management | Flow guidance and facility dispatch | Peak crowding index, egress time | Existing holiday measurement | Manual control | L3 |
| SC-08 | Building energy optimisation | HVAC and lighting control | Energy per square metre, comfort complaints | Same period last year | Manual mode | L1→L2 |
| SC-09 | Enterprise service copilot | Policy matching and application support | Match accuracy, rejection rate | Human advisor control group | Transfer to advisor | L3 |
| SC-10 | Public safety operations review | Incident reconstruction | Review completeness, traceability time | Existing manual review | Full manual review | L2 |
| SC-11 | Construction dust and noise | Site environmental monitoring | False alarm rate, response time | Existing manual inspection | Restore manual inspection | L1→L2 |
| SC-12 | Multi-agent coordination | Right-of-way negotiation between robots | Conflict count, task completion | Single-robot result | Revert to single robot | L1 |
...
**Key results and checks**: the submitted boundary encloses 11,412,825 square metres, deviating +0.113% from the announced 11.4 square kilometres[metric:site_area_sqm][metric:site_area_deviation_ratio]; the land-use partition leaves a residual of 93.8 square metres against the boundary, or 0.00082%, proving the layer is a complete partition rather than partial patches[metric:land_use_coverage_gap_sqm][metric:land_use_total_area_sqm]; and the three key areas deviate by +0.427%, +0.023%, and +0.063%, all within the 3% tolerance[metric:key_area_beijing_ai_origin_community_deviation_ratio][metric:key_area_dazhongsi_ai_industry_cluster_deviation_ratio].
...
9. Historical record of Zhan Tianyou and Jing-Zhang standardisation, covering the 1435 mm standard gauge, the automatic coupler, the Jing-Zhang Standard Drawings, the Chinese Institute of Engineers and its founding purpose of defining construction standards, and the Chinese-English engineering glossary. This is a summary reference to public historical material used as narrative basis, not as a source of spatial or metric conclusions[source:PROCESSED-FACT-PACK].
