...
| 评审维度 | 响应章节 | 主要证据文件 | 代表引用 |
| --- | --- | --- | --- |
| 任务书相关性 | 第二至九章 | compliance_matrix.json、standard_matrix.json | [source:OFFICIAL-ANNOUNCEMENT] [source:AGENT-TASKBOOK] |
| 可实施性 | 第十一章（更新/分期/资金） | risk.json、simulation.json、phasing.geojson | [depth:renewal_project_list] [depth:phasing_implementation] |
| AI 规划创新 | 第六章（场景/脉冲协议） | simulation.json（15 项服务四拍推演） | [source:AGENT-TASKBOOK] |
| 表达完整度 | 全文＋图纸/HTML/视觉 | manifest.json、assets/figures/*、drawings/* | [metric:site_area_sqm] [metric:green_ratio] |
| 原创性 | 执行摘要＋第八章 | 命名体系分级表、波形状态灯语言 | [source:JZ-RAILWAY-CULTURE] |
| 公共利益与包容 | 第六章＋第十二章 | risk.json（equity_inclusion）、compliance_matrix.json | [standard:BARRIER-FREE-ENVIRONMENT-LAW] |
| 风险合规 | 第十二章 | risk.json、standard_matrix.json、copyright_statement.md | [depth:risk_missing_data] |
...
| agent.6 运营转化 | 第十一章 | 年度活动体系、招引转化路径、治理结构 |
...
| 1.4 三层范围 | 统筹研究区/总体设计区/三重点区 | 第三章 |
| 1.5(1) 三区两翼 | 三重点区与双翼协同回路 | 第三章、第五章 |
| 1.5(2)1 产业目标与功能布局 | "1+X+1"功能比例与空间组织模式 | 第三章（表 1） |
| 1.5(2)2 城市更新总体框架 | 更新项目清单与分期实施 | 第十一章 |
| 1.5(2)3 交通轨道市政配套 | 站点一体化、道路微循环、慢行、停车 | 第九章 |
| 1.5(2)4 京张遗址公园活力带 | 绿廊贯通、遗址活化、公园统筹 | 第十章 |
| 1.5(2)5 城市风貌 | 北影等艺术资源、风貌控制 | 第十章 |
...
> agent 响应：本章对应 agent.2（全球 AI 生态）与 agent.6（运营转化），核对见表 A2。
...
> agent 响应：本章对应 agent.1（命名体系的空间落点）与 agent.5（文化叙事载体），核对见表 A2。
...
> agent 响应：本章对应 agent.3（场景落点）与 agent.4（朝圣地标），核对见表 A2。
...
> agent 响应：本章对应 agent.3（场景体系）与 agent.6（运营转化），核对见表 A2。
...
**受益、代价与盲区矩阵（概念建议，公开披露）**：为让评审与公众对场景的预期收益与已知代价同样知情，下表对 8 个关键场景及方案整体如实列示受益、代价与当前盲区——不回避负面面：
...
| 方案整体 | 可复核、可替换边界重算 | 官方边界发布后需重算部分指标 | 产业目录/投资口径为概念假设（[source:ASSUME-005] [source:ASSUME-006]），交通极端情景无实测流量支撑 |
...
| 底线指标 | 量化要求（概念） | 校验方式 |
| --- | --- | --- |
| 非 AI 替代通道保留率 | 100%（人工导览、电话预约、线下人工服务） | 每场景 P1 申报核验 |
| 人工替代率（无障碍导行） | 100%（现场不符即停用） | 场景卡 08 P2 独立复测 |
| 弱势群体代表席位 | ≥1/3（公众委员会） | 委员会构成公示 |
| 普惠时段覆盖 | 场景卡 05/11 每日设普惠时段 | 运营台账公开 |
| 数据最小化 | 不采集敏感信息；留存期上限 30 天（快递记录） | PIA 评估记录 |
...
**三重点区拆改留分档假设（概念建议，待官方现状调查与控规复核）**：为回应公告 1.5(3) 对重点区"明确拆改留分类方案"的必答要求，按公开资料给出分档比例基准点（三档合计 100%）与独立浮动区间，不构成任何拆除、保留或搬迁结论：

| 重点区 | 保留（基准点；浮动区间） | 整治（基准点；浮动区间） | 更新/拆建（基准点；浮动区间） | 复核依据 |
| --- | --- | --- | --- | --- |
| 众智园 | 45%（35–55%） | 35%（25–45%） | 20%（10–30%） | 基准点合计 100%，区间独立浮动；以公开园区信息为底，官方现状调查后复核 |
| 北京AI原点社区 | 40%（30–50%） | 35%（25–45%） | 25%（15–35%） | 基准点合计 100%，区间独立浮动；五道口商住带混合度高，需权属调查 |
| 大钟寺 | 35%（25–45%） | 35%（25–45%） | 30%（20–40%） | 基准点合计 100%，区间独立浮动；站城更新强度高，需控规与文保复核 |
...
> agent 响应：本章对应 agent.5（三线文化叙事与导视系统），核对见表 A2。
...
> agent 响应：本章对应 agent.6（运营转化与治理结构），核对见表 A2。
...
| 项目编号 | 项目名称 | 类型 | 近期动作 | 放行证据 | 牵头建议 | 协议节拍 |
| --- | --- | --- | --- | --- | --- | --- |
| JZ-01 | 中央智脉绿廊贯通工程 | 公共空间/蓝绿 | 沿线步行审计、临时导视、桥下空间清障 | 红线、交通组织与生态复核 | 区园林绿化局+交通部门 [data:geometry/green_space.geojson#GREEN-001] | P3 发布 |
| JZ-02 | 北五环跨环慢行节点 | 交通/慢行 | 断面与高架条件评估 | 结构安全与跨线审批 | 交通委+专业设计单位 [data:geometry/roads.geojson#ROAD-001] | P3 发布 |
| JZ-03 | 众智园门户广场与 AI 之光塔 | 公共空间/地标 | 概念方案与光环境试验 | 权属与景观审批 | 园区运营平台 [data:geometry/public_space.geojson#PUBLIC-003] | P3 发布 |
| JZ-04 | 原点发布厅与代码墙 | 产业服务/文化 | 首层业态策划、开源活动试运营 | 权属与运营主体确认 | 中关村开源社区+属地 [data:geometry/buildings.geojson#BLDG-001] | P1 申报 |
| JZ-05 | 大钟寺站四象限步行连通 | 轨道一体化/慢行 | 过街时长、无障碍、非机动车停放调查 | 轨道站与道路交叉口复核 | 属地+轨道运营方 [data:geometry/public_space.geojson#PUBLIC-001] | P3 发布 |
| JZ-06 | 智脉大道无人接驳示范段 | 交通/新基建 | 法规梳理与信号条件评估 | 路测备案与安全预案 | 区测试办+公交集团 [data:geometry/roads.geojson#ROAD-008] | P2 测试 |
| JZ-07 | 清华东路教育带缝合 | 城市更新/教育 | 校区边界与慢行安全调查 | 权属与校区同意 | 街道+高校 [data:geometry/land_use.geojson#LU-001] | P1 申报 |
| JZ-08 | 南部更新带提升 | 城市更新/住宅 | 现状建筑与用地调查 | 拆改留专项评估 | 属地+规划团队 [data:geometry/phasing.geojson#PHASE-003] | P1 申报 |
| JZ-09 | 低空配送航线验证场 | 新基建/产业测试 | 空域与安全监管梳理 | 空域审批 | 属地+监管机构 [data:geometry/constraints.geojson#CONSTRAINTS] | P2 测试 |
| JZ-10 | 端侧算力与能耗调控示范楼 | 新基建/市政 | 能源与算力需求评估 | 消防与运营主体确认 | 能源企业+园区 [data:geometry/buildings.geojson#BLDG-001] | P2 测试 |
| JZ-11 | 无障碍 AI 导行系统 | 公共服务/无障碍 | 标准与数据授权梳理 | 无障碍设施标准复核 | 残联+运营方 [data:geometry/constraints.geojson#CONSTRAINTS] | P1 申报 |
| JZ-12 | 全球 AI 活动周公共路线 | 运营/品牌 | 活动许可与版权清权 | 公共空间许可与安全预案 | 联合运营机构 [data:geometry/phasing.geojson#PHASE-001] | P2 测试 |
...
| 第 15–30 天 | 评审反馈闭环：按意见修订章节与指标 | 本方案团队 | 修订版（v8.x） |
...
| 拆改留分档 | 基准点归一 100%，区间独立浮动 | 第五章表（ASSUME-005） |
