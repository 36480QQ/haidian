...
**版本可追溯**：本包当前迭代 **v10.12.0**（2026-08-15，frontmatter iteration 17），变更记录逐轮登记于 `changelog.md`，每处主张可沿 changelog 复核引入轮次。**本轮 v10.12.0** 为实施可行性增补轮，纯增补（一行不删）：表 A3 成本复算框架（12 项 JZ 项目逐项「复算基数×单价区间=复算区间」，任一基数或单价变更即按 P4 重算，官方口径发布后自动失效重算）、表 6.8 试点 0–100 日检查点（5 个检查点各带「动作+通过证据+不通过处置」三件套，与证据门 E1–E4 对应）、12 张场景卡 S/M/L 载体面积档（≤500 m²/500–5,000 m²/>5,000 m²，几何落点，概念分级，详规发布后 P4 复核重新分档）。QA 量化口径（表 A10 E12）：图纸像素级复核——双语标题带 46–94 px、顶部留白 1–2%、指标副题零重复行、边界虚线可机检；"标题被裁切"指控经像素扫描证伪。
