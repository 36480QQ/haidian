...
**版本可追溯**：本包当前迭代 **v10.10.0**（2026-08-15，frontmatter iteration 16），变更记录逐轮登记于 `changelog.md`。近四轮按时间序：**v10.7.2** 图纸 QA 修复轮（机器视觉复核：双语标题分置、面板头部留白、指标图副题去重、深底图标题改浅色、边界虚线化、图例移出比例尺区、路名加底衬）；**v10.7.3** 诚实性证据链轮（勘误登记册表 A10 13 条全部可 join 回 changelog、三条红线=现行法规义务条号逐条核实、同场关键词扫描表 A11 801 份、simulate-check 拒绝自测 8/8、verify-counts 零依赖重算 9/9、复算纪律与包级置信度）；**v10.8.0** 证据可见性轮（表 A1 升级为评审一页入口每行给可运行命令、表 A12 可运行证据快照真实 CLI 输出内嵌正文、执行摘要证据四形态导航、表 B1–B3 投入量级与不满足处置列、版权台账逐资产化、多模态交付音频导览与短片见表 A7）；**本轮 v10.9.0** 为评审可读性轮（summary 精简至 3 句、机制总览表 M0、三重点区文本剖面、空间双状态规则表、JZ 表拆分并补审批/备案节点、场地现状研判、场景卡实施阶段列、波形状态灯三态表、证据失效级联规则化并前移、图面防重叠 QA 重渲染）。**本轮 v10.10.0** 为双评回归加固轮（对照 90+ 分同行结构 + 代理模型维度评分）：JZ 主表恢复 12 列（加回资金属性/成本量级/回滚触发器三列，修复上轮拆列造成的实施可行性证据可读性回退）、逐资产权利摘要表（52 行台账进正文，逐类资产给出制作链路与核验位置）、人工签署前的双语实质等价核查专节、表 A2 升级为六项任务可核查索引（新增"进入现实工作的门"列）、公众委员会示范章程要点表（委员产生/任期/议事/申诉/记录公开五要素）。全部为纯增或列恢复，未删改既有主张。评审可沿 changelog 复核每处主张的引入轮次。QA 量化口径（可复验）：图纸视觉复核采用像素级扫描而非模型目测——双语标题带垂直分离实测 46–94 px、面板顶部留白 1–2%、指标副题与主标题去重后零重复行、边界虚线化后线型可机检（12 张图全部通过）；上一轮视觉模型对"标题被裁切"的指控经像素扫描证伪后未改图，记录于表 A10 E12。
