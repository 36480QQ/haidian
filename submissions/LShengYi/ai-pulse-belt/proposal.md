...
**版本可追溯**：本包当前迭代 **v10.15.0**（2026-08-15，frontmatter iteration 20），变更记录逐轮登记于 `changelog.md`，每处主张可沿 changelog 复核引入轮次。**本轮 v10.15.0** 为 rubric 词面逐项对齐轮（纯增补）：可实施性四件事按 rubric 原文词面逐项对齐（阶段路径 / 试点区域 / 参与主体 / 指标——每项给出本稿提供什么、在哪里可打开核验、推进条件），P1 申报台 1:1 最小物理载体原型（人工窗口先行、判定署名、未决栏公开、撤除后保留什么；not_authorized_not_run，不预设尺寸），表 B1 版本漂移修复。上一轮 v10.14.0 为合规修复轮（needs_review 来源降级仅存档；无人机规章更新为 2026 版《北京市无人驾驶航空器管理规定》〔十六届〕第 50 号，北京全域管制空域；指标计数统一为 76 项中 61 项 known）。QA 量化口径（表 A10 E12）：图纸像素级复核——双语标题带 46–94 px、顶部留白 1–2%、指标副题零重复行、边界虚线可机检；"标题被裁切"指控经像素扫描证伪。
...
**1:1 最小物理载体：AI 申报台（把 P1 申报协议缩到一张桌子）**：本方案 AI 服务进入城市的第一个门是 P1 申报——把这一协议落到最小物理单元"申报台"：一张桌子加四件必须同时在场的东西。

| 构成段 | 是什么 | 必须满足才能开台 | 撤除后留下什么 |
| --- | --- | --- | --- |
| ① 人工窗口 | 不依赖任何设备：纸本表单、人工问询、无AI等价申报路径 | **必须先于智能层存在** | **人工窗口保留，这一段不撤** |
| ② 申报位 | 服务登记卡：服务边界、数据使用、失败降级、停用办法 | 缺任一张即不开台 | 卡片归档进台账，台面恢复公共座椅 |
| ③ 判定牌 | 受理 / 暂缓 / 拒收三选一，必须署名 | 申报方与审批方不得为同一角色 | 判定记录归档，判定牌收回 |
| ④ 未决栏 | 上一批未闭合的问题逐条公开挂出 | 存在阻塞性未决项时智能层保持关闭 | 未决项转入公开台账，不随台子消失 |

四件东西顺序不可调换——这正是本方案核心主张在一张桌子上的样子：人工窗口先立，未决栏公开，然后才谈申报与受理。**如果一张桌子只能保留一段，保留人工窗口。**

开台前五条检查（场地权利与责任主体、申报与审批分别指派且非同一人、人工窗口先行并完成一次无AI等价申报演练、回滚演练被观察到通过、五项公共底线评估通过）与 P03 行动包及双联交接账实例共用同一口径，不另立标准。

**当前状态：`not_authorized_not_run`**——没有选址、没有主体、没有建造、没有运行过。位置、尺寸、材料、无障碍净宽与桌面高度须依现场测绘与无障碍规范确定，本方案不预设数字。
...
**可实施性四件事逐项对齐（rubric 原文词组：阶段路径 / 试点区域 / 参与主体 / 指标——每项给"本方案给出什么、在哪里可打开核验、推进条件"）**：

| rubric 要素 | 本方案给出什么 | 在哪里可打开核验 | 推进条件 |
| --- | --- | --- | --- |
| 阶段路径 | 协议四拍链 P1 申报→P2 测试→P3 发布→P4 验收退役；三期概念分期；试点 0–100 日检查点 | `geometry/phasing.geojson`（PHASE-001/002/003）；表 6.8 试点 0–100 日检查点；P4 回滚触发器 R-01–05 | 官方控规与权属发布后按 P4 重算；阶段为概念分期，不写固定年份承诺 |
| 试点区域 | 三重点区（众智园 / AI原点社区 / 大钟寺）；12 张场景卡空间落点（S/M/L 载体面积档）；3 个产业测试验证场景 | `geometry/key_areas.geojson`；场景卡八要素"空间载体"字段；`public_space.geojson`#SCN-01…12 | 几何为 provisional（已披露），官方边界发布后整体重算 |
| 参与主体 | 每卡"运营主体"字段（公交集团+区测试办等）；公众委员会（弱势席位≥1/3、知情/建议/申诉权）；运营秘书处/片区站/专业翼；无AI等价人工路径 | 场景卡八要素"运营主体"字段；公众委员会章程表；意见征集—回应—修订—公示闭环 | 主体角色为建议性描述，正式委托须经法定程序，本稿不替法定主体决定 |
| 指标 | 76 项指标中 61 项 known（EPSG:4548 可复算）；15 项 unknown 分两类（法定控制/产业绩效）——未赋值≠未管理 | `metrics.json`（全量 76 项含公式/status/source_file）；`design_depth_matrix.json`（测量协议十二件事） | 官方指标体系与口径发布后按 P4 重算并公开 diff |

> 表注：四要素按 rubric 原文逐项对齐，全部指向可打开文件；未赋值指标非设计方能定（法定控制指标赋值即伪造法定结论，产业绩效指标无授权基线即编造）。
