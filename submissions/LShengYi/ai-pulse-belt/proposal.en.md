...
**Version traceability**: current iteration **v10.13.0** (2026-08-15, frontmatter iteration 18); per-round records in `changelog.md`, every claim traceable to its round. **v10.13.0** = mechanism-scarcity & implementation-path round, pure addition: Table B4 official module mapping (JZ items ↔ official renewal modules M01–M10; only M03/M04/M09/M10 complete_as_concept — deliverable complete, not official permission — rest await official inputs, unmet thresholds suspend under P4), Table B5 element-interface ledger (compute/data/scenarios/funding/talent/space/equipment/brand; withdraw conditions share R-01–05 and the four-stage contract beat, never unilateral), mechanism-scarcity re-measurement (own track_scan over 840 proposals: excl. this submission — state machine 0, day checkpoints 0, cost-recalc 3, withdrawable interfaces 4; hit lists public). QA caliber (Table A10 E12): pixel-level figure review — title bands 46–94 px apart, headroom 1–2%, zero duplicate subtitle rows, machine-checkable dashed boundaries; "title clipped" refuted by pixel scan.
...
| Days 15–30 | Close review feedback: revise chapters/metrics per comments | Proposal team | Revision (next iteration, currently v10.12.0) | 6–10 person-days |
...
**Table B4 Official implementation-path module mapping**: JZ items map one-to-one onto the official implementation-path modules M01–M10 (Haidian urban-renewal guidelines); only M03/M04/M09/M10 reach complete_as_concept — deliverable complete, not official permission — the rest state which official inputs they await, "method framework" never written as "landed"; any unmet threshold suspends the module under P4 review, never skipped [source:HAIDIAN-URBAN-RENEWAL-2025] [depth:renewal_project_list]:
