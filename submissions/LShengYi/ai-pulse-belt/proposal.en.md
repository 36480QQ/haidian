...
**Version traceability**: this package's current iteration is **v10.11.0** (2026-08-15, frontmatter iteration 17); change records are logged round by round in `changelog.md`, and every claim can be traced to the round that introduced it. **v10.10.0**, the dual-review regression-hardening round (JZ master table restored to 12 columns, per-asset rights summary table, pre-human-signoff bilingual substantive-equivalence check, Table A2 six-task gate column, public-committee model-bylaw essentials table; PR #2708 scored 79/100, confirming the column-restoration fix worked); **this round, v10.11.0, is the expression-density and governance-benchmark round** — the JZ sub-table is removed (capital attribute / cost band / rollback trigger all remain in the 12-column master table), the executive summary and evidence-reading paragraphs are condensed, and five governance-practice benchmark sources are added to the narrative (Haidian Urban Renewal Guidelines 2025, UK Algorithmic Transparency Recording Standard v2.1, the Dutch Algorithm Register, Barcelona Decidim, and the UN-Habitat People-Centred Smart City guide), all registered in `sources.json`. Everything is additive or deduplication; no existing claim was deleted or altered. QA quantitative caliber (re-verifiable): figure review used pixel-level scanning, not model eyeballing — bilingual title bands vertically separated by a measured 46–94 px, top headroom 1–2%, metrics subtitle dedup with zero duplicate rows, provisional boundaries dashed and machine-checkable (all 12 figures pass); the "title clipped" allegation was refuted by pixel scan and no figure was changed (Table A10 E12).
...
| Days 15–30 | Close review feedback: revise chapters and metrics per comments | Proposal team | Revision (next iteration, currently v10.11.0) | 6–10 person-days |
