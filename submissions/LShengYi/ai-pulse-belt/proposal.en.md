...
| Evidence layer | 120 offline synthetic checks (all 105 failure branches blocked) + 13 errata entries + 801 same-field census + 9/9 count recomputation | Every claim verifiable, re-runnable, falsifiable | Tables A10/A11/A12 + simulation.json |
...
**Version traceability**: current iteration **v10.12.0** (2026-08-15, frontmatter iteration 17); per-round records in `changelog.md`; every claim traces to its introducing round. **v10.12.0** = implementation-feasibility enrichment round, pure addition (nothing deleted): Table A3 cost-recalculation framework (12 JZ items × recalculation base × public-market unit price = recalculated range; P4 re-runs on any base/price change; auto-invalidated once official calibers release), Table 6.8 pilot day-0–100 checkpoints (five checkpoints, each an action + passing-evidence + non-pass disposal triple, mapped to gates E1–E4), and S/M/L carrier-size bands on all 12 scenario cards (≤500 m² / 500–5,000 m² / >5,000 m², geometric anchors, concept-level, P4 re-band after the statutory plan). QA caliber (Table A10 E12): pixel-level figure review — title bands 46–94 px apart, headroom 1–2%, zero duplicate subtitle rows, machine-checkable dashed boundaries; "title clipped" refuted by pixel scan.
...
**The reading must stay honest**: the table above is not a scoreboard. The first four rows — non-skippable state machine, five-class rollback triggers, stand-down drill, exit audit — are this package's core mechanism (0–3/800); that scarcity is the re-testable evidence that the mechanism comes from mechanism, not vocabulary. The **baseline item (no-AI-equivalent route, 169/800) shows exactly the opposite: the human-equivalent route is already a de-facto field standard**, so this package treats it as an admission baseline, not an innovation claim. "Offline synthetic rehearsal" appears in 51 packages; this one only claims the same technique at 120 checks with per-check receipt hashes. Word-surface regexes cannot measure quality and do not count counterexamples — they answer only: how many times, and where, these word surfaces occur. The corpus grows daily; re-running build_track_scan.py refreshes counts, and old counts must not be cited.
...
- **Zhongzhiyuan four-band profile (north to south)**: ① **Public observation band** — along the North-5th-Ring green-belt buffer; visitors see the test objects, test status lights, responsible parties, and conclusion grades here, and can verify without entering the controlled zone; ② **Controlled test band** — hosts LLM training/testing, standards workshops, and safety-governance displays, opened by reservation on the P2 beat, with on-site safety officers and physical emergency stops; ③ **Human-takeover band** — retains manual review workstations, equipment maintenance passages, and data-isolation boundaries; any AI conclusion must be reviewed through the human-equivalent path before external display (equivalence registration in cards 06/10); ④ **Ecological-restoration band** — building–green–water integrated buffer, demonstrating site restoration after test exit (P4 spatialization).
...
| --- | --- | --- | --- | --- | --- |
| 01 Rail-inspection AR twin | Central greenway rail segment: AR overlays of century-old Jing-Zhang imagery with an AI digital-twin inspection demo | Aggregated footfall heat only; no personal imagery | Rail-heritage operator + district test office | AR factual accuracy ≥98%; unresolved factual complaints take it offline | P1 near term (JZ-01 greenway pilot) |
| 02 Autonomous shuttle corridor | Pulse-Belt Avenue: campus—station autonomous shuttle demo line (concept) [scenario:ai-traffic-walkability] | Trip data for dispatch only; anonymized after retention | Bus group + district test office | On-time rate ≥85%; any accident stops the line for manual service | P1 near term (JZ-11 wayfinding pilot) |
| 03 AI cycling coach station | Greenway nodes: cycling data visualization with AI coaching | Ride data visible to the user only; one-tap deletion | Subdistrict + greenway operator | Equipment fixed within 24h; privacy complaints pause it | P1 near term (JZ-01 synchronized) |
| 04 Bell-chime metaverse | Dazhongsi station front: digital-twin and interactive performance of the bell culture [scenario:ai-cultural-guide] | No personal behavior tracking | Dazhongsi cultural institution + district | Content complaints answered ≤48h; heritage conflicts remove it | P1 near term (JZ-12 station-city linkage) |
...
| --- | --- | --- | --- |
| 01 Rail-inspection AR twin | S | Recognition posts and guide screens along the alignment, 10–30 m² per point | Greenway rail segment alignment ([data:geometry/green_space.geojson#GREEN-001]) |
...
| --- | --- | --- | --- | --- |
| 01 Rail-inspection AR twin | Century-old memory made visible, low-threshold education | Wayside device maintenance and content-licensing management | AR compatibility with older handsets untested | P1 pilot first-month compatibility sampling (≥98% factual-accuracy caliber measured in the same run) |
| 02 Autonomous shuttle corridor | Commuting efficiency and rail-transfer linkage | Right-of-way negotiation, subsidy during demonstration | Extreme-peak crowd plan pending traffic simulation | Peak-hour passenger-flow simulation completed before P2; simulation inputs and conclusions public |
| 04 Bell-chime metaverse | Cultural reactivation, night-economy stimulus | Display constraints inside heritage protection lines, content oversight | Long-term historical-accuracy calibration to be detailed | Historical-accuracy calibration ledger established before P3 release; first-month calibration records published |
...
| 08 Accessible AI wayfinding | Substantive upgrade of accessibility service | Device dependence and on-site response time | Multi-dialect voice-recognition error rate pending evaluation | P1 pilot measures the error rate with the 100% human-alternative backstop in the same run; results public |
| 10 AI energy-management building | Energy reduction and retrofit demonstration | Retrofit investment, in-unit data boundary | Interaction between control and occupant behavior pending verification | Energy-comparison report published after one season of P2 demo operation |
| 12 Open-air developer workspace code wall | Open-source ecosystem, talent attraction | Manual content-moderation cost | Outdoor equipment dust/sun-protection wear pending estimation | Device-wear and operating-cost report published after one full year of P1 pilot |
| Package as a whole | Verifiable; recomputable on boundary replacement | Some metrics must be recomputed once official boundaries arrive | Industry directory / investment scale are conceptual assumptions (see ASSUME-005, ASSUME-006); extreme traffic scenarios lack measured-traffic support | Recomputation completed within 30 days of official survey and industry-data release, with public diff (P4 procedure); extreme traffic scenarios included in the P2 simulation list |
...
**Clause-level scope limitation (conceptual recommendation)**: legal citations are pinned to specific clauses with explicit scope boundaries, avoiding duty expansion — ① the **Generative-AI Measures** apply only to "generative AI services provided to the public within Chinese territory" per **Article 2**; content-class cards (01/04/12) follow labeling and final-review requirements within that scope only; **Article 14**'s complaint-handling deadlines are not expanded into a general exit right, nor are complaint time limits invented. ② **Article 39 of the Barrier-Free Environment Law** ("providing manual services") applies only to the public-service premises it enumerates; it is not expanded into a statutory duty for all public spaces — the accessible-AI wayfinding station's 100% manual-backup rate is this proposal's self-imposed higher commitment (self-set protocol, not statutory conclusion). ③ The **elderly-friendly policy (2020–2022 stage targets)** window has passed; only its design principle of "traditional and smart services running in parallel" is borrowed, not claimed as a 2026 local implementation fact. ④ Airspace approval under the **"Regulations on the Flight Management of Unmanned Aircraft"** (effective 2024-01-01) and the **Beijing Municipal UAS Measures** (effective 2024-06-01) is a release gate, not a self-set goal; without approval, no opening.
...
| ID | Project | Type | Near-term action | Release evidence | Acceptance indicator (suggested target) | Suggested lead | Capital attribute | Conceptual cost band | Rollback trigger | Approval/filing type | Protocol beat |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| JZ-01 | Central Pulse-Belt greenway connection | Public space/blue-green | Pedestrian audit, temporary wayfinding, under-bridge clearance | Red lines, traffic & ecology review | Through-connection rate ≥90%, net green-space gain | District landscape bureau + transport [data:geometry/green_space.geojson#GREEN-001] | Public | Medium (temporary works first, phased input per release) | Ecology/safety | Green-line/ecology review | P3 release |
| JZ-02 | North-5th-Ring slow-traffic crossing | Transport/slow traffic | Cross-section and overpass-condition assessment | Structural safety & crossing approval | Crossing time ≤60s, accessibility compliance | Transport commission + design firm [data:geometry/roads.geojson#ROAD-001] | Public | Low-medium (assessment first, works pending approval) | Safety | Crossing structural approval | P3 release |
| JZ-03 | Zhongzhiyuan gateway plaza & Tower of AI Light | Public space/landmark | Concept design and light-environment trial | Ownership & landscape approval | ≥20 events/year, 50,000 visits | Park operating platform [data:geometry/public_space.geojson#PUBLIC-003] | Quasi-public | Medium (event operations partially fund input) | Economics/safety | Ownership & landscape approval | P3 release |
| JZ-04 | Origin release hall & code wall | Industry service/culture | Ground-floor use planning, open-source event trial | Ownership & operator confirmation | ≥5,000 registered developers, ≥24 release events | Zhongguancun open-source community + district [data:geometry/buildings.geojson#BLDG-001] | Quasi-public | Low (ground-floor use + trial operation start) | Privacy/economics | Ownership & operator confirmation | P1 declare |
...
| JZ-06 | Pulse-Belt Avenue autonomous shuttle demo | Transport/new infra | Regulation review and signal-condition assessment | Road-test filing & safety plan | Punctuality ≥90%, zero tolerance for accidents | District test office + bus group [data:geometry/roads.geojson#ROAD-008] | Quasi-public | Medium-high (vehicle-road-cooperation equipment input) | Safety (R-01) | Road-test filing (intelligent-connected-vehicle road testing) | P2 test |
| JZ-07 | Tsinghua-East-Road education-belt stitching | Renewal/education | Campus-boundary and pedestrian-safety survey | Ownership & campus consent | ≥3 slow-traffic safety upgrades | Subdistrict + university [data:geometry/land_use.geojson#LU-001] | Public | Low-medium (slow-traffic upgrades in phases) | Privacy (campus data) | Campus ownership consent | P1 declare |
| JZ-08 | Southern renewal band upgrade | Renewal/residential | Existing-building and land survey | Retain-renovate-demolish special study | ≥6 renewal projects in pipeline, ≥2 public-participation sessions | District + planning team [data:geometry/phasing.geojson#PHASE-003] | Public | Medium (survey & public participation first) | Economics/ecology | Retain-renovate-demolish special study (renewal-unit procedure) | P1 declare |
| JZ-09 | Low-altitude delivery route verification | New infra/industry test | Airspace and safety-supervision review | Airspace approval | ≥90 test days, zero tolerance for accidents | District + regulator [data:geometry/constraints.geojson#CONSTRAINTS-01] | Commercial/quasi-public | Medium (test facilities & regulatory support) | Safety (R-01) | Airspace approval (low-altitude flight operations) | P2 test |
| JZ-10 | Edge-computing & energy-control demo building | New infra/utilities | Energy and computing-demand assessment | Fire safety & operator confirmation | Energy use down ≥15%, PUE under control | Energy enterprise + park [data:geometry/buildings.geojson#BLDG-001] | Commercial | Medium-high (computing-equipment input) | Economics/ecology | Fire safety & energy assessment | P2 test |
| JZ-11 | Barrier-free AI wayfinding system | Public service/accessibility | Standards and data-authorization review | Accessibility-standard re-check | ≥30 coverage points, 100% manual backup | Disabled federation + operator [data:geometry/constraints.geojson#CONSTRAINTS-01] | Public | Low (software-hardware combo, deployable in batches) | Privacy | Accessibility-standard re-check | P1 declare |
| JZ-12 | Global AI Week public route | Operations/brand | Event permits and copyright clearance | Public-space permit & safety plan | ≥12 international events/year, ≥40 overseas teams | Joint operating body [data:geometry/phasing.geojson#PHASE-001] | Quasi-public | Low-medium (event operations cost) | Heritage/economics | Public-space permit & event filing | P2 test |
...
| --- | --- | --- | --- | --- |
| JZ-01 Central Pulse Belt greenway | ~3.2 km slow-traffic retrofit along the greenway ([data:geometry/green_space.geojson#GREEN-001] alignment) | 900–2,500 CNY per linear meter (public greenway retrofit price) | 29–80 million CNY | 30–80 |
| JZ-02 5th Ring crossing node | 1 overpass bridge plus ~0.6 km approach links ([data:geometry/roads.geojson#ROAD-001]) | 8,000–25,000 CNY per linear meter (public crossing-structure price) | 48–150 million CNY | 50–150 |
| JZ-03 Zhongguancun Garden gateway plaza and AI Light Tower | Plaza ~1.21 ha ([data:geometry/public_space.geojson#PUBLIC-003] re-computed) | 1,650–5,000 CNY per m² (public plaza + structure price) | 20–60 million CNY | 20–60 |
| JZ-04 Origin release hall and code wall | ~8,000 m² ground-floor retrofit ([data:geometry/buildings.geojson#BLDG-001] concept) | 1,250–3,750 CNY per m² (public existing-building retrofit price) | 10–30 million CNY | 10–30 |
| JZ-05 Dazhongsi four-quadrant walkability | ~1.44 ha four-quadrant retrofit ([data:geometry/public_space.geojson#PUBLIC-001] re-computed) | 700–2,100 CNY per m² (public ground connectivity price) | 10–30 million CNY | 10–30 |
| JZ-06 Pulse Boulevard shuttle demo | ~2 km demo segment ([data:geometry/roads.geojson#ROAD-008] alignment) | 25,000–50,000 CNY per linear meter (public vehicle-road-cooperation demo price) | 50–100 million CNY | 50–100 |
| JZ-07 Tsinghua East Road education belt | ~3 slow-traffic retrofit points ([data:geometry/land_use.geojson#LU-001] alignment) | 6.7–16.7 million CNY per point (public node-retrofit price) | 20–50 million CNY | 20–50 |
| JZ-08 Southern belt upgrade | ~0.8 km² existing-building survey ([data:geometry/phasing.geojson#PHASE-003] re-computed) | 100–250 CNY per m² (public renewal-unit early assessment + light retrofit price) | 80–200 million CNY | 80–200 |
| JZ-09 Low-altitude delivery validation field | ~2 ha test site ([data:geometry/constraints.geojson#CONSTRAINTS-01] vicinity) | 15–30 million CNY per ha (public test-facility price) | 30–60 million CNY | 30–60 |
| JZ-10 Edge-computing and energy-control demo building | ~4.13 ha building cluster ([data:geometry/buildings.geojson#BLDG-001] re-computed) | 1,200–2,900 CNY per m² (public building-smartification price) | 50–120 million CNY | 50–120 |
| JZ-11 Barrier-free AI wayfinding | ~30 wayfinding points ([data:geometry/constraints.geojson#CONSTRAINTS-01] nodes) | 0.33–1.0 million CNY per point (public accessible-information station price) | 10–30 million CNY | 10–30 |
| JZ-12 Global AI Week public route | ~12 events/year (activity-system table in this chapter) | 0.4–1.25 million CNY per event (public large-event price) | 5–15 million CNY/year | 5–15 |
...
| Days 15–30 | Close review feedback: revise chapters and metrics per comments | Proposal team | Revision (next iteration, currently v10.12.0) | 6–10 person-days |
