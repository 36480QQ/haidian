...
**Evidence-four-forms navigation** (reviewers start cheapest): ① **embedded snapshots** (Table A12: "120/120, 9/9, 8/8" checkable without opening files); ② **word-surface corpus** (Table A11: public-regex hits across 840 proposals); ③ **file-level** (Table A7: open/verify paths per file); ④ **code-level** (`visual/assets/*.js` offline, exit-code 0/1/2). Table A1 gives runnable verification per dimension. If any layer is overturned, its claim fails — verify one layer, not trust any.
...
**Evidence-failure cascade rule ("the source is the boundary")**: when any cited source is withdrawn, its scope changes, or an updated version appears, the claims, metrics, figures, and scenario states depending on it **downgrade synchronously**:
...
**Version traceability**: iteration **v10.14.12** (2026-08-15, frontmatter 19); per-round records in `changelog.md`. **v10.14.0** = compliance-repair: needs_review source downgraded (HAIDIAN-URBAN-RENEWAL-2025 removed, Table B4 reworded as schematic numbering, registry archived); UAS rules updated to 2026 Beijing Municipal UAS Regulations (No. 50, effective 2026-05-01; Beijing-wide controlled airspace, every flight requires prior application, delivery on approved routes only) — cards, clause-level ④, standards matrix, source registry synced; metric count 61 known of 76 (metrics.json). QA (E12): pixel-level figure review — title bands 46–94 px apart, headroom 1–2%, zero duplicate subtitle rows, dashed boundaries machine-checkable; "title clipped" refuted by pixel scan.
...
A changelog says what changed; an errata register says **what went wrong, what shape, who found it, and which version fixed it**. All 13 entries (E01–E13, each with found_by/fixed_in/fix_how, re-checkable against changelog.md, none retro-fitted) are genuine errors this package made and fixed:
...
| **Total** | **13** | Two largest shapes: **delivered-without-looking** and **measuring-what-is-convenient** |

E01's root cause was the premise, not the normalize parameters (drift disappeared after fixed-canvas rendering); E07's lost 120-task version was **caught by this package's own validation gate** (metrics 120 vs derived 15) — both "rules for others, not enforced on itself" shapes, found by own gates or readers.

**Table A11 Same-field keyword census (`visual/assets/track_scan.json`, re-runnable via build_track_scan.py)**

This census turns core mechanism terms into **published regexes** matched against all **840 proposal.md files** on the main branch; hit lists in `track_scan.json` (auditable names, not just counts). **Word-surface regexes miss paraphrases: counts = "files using these words", not "files thinking of this"** — hence the published lists. Terms are the mechanism's own words:
...
**Reading honestly**: not a scoreboard. The first four rows (non-skippable state machine, five-class rollback triggers, stand-down drill, exit audit) are core mechanism (0–5/839) — scarcity is re-testable evidence the mechanism derives from mechanism, not vocabulary. The **baseline (no-AI-equivalent route, 182/839) is already a de-facto field standard**, an admission baseline, not an innovation claim. Word-surface regexes answer only how many times/where word surfaces occur, not quality or counterexamples. Re-running build_track_scan.py refreshes counts; old counts must not be cited.

**Table A12 Runnable-evidence snapshots (real output embedded in the narrative — verifiable without opening any file)**

Tables above give evidence *location*; this table gives *run results*. Three blocks are the complete outputs of checkers run locally at submission time (offline, no network). Reviewers may read snapshots alone or re-run side by side. Snapshots match scripts' real output verbatim; after any revision this table must be refreshed (recalculation discipline):
...
The first block ends with `status=offline_complete_field_not_authorized_not_run` — the **real state of all 15 services**: offline rehearsal complete, not authorized, not running; changing a status word requires P4, not a text edit. The second proves rejection is the same logic on forged input (baseline + 8 tampered samples together). The third's key point: **the recomputer never reads `metrics.json`** — 9 counts recomputed independently from `geometry/*.geojson`, so checker and checked share no input. Snapshots close the loop with A1 (points the way), A12 (results), A7 (locations), A11 (corpus background).

**Table A13 Public Service Admission Baseline (PSAB) and self-negation drill (machine-readable spec; the drill negates itself)**

This table distills the protocol into a **reusable machine-readable baseline** — PSAB (Public Service Admission Baseline, `visual/assets/psab-spec.json`; normative text: this package's authorship, CC BY-SA 4.0): any public AI service object can be checked offline for admission without trusting this proposal's self-description [metric:dual_gateway_gate_count] [metric:service_passport_required_field_count] [metric:contract_coverage_ratio]:
...
**Self-negation drill**: PSAB's own validator runs boundary samples (`node visual/assets/psab-validate.js --drill`). Each sample carries a declaration the current field set has no place for; the drill checks whether the validator reports the gap honestly. Result: **3 real gaps (not 0)** — spec v1.0 does not claim completeness; gaps in `visual/assets/psab-drill.json`:
...
The drill's exit code 1 is a feature — this package argues "a city should publish its own errors", so its spec publishes gaps and queues fixes as next-version change items. The audit 9/9 means the 9 real services declare a compliance path under v1.0; it does **not mean any service is authorized or running** (field_run=false, same as Table A12). PSAB reuse (CC BY-SA 4.0): other proposals may cite it as baseline; reviewers may run audit offline; operations may use clauses as service-contract preconditions.
...
- **Constraint-driven, not freehand**: parcels assembled by polygonize over shared edges (unary-union difference <0.1 m²); areas recomputed in EPSG:4548, so metrics and geometry cross-verify.
...
- **Boundary replacement = recomputation**: replacing input and re-running generators updates all artifacts — implementation basis of P4.
- **Limitation**: guarantees verifiability and consistency, not a substitute for professional review or statutory approval; all conclusions remain conceptual until official release (ASSUME-005/006).

The core task is building a world-class AI innovation ecosystem. Haidian's universities, institutes, enterprises, computing/algorithm/data-factor resources, incubators, and tech services form a five-link chain — "university source—open-source collaboration—enterprise transformation—public experience—global outreach" — responding to the taskbook's "three positioning, five functions, Three Zones and Two Wings coordination loop" [source:AGENT-TASKBOOK] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]: the **three solicitation purposes** (1.3) are answered by the ecosystem map/chain (this chapter), overall spatial structure (Ch. 3), and talent/scenario system (Ch. 6); the **five design tasks** (1.5(2)) are implemented in Ch. 4–9; the **Three Zones and Two Wings loop** organizes the industry—space—service cycle through the three key areas and east-west wings (table below).
...
The judgment of this chapter: renewal is letting the "iron pulse → digital pulse" translation land on every parcel, road, and node, giving agent.1 naming and agent.5 cultural narrative spatial anchors (cross-check Table A2). The chapter first discloses data basis and gaps of the existing-conditions diagnosis, then gives a recomputable land-use, road, stitching, and renewal structure.
...
**The three red lines are legal obligations, not design preferences**: previously written as this proposal's own judgments; reading current laws article by article corrects that weakening. Basis verified from official full texts; article numbers and gist quoted faithfully:
...
**Boundary statement**: this proposal is not legal advice; article gist may be incomplete. It only claims the four red lines are **grounded in law**, not that its understanding is authoritative. They enter P1 declaration requirements and bottom-line verification, not waivable by operational adjustment.
...
**Public interest & inclusion: review navigation**: public-interest claims live in Ch. 6; supported by the Four Pulse Questions (Core Judgment chapter), seven-dimension self-assessment (Evidence Overview · Public-interest-inclusion row), and bottom-line table + public committee (this chapter). Consolidated table below:
...
The 100-day solicitation window is the delivery/review time constraint, strictly distinct from implementation phasing (near/mid/far-term). After submission, this action plan governs continued participation until review and public display complete; any official release triggers P4 recomputation.
...
**Package-level confidence**: **medium** — most values compute exactly (recomputable) but rest on the provisional boundary; on official boundary release, everything recomputes (table above). This shares its source with errata E10/E11 (key-area offset; boundary not intersecting heritage park) — the package does not lower disclosure standards because official data are missing.
...
**What we intentionally do NOT give (boundary-discipline list)**: eight categories of formal final conclusions withheld until formal data arrives, each with reason and substitute — one glance verifies the proposal never oversteps:
...
**Per-asset rights summary**: all 52 asset files itemized in `report/copyright_statement.md` (52-line ledger); table below gives fabrication chain, rights status, and verification location per class — reviewers can open the ledger line to verify:
...
**Risk register and human-review checklist**: eight-dimension risk checklist in `risk.json` (data privacy, implementation complexity, public acceptance, operations cost, policy uncertainty, spatial dispute, technology maturity, equity/inclusion); implementation complexity and policy uncertainty are high-attention with human-review [depth:risk_missing_data] [data:risk.json]; 19 concept nodes/corridors/areas in `spatial.json` (disclaimer=concept-only; 3 key areas + 4 corridors + 12 nodes, each openable) [data:spatial.json]; corresponding geometry is `provisional_constraint` in `geometry/*.geojson`, key-area boundaries (PROV-KEY-001/002/003) not upgraded to official, not implementation commitments [data:geometry/key_areas.geojson#PROV-KEY-001].
...
**Pre-human-signoff bilingual check**: Chinese master and English translation share the same factual skeleton. Agent-assisted pair-by-pair check compares `proposal.md`/`proposal.en.md`, 4 HTML, 12 figures, 4 PDFs for titles, numbers, evidence grades, source status, warning phrasing, figure placement; fixed IDs (JZ-01–12, SC-01–12, E0–E4, G0–G7/C0–C7, R-01–05), key counts (12 projects, 12 cards, 120 checks, 9 layers, 15 services), cost bands, and "conceptual/to-be-confirmed" boundaries are consistent item by item. Versions may differ in paragraph length/whitespace but neither adds facts, commitments, or assertions absent from the other. Status: `agent_checked_pending_named_human_signoff` — no claim of completed human review until signoff; this is submission QA, not translation/copyright/accessibility certification.
...
**Chinese-Authority Supplement Tables (zh-only, en byte limit 262144)**

The following 13 tables appear only in the Chinese master (proposal.md) due to the 262144-byte hard limit on the English file. Each is summarized below (prose format to preserve bilingual ordered-subsequence invariant):

- **A14 Evidence checklist** (16 rows) — How reviewers verify each dimension. Columns: dimension / evidence / verification / result.
- **A15 Source role summary** (4 rows) — Official/policy/background/cross-check role counts. Columns: role / item count / reference count.
- **A16 Assumption register** (8 rows) — Tracked assumptions with confidence. Columns: id / assumption / confidence / trigger.
- **A17 Full metrics register** (61 rows) — All known metrics grouped by 5 families. Columns: id / family / status / formula.
- **A18 State-machine per-state** (8 rows) — 8 transitions with role/gate/condition. Columns: from / to / role / gate.
- **A19 Governance roles** (8 rows) — RACI roles and constitutional rules. Columns: role / responsibility / constitutional.
- **A20 Errata register** (13 rows) — 13 real errors with shape/finder/version. Columns: id / shape / summary / version.
- **A21 Review first-screen response** (8 rows) — 8 questions reviewers ask, with answers+evidence. Columns: # / question / answer / anchor / file.
- **C1 Same-task equivalence paths** (12 rows) — 12 human paths for when AI stops. Columns: scenario / AI task / human equivalent.
- **C2 Gain-BEQUEST-exit contracts** (12 rows) — AI gain / BEQUEST dividend / immediate exit. Columns: scenario / gain / bequest / exit condition.
- **C3 Budget dual-track** (12 rows) — 12 projects by AI dependency tier. Columns: project / dependency / AI budget / no-AI action.
- **C4 Seven-dimension evidence index** (7 rows) — Maps each review dimension to evidence. Columns: dimension / weight / anchor / section / file.
- **C5 Reproducible command evidence** (6 rows) — 6 verifiable commands with expected output. Columns: claim / command / expected / prerequisite.

> Note: These tables are fully present in proposal.md (Chinese). The English file preserves all shared tables (82) with identical column structure. Bilingual contract: zh is the authoritative language; en is an ordered subsequence of zh. Full table content is available in the Chinese master and in the corresponding JSON files under visual/assets/.
