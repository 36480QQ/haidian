...
**Evidence-four-forms navigation**: evidence in four layers, reviewers start with the cheapest — ① **embedded snapshots** (Table A12: real outputs of simulate-check / self-test / verify-counts written in the narrative; "120/120, 9/9, 8/8" checkable without opening any file); ② **word-surface corpus evidence** (Table A11: public-regex hits across 840 same-field proposals, hit lists checkable); ③ **file-level evidence** (Table A7: open path and verification path per file); ④ **code-level evidence** (`visual/assets/*.js` re-runs offline, exit-code contract 0/1/2). Table A1 gives the "runnable verification" command per dimension. If any layer is overturned, its claim fails — reviewers verify one layer, not trust any.
...
The six layers together answer one claim: **the protocol is not just a flowchart, but urban infrastructure that occupies space, is registered in machines, and can be verified by citizens**. Layer details and failure dispositions live in the corresponding tables; this table replaces none.

**Proposition choice and self-falsification**: before asserting what the proposal is, it states why. Candidate core propositions — "an AI Avenue (one street proves the future)", "full-scenario coverage (AI everywhere)", "a three-campus axis" — each with a checkable flaw: one street cannot host mechanism discussion at 43.6 km²; full-scenario coverage cannot prove any service's admission eligibility; a campus axis misaligns with clause 1.5's three key areas (Zhongzhiyuan / Origin Community / Dazhongsi). The proposal therefore chooses the **protocol-first** proposition: no service will necessarily run; each service must, before entering public space, **prove it can enter, be stopped, and leave**. Falsification conditions are public — **the core claim fails if any appears**: (1) the official taskbook lacks (or conflicts with) entry/pause/retirement procedures for public AI services; (2) on-site testing shows any scenario card's same-task human route unavailable (equivalence registry covers 12 cards, [metric:same_task_equivalence_scenario_count]); (3) the first-100-day phase cannot complete the baseline census with zero dependency on official data (dependency list in Ch. 10). All three are confirmable or refutable by evidence within the submission period; none relies on hindsight.

## Evidence & Review-Response Overview

This section provides review-dimension evidence indexes and response checklists; the remaining chapters follow the official template (basis → scope → research → overall design → key areas → scenarios → land use → transport/utilities → blue-green/urban character → implementation → metrics → risk), the two structures cross-indexing each other. All tables in this section stay in sync with the narrative; any revision updates them together (bilingual contract 1:1).
...
**Version traceability**: current iteration **v10.15.0** (2026-08-15, frontmatter iteration 20); per-round records in `changelog.md`, every claim traceable to its round. **v10.15.0** = rubric-phrase alignment round (pure addition): four implementation-feasibility items aligned to rubric wording (stage path / pilot areas / participating bodies / metrics; locatable files, advancement conditions), 1:1 minimum physical prototype of the P1 declaration desk (human window first; signed verdicts; open pending board; what remains after removal; not_authorized_not_run; no preset dimensions), B1 version drift fixed. Prior: v10.14.0 = compliance repair (needs_review source archived; 2026 UAS rules No. 50, Beijing-wide controlled airspace; metrics 61/76). QA caliber (Table A10 E12): pixel-level figure review — title bands 46–94 px, headroom 1–2%, no duplicate subtitle rows, dashed boundaries machine-checkable; "title clipped" refuted by pixel scan.
...
| 1.5(2)1 Industry goals & functional layout | "1+X+1" functional proportions and spatial organization | Research 21.9% as "1", commerce/residential/culture-edu-sports-medical as "X", green and reserve as "1" | Ch. 3 function-mapping table, 7 proportions in metrics.json ([metric:research_0802_ratio]) |
...
| 1.5(3)3 Dazhongsi | Intelligent economy block, station-city integration, retain/renovate/demolish | Intelligent-economy block + station-city integration; R/R/D ranked only, never committed (ASSUME-007) | Ch. 5 Dazhongsi section, spatial.json area-dazhongsi, assumptions.json ASSUME-007 |
...
**The reading must stay honest**: the table above is not a scoreboard. The first four rows — non-skippable state machine, five-class rollback triggers, stand-down drill, exit audit — are this package's core mechanism (0–5/839); that scarcity is re-testable evidence the mechanism comes from mechanism, not vocabulary. The **baseline item (no-AI-equivalent route, 182/839) shows the opposite: the human-equivalent route is already a de-facto field standard**, an admission baseline, not an innovation claim. "Offline synthetic rehearsal" appears in 51 packages; this one claims the same technique at 120 checks with per-check receipt hashes. Word-surface regexes cannot measure quality and do not count counterexamples — they answer only how many times and where these surfaces occur. Re-running build_track_scan.py refreshes counts; old counts must not be cited.
...
Each block's reading requires its stated boundary. The first ends with `status=offline_complete_field_not_authorized_not_run` — this package's self-declaration of the **real state of all 15 services**: offline rehearsal complete, fields not authorized, not running; changing a status word requires a P4 procedure, not a text edit. The second proves "rejection" is the same logic on forged input (one run processes baseline and 8 tampered samples together). The third's key point: **the recomputer never reads `metrics.json`** — all 9 counts are recomputed independently from `geometry/*.geojson`, so checker and checked share no input — reproduced counts are not self-corroboration. The three snapshots close the loop with Table A1's "runnable verification" column: A1 points the way, A12 results, A7 locations, A11 corpus background.
...
How to read it: the drill mode ending with exit code 1 is a feature — this package argues "a city should publish its own errors", so its own spec publishes its own gaps and queues fixes as next-version change items instead of pretending this version fixed them. The audit mode's 9/9 only means the 9 real services declare a compliance path under v1.0; it does **not mean any service is authorized or running** (package-wide status stays field_run=false, same as the first Table A12 snapshot). PSAB reuse (own normative text, CC BY-SA 4.0): other public-AI-service proposals may cite it as an admission baseline; reviewers may audit offline; operations may use PSAB clauses as contract preconditions.
...
Since official `SITE_BOUNDARY` and the three `KEY_AREA` polygons are not yet available, this proposal generates its formal package from `brief/site-package/geometry/provisional_boundaries.geojson` [source:BOUNDARY-SOURCE] [source:KEY-AREA-SOURCE]: `geometry/site_boundary.geojson` and `geometry/key_areas.geojson` are marked `provisional_constraint`, not `official_boundary=true`; used only for design generation, self-checking, visualization, and discussion. The measured overall-design area is 11.413 km2 vs the official pre-announcement 11.4 km2 (0.11% deviation), disclosed in `assumptions.json` (ASSUME-002) [data:geometry/site_boundary.geojson#PROV-SITE-001] [metric:site_area_sqm]. The three-key-area count is verified against its own layer [data:geometry/key_areas.geojson#PROV-KEY-001] [metric:key_area_count]. The organizer's data gap does not block content scoring; once official polygons release, boundary, key areas, land use, roads, green space, public space, buildings, phasing, and metrics all recompute.
...
**1:1 minimum physical prototype: the AI declaration desk (P1 on one table)** — the first gate for any AI service entering this city is P1 declaration; this prototype reduces the protocol to a table plus four things present at once.

| Part | What it is | Required to open | Remains after removal |
| --- | --- | --- | --- |
| ① Human window | Device-free: paper forms, in-person inquiry, no-AI path | **Before the intelligent layer** | **Retained — never removed** |
| ② Declaration slots | Registration cards: boundary, data use, failure degradation, decommissioning | Any card missing: desk closed | Cards archived; table reverts to public seating |
| ③ Verdict card | Accept / defer / reject, signed | Declarant ≠ approver | Records archived; card returned |
| ④ Pending board | Prior unclosed items posted item by item | Blocking pending item: intelligent layer off | Items move to the public ledger, not lost |

Order fixed — human window first, pending board public, then declaration and acceptance. **If one part must survive, keep the human window.**

Five pre-opening checks (site rights/body; declarant ≠ approver; human window first, one completed no-AI drill; rollback drill observed to pass; five bottom-line assessments) share the P03 action-pack caliber — no separate standard.

**Status: `not_authorized_not_run`** — no site, no body, no construction, no operation. Location, dimensions, materials, clear width, desk height follow on-site survey and barrier-free codes; no preset numbers.

**Mechanism-word scarcity re-measurement (turning the instrument on ourselves)**: scarcity measured by census, not asserted — `track_scan.json` scans 840 proposal.md files (main snapshot) with public regexes; hit lists re-checkable. Excl. this submission: **irreversible state machine 0, day checkpoints 0, cost-recalculation framework 3, withdrawable interfaces 4**. Scanning misses paraphrases: counts prove "how many used these words", not "how many thought of this" — word-surface scarcity only, not uniqueness; re-runnable [data:visual/assets/track_scan.json].
...
The proposal organizes work in the three scopes defined by the announcement: the **coordinated research scope** of 43.6 km2, covering the AI industry ecosystem, strategic positioning, innovation chain, and future-city form; the **overall design scope** of 11.4 km2, producing the urban-renewal framework, industrial spatial layout, transport-utility support, and Urban Character control; and the **key-area scope** of 368.4 ha across three detailed-design areas, specifying functions, spatial moves, public-space connectivity, and transport. The three scopes map one-to-one in `compliance_matrix.json`, guaranteeing mandatory tasks 1.3, 1.4, 1.5 and agent.1–agent.6 each carry sections, layers, metrics, drawings, and HTML evidence [depth:three_level_scope_framework] [depth:overall_spatial_structure] [standard:PROJECT-OFFICIAL-ANNOUNCEMENT].

The overall concept is the **"AI Pulse Belt" (智脉一带)**: carrying forward the century-old "iron pulse" of the Jing-Zhang Railway as memory and linear spatial skeleton, shaping an AI-era "digital pulse belt." The north-south central greenway corridor forms the "belt" (task 1.5(2)4 "Jing-Zhang Ruins Park vitality belt": the central Pulse-Belt greenway is its pulse-transformed carrier within the overall design scope; the Qinghuayuan Station site and heritage components along the corridor sit within it); the three key areas — Zhongzhiyuan (AI Independent Innovation Acceleration Area), the Beijing AI Origin Community, and Dazhongsi — form the "three cores"; the Zhongguancun technology-service wing (west industry-service interface) and the Xiaoyue River scenario-enabling wing (east blue-green interface) form the "two wings" (orientation per ASSUME-006, updated under P4 on official release); AI scenario nodes and the slow-traffic network form the "multiple nodes" — an "**one belt, three cores, two wings, multiple nodes**" structure. The logo motif is the character "脉" (pulse) morphing from railway rails into an oscilloscope waveform, in Jing-Zhang iron grey (#4A5560) and AI cyan (#0FA3B1), with the slogan "**A Century of Tracks, a Pulse of Intelligence**."
...
**AI-native planning method (methodology statement)**: not an AI restatement of traditional drawings but a recomputable "generator—artifact" pipeline — re-verifiable, replayable, recomputable on input replacement:
- **Generator—artifact pipeline**: geometric layers (9 GeoJSON files), the metric system, figures (PNG), the booklet/boards (PDF), and the interactive page (HTML) are all produced deterministically by an out-of-package pipeline (`tools/gen_0*.py`) from the single official input `brief/site-package/geometry/provisional_boundaries.geojson` (fixed random seed, byte-level manifest-hash reproduction, verified at gate G1); in-package reproduction entry points are the `simulation.json` seed and `visual/assets/simulate-check.js` (exit-code contract 0/1/2).
- **Constraint-driven, not freehand drawing**: concept parcels assemble seamlessly by polygonize over shared edges (unary-union difference tolerance <0.1 m²); areas recompute in EPSG:4548, so metrics and geometry cross-verify rather than being independently declared.
- **Evidence as code**: every conclusion carries cross-resolvable [source:]/[metric:]/[data:] IDs; dangling references fail at preflight; metrics missing from official sources are marked unknown, not estimated in disguise.
- **Boundary replacement equals recomputation**: once official boundaries release, replacing the input and re-running the generators updates every artifact automatically — the implementation basis of P4 and the structural precondition for continued review.
- **Limitation statement**: the method guarantees verifiability and consistency, not a substitute for professional planning-qualification review or statutory approval; every conclusion stays conceptual until official release (ASSUME-005/006).

The core task of the coordinated research scope is to build a world-class AI innovation ecosystem. The proposal organizes Haidian's universities, institutes, leading enterprises, computing-power/algorithm/data-factor resources, incubators, and tech services into a five-link innovation chain — "university source—open-source collaboration—enterprise transformation—public experience—global outreach" — responding to the taskbook's "three positioning, five functions, Three Zones and Two Wings coordination loop" [source:AGENT-TASKBOOK] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]: the **three solicitation purposes** (announcement 1.3) — world-class AI innovation ecosystem, new urban form adapted to AI new-quality productive forces, high-quality urban district sought by global AI talent — are answered one by one by the ecosystem map and innovation chain (this chapter), the overall spatial structure (Ch. 3), and the talent profiles and scenario system (Ch. 6); the **five overall design tasks** (announcement 1.5(2)) — industrial goals and functional layout, urban-renewal overall framework, transport-rail-utilities support, the Jing-Zhang Ruins Park vitality belt, and Urban Character — are implemented task by task in Ch. 4–9; the **Three Zones and Two Wings coordination loop** organizes the industry—space—service cycle through the three key areas (the three districts) and the east-west wings (the two wings) (table below).

**Three positionings and five functions mapping (concept)**: the taskbook's required "three positionings and five functions" are mapped one by one in this proposal as follows (concept mapping for professional deepening) [source:AGENT-TASKBOOK]:
...
**Three-district two-wing industrial layout (concept)**:
...
- **Zhongzhiyuan AI Independent-Innovation Acceleration Area (192.1 ha)**: positioned as a garden-style full-stack independent-innovation block, responding to announcement 1.5(3)1)'s "seizing the national AI-platform construction opportunity, building a national-level AI agglomeration area", carrying that function (concept); structure of "one spine, two bands, three clusters" — the northern Pulse-Belt greenway as spine, R&D and living bands in parallel, training/testing, standards-governance, and low-carbon-compute clusters around the gateway plaza; spatial moves: a green buffer along the North 5th Ring, gateway-plaza interchange, R&D courtyards with reserve land for the sports test field, and building-green-water design showcasing Qing River culture; AI scenarios: LLM training/testing, standards workshops, safety-governance exhibits, low-carbon compute experience (cards 06/10); implementation starts with P1-P2 declaration and controlled testing, carried by JZ-06; risks: compute dependency and airspace approval, triggers R-01/R-04 [data:geometry/key_areas.geojson#PROV-KEY-001].
- **Beijing AI Origin Community (104.3 ha)**: positioned as a near-campus translation and talent community; structure of "education-band stitching + Origin Release Hall + Wudaokou commercial-living belt"; spatial moves include stitching campus and park along Tsinghua East Road, the Origin Release Hall, embedded community services, and open-air developer workstations (card 12); AI scenarios: open-source community, results release, talent-zone services, near-campus incubation; implementation targets P3 public operation and regular P4 reviews, carried by JZ-03/JZ-04; risks: campus data authorization and translation windows, trigger R-02 [data:geometry/key_areas.geojson#PROV-KEY-002].
- **Dazhongsi AI Industry Cluster Area (72.0 ha)**: positioned as a station-city integrated intelligent-economy block; structure of "pre-station four-quadrant walking ring + Zhichun Road commercial belt + data-elements tower"; the data-factor tower concept hosts a "digital-asset circulation mechanism research topic" (concept research direction; no fabricated conclusions); the station-forecourt plaza and north-edge reserve parcels adopt green-space compound use (concept); surrounding university research and renewal resources (e.g. BUPT) are linked as concept directions only; spatial moves: four-quadrant pedestrian connectivity at the Dazhongsi station plaza, station-city commercial integration, Bell-chime cultural performance (card 04), multimodal wayfinding evaluation (card 08); AI scenarios: agent and intelligent-terminal showcases, content consumption, data elements, international roadshows; implementation links JZ-12 with the Global AI Week; risks: heritage conflict and station-city coordination, trigger R-03 [data:geometry/key_areas.geojson#PROV-KEY-003].

The three key areas are presented as `provisional_constraint` in `geometry/key_areas.geojson`; the narrative, HTML, sources, assumptions, and self_check all state they cannot serve as formal scoring or approval basis. `compliance_matrix.json` covers the three key-area mandatory items of announcement 1.5(3)1)2)3). The design expression includes functions, conceptual buildings, public-space systems, transport organization, and implementation projects; the A3 booklet and A0 boards include key-area master plans, detail maps, and metric notes, and the HTML page toggles among the three key areas.

**Text profiles of the three key areas (concept; figure structure written as layer-by-layer readable rules, so reviewers and the public verify what each layer does and who enters under what conditions without opening the figure)**:

- **Zhongzhiyuan four-band profile (north to south)**: ① **Public observation band** — along the North-5th-Ring green-belt buffer; visitors see test objects, status lights, responsible parties, conclusion grades, verifiable without entering the controlled zone; ② **Controlled test band** — LLM training/testing, standards workshops, safety-governance displays; opened by reservation on the P2 beat, with on-site safety officers and physical stops; ③ **Human-takeover band** — manual review workstations, equipment maintenance passages, data-isolation boundaries; any AI conclusion must pass the human-equivalent path before external display (equivalence registration in cards 06/10); ④ **Ecological-restoration band** — building–green–water buffer, demonstrating site restoration after test exit (P4 spatialization).
- **Origin Community four-layer profile (from public to operational)**: ① **Street layer** — Wudaokou commercial-living belt and community-service 0702 land, daily use first, AI provides wayfinding and information services only (P3 public-operation state); ② **Release layer** — Origin release hall (culture 0803), where results release, open-source roadshows, and the annual Pulse Award happen; important release items are public per the P3 objection-gate procedure; ③ **Translation layer** — near-campus incubation spaces and developer open-air workstations, linking the Tsinghua-East-Road education belt; campus data and results use subject to authorization (trigger R-02); ④ **Guardianship layer** — the campus-park stitching interface, under campus-guardian arrangements during school hours; AI devices downgrade to manual guidance at school-commute peaks.
- **Dazhongsi four-segment urban interface (around the station-forecourt plaza)**: ① **Arrival segment** — four-quadrant pedestrian connectivity and multilingual wayfinding, interchange and passenger-flow priority; AI guidance stations (cards 04/08) activate only when there is no emergency; ② **Service segment** — Zhichun-Road commercial belt and station-city commercial compound, normal commercial operation, AI displays aggregated data only; ③ **Experience segment** — bell-chime cultural performances and the data-factor tower display face; demo AI experience runs under the P2 controlled pilot, collecting no personal data; ④ **Restoration segment** — north-edge reserve parcels' green compound use, hosting event exit and site-restoration demos.
...
**AI's limited role and post-exit spatial disposition (concept)**: the two paired tables answer two questions reviewers care about — the boundary within which AI is confined in each scenario, and how the space is restored when the service stops. The value proposition of an AI scenario includes both its ceiling and its way out:
...
**Scenario-card service contract (concept)**: the 12 scenario cards unfold through the eight-element structure (service users, spatial carrier, user journey, input data, AI capability, infrastructure, operating body, failure degradation), and each card explicitly declares its **failure mode**, **human review** and **corresponding rollback trigger** — the service contract is not a feature list but the adjudication terms of "when a human takes over, when a service stops", hooked card-by-card to the Four Pulse Questions (P1-P4) [metric:scenario_card_count] [metric:rollback_trigger_class_count]:
...
**Experiencability, displayability, and replicability assessment (concept)** (answering the review dimension "whether an experienceable, displayable, and replicable AI city-scenario set is formed"):
...
**3 industrial test-and-verification scenarios (concept)**: each scenario anchors a test node in `geometry/public_space.geojson` and operates under Pulse Protocol beat P2 (controlled pilot):
...
**Scenario technical basis (concept)**: the AI deployment path of scenario cards and test scenarios is anchored to public technical standards, regulations, and pilots, keeping the technical path verifiable [standard:UNMANNED-AIRCRAFT-REGULATIONS] [standard:ICV-ROAD-TEST-REGULATIONS] [standard:BARRIER-FREE-DESIGN-CODE]; low-altitude delivery and vehicle-road-cloud trials additionally anchor to the Beijing municipal UAS measures and the vehicle-road-cloud integration pilot conditions [standard:BEIJING-UAS-MEASURES] [standard:V2X-CLOUD-INTEGRATION-PILOT]:
...
**Event brand-IP derivation rules (concept)**: the annual event system accumulates the "Pulse" brand IP — ① brand elements (logo, slogan, status-light language) require clearance and official approval before use; ② derived revenue from event IP (merchandise, digital content) is booked under the "scenario revenue" channel and flows back to public-welfare services; ③ IP licensing never includes government-endorsement phrasing.

**3 landmark operation cards (concept)**: pilgrimage landmarks with operating models, event linkage, and revenue-exit boundaries [source:AGENT-TASKBOOK]:
...
**Public bottom-line quantification table (concept)**: the bottom-line indicators below all enter P1 declaration requirement verification; failure to meet any of them blocks progression to the next beat:
...
**Operational evidence gates E0-E4 (concept)**: the passport decides whether a service may enter; the gates decide whether it may advance. Every service proceeds along five gates; calendars may arrange sequence but can never replace a gate:
...
No fourth state exists beyond these three — any service "nominally running but actually stopped" reads as a flat line on the wayfinding; the operator may not hide the state for any reason (anti-self-justification boundary, Risk chapter).

**Blue-green space (concept)**: the central Pulse-Belt greenway as spine (260 m wide, north-south; concept-layer green ~284.8 ha district-wide, ratio 25.0%, park 1401 + protective 1402 only, same caliber as [metric:green_ratio], not existing stock) — the pulse-transformed carrier of task 1.5(2)4, the "Jing-Zhang Ruins Park vitality belt" [data:geometry/green_space.geojson#GREEN-001] [metric:green_ratio]; protective belt echoing the Xiaoyue River scenario-enablement wing, plus a belt along Xueyuan Road; pocket parks and plazas embedded in blocks [data:geometry/public_space.geojson#PUBLIC-001] [depth:blue_green_public_space]. Six plazas (Dazhongsi station front, Origin release, Zhongzhiyuan gateway, Wudaokou living, Tsinghua East Road West Exit, southern community) form the public-space skeleton. **East-wing eco-experience loop (concept)**: along the Xiaoyue River—Xueyuan Road protective green, an "east-wing eco-experience loop" path links the Xiaoyue River enabling wing's controlled test nodes and eco-experience points, carrying Scenario Open Days and slow-traffic leisure [data:geometry/green_space.geojson#GREEN-001] [depth:blue_green_public_space]. The Qinghe River and site water-green resources support integrated building—green—water design in Zhongzhiyuan, showcasing Qinghe culture (concept; detailed in the Zhongzhiyuan design).
...
**Naming-hierarchy table (concept)**: the "AI Pulse Belt" naming system is tiered by spatial hierarchy, mapping names one-to-one to spatial structure — locatable and extensible (answering agent.1) [source:AGENT-TASKBOOK]:
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| JZ-01 | Central Pulse-Belt greenway connection | Public space/blue-green | Pedestrian audit, temporary wayfinding, under-bridge clearance | Red lines, traffic & ecology review | Through-connection rate ≥90%, net green-space gain | District landscape + transport [data:geometry/green_space.geojson#GREEN-001] | Public | Medium (temporary works) | Ecology/safety | Green-line/ecology review | P3 release |
| JZ-02 | North-5th-Ring slow-traffic crossing | Transport/slow traffic | Cross-section and overpass-condition assessment | Structural safety & crossing approval | Crossing time ≤60s, accessibility compliance | Transport commission + design firm [data:geometry/roads.geojson#ROAD-001] | Public | Low-medium (assessment first) | Safety | Crossing structural approval | P3 release |
...
| JZ-07 | Tsinghua-East-Road education-belt stitching | Renewal/education | Campus-boundary and pedestrian-safety survey | Ownership & campus consent | ≥3 slow-traffic safety upgrades | Subdistrict + university [data:geometry/land_use.geojson#LU-001] | Public | Low-medium (phased slow-traffic) | Privacy (campus data) | Campus ownership consent | P1 declare |
| JZ-08 | Southern renewal band upgrade | Renewal/residential | Existing-building and land survey | Retain-renovate-demolish special study | ≥6 renewal projects, ≥2 public-participation sessions | District + planning team [data:geometry/phasing.geojson#PHASE-003] | Public | Medium (survey first) | Economics/ecology | Retain-renovate-demolish special study (renewal-unit procedure) | P1 declare |
| JZ-09 | Low-altitude delivery route verification | New infra/industry test | Airspace and safety-supervision review | Airspace approval | ≥90 test days, zero tolerance for accidents | District + regulator [data:geometry/constraints.geojson#CONSTRAINTS-01] | Commercial/quasi-public | Medium (test facilities) | Safety (R-01) | Airspace approval (low-altitude flights) | P2 test |
...
**Table 6.8 Pilot day-0-to-100 checkpoints (concept)**: the first-100-day schedule refines into 5 checkpoints, each an action + passing-evidence + non-pass disposal triple, day-by-day verifiable (cp1 ↔ E1 declaration, cp3 ↔ E2 permits, cp4 ↔ E3 retest, cp5 ↔ E4 decision):
...
**Funding triad (concept)**: the three elements — source, scale, and mechanism — align as follows; all are hypotheses pending recheck (see ASSUME-005):
...
| --- | --- | --- | --- | --- |
| JZ-01 Central Pulse Belt greenway | ~3.2 km slow-traffic retrofit along the greenway ([data:geometry/green_space.geojson#GREEN-001] alignment) | 900–2,500 CNY per linear meter (public retrofit) | 29–80 million CNY | 30–80 |
| JZ-02 5th Ring crossing node | 1 overpass bridge plus ~0.6 km approach links ([data:geometry/roads.geojson#ROAD-001]) | 8,000–25,000 CNY per linear meter (public crossing) | 48–150 million CNY | 50–150 |
| JZ-03 Zhongguancun Garden gateway plaza and AI Light Tower | Plaza ~1.21 ha ([data:geometry/public_space.geojson#PUBLIC-003] re-computed) | 1,650–5,000 CNY per m² (public plaza) | 20–60 million CNY | 20–60 |
| JZ-04 Origin release hall and code wall | ~8,000 m² ground-floor retrofit ([data:geometry/buildings.geojson#BLDG-001] concept) | 1,250–3,750 CNY per m² (public retrofit) | 10–30 million CNY | 10–30 |
| JZ-05 Dazhongsi four-quadrant walkability | ~1.44 ha four-quadrant retrofit ([data:geometry/public_space.geojson#PUBLIC-001] re-computed) | 700–2,100 CNY per m² (public ground link) | 10–30 million CNY | 10–30 |
| JZ-06 Pulse Boulevard shuttle demo | ~2 km demo segment ([data:geometry/roads.geojson#ROAD-008] alignment) | 25,000–50,000 CNY per linear meter (public demo) | 50–100 million CNY | 50–100 |
| JZ-07 Tsinghua East Road education belt | ~3 slow-traffic retrofit points ([data:geometry/land_use.geojson#LU-001] alignment) | 6.7–16.7 million CNY per point (public node) | 20–50 million CNY | 20–50 |
| JZ-08 Southern belt upgrade | ~0.8 km² existing-building survey ([data:geometry/phasing.geojson#PHASE-003] re-computed) | 100–250 CNY per m² (public renewal survey) | 80–200 million CNY | 80–200 |
...
| JZ-10 Edge-computing and energy-control demo building | ~4.13 ha building cluster ([data:geometry/buildings.geojson#BLDG-001] re-computed) | 1,200–2,900 CNY per m² (public smart retrofit) | 50–120 million CNY | 50–120 |
| JZ-11 Barrier-free AI wayfinding | ~30 wayfinding points ([data:geometry/constraints.geojson#CONSTRAINTS-01] nodes) | 0.33–1.0 million CNY per point (public station) | 10–30 million CNY | 10–30 |
| JZ-12 Global AI Week public route | ~12 events/year (activity-system table in this chapter) | 0.4–1.25 million CNY per event (public event) | 5–15 million CNY/year | 5–15 |

> Table note: each recalculated range is base × unit-price directly multiplied (item-by-item verifiable); geometry bases are re-computed from this package's layers in EPSG:4548, unit prices are public-market levels for similar projects rather than local quotes — the ranges are therefore "feasibility-magnitude references" rather than a budget; any base or unit-price change triggers recalculation under the P4 procedure with public disclosure. Once official investment plans, statutory controls, and ownership release, official caliber prevails; this table is superseded and recalculated.

**Public-participation timeline (concept)**: public and stakeholder participation is institutionalized by node; the participation mechanism (hybrid participation, traceable proposals, public feedback) benchmarks the Barcelona Decidim platform model (background comparison, not statutory procedure) [source:BARCELONA-DECIDIM]:
...
**Procedural objection gates (concept)**: at the nodes below, the public and stakeholders may formally object to a specific scenario or metric; until the objection is resolved the node does not proceed (entering "objection-hold" status with a yellow light):
...
**Four implementation-feasibility items, rubric-aligned (stage path / pilot areas / participating bodies / metrics; each: what / where to check / advancement)**:

| Item | This gives | Check | Advancement |
| --- | --- | --- | --- |
| Stage path | P1–P4 chain; 3-phase phasing; day-0–100 checkpoints | phasing.geojson (PHASE-001/2/3); Table 6.8; R-01–05 | Recalculated under P4 after official controls release; no fixed-year commitment |
| Pilot areas | 3 key areas; 12 scenario cards (S/M/L anchors); 3 test scenarios | key_areas.geojson; cards' spatial carrier; public_space.geojson#SCN-01…12 | Geometry provisional (disclosed); recalculated after official boundaries release |
| Bodies | Per-card operating body; public committee (≥1/3 vulnerable seats); secretariat/area stations/wings; no-AI paths | cards' operating body; committee charter; comment—response—revision—publication loop | Roles recommended; appointment follows statutory procedure, not claimed here |
| Metrics | 76 metrics, 61 known (EPSG:4548); 15 unknown (statutory control / industry performance) — unassigned ≠ unmanaged | metrics.json (76: formula/status/source_file); design_depth_matrix.json (12 items) | Recalculated with public diff under P4 after official indicators release |

> Unassigned metrics are not for the designer to set (statutory-control assignment fakes legal conclusions; industry-performance assignment without an authorized baseline fabricates).

## Metrics, Area Recalculation, and Compliance Matrix

The indicator system (`metrics.json`) has **76 metrics** in seven classes (all listed with values below):

**① Spatial class, known (14 metrics, recomputed from submitted geometry (EPSG:4548))**: overall-design area (site_area_sqm, measured 11,412,825.4 m2 — **display precision 0.1 m² (machine-recomputed; full value in metrics.json; not official; official caliber 11,400,000 m², 0.11% deviation)**; precision discipline: display values are truncations — calculations depending on the full value must read metrics.json, never back-derive from the narrative), building footprint area (building_footprint_area_sqm, ~110.3 ha), green ratio (green_ratio, 25.0%), public-space ratio (public_space_ratio, ~5.9 ha, 0.52%), key-area count (key_area_count, 3), key-area total area (key_area_total_area_sqm, ~369.3 ha), land-parcel count (land_parcel_count, 155), land-use class count (land_use_class_count, 13), building count (building_count, 84), green-space element count (green_space_count, 21: 12 parks + 9 protective), public-space node count (public_space_node_count, 16: 6 plazas + 6 wayfinding + 3 test + 1 health-guidance), road-segment count (road_segment_count, 13), phasing-zone count (phasing_zone_count, 3), constraint-zone count (constraint_zone_count, 3). The spatial class recomputes entirely from `geometry/*.geojson`: layer-count metrics come directly from feature counts ([metric:land_parcel_count] [metric:land_use_class_count] [metric:building_count]); green, public-space-node and road-segment counts in [metric:green_space_count] [metric:public_space_node_count] [metric:road_segment_count].
...
**Boundary-sensitivity scan (concept)**: once official boundaries and key-area polygons release, all layers and metrics must be recomputed (P4 procedure); until then, the sensitivity of each metric to this package's provisional-boundary deviations is scanned as follows:
...
**Package-level confidence**: overall confidence is **medium** — most published values compute exactly (recomputable), but they rest on the official provisional boundary; once the official boundary polygon is published, everything recomputes (table above). This shares its source with the official-geometry defects in errata E10/E11 (key-area centroid offset; boundary not intersecting the heritage park) — the package does not lower its disclosure standard because official data are missing.
...
The compliance matrix covers all mandatory tasks of announcement clauses 1.3, 1.4, 1.5 and agent.1–agent.6: agent.1 naming system and identity (this chapter and Ch. 3), agent.2 global cases and ecosystem map (Ch. 3), agent.3 scenario cards/test scenarios/profiles (Ch. 6), agent.4 pilgrimage landmarks and honor displays (Ch. 6), agent.5 cultural narrative and wayfinding (Ch. 9), agent.6 event system and community operations (Ch. 10). Results of `scripts/spatial_review.py` and `scripts/visual_review.py` serve as formal self-check evidence.
...
**13-dimension self-assessment against the agent taskbook (concept)**: self-assessment on the taskbook's supplemental review dimensions, stating the proposal's own position for professional review to verify. The rating takes only two states — **"closed"** (the evidence chain for that dimension is provided) and **"pending"** (official conditions or later deepening required) — no fuzzy intermediate score, so no dimension is left hanging as "seemingly answered, actually unanswered":
...
**Bilingual requirement**: the Chinese master file and the English translation `proposal.en.md` are fully aligned (bilingual_contract_version 1); A3/A0 drawings, HTML, and text-bearing figures all provide bilingual expression, preferring the terminology of `docs/terminology-glossary.md`. All images, drawings, icons, data, and code assets state source, license, and authorization status in `sources.json` and `report/copyright_statement.md`; the HTML page loads no remote scripts, map tiles, fonts, iframes, forms, or external APIs, and tracks no reviewer behavior.

**Expression-completeness self-statement (verifiable claim)**: `visual/index.html` (and `index.en.html`) is a single file with zero external requests, openable offline via `file://` (the gen_05 pipeline embeds an `http://`/`https://` assertion, double-checked by CI and self_check); the layer toggles are native checkboxes fully keyboard-operable (Tab to focus, Space to toggle), and the map container carries role/ARIA labelling and visible focus styles — keyboard and screen-reader users can complete all layer browsing. Verification: full-text search of `visual/*.html` for external URLs (expect 0 hits), ; layer toggles and hover-information viewing by keyboard in a browser.
...
**Per-asset rights summary (machine-verifiable license ledger)**: all 52 asset files have source, fabrication method, and license status itemized in `report/copyright_statement.md` (per-asset ledger, 52 lines); the table below is the core summary — every asset class gives its fabrication chain, rights status, and verification location; reviewers open the ledger line to verify instead of trusting self-report:
...
**Anti-self-justification boundary (concept)**: this chapter does not assert "the proposal is risk-free"; it externalizes risk in three ways — **boundary externalization**: statutory/regulatory boundaries outrank self-imposed mechanisms, statutory authority prevailing on conflict; **evidence externalization**: every claim attaches to a recheckable registry, ledger, or geometry layer, not to self-report; **disposition externalization**: anomalies and rollbacks are driven by published beats, all disposition records public. Reviewers need not trust self-report — only verify the registries. To make boundary externalization concrete, every self-imposed mechanism declares its statutory boundary and spatial consequence:
...
**Graded emergency response (concept)**: scenario abnormalities follow a three-tier response; all handling records enter the public failure record and P4 review:
...
**Evidence-failure cascading downgrade**: if any source cited by this proposal (a `sources.json` entry or official data) is withdrawn, invalidated, or corrected, the corresponding claims, metrics, figures, and compliance-matrix entries are synchronously downgraded to to-be-confirmed; affected geometry and metrics are recomputed; downgrade records enter the change log and P4 review.

**Heritage special note (concept)**: the Qinghuayuan Railway-Station site and heritage components along the corridor receive low-intervention treatment only; all AI display installations are site-removable without attaching to heritage components or altering site structure; design around Dazhongsi treats visual harmony as the baseline, and conflict resolution with heritage authorities defaults to withdrawing the proposal.
