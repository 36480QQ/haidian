...
3. **更新类**（~25%）：低效用地、危旧房和临时建筑，可考虑拆除重建

具体到每个地块的拆改留结论需以现状建筑调研、权属核查和控规条件为依据，本方案仅提出方法框架和待校准清单，不编造拆除结论。

### 建筑规模与高度控制

在缺少官方控规条件的情况下，方案提出以下建议方向 [metric:building_footprint_area_sqm] [depth:development_intensity_controls]：

- 总体建筑基底面积约31万平方米（基于临时边界估算）
- 重点区域建议容积率：众智园1.5–2.0，AI原点社区2.0–2.5，大钟寺2.5–3.5
- 建筑高度建议：众智园≤45m（局部60m），AI原点社区≤60m（局部80m），大钟寺≤80m（局部100m）

**以上容积率和建筑高度均为概念建议方向，非审定控规指标。** 待官方控规条件发布后，必须重新校核建筑规模和强度指标 [depth:height_massing_character]。
...
- 绿地覆盖率（约12.3%，基于临时边界）[metric:green_ratio]
- 公共空间比例（约7.3%，基于临时边界）[metric:public_space_ratio]
- 建筑基底面积（约31.1万 m²，基于临时边界）[metric:building_footprint_area_sqm]
...
- **绿地覆盖率 12.3%**：支撑人才生活品质和创新交往空间，为AI人才提供绿色休闲环境
- **公共空间比例 7.3%**：支撑日常非正式交流、开发者社区活动和公共体验
