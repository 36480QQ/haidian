...
summary: "以D—T—S事件链×E0—E3证据等级，把Physical AI调试收进同一处、四逻辑点、六类人工去向的可撤回测量commissioning；三区分别提供三种服务，不组织跨站产品旅行。"
...
公共收益由五个带分母的读数核验：试点导致的普通/无障碍路径不可用分钟除以获批公共服务分钟；授权包络内完成动态运行分钟除以计划且获批分钟；试点直接维护工时除以QA通过的动态运行小时；有效反馈从收到到人工确认的中位数和最大时长；完成预检后进入换位、转专项或取消的签署数除以有签署处置的完整预检数。零分母报告N/A，不伪装为零；方向中性的原始量、分母和比例同时公开，且不预设胜利值。[metric:public_reading_count]
