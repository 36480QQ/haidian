...
- **Coordinated Research Area (about 43.6 km²)**: the strategic research tier. It carries industry positioning, regional coordination, the "Three Zones and Two Wings" structure, and the east–west corridor relationship; no parcel-level design is performed [source:DATA-SRC-OFFICIAL-ANNOUNCEMENT-20260509].
...
**How to verify**: the three area figures and spatial relationships can be recalculated from this package's GeoJSON under the agreed projection (see Chapter 11); the differences between the provisional boundaries and the announced areas, and the recalculation triggers once official polygons arrive, are all registered in the assumptions list.
...
**Two wings — east–west supporting arms** (per the official account: the Zhongguancun Technology Services Wing on the west and the Xiaoyue River Scenario Enablement Wing on the east) [source:DATA-SRC-BEIJING-GOV-20260403]:
...
2. `[source:DATA-SRC-AGENT-TASKBOOK-0518]` Agent-oriented open-call taskbook, the call organization body, 2026-05-18, `https://github.com/open-city-ai/haidian/blob/main/brief/site-package/agent_taskbook.json`. Covers: co-creation principles, three positionings, five functions, six agent tasks. Limitations: only document excerpts available locally.
...
4. `[source:DATA-SRC-REPO-PROVISIONAL-BOUNDARIES]` open-city-ai/haidian repository provisional_boundaries.geojson (provisional rough boundaries), `https://github.com/open-city-ai/haidian/blob/main/brief/site-package/geometry/provisional_boundaries.geojson`. For generation, visualization, and provisional recalculation only; not an official planning boundary.
