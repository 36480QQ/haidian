...
| --- | --- | --- |
| 核心概念是否原创 | 「星轨轻铺连翠岸，银丝漫织绕青栏」——双轨共生（京张物理轨 × AI 数字轨）、AI 时光驿站叙事轴、一带数字孪生沙盘 | 三项机制均有空间落点与可证伪条件（见「三项原创机制」） |
...
## 三项原创机制：与常规做法的差异与证伪条件

1. **双轨共生（物理轨 × 数字轨）**：常规做法是「线下建设 + 线上展示」两张皮；本方案让京张遗址公园物理轨（慢行、站房、道口）与 AI 数字轨（数字孪生、场景卡、治理智能体）共用同一套空间编号，离线可演练、在线可接管。**证伪条件**：若数字轨任一场景在物理轨关闭后无法回到基础服务，则机制失效。
2. **AI 时光驿站叙事轴**：把百年京张站房/道口的史实按时间轴嵌入公共空间导视与 AR 内容，形成可持续更新的文化叙事层，而非静态雕塑。**证伪条件**：若轴上任意导视点在无 AI 环境下无法提供纸本/语音替代内容，则失效。
3. **一带数字孪生沙盘**：以公共数据开放平台 4457 数据集 / 1.95 亿条为底座，形成可测试 AI 城市场景的沙盘（>10 张场景卡、>3 个测试验证场景），所有数字结果须人工复核后才进入设计决策。**证伪条件**：若任一沙盘指标无法追溯到官方数据集（HD-STAT-2024 / BJ-OPEN-DATA），则失效 [source:BJ-OPEN-DATA]。
