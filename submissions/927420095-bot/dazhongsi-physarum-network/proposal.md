...
- 《北京市城市更新条例》（2023 年 3 月 1 日施行）：实行「留改拆」并举、以保留利用提升为主，严控大规模拆除增建；本方案「存量更新、最小干预、可逆措施」与之方向一致（条例名称与施行时间引自公开立法信息，具体条款适用以正式文本为准）。
- 《京张铁路遗址公园规划》：遗产保护边界与其一致，更新建议均在其保护要求内提出，最小干预。公园一期已于 2023 年 6 月建成免费开放（海淀区清华东路至知春路段，约 16.8 公顷）；整体规划自北京北站（西直门）向北延伸至北五环（清河）约 9 公里，大钟寺片区位于该规划走廊沿线。
...
### 算法发现 → 规划含义 → 设计动作（AI 决策链）

为把方法工具的价值显式化，下表把算法运行的真实结果（均为方法验证证据，见上）转译为规划含义，并映射到正文已落实的设计动作。转译是解释性映射，算法结果不作红线、不作审批依据、不直接几何继承 [source:PHYSARUM-RUN-RESULTS] [metric:physarum_efficiency_index]。

| 算法发现（真实运行） | 规划含义（转译） | 设计动作（正文落点） |
| --- | --- | --- |
| 167 边稀疏骨架 + top-10% 传导度边 | 高流量走廊高度集中，更新聚焦少数关键链路而非全域摊开 | 慢行缝合按主静脉—支脉—慢行环层级推进（JZ-01） |
| 锚点式生长（公园/地铁/铁路为养分锚点） | 关键节点是网络「引力源」，先保节点连通再织密网络 | 29 个关键节点按功能分级，优先打通 3 地铁站四象限连通（JZ-04） |
| NSGA-II 四目标 Pareto（无人工加权） | 效率/成本/遗产/覆盖是权衡关系，不存在单点「最优」 | 方案比选呈现 Pareto 选项，推荐 Plan03（UDS 80.34） |
| g1 硬约束（遗产穿越 = 0 恒成立） | 遗产保护是硬边界，不进入优化目标 | 京张遗址公园边界作为不可穿越约束，最小干预 |
| 黏菌多路径冗余（断点可绕行） | 网络韧性来自冗余路径，而非单一路径 | 无障碍降级机制：主路径失效切替代路径（附录 A） |
| 最优效率 19.20（基线 1.143） | 优化收益可量化，可作后评估基准 | 效率指数纳入方案指标体系作后评估 |

以上六条转译均可复核：算法侧数值见 `metrics.json` 与 `simulation.json`，设计侧动作见更新项目清单与试点节点；f3≡f2 的诚实边界见上文，不因本表而弱化。

![AI 决策链图（算法发现 → 规划含义 → 设计动作，概念）](assets/figures/ai_decision_chain.png)
...
3. **轨道站点覆盖不足**：大钟寺站为地铁 13 号线高架站（2002 年随 13 号线西段开通运营）；2024 年 12 月起 12 号线在大钟寺设站，该站成为 13 号线与 12 号线换乘站，现阶段为出站换乘、站内换乘通道待 13 号线扩编改造后启用。站点周边步行接驳与四象限连通不足，站点一体化与产业服务空间的衔接有待加强（站点开通时间与换乘现状引自公开地铁资料，非本方案编造）。
...
**空间锚定（概念落位 · 编号气泡图）**：六个更新项目与三个产业测试场在临时边界、三重点区与概念路网上的大致位置见下图。落位为**概念示意**，非精确红线、门牌或换乘通道；锚点对象以 JZ 证据引用几何为据，精确空间关系待官方数据到位后核实 [depth:renewal_project_list]。

![更新项目与产业验证场景空间落位（概念）](assets/figures/jz_spatial_anchors.png)

| 编号 | 锚点对象（真实） | 概念落位 |
| --- | --- | --- |
| JZ-01 | 京张遗址公园一期两侧慢行断点 | 公园活力带西侧（京张绿廊沿线） |
| JZ-02 | 清河创新界面（众智园临清河） | 众智园（北）蓝绿界面 |
| JZ-03 | 原点社区校区—园区界面 | 北京AI原点社区（中） |
| JZ-04 | 大钟寺站四象限（13号线/12号线换乘） | 大钟寺AI产业聚集区（南） |
| JZ-05 | 总体设计范围端侧算力节点 | 片区中段节点 |
| JZ-06 | 主静脉 ROAD-001 深化复算 | 南北主静脉（中部） |
...
三个测试场编号为 **I-01**（边缘算力感知盲道）、**I-02**（无人清扫与物流走廊）、**I-03**（视觉识别违占治理），空间落位见上「更新项目与产业验证场景空间落位（概念）」图。下表为**闭环矩阵**，补齐「场景定位→测试主体→数据入出→复核→安全边界→试点周期→验收→退出→非数字化降级」全链路字段（均为概念设计，未实际部署或验证）：

| 闭环字段 | I-01 边缘算力感知盲道 | I-02 无人清扫与物流走廊 | I-03 视觉识别违占治理 |
| --- | --- | --- | --- |
| 空间锚点 | 京张绿廊（盲道连续段） | 众智园（园区内部路网） | 大钟寺商业街区（公共空间） |
| 测试主体类型 | 触觉铺装 / 坡度 / 障碍物 | 低速无人清扫车 / 物流车 | 公共空间占用图像 |
| 数据输入（概念） | 铺装状态、坡度、障碍 | 园区路况、时段 | 占用图像 |
| AI 输出（概念） | 盲道连续性与破损告警 | 清扫 / 物流路径调度 | 违占告警 + 处置工单 |
| 人工复核机制 | 网格员现场复核 + 视障用户反馈 | 现场安全员 + 异常人工接管 | 复核告警后再派单 + 申诉纠错 |
| 安全边界 | 不采集个人身份，端侧推理 | 非高峰作业、远程监控、限速 | 不采集个人敏感信息 |
| 试点周期（建议） | 1 个季节（盲道养护周期） | 3 个月（含非高峰时段） | 1 个季度（含节假日） |
| 成功指标（概念） | 盲道连续率提升、告警处置闭环率 | 清扫覆盖率、物流时效、零重大安全事件 | 违占发现率↑、误报率↓、处置闭环率 |
| 失败退出条件 | 误报率高或用户投诉上升则退出 | 发生重大安全事件或物流时效不达标则退出 | 误报/漏报超阈值或申诉未闭环则退出 |
| 非数字化替代 | 人工巡检 + 无障碍登记点 | 人工清扫 + 传统物流调度 | 人工巡查 + 网格派单 |
...
本方案把无障碍与包容性作为路网更新的硬性设计前提，而非附加项。检验依据为 GB 50763-2012《无障碍设计规范》（现行；其部分强制性条文已被 GB 55019-2021《建筑与市政工程无障碍通用规范》取代，自 2022-04-01 起实施，强制性要求以 GB 55019-2021 为准，本方案并列引用）及《无障碍环境建设条例》（国务院令第 622 号），落到本项目表现为：慢行环与接驳轴的**缘石坡道、盲道、无障碍坡道/电梯、无障碍卫生间、无障碍标识与低位服务设施**在全部更新路段与公共空间连续可达 [depth:traffic_rail_slow_parking]。
...
为堵住评审可能的一票否决点，就最可能被追问的七个问题给出预案（每问含落点与要点，不新增外部文件）：

| # | 评审可能追问 | 预案要点 | 落点/证据位置 |
| --- | --- | --- | --- |
| 1 | 是否冒充红线、或声称获得官方背书？ | 否；全文标注「概念/参考/建议性」，临时边界为 provisional，不冒充红线或审批几何 | 「方法论（方法工具 · 诚实说明）」+ R-02 |
| 2 | 真实 Physarum 实验与场地几何是什么关系？ | 方法验证证据（真实运行），位于临时边界以西 2–3 km，仅作方法有效性证据，不作红线/审批几何；f3≡f2 已诚实披露 | R-02 / 方法诚实声明 |
| 3 | 产业测试场是否已实际部署或验证？ | 否；均为概念建议/建议性框架，未部署；闭环矩阵与退出条件为概念设计 | 「产业测试验证场景」诚实声明 |
| 4 | 造价与经济数字是否真实测算？ | 概念估算（示意），参考同类项目公开区间，非真实测算；仅路网市政造价 2965.5 万元为建议性参考 | 「经济可行性」诚实声明 |
| 5 | 无障碍依据是否引用最新强制性规范？ | GB 50763-2012 现行，但其部分强制性条文被 GB 55019-2021（2022-04-01 实施）取代；本方案并列引用 | 「无障碍与包容性设计」 |
| 6 | 版权/来源是否账实相符？ | 已逐项审计：全部资产列明来源/许可（含 vendored Leaflet BSD-2-Clause、AI 生成效果图标注非数据图） | `report/copyright_statement.md` |
| 7 | 官方数据缺失时结论是否可信？ | 关键结论均标「待确认」，数据到位后全量复算；不预设验收阈值、不虚构红线 | 「风险、版权与合规说明」+ R-01 |
...
**无障碍检查项清单（参考 GB 50763-2012《无障碍设计规范》，标注为「参考规范」）**：缘石坡道坡度不大于 1:12（全宽式单面坡不大于 1:20）；轮椅坡道纵向坡度不大于 1:12、净宽不小于 1.00 m；行进盲道宽度 250–500 mm、连续铺设无中断；过街信号声音提示；座椅间距 ≤100 m；无障碍卫生间可达；低位服务设施。以上阈值为 GB 50763-2012 公开条文（现行；强制性条文以 GB 55019-2021《建筑与市政工程无障碍通用规范》为准，2022-04-01 起实施），本方案沿用为概念设计参考；覆盖率与现状缺口须经现状无障碍调查与正式设计核定，不预设达标数值。
