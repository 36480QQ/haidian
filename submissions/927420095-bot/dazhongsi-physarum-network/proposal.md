...
## 六张更新项目实施卡（极简实施卡，概念建议）

> 本卡片为概念建议 / 参考方案，不构成正式工程文件；责任主体只写**单位类型**，不编造具体单位名称；道路/站口/节点名称均为 provisional 概念落位，待官方数据核实。

以下 6 张实施卡给出每个项目的**空间锚点**（落在哪条路/站口/节点）、阶段、核心动作、责任类型、主要风险、验收方向与概念成本等级；均为建议性，具体以控规与主管部门批复为准 [depth:renewal_project_list] [depth:phasing_implementation]。

**JZ-01 京张遗址公园慢行断点缝合**
- 空间锚点：京张铁路遗址公园一期两侧（京张铁路与慢行环交叉段，provisional）
- 阶段：近期（试点）
- 核心动作：缝合慢行断点，增设无障碍坡道与双语导视，恢复连续步行路径
- 责任类型：街道协调 + 公园管理单位（类型，不写具体单位）
- 主要风险：涉及铁路/公园/市政多产权界面，需进一步核实
- 验收方向：连续步行无障碍性改善，待现状基线确定后评估
- 概念成本：中（conceptual estimate only）

**JZ-02 众智园清河创新界面**
- 空间锚点：众智园临清河段（北翼，清河蓝线南侧，provisional）
- 阶段：中期
- 核心动作：滨水蓝绿贯通 + 低影响开发雨水系统 + 雨洪韧性改造
- 责任类型：区水务局 + 园区（类型，不写具体单位）
- 主要风险：河道蓝线与防洪条件不满足即退出
- 验收方向：蓝绿廊道连通 + 雨洪韧性指标达标，待基线确定
- 概念成本：中（conceptual estimate only）

**JZ-03 原点社区近校成果转化街**
- 空间锚点：北京AI原点社区校区-园区界面（中区，provisional）
- 阶段：近期
- 核心动作：首层业态活化 + 成果转化空间启用
- 责任类型：高校 + 区科信局（类型，不写具体单位）
- 主要风险：校区边界与首层权属争议未决则暂停
- 验收方向：转化空间启用 + 首层业态清权
- 概念成本：中（conceptual estimate only）

**JZ-04 大钟寺站四象限步行连通**
- 空间锚点：大钟寺站 13号线/12号线换乘节点四象限（站点辐射区）
- 阶段：中期
- 核心动作：四象限步行连通 + 无障碍换乘 + 站点一体化
- 责任类型：轨道运营单位 + 区交通委（类型，不写具体单位）
- 主要风险：站点改造时序与市政管线迁改不确定性
- 验收方向：四象限步行连通 + 无障碍换乘达标，待基线确定
- 概念成本：高（conceptual estimate only）

**JZ-05 AI公共服务与端侧算力节点**
- 空间锚点：大钟寺AI产业聚集区门户（南区，邻近大钟寺站，provisional）
- 阶段：长期（治理）
- 核心动作：部署端侧算力节点，开放公共服务，安全合规审计
- 责任类型：区科信局 + 运营主体（类型，不写具体单位）
- 主要风险：能源/算力/安全/运营主体缺位则退出
- 验收方向：算力服务开放 + 安全合规审计通过
- 概念成本：高（conceptual estimate only）

**JZ-06 黏菌路网概念深化与真实场地复算**
- 空间锚点：概念路网主静脉 ROAD-001 沿线（全场，待官方边界与道路红线对齐）
- 阶段：长期（持续研究）
- 核心动作：官方数据到位后全量复算并更新图层
- 责任类型：设计方 + 高校（类型，不写具体单位）
- 主要风险：官方数据长期缺失，保持「待确认」口径
- 验收方向：官方数据到位后全量复算通过
- 概念成本：低 / unknown（研究性，conceptual estimate only）
