...
- *Beijing Regulations on Urban Renewal* (effective 1 March 2023): applies a combined "retain-renovate-demolish" approach centered on retention, utilization, and improvement, strictly controlling large-scale demolition and addition; this proposal's "stock renewal, minimal intervention, reversible measures" aligns with it (the regulation name and effective date are from public legislative information; specific clause application is subject to the official text).
- *Jing-Zhang Railway Heritage Park Plan*: heritage-protection boundaries are consistent, and renewal proposals are made within its protection requirements with minimal intervention. Phase 1 of the park opened free to the public in June 2023 (the Qinghua East Road–Zhichun Road section in Haidian District, about 16.8 hectares); the overall plan extends about 9 km from Beijing North Station (Xizhimen) northward to the North Fifth Ring Road (Qinghe), and the Dazhongsi area lies along this planned corridor.
...
### Algorithm Finding → Planning Meaning → Design Action (AI Decision Chain)

To make the method tool's value explicit, the table below translates the real algorithm-run results (all method-validation evidence, see above) into planning meanings and maps them onto design actions already implemented in the main text. The translation is an interpretive mapping; the algorithm results are neither redlines, nor approval basis, nor direct geometric inheritance [source:PHYSARUM-RUN-RESULTS] [metric:physarum_efficiency_index].

| Algorithm finding (real run) | Planning meaning (translation) | Design action (main-text landing) |
| --- | --- | --- |
| 167-edge sparse skeleton + top-10% conductivity edges | High-flow corridors are highly concentrated; renewal focuses on a few key links rather than spreading everywhere | Slow-travel stitching proceeds by primary-vein–branch–slow-loop hierarchy (JZ-01) |
| Anchor-based growth (parks/subways/railways as nutrient anchors) | Key nodes are the network's "attractors"; secure node connectivity before densifying | 29 key nodes are tiered by function, prioritizing four-quadrant connectivity of the 3 metro stations (JZ-04) |
| NSGA-II four-objective Pareto (no artificial weighting) | Efficiency/cost/heritage/coverage are trade-offs; there is no single "optimum" | Plan comparison presents Pareto options, recommending Plan03 (UDS 80.34) |
| g1 hard constraint (heritage crossing = 0 always) | Heritage protection is a hard boundary, not entering the optimization objectives | The heritage-park boundary is treated as an uncrossable constraint with minimal intervention |
| Physarum multi-path redundancy (detour on breakage) | Network resilience comes from redundant paths, not a single path | Accessibility degradation mechanism: switch to alternative paths when the primary fails (Appendix A) |
| Optimal efficiency 19.20 (baseline 1.143) | Optimization gains are quantifiable and can serve as a post-evaluation baseline | The efficiency index is added to the proposal indicator system for post-evaluation |

These six translations are all reviewable: algorithm-side values are in `metrics.json` and `simulation.json`; design-side actions are in the renewal-project list and pilot node; the honest f3≡f2 boundary is stated above and is not weakened by this table.

![AI decision chain (algorithm finding → planning meaning → design action, conceptual)](assets/figures/ai_decision_chain.png)
...
3. **Insufficient rail-station coverage**: Dazhongsi station is an elevated Metro Line 13 station (opened with the western section of Line 13 in 2002); from December 2024 Line 12 began serving Dazhongsi, making it an interchange between Lines 13 and 12, currently as out-of-station transfer with the in-station transfer passage pending the Line 13 capacity-expansion (8-car) works. Pedestrian interchange and four-quadrant connectivity around the station are insufficient, and station integration with industrial service space needs strengthening (opening dates and interchange status are drawn from public metro sources, not fabricated by this proposal).
...
**Spatial anchoring (conceptual placement · numbered-bubble figure)**: the approximate positions of the six renewal projects and three industry testbeds, overlaid on the provisional boundary, three key areas, and concept network, are shown below. All placements are **conceptual illustrations**, not precise redlines, house numbers, or interchange passages; the anchor objects are based on each JZ project's evidence-cited geometry, and precise spatial relations are to be verified after official data arrives [depth:renewal_project_list].

![Spatial placement of renewal projects and industry testbeds (conceptual)](assets/figures/jz_spatial_anchors.png)

| ID | Anchor object (real) | Conceptual placement |
| --- | --- | --- |
| JZ-01 | Slow-travel break points on both sides of Jing-Zhang Heritage Park Phase 1 | West side of the park vitality belt (along the Jing-Zhang green corridor) |
| JZ-02 | Qinghe innovation interface (Zhongzhiyuan facing the Qinghe) | Zhongzhiyuan (north) blue-green interface |
| JZ-03 | Origin Community campus-park interface | Beijing AI Origin Community (central) |
| JZ-04 | Dazhongsi station four quadrants (Line 13 / Line 12 interchange) | Dazhongsi AI industry cluster (south) |
| JZ-05 | Edge-computing node in the overall design scope | Central segment node |
| JZ-06 | Primary vein ROAD-001 deepening & recalculation | North-south primary vein (central) |
...
The three testbeds are numbered **I-01** (edge-computing tactile-paving sensing), **I-02** (unmanned sweeping and logistics corridor), **I-03** (visual-recognition encroachment governance); their spatial placement is shown in the "Spatial placement of renewal projects and industry testbeds (conceptual)" figure above. The table below is the **closed-loop matrix**, completing the full "positioning → test subject → data in/out → review → safety boundary → trial period → acceptance → exit → non-digital degradation" chain of fields (all conceptual, none deployed or verified):

| Closed-loop field | I-01 tactile-paving sensing | I-02 unmanned sweeping & logistics | I-03 encroachment governance |
| --- | --- | --- | --- |
| Spatial anchor | Jing-Zhang green corridor (continuous tactile-paving segment) | Zhongzhiyuan (internal park road network) | Dazhongsi commercial block (public space) |
| Test subject type | Tactile paving / slope / barriers | Low-speed unmanned sweeper / logistics vehicle | Public-space occupancy images |
| Data input (conceptual) | Paving status, slope, barriers | Park road conditions, time slots | Occupancy images |
| AI output (conceptual) | Tactile-paving continuity and breakage alerts | Sweeping / logistics route dispatch | Encroachment alerts + handling work orders |
| Human review mechanism | Grid-worker on-site recheck + visually-impaired user feedback | On-site safety officer + manual takeover on anomaly | Human recheck before dispatch + appeal & correction |
| Safety boundary | No personal identity collected; edge inference | Off-peak operation, remote monitoring, speed-limited | No sensitive personal data collected |
| Trial period (suggested) | 1 season (tactile-paving maintenance cycle) | 3 months (including off-peak hours) | 1 quarter (including holidays) |
| Success metric (conceptual) | Higher tactile-paving continuity, alert closed-loop rate | Sweeping coverage, logistics timeliness, zero major safety incidents | Higher detection rate, lower false-positive rate, closed-loop handling rate |
| Failure exit condition | Exit if false-positive rate is high or user complaints rise | Exit on major safety incident or unmet logistics timeliness | Exit if false-positive/miss rate exceeds threshold or appeals unclosed |
| Non-digital alternative | Manual inspection + accessibility registration point | Manual sweeping + traditional logistics dispatch | Manual patrol + grid dispatch |
...
### Review cross-examination preparation (7 questions · defense plan)

To close potential one-vote-veto points, the following defense plan covers the seven most likely review questions (each with location and key points; no external files added):

| # | Likely review question | Defense key points | Location / evidence |
| --- | --- | --- | --- |
| 1 | Does it pass itself off as an approved redline or official endorsement? | No; the text marks "conceptual / reference / suggestive" throughout, the provisional boundary is provisional, and nothing is presented as a redline or approval geometry | "Methodology (Method Tool · Honest Account)" + R-02 |
| 2 | What is the relationship between the real Physarum experiment and the site geometry? | It is method-validation evidence (a real run), located 2–3 km west of the provisional boundary, used only as method-validity evidence, not as redline/approval geometry; f3≡f2 is disclosed honestly | R-02 / methodology honesty statement |
| 3 | Have the industry testbeds been actually deployed or verified? | No; all are conceptual suggestions / a suggestive framework, none deployed; the closed-loop matrix and exit conditions are conceptual designs | "Industry Testbed Scenarios" honest statement |
| 4 | Are the cost and economic figures real calculations? | They are conceptual estimates (illustrative), referencing public ranges of comparable projects, not real calculations; only the road-network municipal cost of 29.655 million CNY is a suggestive reference | "Economic Feasibility" honest statement |
| 5 | Does the accessibility basis cite the latest mandatory standards? | GB 50763-2012 is current, but some mandatory clauses are superseded by GB 55019-2021 (effective 2022-04-01); the proposal cites both in parallel | "Accessibility and Inclusivity" |
| 6 | Are copyright and provenance accounts consistent with the assets? | Audited item by item: every asset lists origin/license (including vendored Leaflet BSD-2-Clause, and AI-generated effect images labeled as non-data figures) | `report/copyright_statement.md` |
| 7 | Are conclusions credible given missing official data? | Key conclusions are marked "to be confirmed" and fully recalculated once data arrives; no acceptance thresholds are preset and no redlines fabricated | "Risk, Copyright, and Compliance" + R-01 |
...
**Accessibility checklist (referencing GB 50763-2012 *Code for accessibility design*, marked "reference standard")**: curb-ramp slope not steeper than 1:12 (full-width single-sided ramps no steeper than 1:20); wheelchair-ramp longitudinal slope no steeper than 1:12 and clear width no less than 1.00 m; directional tactile paving 250–500 mm wide and continuous (no breaks); audible crossing signals; seat spacing ≤100 m; accessible toilets reachable; lowered service facilities. These thresholds are public clauses of GB 50763-2012 (current; mandatory clauses follow GB 55019-2021 *General Code for Accessibility and Universal Design of Buildings and Municipal Works*, effective 2022-04-01), adopted here as conceptual design reference; coverage and existing-condition gaps must be verified by a barrier-free survey and formal design, with no preset compliance value.
