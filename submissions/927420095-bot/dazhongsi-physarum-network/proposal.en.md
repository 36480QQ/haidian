...
**Governance mechanism (ordinary path first + graded release G0-G3)**: every AI scenario moving from demonstration to daily use must pass a graded ladder, with all 8 declared scenarios bound per-scenario to five fields (ai_off_path / human_handoff / gate_id / operating_mode / responsible_role, see `simulation.json`); stop conditions, reversible measures, responsibility matrix, and receipts are machine-registered in `risk.json`, `governance-raci.json`, and `governance-receipts.json`, with the ordinary path always available and AI only adding reversible gains.
...
**Honest boundary**: this proposal is a conceptual urban-renewal study; all spatial recommendations must be coordinated with statutory planning and approved by the competent authority, and are never presented as redlines, approval geometry, or approved declarations; building height and development intensity are conceptual suggestions only.
...
**Governance and release (the algorithm-to-deployment boundary)**: the algorithm's conclusions serve only as design input and topological preference, and never move directly into daily operation; every AI scenario moving from demonstration to daily use must pass the graded release G0-G3 (definition in "Test and Validation Scenarios"; per-scenario bindings and stop/recovery rules machine-registered in `simulation.json` and `risk.json`), falling back on any failed level, with the ordinary path always available.
