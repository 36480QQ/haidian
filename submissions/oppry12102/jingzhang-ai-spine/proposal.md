...
summary: "以'可复算—可停—可问责'三可机制，把纵贯全国最密集高校院所集群的京张绿廊转译为公共创新界面；基于 provisional boundary 与结构化自检生成 formal 包，保留精度警示与复算要求，组织方数据缺口不阻断内容评分。"
...
方案把关键缺口改写为四行“证据账”，每行区分本轮可核事实、仍不能证明与设计后果；新事实只缩小未知范围，不自动补线、不激活服务、不制造精确感 [depth:existing_conditions_diagnosis]：
...
| 建筑/控规 | 建筑基底面积可由提交几何复算（约 42.79 万平方米，provisional）[metric:building_footprint_area_sqm] | 全域现状建筑轮廓、权属、结构、拆改留、容积率、高度、退线 | `floor_area_ratio` 保持 unknown；拆改留只给方法不给结论，不编造拆改留表 [depth:retain_renovate_demolish] |
...
这份证据账同时约束“可复算、可停、可问责”：能复算的写数值，不能复算的写 unknown；能核验的写事实，不能核验的写“待核不画”；能负责的写角色，不能负责的写未激活。
...
本方案建议的总体概念为“**京张智脊**”，并把“脊”从单纯的形态隐喻转译为一套贯穿全篇的**三可机制**：**可复算（Recalculable）—可停（Stoppable）—可问责（Accountable）**。
...
- **可停**：每一个 AI 场景都必须有“无 AI 等价路径”与停止条件——人工替代、数据边界、服务窗口或安全底线任一不满足时，场景关闭而非降级硬撑 [metric:public_space_ratio] [depth:metrics_recalculation]。
- **可问责**：每一个更新项目都有牵头/参与角色、前置条件、KPI、人工回退与治理接口；缺主体责任或预算即列为未激活，不写成落地承诺 [depth:renewal_project_list]。

这条“智脊”的空间本底是一条**纵贯全国最密集高校院所集群的约 9 公里绿廊**：南端西直门紧邻北京交通大学，中段学院路沿线依次分布八大学院与中国科学院多家院所，北段五道口汇聚清华大学、北京大学 [source:DATA-ALONG-UNIVERSITIES]。三可机制的意义，正是把这条“存量高密度”的创新要素走廊，转译为一条“可复算、可停、可问责”的公共创新界面——不是在要素密集处简单扩张研发建筑量，而是在要素最高频相遇处，把验证、开放与问责做成公共产品。

空间组织仍沿用“一带三核、多点场景、蓝绿慢行复合环”：这里的“一带”不是额外画出的新红线，而是把公告中的三层范围转译为工作方法；“三核”对应众智园、北京AI原点社区、大钟寺三处重点区域；“多点场景”对应AI+公共服务、产业服务和城市生活的可运营节点；“复合环”对应慢行、绿地、公共空间和活动路线的联动。
...
十张场景卡共同遵守“**可停**”纪律：每一卡都必须声明无 AI 等价路径（人工/纸面/静态指引兜底）、停止条件（何时关闭而非降级硬撑）与失败回退。若人工替代、数据边界、服务窗口或安全底线任一不满足，该场景不得对公众开放或复制；这一条不因“总体体验更好”而豁免 [source:AGENT-TASKBOOK] [depth:metrics_recalculation]。五张产业测试场景（02 安全治理沙盒、03 端侧算力驿站、04 AI慢行导航、07 近校成果转化街、08 数据要素会客厅）还必须给出沙盒隔离、失败回滚与人工复核的验收口径，测试未通过不得向公众或企业开放。
...
把六项更新项目合并为三个**可停止工作包**，每个工作包都带明确停止条件 [depth:phasing_implementation]：

- **WP-01 证据底盘**：替换 official polygons，完成建筑/权属/控规/交通市政底数与数据清权，冻结六项可复算指标。输入不全即维持 unknown，不进入工程或法定结论。
- **WP-02 三核原型**：用品牌组件库与场景卡承载三处重点区原型（众智园全栈核、AI原点策源核、大钟寺智能原生核），落地沙盒、算力驿站、慢行缝合与地标。缺无 AI 等价、停止条件、数据隔离或人工复核即不开。
- **WP-03 年度运营与区域接口**：运行活动周、开发者社区、场景开放日与区域协同接口，按季度复盘。缺真实主体、预算、日志或代表性评议即不复制、不扩张。

三个工作包遵守同一停止纪律：前置条件不满足就停在上一阶段；单一群体失败不能用总体平均抵消；official 几何变更即整体重算。这使实施不是线性承诺，而是带闸门的证据闭环 [assumption:A-OPS-001]。
