...
summary: "以混合渐进更新、低碳可达和可停留公共生活三项空间原则组织铁路遗产、创新协作与日常服务；先用合规开放数据建立网络、形态、服务和蓝绿代理基线，再由本地独立观察验证真实公共生活，且不把代理数据冒充现场事实。"
...
![概念性用地供给与纹理化介入场图](assets/figures/land-use-structure.png)

三层工作不是互相割裂的图纸集合。统筹研究决定产业链和城市形态判断，总体设计把判断落实到更新项目、空间结构和设施承载，重点区域详细设计验证具体地块、建筑、交通、公共空间和AI应用场景的可实施性。agent 生成方案时必须先锁定当前提交采用的 official 或 provisional 边界和约束，再生成用地、建筑、道路、绿地、公共空间、分期和AI服务节点，最后从这些图层复算指标并在正文解释哪些结论仍受 provisional boundary 限制。任何无法从结构化数据复算的面积、比例、规模或项目数量，不得写入正式结论。

本方案的总体概念为“京张AI新篇章：人—城—伟业”：以京张遗址公园为历史与公共空间主轴，以众智园、北京AI原点社区、大钟寺三处重点片区为创新锚点，以高校、企业、社区和轨道站点为日常网络，形成“一条公共生活链、三处重点区及其公共节点、多点可退出场景”的空间组织。“新篇章”不是额外画出的新红线，也不是把 AI 设备铺满走廊；它要求先以空间动作组织公共生活，再以三项人本空间检验决定场景是否可以推进。图 01 的 01/02/03 是重点区编号，H/O/P 是同时贯穿 01/02/03 的三条叙事线，二者不是同一套分类；图 02、图 03、图 04 分别解释混合更新界面、重点区公共生活节点和慢行—蓝绿系统；图 05 与 A0 展板以“混合更新 / 可达网络 / 公共生活”把三类动作重新指回同一张总体图。所有地图采用 WGS84 北朝上示意框架；不同图中的颜色、线型和编号只在该图的图例内生效，不跨图承担同一含义。

铁路遗产、开放创新与公共生活不是三块彼此覆盖的功能区。铁路遗产是贯穿三处重点区的历史与空间线索；开放创新是发布、学习、测试和跨组织协作的使用机制；公共生活是首层界面、步行、停留和公共服务组成的日常网络。它们可以在同一地点共同发生，但分别以“线索、机制、空间网络”接受复核，不以三套重合色块表达。AI 则是满足公共可达、空间承载、人工后备和治理条件后才接入的服务层。
...
三项检验把已登记的城市设计研究转译成空间决定，而不是把研究者姓名变成图例。第一项是**混合与渐进更新**：用不同主导用途、可选择的短路径、可逐步改善的首层界面支撑全天候日常活动；图 02 的 M 介入场和图 05 的 M 锚点使用同一组展示坐标。第二项是**低碳可达网络**：站点、步行、骑行和公共交通的连续性先于智慧交通界面；图 04 的 A 只表示沿开放街路形态提出的慢行与公共服务候选联系片段，必须经站口、过街和屏障数据核对后才能延展为连续廊道，图 05 的 A 与 A0 使用同一锚点。第三项是**可停留、可观察、可调整的公共生活**：先理解人们如何走、停、坐、交谈和取得非数字服务，再决定空间构件和 AI 场景；图 03 的 01–03 是三个观察节点，图 05 用中性节点环和汇总 P 标记明确一对三关系。[source:JACOBS-1961-DEATH-LIFE] [source:NEWMAN-KENWORTHY-1999-SUSTAINABILITY-CITIES] [source:GEHL-2010-CITIES-FOR-PEOPLE]

| 空间检验 | 可见空间动作 | 图面着眼点 | 决定是否推进的复核 |
| --- | --- | --- | --- |
| 混合与渐进更新 | 让研发、产业服务、社区配套和开敞空间在共享首层界面相遇；优先保留可用建筑并以小尺度改造启动 | 图 02 的 M；图 05 / A0 的“混合更新” | 以清权或官方用地、首层用途、建筑状况和时段调查核对；未证明可混合或可进入时，单一企业专用公共空间不进入试运行 |
| 低碳可达网络 | 先沿可核验街路提出步行、骑行、公共服务和遗产记忆的候选联系，再接入 AI 服务 | 图 04 的 A；图 05 / A0 的“可达网络” | 以站口、公交、过街、屏障和网络分析核对；未形成可核验网络时，不声称已形成连续慢行廊道 |
| 可停留、可观察、可调整的公共生活 | 在三处重点区安排可进入、可停留、可观察、可调整的公共生活节点和非数字服务后备 | 图 03 的 01–03；图 05 / A0 的“公共生活” | 由本地独立团队进行匿名聚合观察；未完成观察时，不声称现场已安全、舒适或有活力 |

三项原则都是可否决的准入条件，而非审美标签。数据审计和人工复核不属于图中的候选路径、候选绿荫/开放空间场、片区、边界或用地类别，也不与它们并列：其顺序是“空间动作提出 → 合规来源与可复算数据审计 → 人工专业复核 → 允许试运行、调整或退出”。空间图只画前一步提出的对象；审计和复核只决定对象与 AI 场景能否进入下一步。因参赛者无法到场，本轮仅以合规的开放网络、公开目录和遥感资料建立选点与形态代理基线，并逐项保留许可、查询、原始哈希、坐标系和处理局限；后续须由本地专业团队或清权社区伙伴完成匿名观察。已取得的 OSM mobility 与 buildings 仅作为低对比度背景形态，不作为官方边界、现状红线、现场观察或正式指标依据；未取得或不能复算的数据不在图中以“待补充”或 `unknown` 充当设计信息，而在 `assumptions.json`、`metrics.json` 和 `visual/assets/people-first-research.json` 中保留其来源边界、补数条件与复核路径。代理基线可支持选点和诊断，不能替代人工复核。

## 总体设计范围城市更新与控规深度城市设计

总体设计范围的工作目标是把城市更新判断落实到可复核对象：用地与首层用途支撑混合日常活动，道路和站点网络支撑低碳可达，公共空间与界面调查支撑可停留公共生活。当前 `land_use.geojson`、`buildings.geojson` 和 `roads.geojson` 仍是验证结构，不是完整现状基线；在官方或清权数据到位前，本方案不以这些对象推断真实街区长度、建筑年代、首层活力或站点覆盖。
...
重点区域详细设计是必选项。三处重点区的正式名称来自任务书，因而保留“AI”作为征集范围标签；这不意味着每一项空间任务都应是 AI 装置。众智园应把清河与铁路记忆转为可进入的展示、学习、开放发布和低碳交往界面；北京AI原点社区应以校区—邻里—公园之间的共享服务、日常配套和短路径联系为主，AI 场景只作为可退出的服务层；大钟寺应优先解决站城四象限步行连通、产业服务首层、规划绿地复合利用和渐进更新，再讨论智能终端、内容消费或国际路演。图 03 的 01–03 均为可观察、可调整的公共生活节点，而不以片区色彩表示用地类别。
...
图 02 将四类**建议用地**只作为概念性供给比例汇总，以 100% 概念供给台账表达四类比例，不在地图上填色定位。`land_use.geojson` 中四条贯穿场地的带状多边形是早期用于闭合拓扑和面积复算的设计脚手架，不是现状用地调查，也不替代地块边界、控规用途或产权边界；图中的 M / A / P 概念介入场仅沿 OSM 街路与建筑纹理组织可讨论的空间动作，同样不构成正式地块或道路红线。现有资料尚未提供可靠的地块用途、首层业态、建筑年代和状态、空置、权属、文保控制、污染与结构安全信息；因此，图面不把任一色块解释成“应整体拆除”或“现状已为此用途”。建筑方案应区分保留、改造、更新、新建或待确认对象，明确建筑基底、功能、规模、风貌、屋顶、体量和高度控制的建议层级。若缺少现状建筑、权属、控规和工程条件，方案只能提出方法和待校准清单，不能编造拆改留结论。

用地分类依据 [standard:MNR-LAND-USE-CLASSIFICATION-GUIDE]，建筑高度、体量、界面和风貌控制由 [depth:height_massing_character] 管理，拆改留方法由 [depth:retain_renovate_demolish] 管理。用地和建筑的主要证据是 [data:geometry/land_use.geojson#LU-001]、[data:geometry/buildings.geojson#BLDG-001] 和 [metric:building_footprint_area_sqm]。

改造难度不以颜色或概念分区替代判断，而以“先轻后重”的更新决策序列进入设计。每一类建议用地都优先寻找低扰动的公共空间、首层和服务改善机会，只有在证据齐备后才进入建筑改造、增建或重建讨论。这个序列让“混合与渐进更新”成为空间约束，而不是抽象口号：

| 更新层级 | 可以提出的空间动作 | 必须取得的依据 | 当前结论 |
| --- | --- | --- | --- |
| 01 运营与公共界面修补 | 导视、遮阴、座椅、无障碍、临时活动、首层开放协商和慢行断点修补 | 公共可进入性、管理许可、消防与无障碍复核 | 可作为概念试点，但不预设场地已可用 |
| 02 适应性再利用与小尺度改造 | 既有建筑首层、闲置空间和沿街界面改造成社区服务、学习、展示或产业服务 | 建筑用途、产权、租约、结构、消防、文保和经营连续性调查 | 不对任何具体建筑作既定拆改留判定 |
| 03 设施补足与审慎增建 | 在已确认的公共服务缺口、交通换乘和开敞空间边缘补足设施 | 官方控规、道路红线、市政容量、日照、生态与审批条件 | 仅作为待校准的专业深化方向 |
| 04 条件性重建 | 仅在更新收益、既有建筑不可适配和公共利益均被证明时讨论重建 | 权属与安置、文保、结构安全、污染、融资、审批和公众协商 | 当前不划定重建范围，也不计算拆除量 |

四类建议用地对应的优先顺序亦不同：公园绿地与开敞空间优先采取 01 的修补与连续化；社区服务和产业服务优先采取 01–02 的首层和适应性再利用；研发创新空间优先复用可用建筑和共享设施，任何新增开发都须在 03–04 的条件满足后再判断。这些是设计方法，不是对现状地块的事实分类；后续应以建筑—地块—首层联合清查覆盖率、可保留/可改造/待确认建筑面积和公共可达界面长度等指标复核 [data:geometry/land_use.geojson#LU-001] [depth:retain_renovate_demolish]。
...
本轮新增用地混合熵、步行路口密度、街区长度、活跃界面率、公共生活观察、无障碍连续率、站点步行覆盖、路径直接度、高频公交可达和遮阴覆盖十项人本指标，并新增建筑—地块—首层联合清查覆盖与改造决策所需资料清单。它们不是用于填满展板的数值清单：先公开公式和所需证据，再把可复算的开放数据结果标为“代理基线”，把必须现场验证的停留观察留在机器可读的待复核记录中，最终由本地独立观察和专业复核闭环。图 05 与 A0 只展示可由提交几何复算的方案量，以及“混合更新 / 可达网络 / 公共生活”与空间动作的关系，不以推测值制造表面完整。
