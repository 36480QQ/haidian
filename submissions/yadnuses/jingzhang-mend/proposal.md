...
本方案对 5 张核心照护契约卡（康复机器人实测、AI 陪诊、认知症防走失、无障碍导航、社区助餐配送）进行了离线桌面状态机推演，验证协议「能够被执行和审计」而非停留在概念层面。每张卡推演三条路径：①正常服务路径（服务启动→数据采集合规→人工复核就位→服务运行→服务周期完成）；②硬停止路径（触发契约硬停止条件→服务暂停→人工复核→恢复证据检查→服务恢复）；③变异测试路径（故意跳过人工复核，验证演练系统能检测出违规并拦截）。推演共生成 15 条任务记录（5 卡 × 3 路径），结果汇总如下：

| 卡片 | 正常路径 | 硬停止路径 | 变异测试（跳过人工复核） |
| --- | --- | --- | --- |
| CC-04 康复机器人实测 | ✓ 通过 | ✓ 硬停止→人工复核→恢复 | ✓ 检测到违规并拦截 |
| CC-01 AI 陪诊 | ✓ 通过 | ✓ 硬停止→人工复核→恢复 | ✓ 检测到违规并拦截 |
| CC-02 认知症防走失 | ✓ 通过 | ✓ 硬停止→人工复核→恢复 | ✓ 检测到违规并拦截 |
| CC-03 无障碍导航 | ✓ 通过 | ✓ 硬停止→人工复核→恢复 | ✓ 检测到违规并拦截 |
| CC-05 社区助餐配送 | ✓ 通过 | ✓ 硬停止→人工复核→恢复 | ✓ 检测到违规并拦截 |

概念验证结论：全部 5 张核心卡的正常路径和硬停止路径均按契约执行，变异测试（跳过人工复核）全部被系统拦截——照护契约卡的人工复核机制不是软约束，而是可被机器验证的硬闸门。详细推演记录见 `simulation.json`（15 条任务，成功率 66.7%，违规检测率 33.3%，人工复核拦截率 100%）[metric:simulation_task_count]。该演练为离线概念验证，不替代现场实测；T1-T3 测试验证场景仍需完成测试区备案、保险与安全员就位后方可启动。
