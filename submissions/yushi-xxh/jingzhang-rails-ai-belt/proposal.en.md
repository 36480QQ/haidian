...
This proposal reads `data/processed/*` (fact pack, 6 scope rows, 14 task rows, 6 source-usability rows, 9 GAP rows) and the 6 official snapshots under `brief/site-package/standards/references/*` per the 6-step workflow in `docs/data-workflow.md`, citing only formal and provisional_only material; all 9 GAPs are written into `assumptions.json` and the Data Gap Matrix below, and the architectural design-depth regulation is recorded as a gap due to `missing_source_url`.
...
Aligned with the 13 unified review dimensions of the taskbook (goal alignment, function matching, brand recognition, regional synergy, planning innovation, industry support, scenario perceptibility, spatial clarity, transferability, expression completeness, public compliance, international reach, long-term operation value), this proposal lands each dimension onto a prose section and `compliance_matrix.json`; this section is an internal alignment reference only — **not a self-score**; final review rests with the maintainer.
...
The Switchback Protocol does not only register a path; it also registers how every pilot KPI is measured, who measures it, and how long before it is recomputed. Targets stay `null` until a real operator is confirmed, leaving only the binary "missing one, do not open" gate and the recalculation trigger — writing the unknown into the deliverable instead of filling the table with estimates (`[data:visual/assets/governance/measurement-protocol.json]`):

| Pilot metric | Baseline source | Method | Cadence | Measurer | Target | Recalculation trigger |
| --- | --- | --- | --- | --- | --- | --- |
| k1_pilot_entry_throughput | manual patrol counts on the existing park-edge path | aggregate flow counters plus manual cross-checks; device-level data deleted after aggregation | weekly during pilot, monthly report | R-PUBLIC-SPACE | `null` | official geometry or park boundary update (A-BOUNDARY-001) |
| k1_green_buffer_area_sqm | submitted green_space.geojson buffer polygon, recomputed in EPSG:4548 | polygon-area recomputation | per submission version | R-INDEPENDENT-REVIEW | `null` | river blue-line verification and flood assessment (A-GREEN-001) |
| k1_accessibility_crossing_score | accessibility assessment of the current unsignalized crossing (before pilot) | accessibility audit checklist with a named specialist | before pilot, then per audit cycle | R-PUBLIC-SPACE | `null` | road crossing engineering permit conditions (A-ROAD-001) |
| k2_zero_barrier_link_completion | current walking continuity between Wudaokou and the west end of Tsinghua East Road | route-continuity audit | per audit cycle | R-PUBLIC-SPACE | `null` | official regulatory plan and property conditions (A-PARCEL-001) |
| k3_quadrant_pedestrian_continuity | current pedestrian continuity across the Dazhongsi station four quadrants | route-continuity audit | per audit cycle | R-INDEPENDENT-REVIEW | `null` | station integration plan and property conditions (A-ROAD-001) |
| high_impact_human_review_ratio | share of high-impact scenarios per submission that passed human review | review-record statistics | per submission version | R-INDEPENDENT-REVIEW | `1.0` | — |

The contract consists of the files under `visual/assets/governance/`: `switchback-protocol.schema.json` (the structure), `switchback-instance-suite.json` (10 instances, all sandbox_only / draft — none claims to be live), `switchback-rule-check-report.json` (80 checks), `role-spec.json` (7 R- roles: R-PUBLIC-SPACE / R-TEST-SAFETY / R-SERVICE-OPS / R-COMMUNITY-CARE / R-CULTURE-VENUE / R-EVENT-OPS / R-INDEPENDENT-REVIEW, each with qualifications, authority, absence prohibition and incompatibilities), and `measurement-protocol.json`. "0 structural errors" proves only that the instances are machine-parseable and the rules catch their defined defects — not that the path is correct, the service is available, the arrangement is legally compliant, or the instances are field-ready (`[data:visual/assets/governance/validation-report.json]`, `[data:visual/assets/governance/switchback-rule-check-report.json]`).
