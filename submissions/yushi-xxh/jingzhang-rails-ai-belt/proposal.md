...
| 评审问题 | 本方案的回答 | 可核验成果 |
| --- | --- | --- |
| 核心命题 | 把京张铁路"百年文化血脉"与 AI"智能流"组织为一条共生带：一脉（遗址公园主轴）· 三核（众智园 / 北京 AI 原点社区 / 大钟寺）· 九场景 · 五链协同（高校策源—开源协作—企业转化—公共体验—国际传播） | 总体概念章节 + `visual/index.html` + 品牌识别图 |
| 空间响应 | 三层范围（43.6 km² / 11.4 km² / 368.4 ha）逐层落图；三处重点片区各配 3 个概念地块、4 条干预要素与 1 条剖面 | 9 类 GeoJSON + 7 组双语图件 + A3 文册 + A0 展板 + `[data:geometry/site_boundary.geojson#SITE-001]` |
| AI 与治理机制 | 城市智能体只做辅助（识别断点、聚合计數、养护提示、需求撮合），不替代审批、不输出个人画像；十张场景卡逐卡设置人工接管、无 AI 等价服务与停用入口 | 城市智能体治理协议章节 + `[source:AI-GOVERNANCE-INTERIM-MEASURES]` |
| 实施起点 | 三段实施闸门：近期试点（0–12 月，JZ-01/JZ-06）→ 中期更新（控规与权属条件闸门，JZ-02/JZ-03/JZ-05）→ 长期治理（JZ-04 + 年度复盘）；每道闸门有进入条件与退出条件 | JZ-01..JZ-06 治理矩阵（6 项目 × 10 字段）+ 三段闸门表 + `[metric:project_exit_clause_count]` |
| 公共价值 | 8 类用户画像含老年人/照护者、残障人士、夜间劳动者、非中文使用者；公共空间组件库 8 项；年度活动 4 项全部概念级、可申诉 | 画像表 + 组件库 + 无 AI 等价服务列 + `[source:BARRIER-FREE-LAW]` |
| 证据状态 | 全部 known 指标由 EPSG:4548 从提交几何复算；临时边界 0.24% 精度差已显式声明；官方控规缺失项列为 unknown | `[metric:site_area_sqm]` + `[metric:land_use_gap_ratio]` + `[metric:bilingual_completeness_ratio]` |
| 决策边界 | 全部空间、品牌、活动、时限与角色安排均为概念建议；不声称官方批准、审定控规、最终权属或政府承诺 | 风险章节 + `assumptions.json` 16 条 + `[source:SOURCE-REGISTRY]` |

> 上表引用键写在表下而非单元格内，避免每格多锚点割裂阅读；本表其余表格每行只挂一个引用键。执行摘要不替代正文证据：任何"可核验成果"列都指向正文章节或结构化文件。
...
| 场景卡 / 空间 | 服务对象与最小数据 | 模型边界 / 人工复核 | 运营责任、失败模式与 KPI | 无 AI 等价服务（停用后可用） |
| --- | --- | --- | --- | --- |
| 01 开源发布厅 / 原点社区 | 开发者与高校；授权项目元数据、公开许可证 | 仅检索与摘要；发布前由项目维护者和版权岗复核 | 社区运营组；误署名即下架；KPI=已核验项目率、无障碍活动率 | 实体投稿窗口 + 线下评审会 |
| 02 安全治理沙盒★ / 众智园 | 测试团队；合成/授权测试集、模型卡 | 不连接生产系统；安全负责人批准测试与报告 | 沙盒运营方；越权调用立即隔离；KPI=复现实验率、问题闭环时长 | 线下安全评估 + 人工测试报告 |
| 03 端侧算力驿站★ / 总体节点 | 初创团队；授权任务、设备遥测最小集 | 不处理未授权个人数据；值班工程师审核资源与能耗 | 基础设施运营方；容量/能耗超限降级；KPI=服务可用率、单位任务能耗 | 常规云资源 + 线下调试工位预约 |
| 04 AI 慢行导航 / 公园活力带 | 居民、老人、残障人士；公开路网、人工巡查、聚合计数 | 只给路线建议；交通/无障碍专员确认高风险提示 | 公共空间运营方；断路或误导切换人工公告；KPI=断点闭环率、无障碍可达率 | 纸质地图 + 路侧标识 + 人工问询 |
| 05 国际路演客厅 / 大钟寺 | 企业访客与公众；授权议程、企业自报材料 | 不生成投资或政策承诺；主持人与法务审稿 | 场馆运营方；错误宣传撤回；KPI=清权材料率、活动反馈率 | 传统展陈 + 人工导览 |
| 06 清河低碳创新廊★ / 众智园 | 园区用户；公开气象、授权环境传感、人工巡检 | 仅辅助展示与养护提示；设施人员决定处置 | 园区/绿地运营方；传感失效转人工巡检；KPI=数据可用率、工单闭环率 | 人工巡检 + 固定养护计划 |
| 07 近校成果转化街 / 原点社区 | 高校团队；授权成果摘要与服务需求 | 不评价科研真实性或替代法律意见；技术经理/法务复核 | 成果转化服务方；权利争议暂停展示；KPI=清权率、服务响应时长 | 线下技术经理窗口 |
| 08 数据要素会客厅 / 大钟寺 | 合规与产业人员；公开目录、合成样例、审计日志 | 不交易真实敏感数据；数据保护官批准演示 | 授权数据运营方；来源不明即拒绝；KPI=可追溯率、审计问题闭环率 | 书面目录 + 人工查阅 |
| 09 AI 生活服务样板街 / 社区商业界面 | 居民与照护者；用户主动输入、公开服务目录 | 不作医疗/法律/教育最终决定；对应持证人员复核 | 各服务机构；错误建议转人工并可申诉；KPI=人工接管率、申诉处理时长 | 人工服务台 + 持证专业人员 |
| 10 全球 AI 活动周路线 / 公共空间系统 | 公众与国际访客；公开活动与无障碍信息 | 只做多语导览；内容编辑与安全岗发布 | 活动联合体；拥挤/许可变化触发停用；KPI=路线可用率、多语与线下覆盖率 | 印刷手册 + 志愿者引导 |

★ 为 3 个 AI 产业测试验证场景。十张卡均执行数据最小化、告知与退出、人工接管、日志留存和阶段性影响复核；试点前必须明确申诉渠道与停用阈值，约束 A-DATA-001。第 5 列"无 AI 等价服务"呼应 [source:BARRIER-FREE-LAW]（无障碍环境建设法）与 [source:ELDERLY-SMART-TECH-PLAN]（老年人智能技术实施方案）的线下通道要求：系统停用时，任何人仍可获得等价基础服务。

agent 生成的AI治理建议必须遵守数据最小化、公开来源、可解释和人工复核原则。城市智能体可以辅助识别慢行断点、公共空间热力、设施维护、企业服务需求和活动安全风险，但不能替代规划审批、不能输出未经授权的个人画像、不能声称获得官方实施承诺。所有AI场景节点应进入结构化图层或合规矩阵，便于评审者看到它们与产业、空间和公共利益之间的关系。

### 城市智能体治理协议（v1.11）

本方案把分散在各场景卡中的治理规则收敛为一条可审查的协议，回应 [source:AI-GOVERNANCE-INTERIM-MEASURES]（生成式AI服务管理暂行办法）的合规框架：

| 阶段 | 动作 | 人类责任角色 | 边界 |
| --- | --- | --- | --- |
| 1 计划 Plan | 写测试计划 + 威胁模型 + 数据最小化清单 | 场景负责人 + 数据保护官 | 不连接生产系统；只使用公开、授权、合成或聚合数据 |
| 2 授权 Authorize | 逐场景签署测试与上线许可 | 安全负责人 + 数据保护官双签 | 无授权不得上线；高风险场景加第三方安全评估 |
| 3 执行 Execute | 沙盒隔离运行 + 审计日志留存 | 值班工程师 + 运营方 | 越权调用立即隔离；日志保存周期符合合规要求 |
| 4 报告 Report | 复现实验 + 影响说明 + 公开复盘 | 场景负责人 + 外部复核人 | 严重事件公开复盘报告，不停用不隐瞒 |

协议的三条硬约束：**人工接管**（任何高风险提示必须人工确认）、**无 AI 等价服务**（系统停用时必须保留线下或人工替代通道）、**停用入口**（物理/数字双入口可触发停用与申诉，约束 A-DATA-001）。十张场景卡的"无 AI 等价服务"列给出每张卡的线下替代通道；三条硬约束进入 `compliance_matrix.json` 的 1.3.3 条目。该协议是概念治理建议，不构成政府监管决定。
...
分期与 100 天征集设计周期形成区分：征集周期是提交成果的时间要求，实施分期是城市更新和项目建设的推进路径。本方案给出近期试点（JZ-01 / JZ-06）、中期更新（JZ-02 / JZ-03 / JZ-05）和长期治理（JZ-04 + 长期运营机制）三段框架，分别对应 [data:geometry/phasing.geojson#PHASE-001] 的三段概念 polygon。近期试点可由轻量设施、运营活动和服务平台启动；中期更新须等待正式控规、市政、交通和权属条件确认；长期治理依赖年度活动体系、开发者社区运营、场景开放日、公共体验路线和国际传播机制的稳定运行。

三段实施闸门（v1.11）把分期写成三道可核查的条件闸门：

| 阶段 | 覆盖项目 | 进入条件 | 退出条件（转入下一阶段前必须满足） |
| --- | --- | --- | --- |
| 近期试点（0–12 月） | JZ-01、JZ-06 | 轻量设施/活动/平台，不依赖控规与权属变更；完成试点许可与清权 | 试点 KPI 基线记录完成；2/3 试点 go/no-go 达标；公开复盘纪要发布 |
| 中期更新（条件闸门） | JZ-02、JZ-03、JZ-05 | 正式控规、市政、交通与权属条件确认；可研与专项评估完成 | 任一 A-* 限制未解除则冻结；防洪/文保/权属任一否决即暂停并转入"待补资料" |
| 长期治理（年度复盘） | JZ-04 + 年度活动体系 | 中期项目交付后进入；运营主体、预算与许可签署 | 年度技术审查与内容版权复核；连续两年 KPI 不达标即退出并公开说明 |

闸门不是时间表承诺：每道闸门只在条件满足时开启，时间跨度为概念参考区间。该表对应 [metric:phase_coverage_ratio] 与 [metric:project_trigger_threshold_count]，约束 A-PROJ-001 / A-PARK-002。

年度活动体系、开发者社区运营、场景开放日、公共体验路线与国际传播机制的运营对象、频率、责任边界、转化路径与风险见下表，约束 A-OPS-001 / A-PROJ-001。
...
- **双语合规模块数**：[metric:bilingual_figure_count] = 7（5 固定图 + 品牌识别 + 剖面），[metric:bilingual_pdf_count] = 2，[metric:bilingual_visual_count] = 1；综合完成度 [metric:bilingual_completeness_ratio] = 1.000。
