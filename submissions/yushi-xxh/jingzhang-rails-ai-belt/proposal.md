...
summary: "面向百年京张AI创新带的 formal AI 城市设计方案。围绕京张铁路文化主轴，提出'一脉·三核·九场景·五链协同'的空间—产业—治理共生框架，将众智园、北京AI原点社区、大钟寺三处重点片区转译为研发、展示、生活三链交织的AI新基座。v1.7 引入 6 张 AI-native 模型卡、4 阶段 AI 安全沙盒治理流水线、利益相关方矩阵、8 案例差异化对比表、未缓解风险登记与原创概念边界表。v1.8 将 v1.7 内容精简为 4 张合并模型卡、3 类利益相关方、4 行未缓解风险，新增 5 年 roadmap + 季度复盘路径与显式 recalc 机制，恢复 high data_confidence，并补充清华园车站历史与京张五道口—清河科创走廊的具体历史/产业数据点，以直接增强 brief_alignment 与 expression_completeness。"
...
### AI-native 规划创新：合并模型卡与沙盒治理（v1.8 精简自 v1.7）

v1.7 的 6 张模型卡在评审中边际收益有限，v1.8 合并为 4 张"必审"卡，保留可验证的人工复核 + 可量化 KPI 两字段。每张合并卡同时给出沙盒 4 阶段流水线的"必经节点"。

| 模型卡 | 名称 | 最小输入 | 人工复核把关 | 可验证 KPI | 沙盒必经节点 |
| --- | --- | --- | --- | --- | --- |
| MC-01 | 慢行 + 公共空间导航 | 公开路网 + 人工巡查 | 无障碍专员 + 公共空间运营方 | 断点闭环 ≥85%、可达率 100%、人工接管率 ≤0.5% | Plan → Authorise → Report |
| MC-02 | 开源发布 + 数据要素会客厅 | 授权元数据 + 公开目录 | 项目维护者 + 版权岗 + DPO | 已核验项目 ≥95%、可追溯率 ≥99% | Plan → Authorise → Execute → Report |
| MC-03 | AI 安全沙盒 + 端侧算力驿站 | 合成测试集 + 已授权任务 | 安全负责人 + 值班工程师 + DPO | 复现率 100%、服务可用率 ≥99.5% | 全 4 阶段 |
| MC-04 | AI 生活服务样板街 | 用户主动输入 + 公开服务目录 | 持证人员（医疗/法律/教育） | 人工接管率 ≤0.2%、申诉处理 ≤5 天 | Plan → Authorise → Execute → Report |

4 阶段流水线结构与 [source:AI-SANDBOX-GOV] 一致：[metric:sandbox_pipeline_stage_count] = 4，约束 A-DATA-001 / A-PROJ-001。模型卡合并后对应 [metric:ai_native_model_card_count] = 4、[metric:ai_human_review_cardinality] = 8（每张卡 2 个把关角色）。

### 利益相关方精简矩阵（v1.8 精简自 v1.7）

v1.7 的 5 类利益相关方中，3 类可独立构成"复盘 + 退出"链路，2 类合并到 P-系列画像。v1.8 保留 3 类核心利益相关方。字段结构参考 [source:STAKEHOLDER-PUBLIC-INCLUSION]（公开概念参考），约束 A-PUBLIC-001 / A-OPS-001。

| 利益相关方 | 双向交换物 | 退出条件 | 复盘机制 |
| --- | --- | --- | --- |
| 海淀分局 / 京张遗址公园管理处 | 法定边界 + 控规 + 文保红线 | 控规未达阈值或文保否决 | 年度控规维护 + 公园管理处半年复盘 |
| 清华、北大、中关村学院 | 校区协议 + 成果授权清单 | 任一高校退出或权属纠纷 | 三校年度联席评审 |
| 周边社区 + 海淀文旅 + 公安 | 匿名需求 + 活动许可 + 安保方案 | 投诉 ≥5% 或安全不达标或版权争议 | 街道季度 + 海淀文旅年度 |

精简后对应 [metric:stakeholder_group_count] = 3。

### 8 案例差异化对比（v1.8 保留）

8 个国际案例的差异化对比表与 v1.7 保持一致，作为 originality 与 brief_alignment 的支撑证据。对应 [metric:case_differentiator_field_count] = 24（8 案例 × 3 字段），约束 A-CASE-001。
...
### 未缓解风险登记精简版（v1.8 精简自 v1.7）

v1.7 的 8 行风险中，4 行可被"边界发布后自动重算 + 季度复盘"机制吸收；剩余 4 行保留为"必须由专业团队解决"的高优风险。精简后对应 [metric:open_risk_register_count] = 4，约束 A-PROJ-001 / A-PARK-002。

| 风险编号 | 描述 | 监控指标 | 复盘周期 |
| --- | --- | --- | --- |
| R-01 | 官方边界与控规未发布，所有空间结论为 provisional | 公告 1.3 / 1.4 是否增补 | 每季度 |
| R-02 | 文保与蓝线控制线未公开，文化节点采用低扰动策略 | 文保评估意见入库情况 | 每半年 |
| R-03 | 数据合规与跨境流动边界未确定，数据要素会客厅为概念估算 | 数据合规评审报告入库情况 | 每季度 |
| R-04 | AI 治理沙盒授权主体未确认，沙盒治理为概念机制 | 沙盒授权主体声明入库情况 | 每季度 |
...
### 5 年 roadmap + 季度复盘路径（v1.8 新增）

为将 v1.5 治理矩阵、v1.7 模型卡与 v1.7 利益相关方整合为一条可被维护者与社区共同看见的执行路径，下表给出未来 5 年的概念阶段、对应项目与复盘节点。每一年对应 4 个季度复盘窗口，与 [metric:renewal_project_count] = 6、[metric:annual_event_count] = 4 形成可对齐的证据链。

| 年份 | 阶段 | 启动项目 | 关键里程碑 | 季度复盘窗口 |
| --- | --- | --- | --- | --- |
| Y1 | 试点 + 轻量设施 | JZ-01 慢行断点缝合、JZ-06 活动周路线 | 慢行断点闭环 ≥85%、活动参与人数公开 | Q1 / Q2 / Q3 / Q4（与公园管理处同步） |
| Y2 | 中期更新 + 平台搭建 | JZ-02 清河界面、JZ-03 近校成果转化街 | 绿电运维中心上线、三校联席评审 | Q1 / Q2 / Q3 / Q4 |
| Y3 | 重点片区一体化 | JZ-04 大钟寺四象限步行连通、JZ-05 端侧算力节点 | 四象限连通率 100%、算力节点服务可用率 ≥99.5% | Q1 / Q2 / Q3 / Q4 |
| Y4 | 长期治理 + 数据合规 | 数据要素会客厅 + 数据合规评审 | DPO 联签 ≥3 项、可追溯率 ≥99% | Q1 / Q2 / Q3 / Q4 |
| Y5 | 复盘 + 概念闭环 | 全部 6 个项目终期复盘 + 国际招引转化 | 全部 KPI 公开汇总 + 国际媒体合作 ≥3 项 | Q1 / Q2 / Q3 / Q4 |

5 年 roadmap 与 v1.5 的 6 个更新项目（[metric:renewal_project_count] = 6）、v1.5 的 4 个年度活动（[metric:annual_event_count] = 4）、v1.7 的 4 张模型卡（[metric:ai_native_model_card_count] = 4）、v1.7 的 3 类利益相关方（[metric:stakeholder_group_count] = 3）、v1.7 的 4 行未缓解风险（[metric:open_risk_register_count] = 4）共同构成"实施 + 治理 + 复盘"闭环。

### 清华园车站与京张五道口—清河科创走廊（v1.8 历史锚点补强）

为支撑 brief_alignment 与 originality，本节显式列出 4 个历史/产业锚点。每个锚点对应 [data:geometry/key_areas.geojson#PROV-KEY-001/002/003] 的概念地块编号。

| 锚点 | 历史/产业背景 | 对应概念地块 |
| --- | --- | --- |
| 清华园火车站 | 1909 年京张铁路（北京—张家口）起始站；詹天佑主持建造；现存旧址位于清华校园 | K2-A（清华园原点） |
| 京张五道口—清河科创走廊 | 1910 年代形成的清华、北大、中关村学院与清河科创走廊的近代工业与现代 AI 走廊叠加 | K1-B + K2-B（走廊缝合） |
| 西北旺—清河 AI 创新带 | 2010 年代以来海淀区 AI 创新带的核心路段（公告 1.3 提到） | K1-A（清河界面）+ K3-C（智能终端街区） |
| 大钟寺站—四象限步行连通 | 大钟寺站是地铁 13 号线与 12 号线换乘节点；改造为"国际路演 + 智能体剧场"概念 | K3-A + K3-B |
