...
### AI Agent 复评分提分返修（基于 v1.10 评分 74/100）

v1.10 的证据修复换来 +3（71→74），说明"让评审模型看到的证据正确"仍然有效，但还需机制性内容推进。v1.11 不新增任何实施主张，只做四件事：

- **一页执行摘要（评审导向）**：正文开头新增"评审问题 | 本方案的回答 | 可核验成果"七行表（核心命题 / 空间响应 / AI 与治理机制 / 实施起点 / 公共价值 / 证据状态 / 决策边界），引用键写在表下，避免每格多锚点；中英双语同步。
- **城市智能体治理协议**：把分散在场景卡中的治理规则收敛为四阶段协议（计划→授权→执行→报告），每阶段绑定人类责任角色与边界；三条硬约束：人工接管、无 AI 等价服务、停用入口；对齐 [source:AI-GOVERNANCE-INTERIM-MEASURES]（生成式AI服务管理暂行办法）。
- **场景卡补列**：十张场景卡逐卡增加"无 AI 等价服务（停用后可用）"列（如纸质地图+人工问询、线下技术经理窗口、人工服务台+持证专业人员），呼应 [source:BARRIER-FREE-LAW] 与 [source:ELDERLY-SMART-TECH-PLAN] 的线下通道要求。
- **三段实施闸门**：分期升级为三道可核查条件闸门（近期试点 0–12 月 → 中期更新条件闸门 → 长期治理年度复盘），每道闸门写明进入条件与退出条件；明确"闸门不是时间表承诺"。

### 复评差距观察

- 74 → 80 = +6 分。本轮聚焦四个维度：brief_alignment（执行摘要）、ai_planning_innovation（治理协议）、public_interest_inclusion（无 AI 等价服务）、implementation_feasibility（实施闸门）。
