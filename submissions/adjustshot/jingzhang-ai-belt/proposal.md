...
- **《京张铁路遗址公园沿线街区控制性详细规划（2024—2035 年）》获批（2026.8.12）**：本方案最权威、最直接的规划依据——官方给出"一带一轴两心多点"空间结构，以绿色公共空间引领城市更新、支撑人工智能创新街区建设，发展大模型、智能体、具身智能等新业态 [source:PROCESSED-FACT-PACK]。
...
主品牌"JZ-IA / 智脉京张"采用三个候选方向，均为 AI 辅助的**概念探索**，尚未注册商标，正式落地前需由专业设计团队与法务团队深化：

![JZ-IA 三个视觉识别方向（概念探索）](assets/figures/c06_logo_three_directions.png)
...
**对标结论**（概念阶段）：全球 AI 创新带的共性是「**政府主导+大学/科研核心+旧工业用地再生+地铁/高铁门户+全球资本与技术链接**」五条腿组合——本方案的五要素（智源/智转/智用 + 9km 廊 + 两大高铁门户）已在结构上对位。但**具体到产业规模、TAM、资本规模等定量对标，需待深化阶段按官方数据重新校核**。
...
**协同原则**（概念建议）：尊重各板块官方边界与主导权，本方案仅输出**通道、链接、共创机制**三件事，不越权、不抢夺主导权、不重复建设。所有数据来自公开规划文件，不构成对其他板块的承诺或评判。
...
| 场景 | 空间落位 | 对应节点 | 运营主体（概念） | 数据治理/模型责任 | 故障降级 | 验证门槛 | KPI（草案） | 分期 |
|---|---|---|---|---|---|---|---|---|---|
| S1 智轨接驳 | 绿廊北段 | A1 京张之门 | 政企合作试点 | 只留 OD 匿名数据；模型可解释性审计 | 自动驾驶→人工接管 | 试运行 3 个月安全数据 | 准点率 ≥95% | 近 |
| S2 人字课堂 | 清华园车站 | B2 原点对话轴 | 文博+教育机构 | 不采集儿童生物信息 | 现场讲解员兜底 | 课程审核制 | 年服务 ≥1 万人次 | 近 |
| S3 全栈开源 | 众智园 | B1 全栈圣殿 | 开源社区+企业 | 代码贡献公开署名；容器隔离 | 社区评审回滚 | 双周发布+评审 | 月活跃贡献者 ≥200 | 近 |
| S4 OPC 发布厅 | 原点社区 | B2 原点对话轴 | 园区运营方 | 数据不出域；合同约定用途 | 人工申诉窗口 | 平台审核制 | 月路演 ≥30 场 | 近 |
| S5 健康舱 | 社区驿站 | C 类节点 | 卫健+企业 | 健康数据最小化+知情同意 | 人工护士值守 | 医疗级合规评测 | 异常告警响应 ≤15min | 近 |
| S6 绿廊智导 | 绿廊全线 | 全线 | 公园运营方 | 摄像头自动打码；7 天留存 | 志愿者巡查补位 | 月度审计 | 无障碍通过率 ≥95% | 近中 |
| S7 邻里法理站 | 大钟寺/社区 | B3/C5 | 司法所+律所 | 案件数据加密+访问留痕 | 人工律师复核 | 司法合规审查 | 年咨询 ≥5000 件 | 中 |
| S8 无人配送 | 小月河翼 | C6 场景试验场 | 平台企业 | 路径数据匿名化 | 人工配送兜底 | 道路许可+试运行 | 配送时长下降 30% | 中 |
| S9 环境守望 | 蓝绿网络 | C1 滨水驿站 | 市政+生态企业 | 坐标+时序即可，不留身份 | 现场人员处置 | 数据质量校验 | 异常 ≤24h 上报 | 中 |
| S10 科创市集 | 四道口 | C5 铁路记忆 | 市集品牌 | 交易数据脱敏 | 现场人员补位 | 场地安全许可 | 年客流 ≥100 万 | 中 |
| S11 机器人驿站 | 五道口 | C3 青年共创 | 企业+孵化器 | 不采集身份；实时擦除 | 人工通道优先 | 安全评测 | 服务响应 ≤30s | 中 |
| S12 安全沙盒 | 众智园 | B1 全栈圣殿 | 研究院+标准机构 | 测试集脱敏；隔离网络 | 隔离网回滚 | 3 国标准通过 | 漏洞闭环 ≤90 天 | 中远 |

映射逻辑：近分期场景优先落在已点燃的"原点—众智园"核与绿廊骨干上（轻量启动）；中分期沿两翼与支撑节点展开（均衡带动）；所有运营主体、KPI 与治理机制均为概念建议，待专业团队深化。
映射逻辑：近分期场景优先落在已点燃的"原点—众智园"核与绿廊骨干上（轻量启动）；中分期沿两翼与支撑节点展开（均衡带动）；所有运营主体均为概念建议，待专业团队深化。
...
- **规划衔接**：方案空间结构与官方街区控规"一带一轴两心多点"的提法呼应（具体获批状态待官方公告确认）；待官方 polygons 发布后按 EPSG:4548 全量复算，正式控规条件到位后逐项落图 [source:PROCESSED-FACT-PACK]。
