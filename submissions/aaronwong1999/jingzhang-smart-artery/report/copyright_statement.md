...
- 渲染 PNG 与 PDF 时使用 macOS 系统自带中文字体 `STHeiti Medium.ttc`（黑体-简）。该字体文件未随本包重新分发，仅用于本地生成设备上的渲染输出；图片/PDF 中的文字已栅格化/嵌入为图形，不构成字体软件的再分发。
- 几何与制图依赖 Python 开源库 `shapely`、`pyproj`、`matplotlib`、`reportlab`，均按各自开源许可证使用。

## 第三方数据

- 场地边界采用仓库维护者提供的 `provisional_boundaries.geojson`（临时粗略边界），明确标注为 `provisional`，仅用于生成、可视化与自检，不作为官方红线或精确面积依据。
- 三层范围与三处重点区面积引用自公开资格预审公告。
