...
- **蓝线管控**：清河、小月河蓝线外50米范围内以绿地和低强度公共服务设施为主，不布置高强度产业用地
...
- 沿京张绿脉设置独立路权的"AI慢行专用道"——宽8-12米的林荫慢行主轴，兼容步行、骑行、自动驾驶接驳和AI场景展示
...
[source:SOURCE-TASKBOOK][depth:agent.3]本章节详述面向"绿色智慧"主题的AI创新生态、五类人才用户画像及10张AI+场景卡的设计方案。
...
以下是10张AI场景卡的核心信息[agent:3]。完整细节见`scenarios/`下的场景注册文件。

**场景1：绿脉AI慢行导航系统**
- 位置：京张遗址公园AI慢行专用道全线
- 用户：全体
- 数据源：环境传感器、人流摄像头、用户手机信令（匿名化）
- 隐私边界：不使用人脸识别，数据在边缘节点处理不上传云端
- 运营模式：市政公共基础设施
...
**场景8：AI公共空间活力监测与自适应管理**
- 位置：全线公共空间
- 用户：空间管理者、公众
- 数据源：热力传感器、Wi-Fi探针（匿名化）
- 运营模式：市政公共管理
...
建筑总规模控制在1200-1400万平方米（毛容积率约1.1-1.2），其中产业研发约550万m²、商业商务约200万m²、居住约400万m²、公共服务约60万m²[metric:total_gfa]。

**重要提示**：以上用地分类和建筑规模均为概念方案，在缺乏官方控规条件和现状准确数据的情况下，仅作为方向性建议。正式规划中必须依据官方控规条件进行精确测算。
...
**新型能源体系**：
- 分布式光伏：利用新建建筑屋顶和京张绿脉廊架，装机容量约15MWp
- 地源热泵：三个重点区各建一处区域能源站，为周边500米半径提供集中供冷供热
- 清河地表水源热泵：利用清河地表水作为冷热源，服务众智园片区
- 目标：可再生能源占区域总能耗的20%以上
...
[source:SOURCE-SITE][source:SOURCE-TASKBOOK][source:SOURCE-OSM]本章节列出本方案编制过程中参考的全部资料。
...
9. OpenStreetMap贡献者，北京海淀区基础地理数据（ODbL许可）
10. 北京市公共数据开放平台，交通、环保和公共服务统计信息
