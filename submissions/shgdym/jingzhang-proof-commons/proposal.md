---
title: "X京张 X JINGZHANG：一件产品，三次决定，一次RETURN"
author_github: "shgdym"
language: "zh"
translation_file: "proposal.en.md"
license: "CC-BY-SA-4.0"
summary: "公众走正线，AI走侧线；一件低速具身AI产品在众智园TEST、AI原点RELEASE、大钟寺USE，任一失败即RETURN，而公共城市主线始终运行。"
...
> **一件产品，三次决定，一次 RETURN。** 公众走正线，AI走侧线；每次并线，都必须经过城市的公开交叉验证。

![X京张一件产品、三站与RETURN运行总图](assets/figures/x-operating-proof.png)

## 30秒读懂：一件产品如何被城市允许，又被城市退回
...
这就是X：不是一个Logo，也不是一串后台审批，而是城市里真实发生冲突、由人决定是否放行的空间交叉点。技术通过不能替代权利判断，有限发布不能替代公众决定，新版本也不能覆盖0.8版留下的失败历史。[data:visual/assets/lived-lifecycle.json]

| 城市决定 | 这一站只回答什么 | 真实空间中的X | 失败后城市怎样继续 |
| --- | --- | --- | --- |
| 众智园 TEST | 它能否在声明边界内工作？ | 公众主路径与机器测试路径相交；值守人、物理急停和透明边界控制交叉 | 机器撤出；院落、步行与开放绿地继续 |
| AI原点 RELEASE | 它是否有权被有限发布？ | 方法公开台与权利说明台在撤回界面前交叉；发布状态与责任人同场可见 | 发布撤下；教学、协作与普通展览继续 |
| 大钟寺 USE | 城市是否愿意继续使用？ | AI服务流与日常生活流在投诉节点交叉；无障碍和人工服务拥有正线优先 | 设备折返；商业、休息、人工服务与无障碍主链继续 |

### 三个真实空间X：同一条正线，三种不同城市冲突

**众智园是机器X。** 连续公众正线穿过日常院落，可封闭机器测试侧袋只经一个有人值守的交叉点接入；观察缓冲、实体急停、透明安全边界、失败记录位和设备撤出路径共同构成TEST。运行前是普通院落，运行中只打开机器侧袋，RETURN后拆除围界并恢复座椅、步行和绿地。[data:visual/assets/station-topology.json#ZHONGZHI_TEST]

**AI原点是权利X。** 贯通首层保持教学、协作、咖啡和社区活动，方法台与权利台在公开发布剧场汇合，人工撤回台位于同一视线内。技术可以HOLD、产品可以撤下，但公共长桌、旁观阶梯和普通展览不随发布状态关闭。[data:visual/assets/station-topology.json#AI_ORIGIN_RELEASE]

**大钟寺是使用X。** 无障碍主链、人工等价服务和非数字路径先形成连续日常界面，AI只进入偏置试用侧袋；投诉点从主链直接到达，设备沿独立接口退出。AI ON、RETURN和AI OFF三种状态下，商业、休息、交通到达和人工服务都能继续。[data:visual/assets/station-topology.json#DAZHONGSI_USE]

![三个真实空间X的平面、剖面与三态](assets/figures/key-areas.png)

### 90天最小试点：先建公共空间，再让AI进入

三站不等待永久工程同时启动，而各自用90天检验一个小而完整的空间原型。D01—14只记录无AI普通使用；D15—45搭建可逆构件并完成撤出、急停和人工服务演练；D46—75才允许有限开放；D76—90根据异议、维护和复原结果决定保留、修改或拆除。面积、实施强度和投资只表达城市设计阶段量级，不是测绘、概算、招标预算或实施承诺。[assumption:A-RESOURCES-001]

| 站点 | 试点面积量级 | 建设与投资量级 | 所需主体类型 | 立即停止 | AI失败后留下的城市资产 |
| --- | --- | --- | --- | --- | --- |
| 众智园 TEST X | 800—1,500 m² | 轻建造级；数百万元级 | 场地运营、产品团队、安全责任、独立观察 | 越界、急停失效、公众正线冲突 | 公众院落、安全模块、失败档案、可复用撤场接口 |
| AI原点 RELEASE X | 300—800 m² | 家具与轻改造级；百万元级 | 场地运营、权利复核、发布责任、社区代表 | 许可HOLD、撤回失灵、普通使用受阻 | 公共长桌、教学空间、开放方法档案、人工撤回台 |
| 大钟寺 USE X | 500—1,000 m² | 家具与轻建造级；百万至数百万元级 | 公共服务、无障碍顾问、维护团队、用户代表 | 无AI服务中断、无障碍受阻、异议未闭环 | 人工窗口、触觉地图、座椅、普通商业与休息界面 |

![三站90天最小试点、退出与保留资产](assets/figures/implementation-roadmap.png)

> **证据边界。** 当前场地几何为临时资料，仅用于概念设计、相对拓扑和包内复算；正式边界、控规、权属、现状建筑、市政、消防、文保、预算、运营主体和审批仍待正式资料确认。所有机器检查只证明提交包内部可重跑，不代表现场认证、产品批准或实施授权。[assumption:A-BOUNDARY-001]
...
票据链只保留“上游票据、当前版本、人工签注、到期时间”四项关联。版本、路线、速度或感知范围变化时必须重新判断上游结论；公众异议揭示新使用方式时，Public Verdict生成RETURN任务，而不是只写“优化体验”。可重跑规则与测试结果统一下沉到专业附录。[data:visual/assets/lived-lifecycle.json] [metric:ticket_hash_chain_target_percent]

![Test Receipt、Release Ticket与Public Verdict的字段、签注人、拒绝条件和返回路径](assets/figures/station-handover.png)

### 90天试点的详细运行顺序

90天试点不追求活动规模，只回答“一个明确版本能否在一个受控空间中，经三站审查后接受一次有限公众使用，并在失败时恢复日常状态”。建议上限是一台已指定设备、一个机器测试口袋和一条始终开放的无障碍公众正线；精确尺寸、距离、速度和人数必须等待正式边界、现场测量、风险分析和批准，不能由概念方案预填。没有候选运营主体、保险、安全责任、无障碍复核和复原资源时，90天倒计时不启动。
...
| D0 | 锁定候选场地、角色、版本、普通使用基线和退出资源 | 启动清单 | 工况、版本、人员或资源不一致 |
| D1—D14 | 只运行步行、无障碍、人工服务、教学、商业和休息等普通状态 | 人流断点、服务班次、投诉与维护基线 | 以模型估算代替现场记录 |
| D15—D30 | 安装可拆围界、公共长桌、人工窗口、触觉导引和设备撤出接口 | 构件清单、材料与维护说明 | 构件侵占公众正线或无法拆除 |
| D31—D45 | 演练急停、发布撤回、投诉响应、设备撤出和普通用途复原 | 五类演练记录 | 无人负责或任何一项复原失败 |
| D46—D60 | 在机器口袋内封闭测试，完成独立复测和权利核对 | Test Receipt与Release Ticket候选稿 | 越界、失联、接管失败或权利不清 |
| D61—D75 | 有人值守的有限公众使用，同时保持人工与非数字服务 | 开场检查、匿名投诉和服务日志 | 人工缺席、替代服务中断或重大异议 |
| D76—D83 | 关闭AI，完成设备撤出、构件清点和AI OFF普通使用复测 | 空间复原与公共连续性记录 | 设备、围界、空屏或维护债务遗留 |
| D84—D89 | 由独立观察、无障碍与用户代表复核利益、负担和未闭合问题 | 去标识化问题清单 | 投诉、差异或失败被遗漏 |
| D90 | 作出保留、修改、拆除或RETURN决定 | Public Verdict与城市资产清单 | 不允许以活动结束代替正式决定 |

90天结束不默认续期。通过只允许同一版本、同一边界和同一用途进入下一次有期限复核；HOLD让责任道岔保持朝向公众正线；RETURN关闭侧线、设备折返并生成新任务；RESTORE只有在构件清点、普通用途复原和无障碍主链复核完成后才结束。真实试点要证明的是值守是否到位、公众是否理解、急停是否真实有效、投诉是否改变决定，以及撤场后空间是否仍然好用。[metric:minimum_pilot_duration_days]
...
三站拓扑只表达节点、连接、AI依赖、无障碍属性、可独立关闭和RETURN后的普通用途，不虚构现场尺寸。空间检查逐站关闭全部AI节点，确认公众主链、侧袋隔离、无AI服务、无障碍独立性和三站差异；结果与修复历史统一下沉到专业附录，不作为现场证据或首层价值主张。[data:visual/assets/station-topology.json] [data:visual/assets/station-topology-results.json]
...
90/180日均是**建议验收门槛而非已实现绩效**。候选主体、现场尺寸、预算、保险、许可和公众接受度仍待确认；合同结构的可重跑检查统一收录于专业附录，不替代现场、安全、无障碍、认证或审批判断。[data:visual/assets/three-station-flagship-contract-results.json]
...
生命周期与三站拓扑继续作为底层证据保存：固定用例记录允许与拒绝路径，以及AI节点关闭后的公共连续性。[data:visual/assets/x-lifecycle-test-results.json] [data:visual/assets/station-topology-results.json]

三站合同和SC-03合成桌面演练记录三站不可互换条件与合成工况响应。这些PASS只证明仓库内规则和概念关系可重跑，不证明真实产品、现场绩效、公众接受、认证、审批或实施授权。[data:visual/assets/three-station-flagship-contract-results.json] [data:visual/assets/sc03-tabletop-evidence.json]
