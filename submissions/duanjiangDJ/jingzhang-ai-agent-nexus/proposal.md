...
信号生命周期让”可停止”成为制度；**信号兼容分区（Signal Compatibility Zoning）**则把三态语言**下沉到用地控制层**，让”智能体能不能进入某个空间”成为规划图上一眼可读的条款——这是 USR 的第三层应用（前两层为：场景信号状态·运营层、节点信号装置·设施层）[depth:urban_signal_regime] [depth:signal_compatibility_zoning]：

| 兼容区 | 智能体权限 | 空间类型（概念） | 人类市民可读到什么 |
| --- | --- | --- | --- |
| 🟢 绿态兼容区 | 常态可达，按开放协议运行 | 京张遗址公园开放段、原点社区公共客厅与开源发布厅前广场、大钟寺产业与商业街区 | 智能体在此为常态；状态可读、可停 |
| 🟡 黄态兼容区 | 需人工交接 / 时段限定后进入 | 众智园受控试验段、大钟寺站轨道与换乘庭界面、校园与医院等敏感界面 | 进入需交接入场；高峰时段人机共操 |
| 🔴 红态兼容区 | 智能体禁止进入 | 众智园安全治理沙盒（红队评测与数据隔离区） | 此空间只由人类运营；智能体不进入 |

三态兼容区作为**城市设计导则的”智能体兼容度”条款方向**（概念），写入用地兼容度控制，待官方控规与现状底数补齐后由专业团队核定，不构成法定控制结论；网关在入口执行分区：智能体入城先按目标空间的兼容状态鉴权，绿态直接进入、黄态转交接入场、红态拒绝并写入审计记录 [metric:green_compat_zone_count] [metric:yellow_compat_zone_count] [metric:red_compat_zone_count]。

分区与”无 AI 等价”原则互为表里：红态与黄态空间本就由人工等价路径完整覆盖，智能体从不是必需项；分区把 DCC 的义务条款（②可停止、③无 AI 等价）从运营建议变成**控规层的可执行约束**，使”数字市民”的准入与退出在规划文件层面可审查、可审计、可撤回 [depth:no_ai_equivalence] [depth:risk_missing_data]。
...
三跃迁与"近期试点—中期更新—长期治理"分期一一对应，并让**信号兼容分区**作为近期试点的可移除装置之一（信号柱 + 兼容度条款）先行验证"分区鉴权—人工交接—红态拒绝"的完整链路 [depth:signal_compatibility_zoning] [depth:pilot_demo_scale]。所有跃迁条件为概念阈值，正式阈值由运营实施阶段以公开程序确定，不构成任何政府实施安排或投资承诺 [depth:phasing_implementation]。
...
| red_compat_zone_count | 1 | known | 红态兼容区数量：智能体禁止进入的安全治理沙盒（红队评测与数据隔离区）[metric:red_compat_zone_count] |
