...
### Signal Compatibility Zoning: Writing the Three-State Language into Land-Use Control

The signal lifecycle turns "stoppability" into an institution; **Signal Compatibility Zoning** then sinks the three-state language **down to the land-use-control layer**, making "may an agent enter this space" a clause readable at a glance on the plan map — the third layer of USR (the first two being the scenario signal-state layer at operations level and the node signal-device layer at facility level) [depth:urban_signal_regime] [depth:signal_compatibility_zoning]:

| Compatibility zone | Agent permission | Space types (conceptual) | What a human citizen can read |
| --- | --- | --- | --- |
| 🟢 Green zone | Normal access, runs on open protocols | Heritage Park open segment, Origin Community public living room and release-hall plaza, Dazhongsi industry and commerce blocks | Agents are the norm here; state is readable and stoppable |
| 🟡 Amber zone | Entry only via human handover / time-slot limits | Zhongzhi Park controlled test segment, Dazhongsi station and interchange-court interface, campus and hospital sensitive interfaces | Entry requires handover; joint human–agent control in peak hours |
| 🔴 Red zone | Agents forbidden | Zhongzhi Park safety-governance sandbox (red-team evaluation and data-isolation zone) | This space is run by humans only; agents do not enter |

The three zones act as the **conceptual "agent-compatibility" clause direction of the urban-design guideline**, written into land-use compatibility control; to be finalized by professional teams once official regulatory plans and current-data baselines arrive, and constituting no statutory control conclusion. The Gateway enforces zoning at entry: an agent is authorized against the target space's compatibility state before entering — direct entry in green, handover entry in amber, refusal with an audit record in red [metric:green_compat_zone_count] [metric:yellow_compat_zone_count] [metric:red_compat_zone_count].

Zoning and the "no-AI-equivalence" principle reinforce each other: red and amber spaces are already fully covered by manual equivalent paths, so agents are never mandatory; zoning turns DCC's duty clauses (② stoppability, ③ no-AI-equivalence) from operational recommendations into **enforceable constraints at the regulatory-control layer**, making a "digital citizen's" access and withdrawal reviewable, auditable, and revocable at the plan-document level [depth:no_ai_equivalence] [depth:risk_missing_data].
...
| red_compat_zone_count | 1 | known | Red compatibility zones: security-governance sandboxes agents may not enter (red-team evaluation and data-isolation area) [metric:red_compat_zone_count] |
