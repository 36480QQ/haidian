...
**The certified time-checker system (education as the operating workforce).** Twelve services checked on periodic windows — where does the workforce come from? Not from hiring a standing evaluation corps, but from the education pipeline itself. The co-build room hosts a 16-week "assessment literacy" course (course card in `visual/assets/timecheck-runtime.json`): student teams adopt one class of civic service, start by re-running this package's three drill suites, then draft baseline entries and tolerances, hold joint review with the professional maintenance role, rehearse one time check, and publicly explain one degrade decision to resident representatives. Graduates are certified as **time-checkers**, staffing Time-Check Points in pairs with the professional role on call; senior checkers join the red-probe maintenance group. A concept estimate: twelve services need roughly 50–100 person-hours of checking per month — the practicum hours of one 40-student course — so **operating capacity reproduces itself every semester through the education pipeline**, which is the real answer to long-term operations. Three guardrails are fixed: minors never staff shifts (the course may be audited); checkers recuse themselves from services they helped build; final adjudication always rests with the professional maintenance role — students replay and record, they do not judge. What students learn is not "using AI" but **setting the passing standard for AI**; revocable attribution and data minimisation follow scenario 04's boundaries, and course delivery requires separate confirmation by education authorities and universities. [source:AGENT-TASKBOOK] [assumption:A-OPERATIONS-001]
