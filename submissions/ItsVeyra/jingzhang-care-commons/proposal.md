...
百工留存站负责采集与著录，万技开局厅把技能转成活动，传承作品场让经授权成果进入公共叙事。撤回采用分层生命周期。**拟议服务标准**（须经法律与数据保护复核）为：活跃/公开副本在 30 天内去索引或删除；搜索索引、嵌入和派生输出在 90 天内重建或删除；缓存/备份按正常轮换在 90 天内到期，并禁止恢复到活跃系统；内容为空的审计墓碑最多 1 年。墓碑只含记录 ID、请求范围、时间戳和完成状态，不含作品或故事内容。记录化法律保留例外可暂停相应删除，但须限制访问、说明依据并在保留结束后继续执行。上述期限不是现行法律或既有承诺。AI 生成摘要、修复图像或字幕都与原作分层显示。
