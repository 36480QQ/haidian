...
| `assets/figures/*.png`（7 张） | 仓库级脚本 `diagnosis/figure-sources/generate_same_corridor_figures.py` 用 matplotlib 从包内 GeoJSON + metrics.json 程序化绘制 | 包内几何数据；渲染时使用微软雅黑字体（位图化进像素） | 微软雅黑：专有字体，著作权属方正，微软随 Windows 分发（见第 3 节） | ⚠️ 评审展示可用；公开再分发建议执行第 5 节字体替换 |
...
| 生成代码（仓库级 `diagnosis/figure-sources/*.py`，不在本包内） | AI agent 编写 | matplotlib（BSD 兼容许可）、Python 标准库（PSF 许可） | 均为宽松开源许可，无 copyleft 义务 | ✅ 可用 |
...
- 使用字体：**微软雅黑（Microsoft YaHei，`C:/Windows/fonts/msyh.ttc`）**，用于 7 张 PNG 图件的位图渲染与 A3/A0 PDF 的子集嵌入。
