...
1. `visual/assets/renders/` 下七张概念意象图，分两批生成，来源如下：
   ...
   - **GPT image 2.0（经用户 ChatGPT Pro 订阅调用）五张**：`render-ren-bridge.jpg`（人字之跃片区鸟瞰）、`render-answer-gate.jpg`（应答门·机器人敲门）、`render-spike-belt-night.jpg`（道钉带夜景）、`render-disclosure-wall.jpg`（算法公示墙）、`render-lesson-gate.jpg`（无墙学期课门）。
   - **两张前版已作废并整条替换，不再随包提交**：旧 `render-ren-bridge` 表现为下承式桁架桥，与正文「桥不做塔、不做拱、不做门架」「桥面之上不叠加构筑物」等五项口径不符；旧 `render-answer-gate` 表现为青铜卷轴纪念门，与正文「46 座社区出入口」的定义不符。作废原因在此写明，以免读者从早期版本产生误解。
...
2. `assets/media/narrative.mp4` 概念短片及其海报帧 `narrative-poster.jpg`——五段静帧均由 Lib Image 生成；其中四段（道钉带、鸣钟之门、人字之跃、应答门）经 Seedance 2.5 图生视频生成动态画面，各 10 秒，应答原点一段为静帧缓慢推镜、未经视频模型，8 秒；另有 Python 绘制的片头片尾与字幕条，全片 55 秒。
