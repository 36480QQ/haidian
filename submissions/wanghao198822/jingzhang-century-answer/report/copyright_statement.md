...
1. `visual/assets/renders/` 下七张概念意象图，分两批生成，来源如下：
   ...
   - **`b3d-poster.jpg` 的已知形式差异（据实声明）**：图中两端折返坡道为**上下叠置**，而正文口径是「两段坡道在平面上并排展开、不上下叠置，每侧只设一处折返」——该图属气氛与意象表达，形式一律以断面 ZZY-1／ZZY-2 与正文为准。此前该位置为确定性脚本绘制的正投影技术图，本轮按效果优先替换为概念意象图；绘图脚本 `build-staging/make_b3d_form.py` 保留在仓库外的构建目录，不随包提交。
...
全部提示词由本方案自行撰写，依据为本包 `proposal.md` 正文口径；除上述已注明的 CC 许可参考图外，未使用任何第三方图像、影像、音频或受版权保护的素材作为输入。AI 生成画面仅用于表达空间意象，**不作为形式、尺寸、材料、树种或工程做法的设计依据**；凡与正文口径不一致之处，一律以断面图与正文为准，已知差异已在 `narrative.vtt` 与 `narrative-transcript.md` 中逐条声明，`b3d-poster.jpg` 的折返坡道形式差异见上文第 3 条。
