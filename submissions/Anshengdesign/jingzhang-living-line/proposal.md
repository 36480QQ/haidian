...
**体检结论转设计路径（依据诊断篇，不重复分析）**：诊断篇的四把尺子指向同一个方向——骨架健康（D=1.746、H=1.556）、连接被切（λ 十对核心配对仅一对过线）、职住错配（匹配率 46.2%、45 个欠开发单元、纠缠法则见 LUTI 专章）、中间层未度量。**空间不是画出来的，是由这些看不见的动力学支撑的**：λ 给出“缝哪里”（七个断点）、句法给出“缝完之后谁受益”（大钟寺 +70%）、LUTI 给出“补什么、怎么补”（成对补位、混合楼宇）、CA 给出“为什么必须干预”（不干预=路径锁定）。据此，总体结构因此是一条线、三个折、两扇翼、七个点。用地分区不是我们拍脑袋画的：400 米网格里，不干预的街区保留 POI 认出来的现状特征，站域和主脊两侧才做更新干预——科研 31.5%、商业 25.6%、教育 18.2%、居住 16.5%、绿地 8.2% [data:geometry/land_use.geojson#LU-001] [depth:overall_spatial_structure]。法定容积率、高度、密度、退线未公开，全部写"待正式数据补齐"，只给概念体量（782 栋、基底密度 17.4%）用于空间讨论，不下逐栋结论 [metric:building_density] [depth:development_intensity_controls]。
