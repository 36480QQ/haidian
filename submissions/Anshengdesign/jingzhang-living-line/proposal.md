...
**颠覆概念：职住摩尔纹 LIVE-WORK MOIRÉ——溶解通勤。** 传统城市设计把职住平衡当作"减少通勤"的改良；LUTI 的深度读法允许我们走得更远：把 45 个"欠开发"单元（黄金位被低用）重构为**职住比 1.0±0.2 的互锁单元**——办公与居住在同一 400 米单元内耦合，通勤从"被缩短的负担"变成"被溶解的类别"。实算：现状通勤自足率 78.0%（Σmin(职,住)/Σ职，职住为 POI 代理），互锁重构后 89.5%（+11.5pp）——**这条带的目标不是"少通勤的城市"，是"不需要通勤的城市"**；每多一个互锁单元，就少一条跨单元通勤流。三条线合起来才是完整的 LUTI 证据链：错配四分图（45 欠开发/4 过密/42 匹配）说"哪里错"、用地-交通匹配率 46.2%→61.5% 说"缝上之后对多少"、溶解通勤 +11.5pp 说"重构之后通勤还剩多少"。职住为 POI 代理口径，官方人口与岗位数据到位后重算。
...
| 表达完整性 | 强 | 22 章双语 + 43 组图 + 9 张效果图 + A3/A0 + 微信版 |
