...
**【诊断环·第 3 步：功能流体检——LUTI 评价（土地利用-交通循环互动的完整评价）。】**

LUTI 评价的第一问不是"画了多少用地"，而是**"用地强度是否与交通可达性匹配"**。本方案对 91 个 400m 单元做三件套评价（全部实算）：

**① 职住平衡指数 JHR=就业/(就业+居住)**：现状均值 **0.393**（就业主导），落在健康带 0.4–0.6 的单元仅占 **38.4%**——职住失衡是系统性病，不是局部病 [metric:land_use_area_0701_sqm]。

**② 用地-交通匹配度（LUTI 错配诊断）**：现状 **45 个单元"欠开发"**（站点/主脊可达性高但用地强度低——高可达的黄金位被低强度使用）、4 个单元"过密"（高强度楼宇卡在低可达位）、匹配率仅 **46.2%**；七节点缝合后匹配率升至 **61.5%**——**缝合把 LUTI 循环接通了 15 个百分点**。

**③ 四部门流（SIMULACRA E→P→R→M）**：就业 727、居住 491、零售 1100、本地服务（Putnam 式）——通勤流束图显示主流集中在学院桥-五道口走廊（就业的"嘴"）。

![LUTI 评价：职住平衡 × 用地-交通匹配 × 通勤流](assets/figures/c15-luti.png)

**怎么读这张图**：左=错配地图（绿=欠开发、红=过密、灰=匹配）+ 紫色通勤流束；中=缝合前后匹配度（46.2%→61.5%）；右=职住平衡带占比（38.4%，健康目标 60%）。**一句话结论：现状是"黄金位被低用、好楼被错放"的错配格局，缝合+补位就是修正这张图。**
...
**三区量化图（每区：POI 功能结构 + 句法锚点 + 指标卡 + 更新抓手）**：

![众智园量化分析](assets/figures/keyarea-zhongzhiyuan_ai_acceleration_area.png)

**怎么读这张图**：左=POI 功能结构（短板文旅/教育/金融一目了然）；右=句法锚点学知园 84.0%（全栈段是"高选择度枢纽"，过路多、停留少——补公共空间让它从"过路"变"停留"）。指标卡：多样性 0.847、供职比 0.747。

![原点社区量化分析](assets/figures/keyarea-beijing_ai_origin_community.png)

**怎么读这张图**：左=POI 结构（医疗 0.99% 为三区最低）；右=清华园车站遗址整合度 **93.5%——原点不是我们命名的，是结构自己长出来的**；指标卡：多样性 0.863、医疗密度 2.88/km²。

![大钟寺量化分析](assets/figures/keyarea-dazhongsi_ai_industry_cluster.png)

**怎么读这张图**：左=POI 结构（三区最均衡）；右=大钟寺站整合度 20.2%——句法末梢，**必须靠四象限缝合与功能激活补配置的短板**；指标卡：供职比 0.923、医疗密度 31.92/km²。

![三处重点区域索引与设计任务图](assets/figures/key-areas.png)

**怎么读这张图**：三张小图是三张"诊断处方单"——上方定位、中部策略、底部红字为 POI 缺口清单。**缺什么补什么，不是设计院觉得该有什么。**
...
1. **病**：这条带的骨架是健康的（分形维 D=1.746、健康因子 H=1.556），但它断了——主脊被 7 处干道切断、三区两翼 5 核只有 2/5 连通、45 个单元"黄金位被低用"、职住平衡带仅 38.4%、协同博弈锁死在"各自为战"。
2. **药**：全部干预只有一类动作——**缝合、解锁、补位、度量**：七节点缝合（整合度 +65%~+132%）、CA 解锁（42 单元再分配）、POI 缺口补位（金融 −63%、科研 −45.9%、医疗 2.88/km²）、中间层度量（可支付性/转化率/普惠算力年度化）。
3. **效**：三个模型验证疗效——SIMULACRA 重跑（25 分钟圈就业可达 +15.9%、通勤 −3.4%）、LUTI 匹配度 46.2%→61.5%、纳什均衡从 D,D 翻转为 C,C（η +60%）。**干预不是愿望，是三个模型各自独立收敛的同一个结论。**
4. **归**：超线性收益经五平权机制回到市民（算力券/数据湖/夜校/空间平权/署名平权）——"让曲线向每位市民弯曲"。
5. **尺**：四把尺子（λc=0.42、p_c=59.27%、D=1.6–1.8、H=1.35）与相变论坛年度发布，让这条带**每年可体检、可校准、可证伪**——这不是一张画完就挂墙的蓝图，而是一套能随城市一起演化的城市操作系统。

> 百年前詹天佑在八达岭用"人字形"回答了约束；今天，我们用 15 项可复算的计算回答同一个问题——**约束不是设计的敌人，是涌现的条件**。京张 Hyper Line 的使命，就是让这条百年的线，在可计算的城市科学里，开始上翘。
