...
| Area | Disease (c7 measured [metric:poi_total_count]) | Cure | HOLD condition |
|---|---|---|---|
...
**① The minimal unit of emergence: the SCALE — space as cellular automaton, and the spatial translation of scaling laws.** The scale is not just a design language — it is the spatial organizing principle: each scale is a CA cell obeying local rules (wind, light, temperature), and the global order (starling wave, light-shadow ripple) **emerges** from interaction. Paving is a scale (percolation strip), shading is a scale (facade module), the landmark is a scale (Thousand-Scale Grove), seating is a scale (row arrangement) — four scales sharing the same "cell". **This is precisely the design meaning of urban scaling laws: a fractal city is mathematically multi-scale self-similar (the D=1.746 skeleton already verifies this), and spatially it is the same generative rule nesting from 1m paving to 40m plaza to the 7.9km belt** — the small-scale rule, magnified, is still the rule; energy conducts losslessly along this fractal skeleton. This is the physical realization of "a sublinear skeleton carrying a superlinear soul". The spatial recursion is here: given the scale's rules, users can compose new spatial events themselves — the scenario cards on the spine are not "placed" functions but events "grown" from the scale's rules. The motif decorates no surface — it is the touchable form of the mechanism itself. Iron rule: scales appear only where a mechanism has semantic meaning, never pasted onto unrelated facades.

**①b The spatial carrier of emergence: self-organization reserves — places deliberately "not designed".** Emergence cannot be designed; it can only be given room. This plan deliberately leaves blanks at three scales: **street scale** — six "undefined interfaces" of 30–50m each along the spine (no commerce, no fixed function, only scale-pattern paving and basic lighting); residents and entrepreneurs may apply for 90-day temporary use (furniture / pop-ups / experimental installations, open admission rules). **Block scale** — one "blank ground" per area (1,500–3,000 m² hardened vacant lot with no planned function, only water/power stubs), its use decided by annual community assembly. **Building scale** — 15% of new-build floor area as "unleased elastic layers" (structural load reserved, services to the floor, demountable partitions), uses determined by market self-organization. **These blanks embody the "self-organization reserve" principle — the designer's restraint is itself the design**: we designed 85%, leaving 15% for emergence. CA simulation verifies: without intervention the path locks (0 change) — it is precisely this 15% of blank space that gives the system the physical possibility to flow again [metric:ca_intervention_change].

**①c The physical carrier of superlinearity: seven phase-change points = seven growth poles in gradient design.** Superlinear growth (β>1) does not unfold uniformly along the line — it concentrates and bursts at **phase-change points**, just as percolation's giant component suddenly connects near the critical threshold. The seven stitches are this belt's seven phase-change points: within 300m of each stitch is the "growth-pole core" (CA-verified commercial clustering); between cores are "conduction segments" (kept low-intensity, awaiting core spillover). **Gradient design**: stitch core (0–100m) allows maximum intensity and mix; transition zone (100–300m) decreasing intensity; conduction segment (300m+) maintains the existing fabric. This is not uniform development — it **focuses the growth potential of the entire 11.41 km² onto 7 points for ignition**, letting superlinear growth spread from the phase-change points outward. Percolation computation verifies this logic: the seven-point stitch jumps the giant fraction from 0.840 to 0.904 [metric:percolation_giant_fraction_design] — the system's connectivity changes qualitatively; that is the physics of phase-change points.
...
- EI0 = 0.795 [0.775, 0.848] yellow → EI1 = **0.9625 [0.9125, 0.985] all green** [metric:ei1_design_s4]
- Sensitivity: weights ±0.05 → [0.799, 0.818]; inputs ±10% → [0.779, 0.832] (intervals, not single numbers) [metric:ei1_sensitivity_low]
