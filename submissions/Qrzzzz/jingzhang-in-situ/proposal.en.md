---
title: "Jing-Zhang In Situ: Making AI Visible, Human-Takeover-Ready, Verifiable, and Withdrawable"
...
summary: "One in-situ spine, three in-situ prototypes, twelve candidate responsibility cross-sections, and one time-bounded reversible first-use trial turn AI into urban interfaces that people can see, take over, verify, and withdraw."
...
The Phase 2 source freeze records publisher, URL/file, publication or retrieval date, spatial and temporal scope, reuse boundary, acquisition-to-transformation chain, SHA-256, evidence grade, limitations, and prohibited uses for every item. Existing repository snapshots remain governed by their item-level rights records; URL-only cases retain links and paraphrase only. OSM retains only the 2026-08-14 background check’s query/response hashes and the warning that the raw response is absent and not package-replayable. It never enters required design GeoJSON or becomes a surveying claim.[source:DATA-SRC-PROVISIONAL-BOUNDARY-BASIS-20260814] [data:visual/assets/phase2-source-freeze.json#open_data_separation]
...
![F01 Three-Level Scope, Three Zones and Two Wings, Cross-Section Status, and Candidate Regional Interfaces (Provisional Boundaries)](assets/figures/site-overview.en.png)
...
![F06 Jing-Zhang In Situ Identity System and Logo Logic](assets/figures/identity-system.en.png)
...
![F02 KEEP / OPEN / INTENSIFY and Conceptual Land-Use Structure](assets/figures/land-use-structure.en.png)
...
SC01–SC12 share one complete operating contract: purpose and spatial carrier; primary and affected users; allowed/prohibited inputs; source and evidence grade; minimum necessary data; output; AI authority; human-only decisions; frontstage notice; lawful basis or no-collection statement; Human Takeover; low/no-AI alternative; complaint and appeal; logging and audit; retention and deletion; lifecycle; evaluation and review; dependencies; qualitative cost; maintenance; and exit. Machine authority lives in `visual/assets/phase3-protocol-contracts.json`; the prose keeps the review judgment instead of dumping fields. No reachable accountable person, accessible complaint entry, or high-risk closure means no entry or immediate stop.[source:DATA-SRC-AGENT-TASKBOOK-20260518] [depth:municipal_new_infrastructure]
...
The shared lifecycle is `preflight → limited-open → operating → degraded → human-takeover → paused → stopped → removed → archive`. Every transition records trigger, authorized role, required evidence, public status, rollback, and prohibited automation. Its mapping to frozen F10 terms is: preflight = “Conceptual Recommendation / Pending Site Verification”; limited-open = “Controlled Test”; human-takeover = “Human Takeover”; paused = “Paused.” AI may only suggest, sort, translate, assist verification, or prompt. Implementation approval, rights changes, sanctions, resource denial, stop, and reopening are human-only. Default facial/emotion recognition, social scoring, predictive policing, and undisclosed sensitive inference are prohibited.
...
The three flagships expand that same contract into end-to-end sequences and role swimlanes. **SC01 Model Evaluation Sandbox** runs test submitter → test steward → safety/professional reviewer → public observer; source/protocol failure degrades, safety staff take over, and unresolved issues stop and archive. **SC05 Research-to-Application Matching Clinic** runs visitor → service staff → human adviser → appeal reviewer; AI only ranks candidates, voluntary summaries can move to paper/counter during takeover, and leakage, repeated mismatch, or no adviser pauses service. **SC10 AI-Native Product Public Trial Room** runs product provider → on-site host → safety/rights reviewer → public/complaint role; AI never buys, admits, or markets autonomously, while harm, rights issues, misleading claims, absent handover, or unreachable complaints trigger takeover, stop, and removal. Their normal, degraded, takeover, and removal paths use one lifecycle without adding buildings or product schemes.
...
![F04 North–South Spine, East–West Cross-Sections, Blue-Green Mobility, and Public Experience Routes](assets/figures/mobility-bluegreen.en.png)
...
![F09 Three Conceptual Landmarks, the Open Contribution Ledger, and Public-Space Components](assets/figures/landmarks-components.en.png)
...
![F10 Jing-Zhang Cultural Narrative, Six-Level Wayfinding, and International Communication](assets/figures/culture-wayfinding.en.png)
...
![F11 shared lifecycle, accountable evidence, complaint/appeal, SC10 + IM06 preflight, stop, and removal](assets/figures/operations-pathway.en.png)
...
![F05 Metric Authority, Recalculation Chain, and Implementation Gates](assets/figures/metrics-evidence.en.png)
