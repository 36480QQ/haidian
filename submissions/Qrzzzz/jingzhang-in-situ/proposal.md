...
“京张”锚定铁路遗产、空间连续与公共记忆；“在场”要求人能进入、证据能追溯、责任能接管。品牌合同规定英文规范名唯一为 **Jing-Zhang In Situ**；F06 的 Logo 图形可把连字符省略为纯图形排印变体，但正文、元数据、标题和对外名称不得写成无连字符的第二个项目名。该名称强调基于现场和就地修正，不暗示已经实施。F06 以两条轨迹、十二刻度、三个节点和开放弧线表达一带、十二横断面、三区两翼及可修订接口；不使用企业商标或未授权铁路标识。[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:overall_spatial_structure]
...
SC01—SC12 共用同一份完整运行合同：目的与空间载体、主要及受影响用户、允许/禁止输入、来源与证据等级、最小必要数据、输出、AI 权限、必须由人决定的事项、前台告知、合法依据或不采集说明、Human Takeover、低技术/无 AI 替代、投诉申诉、日志审计、保留删除、状态、评价复核、依赖、定性成本、维护与退出。机器权威位于 `visual/assets/phase3-protocol-contracts.json`；正文只保留评审判断，不倾倒字段。没有可到场责任人、可达投诉入口或高风险闭环，即不进入或立即停止。[source:DATA-SRC-AGENT-TASKBOOK-20260518] [depth:municipal_new_infrastructure]
...
共用状态机只使用 9 个连字符 ID：`preflight / limited-open / operating / degraded / human-takeover / paused / stopped / removed / archive`，并以 15 条权威边运行，不是一条可自动逐级前进的箭头链。每条边都写明双语触发条件、可执行规范角色、必留证据、公众状态、回退和禁止自动事项；F10 冻结词对应为 preflight＝“概念建议 / 待现场核验”、limited-open＝“受控测试”、human-takeover＝“人工接管”、paused＝“暂停服务”。同一实例只有 `paused→limited-open` 可由具名服务责任人与安全专业角色共同批准限量复测；`archive` 是当前 `run_instance_id` 终态，没有 `archive→preflight`，未来使用必须新建实例从 preflight 重新取证。AI 只可建议、排序、转译、辅助验证或提示；实施批准、权利变化、惩戒、资源剥夺、停止与复测只能由人决定。禁止默认人脸识别、情绪识别、社会评分、预测性执法和未说明的敏感推断。

三项产业测试验证编号继续锁定：TVS-1＝SC01 模型评测，TVS-2＝SC02 能效与热安全，TVS-3＝SC03 具身智能无障碍共测；3+9 的竞争性层级不删除这组三项专业验证义务。

### 十二张场景卡

三旗舰把同一合同展开为端到端时序和 12 条可解析泳道。**SC01 模型评测沙盒**由外部提交者→数据治理责任人→安全专业 Gate→受影响公众运行；来源或协议失效从 operating 经 degraded 接管，问题未解则 paused→stopped→removed→archive。**SC05 成果匹配诊所**由受影响用户→现场人工服务→场景服务责任人→独立申诉复核人运行；AI 只排候选，自愿摘要可转纸面/柜台，泄露、持续误配或无顾问即接管或暂停。**SC10 AI 原生产品公共试用室**只到 limited-open，由外部产品方→现场主持→安全专业 Gate→投诉闭环责任人运行；伤害、权利、误导、无人接管或投诉不可达即 human-takeover→stopped→removed→archive。三者的正常、降级、接管与撤场路径全部由同一 15 边合同组成，不新增建筑、产品或 pilot。
...
数据治理按六类分开：公开/仓库证据、运营非个人数据、个人数据、敏感个人数据、模型推断/评分、日志与申诉材料。每类登记目的、最小字段、收集/控制/运营角色、权限、保留删除、本地/远程边界、审计、申诉、禁止用途和证据状态；12 个场景画像以可解析继承加场景 override 闭合这些控制，12 个 controller 仍为 unknown。九个核心路径明确 `no personal data required`；七条且仅七条窄条件路径是 SC04 署名/回复、SC05 用户请求引荐、SC06 预约确认、SC07 人工回访、SC10 投诉回复、SC11 可识别口述贡献、SC12 人工回复，全部须先完成法律审查，绝不表示核心服务必须采集个人数据。敏感个人数据必需项为 0 并默认禁止。每项投诉都保留非 AI 入口、回执、独立人工复核、结果告知和由不同人员承接的再申诉；投诉不可达即 Human Takeover 或停止。老年人、残障人士、儿童、非智能设备用户、非中文用户及拒绝 AI 者均保留等效服务、非数字入口、人工解释和退出；这是一项 `design-target` 服务合同，不宣称已配备人员或已完成法律/无障碍审查。[source:DATA-SRC-BARRIER-FREE-ENVIRONMENT-LAW] [source:DATA-SRC-ELDERLY-SMART-TECH-PLAN-2020-45]
...
前台运行路径采用共用状态机；公众首先看状态牌与人工入口，责任角色再核对触发、证据和回退。投诉/申诉可从柜台、纸面、电话或可访问数字入口进入，形成记录与回执，转独立人工复核，再决定纠正、降级、暂停、停止或撤场并告知结果；任何数字响应窗口均为 `design-target`，不是法定期限或既有承诺。参与者后续路径止于自愿的候选转介，不承诺岗位、资金、采购、空间或背书。
...
**唯一首用试验**仍只绑定 **SC10＋IM06**。`preflight` 必须先留存书面期限、批准与责任角色、最小数据和禁止数据清单、产品/场地/消费者权益/无障碍/投诉/撤场开放条件、人工值守与低技替代；任一条件未证实即不开。运行中，伤害或近失、权利或身份冒用、误导营销、人工无人接管、投诉不可达、强制日志缺失或撤场失败，均按 `design-target` 阈值立即接管，并由责任人决定暂停或停止。停止后断开服务、封存/删除规定数据、撤除前台状态与设备、归档证据、公开复盘；`archive` 后原实例永不复开，未来只有在书面独立人工/专业复核、全部开放条件重证和新公众告知齐备时，才能创建新的 `run_instance_id` 从 preflight 开始。具体时限、响应窗口与阈值都标 `design-target` 或 `unknown`，不是 SC13、IM14、其他 pilot、法定数字期限、运营承诺或真实绩效。

独立反方台面测试共 16 项，覆盖错误建议、偏差不平等、漂移、身份冒用、恶意输入、断网、断电、传感/接口、无人接管、无障碍、语言误导、投诉不可达、撤场失败、日志不完整、权利撤回与权限越界；12 项保留 `fail_stop`、`unknown_stop` 或 `fail_pause` 负结果，未把测试全部写成通过。每项都追溯到受影响 SC、协议字段、F08/F11、正文和矩阵；残余风险与复测条件仍在 `visual/assets/phase3-red-team.json`，不把台面推演冒充现场、安全或法律验证。[depth:risk_missing_data]
...
第三期协议仍保留具名运营主体、个案法律依据、个人数据处理条件、投诉数字窗口、现场人工配置、无障碍/消防/能源/撤场可行性为 `requires legal review`、`design-target` 或 `unknown`。`controller`、`operator` 和专业角色均是责任占位而非既有委任；来源仅支持背景边界，不能证明合规、同意、政府/企业合作或资金。
