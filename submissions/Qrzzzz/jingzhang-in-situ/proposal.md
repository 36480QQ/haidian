...
共用状态机为 `preflight → limited-open → operating → degraded → human-takeover → paused → stopped → removed → archive`。每个转换都写明触发条件、可执行角色、必留证据、公众可见状态、回退路径与禁止自动执行事项；它与 F10 冻结词的对应为：preflight＝“概念建议 / 待现场核验”，limited-open＝“受控测试”，human-takeover＝“人工接管”，paused＝“暂停服务”。AI 只可建议、排序、转译、辅助验证或提示；实施批准、权利变化、惩戒、资源剥夺、停止与复开只能由人决定。禁止默认人脸识别、情绪识别、社会评分、预测性执法和未说明的敏感推断。

三项产业测试验证编号继续锁定：TVS-1＝SC01 模型评测，TVS-2＝SC02 能效与热安全，TVS-3＝SC03 具身智能无障碍共测；3+9 的竞争性层级不删除这组三项专业验证义务。

### 十二张场景卡

三旗舰把同一合同展开为端到端时序和角色泳道。**SC01 模型评测沙盒**由测试提交者→测试管家→安全/专业复核者→公众观察者顺序运行；来源或协议失效即降级，安全人员接管，问题未解则停止归档。**SC05 成果匹配诊所**由来访者→服务人员→人工顾问→申诉复核者顺序运行；AI 只排候选，自愿摘要可在接管时转纸面/柜台，泄露、持续误配或无顾问即暂停。**SC10 AI 原生产品公共试用室**由产品提供者→现场主持→安全/权利复核→公众/投诉角色顺序运行；AI 不自动购买、准入或营销，伤害、权利、误导、无人接管或投诉不可达即人工接管、停止并撤场。三者的正常、降级、接管与撤场路径均落在同一状态机，不新增建筑或产品方案。
...
**唯一首用试验**仍只绑定 **SC10＋IM06**。`preflight` 必须先留存书面期限、批准与责任角色、最小数据和禁止数据清单、产品/场地/消费者权益/无障碍/投诉/撤场开放条件、人工值守与低技替代；任一条件未证实即不开。运行中，伤害或近失、权利或身份冒用、误导营销、人工无人接管、投诉不可达、强制日志缺失或撤场失败，均按 `design-target` 阈值立即接管，并由责任人决定暂停或停止。停止后断开服务、封存/删除规定数据、撤除前台状态与设备、归档证据、公开复盘；只有书面人工/专业复核和全部开放条件重证后才能复开。具体时限、响应窗口与阈值都标 `design-target` 或 `unknown`，不是 SC13、IM14、法定数字期限、运营承诺或真实绩效。
