...
**A conceptual suggestion for regional coordination.** Relations with the Future Science City, Huairou Science City, and the economic development area are better organised around "mutual recognition of verification" than around competition for projects: the belt produces recomputable ledgers and admission criteria, and other districts can reuse the same standard, cutting duplicated verification. The same applies to Beijing-Tianjin-Hebei coordination: agree the standard first, then discuss projects.
...
Cases were chosen for offering a reusable verification mechanism, not for scale. All descriptions rest on public information and contain no unverified investment figures, output values, or policy commitments [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK].

| # | Case direction | Reusable mechanism | Implication for this belt |
|---|---|---|---|
| 1 | Autonomous-driving open test zones (common international practice) | Graded road-test permits: closed ground → limited public road → full area | The direct source of our three rehearsal grades |
| 2 | Phased clinical validation of drugs and medical devices | Phased trials + mandatory adverse-event reporting + withdrawal | "Failure must be reported" is written into the ledger |
| 3 | Aviation's accident and near-miss reporting culture | Blame-free near-miss reporting bought with systemic safety | Rehearsal failure is not punished; concealment is |
| 4 | Continuous integration and regression testing in open source | Frozen test set + rerun on every change + public red/green | The live readings on the Ledger Tower |
| 5 | Urban digital twins and simulation platforms | Try it in the model before trying it in reality | Offline synthetic rehearsal as a grade-1 precondition |
| 6 | Minimum service guarantees in public services | Basic service must not be interrupted whatever the technology | The AI-off baseline as a hard floor |
