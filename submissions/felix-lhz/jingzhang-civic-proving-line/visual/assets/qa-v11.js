...
if(answers.schema_version!=='1.10.0')fail('two-answers schema is not 1.10.0');
...
if(Object.keys(metrics).length!==18)fail(`metric count is ${Object.keys(metrics).length}, expected 18`);
...
for(const rel of ['visual/index.html','visual/index.en.html']){
  ...
  for(const token of ['data-alt="ALT-A"','data-alt="ALT-B"','data-alt="ALT-C"','data-mode="baseline"','data-mode="ai"','data-mode="compare"','NOT FIELD-RUN','decision-data','alternative-data'])if(!html.includes(token))fail(`${rel} missing ${token}`);
}
...
console.log(JSON.stringify({ok:true,schema:'1.10.0',measurement_contracts:12,field_results:0,metrics:18,decision_counts:counts,input_hash:decision.summary.input_hash,scenarios:12,core_figures:10,html:'static-offline-pass',browser_runtime:'user_review_required_file_url_policy',package_mib:Number((bytes/1048576).toFixed(2))},null,2));
