...
本次 V6 深化把方案压缩为五个可交接成果：**一套六步市民智能协议、一个“普通主线 + AI 试验支线”的空间原型、三份首发实施合同、一套首百日 RACI、七项公共权利**。六步协议把 SENSE—ASK—PLAN—ACT—VERIFY—LEARN 分别绑定到空间载体、人工责任、普通等价路径、证据输出和停止条件；三份合同分别绑定三处重点区的牵头建议、审批前置、成本等级、阶段、指标、运维缺口与退出条件；首百日把“谁负责、何时交付、如何退出”变成可核对清单；七项权利保证无设备、匿名、退出、人工复核、状态知情、申诉和无障碍不是口号，而是空间规则。[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:phasing_implementation]

> **一句话交付：AI 可以成为城市支线，但普通公共服务永远是主线；任何智能节点都必须可断网、可停机、可拆除、可由人接管。**
...
图 1 的 OpenStreetMap 底图只承担真实方位识别，不承担红线、面积或现状权属证明；红色纵线是概念公共基线，三处彩色锚点是重点区任务索引。审图时先看真实方位，再看右侧六步协议，最后进入三处重点区合同，可避免把抽象治理流程误读为脱离场地的数字平台。[source:OSM-ORIENTATION] [data:geometry/key_areas.geojson#PROV-KEY-003]
...
### V7 场地锚点校核：三段独立可用，而非一条被承诺的连续步道

以清华园车站旧址、清华东路西口地铁站（15号线）和大钟寺地铁站（12、13号线）作为体验推演锚点。2026年8月21日的高德服务快照给出清华园车站旧址至大钟寺站约 4.13 km、55 分钟的步行估算；因此本方案将文化解释入口、知识抵达界面与日常服务界面设计为可各自成立的公共片段，不将其表述为一条无障碍认证路线、建设承诺或法定空间结论。[source:AMAP-SPATIAL-SNAPSHOT-20260821]
...
V6 不把“覆盖任务”理解为在矩阵里打勾，而把任务书要求压缩成可由专业团队继续工作的六份交接合同。每份合同都说明空间对象、使用者、运营者、证据、停止条件和下一步专业责任；任何尚未获得授权的主体均标记为“建议角色”，不写成既定政府安排。[source:AGENT-TASKBOOK] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]

| Agent 任务 | 本方案的核心交付 | 可核验载体 | 专业深化的第一问 |
| --- | --- | --- | --- |
| agent.1 总体概念与品牌 | 市民智能协议、命名树、标志构形、三定位五功能、三区两翼接口 | 正文、五张主图、品牌规则 | 名称、符号和空间是否形成同一套公众语义 |
| agent.2 创新生态 | 七段创新链、八类要素保障、六个全球案例、五类区域接力 | 案例表、生态链、指标与来源 | 研究如何经过验证进入城市而不越权承诺 |
| agent.3 AI+ 场景 | 八类人物、十二张场景卡、三项行业硬测试、84 条桌面演练 | 场景表、指标、机器回执 | 无 AI、断网和人员缺席时同一任务是否仍能完成 |
| agent.4 公共空间与地标 | 普通主线、八件组件、四处地标、一条 90 分钟公共路线 | 空间图、组件合同、样机剖面 | 地标是否同时可用、可停、可拆、可维护 |
...
**正式主名称：京张市民智能协议；英文：Jing-Zhang Civic Intelligence Protocol；公众短句：城市有智，人民作主；国际短句：A city that can answer, and people who can take over。**“协议”不是软件品牌，而是所有节点共同遵守的公共约定。三处重点区使用同一命名句法：地点名 + 公共任务 + 动词状态；九道城市接口使用“对象 + 接力门”；临时项目使用“问题编号 + 当前 Gate”，从名称上避免把试验误写成建成项目。
...
首百日把八个项目压缩为五个工作包：D0—15 锁定资料和责任；D16—30 完成公众与同任务人工基线；D31—60 搭建一个 1:1 可逆样机；D61—85 运行 84 条桌演并主动测试断电、接管与恢复；D86—100 由具名人类责任方作出继续、修改或退出决定。RACI 必须至少覆盖规划、场地运营、高校、社区、安全/权利与维护六类角色；它是建议责任矩阵，不是假定机构已授权。[metric:first_100_days_work_package_count] [metric:first_100_days_action_count]
...
首发资源采用等级而非虚构投资额：L1 为现状空间内的标识、纸面工单和移动家具；L2 为 3 米可逆样机、临时电源和人工值守；L3 为需要专项设计和审批的半永久节点。未核实场地、运维预算、消防、结构和夜间管理前，不进入 L3。每一项年度运营费必须覆盖人工窗口、无障碍维护、投诉处理、系统停用和拆除恢复，而不只覆盖软件订阅。
