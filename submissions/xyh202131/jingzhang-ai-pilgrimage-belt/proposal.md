...
> **四态结论：** 普通任务先成立；可选验证只在全部前置条件满足后出现；故障仅停止验证叠层，连续日常轨、完整非 AI 路径、人工交接、撤回与申诉继续；恢复先还普通使用，但不等于授权、重启或 G1。48 秒只是展示节奏，不是现场故障恢复时长。

### 已完成的 G0 证明与下一步候选

| 评审现在可核查什么 | 本包给出的单一答案 | 仍不得推出什么 |
|---|---|---|
| **已完成证明** | T-02 无个人信息合成治理回放已完成 1 次：10/10 决策与冻结预期精确匹配，4/4 个声明停止事件精确映射恢复动作，13/13 负向变异控制均按预期 fail-closed。 | 只证明 G0 合同的确定性与拒答优先级；不证明现场、服务、无障碍、公众接受、批准或 G1。 |
| **下一步候选** | 在大钟寺人工服务门户，用既有 `JZ-05 × SCENE-011 × T-02` 形成同一冻结问题集的“人工窗口／静态指南基线—旁侧影子验证—停止与人工接管—独立复测”候选链。 | 这是本轮的**编辑审查顺序**，不是新增项目、选址、优先级、排班、伙伴、获批试点或实施承诺。 |
| **当前阻断** | 责任主体、准确场地、现实阈值、来源覆盖、人工容量、批准、现实服务与独立复测均为 0 或 `unknown`；任一项未闭合即保持 G0 / NO-GO。 | 文档完整、机器 PASS、恢复普通使用或图面清楚，都不能自动生成授权、批准、重启或成熟度升级。 |

![双轨京张公共地面结构：连续日常骨架与六处横向联系串联众智园平行绕行、原点社区一街两院和大钟寺四向通勤三种不可互换原型；旁置验证、人工交接和失败对象均退出普通路径；G0、临时几何、不按比例](assets/figures/site-overview.png)

> **编辑层级：13 个必答章 + 8 个扩展证据单元。** 必答章由“设计依据与资料清单”至“风险、版权与合规说明”的第 1—12 章，以及文末“参考资料”第 13 章组成。本“阅读入口”为扩展证据 E0；第 12 章后、第 13 章前的七个 H3 深读单元为 E1—E7。扩展证据用于展开核验、专业交接和现场替换规则，不替代必答章，也不改变任何 G0、临时边界、未知项、非 AI 路径、权利阻断或专业否决。
...
| 3 分钟 | 已完成的 T-02 合成证明、`JZ-05 × SCENE-011 × T-02` 下一步候选、同一普通任务的非 AI 完整旅程、四态，以及 D01—D08 / H01—H07 为什么仍挡住现实升级。 | 图 02—05、三处换轨场和证据门。 | 候选不是优先级或批准；关系图不是现状平面；机器 PASS 不是现实质量、专业责任或授权。 |
...
**包内导航与评审交接索引。** 本包全部 156 个路径的评审定位统一见 `visual/assets/review-handoff-index.json`：七条阅读路线（30 秒／3 分钟／15 分钟、五步可访问漫游、全部 21 个章节阅读单元、D01—D08/H01—H07 交接、[八问冷读答案地图](visual/index.html#cold-read-answer-map)）与逐文件登记（现行／历史快照／机器输入／冻结／临时五类状态、语言对、轮次来源与权利回链）。`round15-baseline.json` 是历史快照而非现行状态，`t02-g0-g1-replay-fixtures.json` 是合成回放机器输入，两者均不得被误读为当前证据。索引只负责定位，不生成新证据、不改变成熟度或权利状态 [data:visual/assets/review-handoff-index.json]。

图 01 只画**概念空间接力**：连续日常轨先贯穿三座换轨场，间歇验证轨只在旁侧出现，两条供给翼输入问题与专业建议，失败侧线只停止验证叠层；外层虚线仅表示临时设计容器。图中南北是阅读顺序，不表示坐标、距离、站点锚点、批准顺序或建设线位；公开背景定位继续留作现场问题，不与设计关系套准成“看似真实”的场地图。既有 OSM 背景核查所记录的 0% 相交和约 412.5 米间距仍是未裁决的背景差异；`PROV-KEY-003` 仍未与大钟寺站、道路、地块或建筑锚定。geometry 因此保持原位，只把重算触发写清 [data:visual/assets/site-grounding-register.json#JZ-SITE-READING-R17] [assumption:A-SITE-READING-020]。
...
![嵌套空间图集：上部以一条连续公共地面串联三处，下部显示众智园平行绕行与证明庭、原点社区一街两院四撤回点、大钟寺十字通勤与路外服务；三处均先保普通非 AI 路径。](assets/figures/spatial-atlas.png)

**嵌套空间图集读法。** 先读上部一条连续公共地面与三个任务节点，再下钻到三种不可互换原型：众智园用平行绕行保护日常任务并把证明庭置于旁侧；原点社区以居民街连接两院，四个节点可逐点撤回；大钟寺保持十字通勤中心净空，把信息厅与人工台退到路外。故障只停止验证／服务叠层，普通生活、完整非 AI 路径、人工交接与安全离开继续；恢复只还被命名的普通空间关系，不构成授权、批准、G1 或重启。该图是 G0、临时关系、非比例的编辑性概念图，不证明真实地点、已建成状态、无障碍结果、人员、工期、恢复时长或公众意见 [data:visual/assets/spatial-atlas-contract.json]。
...
![三处状态站的差异化概念平面：众智园是平行绕行与旁置证明园，原点社区是一街两院四撤回点，大钟寺是四向通勤与路外人工服务；G0、临时几何、不按比例，不是现场证据、现状建筑、批准方案或无障碍结果](assets/figures/key-areas.png)
...
![季节公共地面剖面：普通通行与安静休憩、遮阴种植、雨洪生态缓冲、人工服务与维护净空、可拆气候验证边依次向外叠加；下部以普通—可选—停止—恢复四态说明只停验证边（G0；实测、设备与责任确认均为 0）。](assets/figures/climate-resilience-corridor.png)
...
| 主张／对象 | 工件 | 核验方法 | 失败条件 | 非 AI 回退 | 建议专业类型 | 当前状态 |
|---|---|---|---|---|---|---|
| D01 官方边界与重点区 | D01 官方 polygon；受影响的 containment、面积指标、全部空间图、HTML、A3/A0 | 核对签发主体、日期、CRS、版本与用途权限；差异比对后以 EPSG:4548 全量复算 | 缺任一权威字段，或正式资料与临时几何冲突而未重算 | 仅保留明确标注的临时 G0 容器，不作精确落位或审批判断 | P01 | `provisional_only`；材料 0；NO-GO |
| D02 现状与竣工测绘 | D02 测绘包；日常路径、首层界面、剖面与恢复基线 | 核对地形、标高、出入口、铺装、植被、设施和开放条件，逐项核实、修正或删除概念关系 | 测绘缺失、范围不全或 before-state 不可追踪 | 普通路径与既有服务优先；可选叠层保持关闭 | P01、P02、P03、P04 | `not_measured`；材料 0；NO-GO |
| D03 控规与权属 | D03 有效控规、权属／使用权、四线和审批台账；受影响的用地、建筑／更新语言、构件位置与阶段声明 | 先做专业冲突审查，再讨论任何精确落位 | 有效性、权利范围或审批条件为 unknown / not_authorized | 保持概念表达，不进入场地、建设或阶段授权 | P01、P04、P05 | `unknown / not_authorized`；材料 0；NO-GO |
| D04 无障碍与同任务共测 | D04 实测路线与分群同意后的同任务共测；连续路径、人工交接、无屏完成与同权结果 | 按群分别复核并改图，不以总体平均掩盖断点 | 任一群体无法完成同任务非 AI 路径，或同意、撤回、人工兜底不闭合 | 恢复完整非 AI／无屏任务与安全人工拒绝；停止受影响叠层 | P02、P06、P07 | `design_intent_only`；现实结果 0；NO-GO |
| D05 交通与高峰连续性 | D05 行人／骑行／公交、愿望线、站点接口与高峰冲突；六接口和大钟寺四向通行 | 复算并压力测试普通通行、活动避让与时段 | 分母、高峰定义、站点接口或侵路冲突未闭合 | 保持普通通勤与安全离开，活动和验证退出连续路径 | P03、P07 | `conceptual`；材料 0；NO-GO |
| D06 专项约束 | D06 铁路、文保、水务、生态、市政、消防、应急、结构、声光等意见；可拆构件、服务边、夜间静音与恢复验收 | 由相应专业在接受范围内复核、附条件、覆盖或删除概念 | 缺专项意见、出现冲突或专业拒绝 | 移除受影响构件／服务叠层，保留普通公共使用 | P02、P03、P04、P05 | `not_specialist_verified`；意见 0；NO-GO |
| D07 责任与运营 | D07 书面责任接受、停机／重启链、替补、预算、维护与退役；公共时刻表、人工在线、事故值班与年度决定 | 核对责任范围、可达联络、覆盖时窗、替补和资源，不以角色模板代替接受 | 任一关键责任、停止权、预算、维护或退役去向未确认 | 不排期、不试用；人工只在真实在线时显示，普通／非 AI 任务继续 | P05、P06、P07 | `role_types_only`；责任接受 0；NO-GO |
| D08 权利与复用 | D08 完整条款、书面同意、ODbL、字体／工具／媒体和逐文件审计；展示、翻译、专业深化与复用输出 | 逐对象、版本、地域、期限和用途核权，并保留撤回与独立审计 | 任一对象未清权、范围不明或撤回链不可执行 | 下架未清权内容；普通通行和同任务服务不得依赖该内容 | P06、P07 | `not_fully_cleared`；独立逐文件审计 0；NO-GO |
| T-02 现实项目／`t02` 合成复演 | `t02-g0-g1-replay-fixtures.json`、`t02-g0-g1-replay-result.json`、T-02 预注册与 H01—H07 | 合成层复核 10/10、4/4、13/13 的确定性 fail-closed；现实层另核来源、人工接管、批准、责任主体、场地、服务与独立复测 | 把合成 PASS 当作现实证据，或任一现实前置条件未闭合 | 同一冻结问题集使用人工窗口或可追溯静态指南；自动化保持关闭 | 按 D01—D08 对应的 P01—P07；未接受责任 | **`t02` 合成复演 PASS；T-02 现实项目仍为 G0 / NO-GO，现实交互、批准与复测均为 0；两者不是同一件事** |

本索引只把 Q8 的闭合路径收束在一处。P01—P07 不是人员名单、已确认专业方或已接受责任；只有未来胜任且接受明确范围的专业方，才可在该范围内复算、修改、附条件、拒绝或删除。新材料还必须保留来源、版本、日期、空间／时间范围和使用权限，逐项触发冻结、替换、复算、复核或退役，并同步正文、GeoJSON/JSON、图件、PDF、三载体、manifest 与权利台账；若与概念冲突，优先保护普通公共权利并停止受影响叠层，不得改写官方或锁定图层迁就方案 [data:visual/assets/implementation-handoff-matrix.json#professional_discipline_matrix] [data:visual/assets/implementation-handoff-matrix.json#official_data_replacement_register]。

#### 七专业交接与停止线

规划城市设计、景观/无障碍、交通、建筑/公共界面、铁路/文保/市政/消防等专项安全、数据/AI/权利、运营/社区七类专业分别接收与其判断相称的 D01—D08 材料；概念图不能代替规划判断，绿线不能代替无障碍合规，OSM 不能代替交通或边界，体量原型不能代替现状建筑，完整模板不能代替专项安全意见，合成 PASS 不能代替数据与服务表现，活动日历不能代替责任和预算。
...
本图继承扩展单元共用边界，仍只覆盖 12 个场景、8 个项目、3 个重点区及既有 geometry / metrics；它不构成正式规划、政府审定、场地、控规、建筑、交通、铁路、文保、市政、消防、无障碍、数据、运营、权利或部署授权。
...
![三处可逆构造与还场图集：众智园拆围护和临时设备但保平行绕行；原点社区撤四节点与院内插件但保居民街；大钟寺关闭路外厅台但保十字通勤；六阶段从证据前置到还场验收。](assets/figures/reversible-components.png)
...
本漫游继承扩展单元共用边界，并定位 12 场景、8 项目、3 重点区与 99 个关闭槽；批准、现场测试、运营、专业责任接受和权利清除均为 **0**。只有未来胜任且明确接受范围的专业方，才可在该范围内复算、修改、附条件、拒绝或删除；重大资料、普通权利或权利缺口继续 NO-GO。
...
八类资料包当前均为 **0 材料 / `null`**；以下每项先写最小进入条件，再写证明上限，避免把同一个空白状态重复八次：

- **D01 官方几何**：须有签发主体、依据、范围 ID、CRS、精度、版本、用途权限和与临时几何的差异报告；即使完整，也不能证明本方案获批、权属、可建性或现场表现。
- **D02 现状测绘**：须有覆盖、方法、日期、精度、标高／出入口／表面／设施和 before-state；即使完整，也不能证明全时段现状、结构安全或普遍无障碍合规。
- **D03 控规权属**：须有有效控规、权利依据、四线、活动范围、条件、期限和冲突台账；即使完整，也不能证明本方案的场地、建设、消防或未来同意。
- **D04 同任务同权**：须有同一任务、路线实测、分群同意、撤回、非 AI 完成、人工兜底和人群差异；即使完整，也不能证明普遍合规、总体接受或未参与人群表现。
- **D05 交通高峰**：须有方式、分母、计数方法、愿望线、站点接口、高峰定义和侵路冲突；即使完整，也不能证明未来需求、道路红线、因果或活动容量。
- **D06 专项约束**：须有专业资质类型、适用范围、规则版本、结论、条件、否决／修改和未决冲突；即使完整，也不能证明全专业放行或施工批准。
- **D07 责任运营**：须有书面责任接受、可达联络、覆盖时窗、替补、停／启权、预算及维护退役责任；即使完整，也不能证明未来在线、志愿劳动充分、服务质量或采购。
- **D08 权利复用**：须有对象／版本、权利人依据、条款、同意、地域、期限、翻译／衍生权、撤回和独立审计；即使完整，也不能证明全包清权、内容真实或权利人背书。
...
本合同继承扩展单元共用边界并覆盖 12 场景、8 项目、3 重点区、99 槽及既有 geometry / metrics；它不是现场任务书、采购单、调查结果、专业意见、审批记录或实施授权。
