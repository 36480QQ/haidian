...
> **一句话判断：** 先以连续日常轨保证任何人不注册、不扫码、不使用 AI 也能完成普通任务；再把间歇验证轨作为自愿、公告、限域、可停止、可拆除的旁侧叠层。三座换轨场处理共创、验证与发布，失败侧线负责停机、人工接管和还场，公共时刻表说明当下能做什么。后台治理仍为 JZ-AIOS、G0—G3、证据门与权利边界。

![双轨京张：连续日常轨、间歇验证轨、三座换轨场、失败侧线、四态与公共时刻表；概念 G0，临时几何，不按比例](assets/figures/site-overview.png)

| 评审第一问 | 当前可核对回答 | 现实边界 |
|---|---|---|
| 普通生活能否先成立？ | 连续日常轨、人工站房、无屏节点、非 AI 完整任务与清晰离开路径先成立。 | 无障碍连续性仍是设计意图，须经实测和共同测试；现场完成数为 0。 |
| 三处是否真正不同？ | 众智园是设备隔离的平行验证庭；原点社区是一街两院四节点；大钟寺是四象限通勤与一厅一台。 | 三处均在临时重点区内表达概念关系，不是现状建筑、精确位置或已批改造。 |
| 失败时普通任务是否继续？ | 自动化先停，人工接管、非 AI 绕行和退出继续；恢复必须先还原普通路径与服务。 | 现实事故、班次、恢复时长、场所验收和重启决定均为 0 或 unknown。 |
| 哪些内容仍不能落地？ | D01—D08 真实资料与 H01—H07 专业判断尚未闭合；重大缺口保持 NO-GO。 | 官方红线、控规、权属、专项、运营责任、预算和逐文件权利清除均未确认。 |

本包保留 12 个既有机器可读场景、8 个项目和 3 个重点区；页首 `scenarios` 仅使用仓库现行 6 个场景登记族作为检索元数据，不删除、不重编号也不升级任何 `SCENE-*` 对象。中英文正文、结构化证据、图件和离线入口共同表达同一结论：**文档完整不等于现场成熟，PR 合并不等于批准。** [assumption:A-JURY-CONVERGENCE-019]
