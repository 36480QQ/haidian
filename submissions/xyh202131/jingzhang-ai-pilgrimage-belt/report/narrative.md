...
PDF 触发审查把 A3 第 13—14 页和 A0 第 7—8 板缺少同一风险摘要与唯一机器入口记录为修复前 RED，并选择 `REBUILD`。四份既有出版物在 14+14 与 8+8 页数不变的前提下原位重建；两次新进程逐文件字节一致，44 页全页渲染、彩色／灰度缩略图和目标页细节冷读均通过，未发现空白、加密、裁切、越界、替换字形、遮挡或层级倒退。出版复现只证明文件级一致与可读，不证明字体许可、现场、无障碍认证、专家接受、批准、清权或成熟度。
...
The PDF trigger audit records a before-fix RED: A3 pages 13–14 and A0 boards 7–8 lacked the same risk summary and sole machine entry, so the decision was `REBUILD`. All four existing publications were rebuilt in place while retaining 14+14 and 8+8 pages. Two fresh processes are byte-identical per file; full-page renders, colour/grayscale contact sheets and target-page detail cold reads pass across all 44 pages, with no blank, encrypted, clipped, out-of-bounds, replacement-glyph, overlap or hierarchy-regression page. Publication reproducibility proves file-level consistency and legibility only—not font licence, field evidence, accessibility certification, expert acceptance, approval, rights clearance or maturity.
