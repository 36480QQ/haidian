...
| Rights / brand | Contributor content is CC BY 4.0, code MIT, the OSM-derived database ODbL and bundled fonts OFL; external pages, standards, cases, marks and repository inputs are not relicensed. The final package has no model-generated media. Component licences permit reuse of this exact version, but prove no factual authority, approval, field result, accepted professional duty or trademark registrability [data:visual/assets/rights-clearance-ledger.json] [data:visual/assets/submission-use-rights-matrix.json]. |
