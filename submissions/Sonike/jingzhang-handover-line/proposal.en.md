...
**But this stretch has a problem that has to be stated head-on: the waiting area cannot rely on the street trees in band E for shade.** One pass of arithmetic makes it clear. At Dazhongsi A ＝ 8.5 m (A1 3.5 ＋ A2 3.0 ＋ A3 1.5 ＋ 0.5 slack), and between the outer edge of A1 and the inner edge of E lie A2 3.0 ＋ A3 1.5 ＋ B 0.6 ＝ **5.1 m** (C and D are zero here); a 1.5 m tree pit against E's inner edge puts the trunk about **5.85 m** from A1's outer edge and 9.35 m from the building face. At a latitude of about 39.9°N the summer-solstice noon solar altitude is about 73.5°, so a 10 m tree casts a shadow only about **2.96 m** long — **still nearly three metres short of A1's outer edge**. In the half hour when shade is most needed, the street trees shade their own feet.
...
**All of the above remains a conceptual proposal, and the solar conclusion is qualitative only.** Solar altitude and shadow length are computed directly from latitude and date, with inputs and formula stated in the paragraph above so anyone can recompute; but **this proposal has run no insolation or wind simulation**, so actual shading must be rechecked with professional software once building and tree survey data exist, and the overhang depth and tree-pit position may change then [depth:three_key_area_detailed_design] [standard:MOHURD-URBAN-DESIGN-MEASURES].
...
**Rule three: not one cell fronts the spine directly.** The perpendicular distance from each cell to the spine is 245–414 m, mean 330 m, median 337 m. **"One line threading twenty cells" therefore does not hold geometrically**: roughly three hundred metres of urban fabric lie between the spine and every cell.
...
**These 96 checks and the 48 assertions in the previous section can now be recomputed on the spot by a script shipped in the package.** Every other number in this package — areas, lengths, ratios, the nine-segment section totals, the positions of the twenty renewal cells — can be recomputed directly from `geometry/` and `metrics.json`; **these two alone could not, because the script that produced them lived outside the repository**. That was a hole in this package's own methodology, and it is now closed: `visual/assets/governance/protocol-check-runner.js` is a read-only, offline Node script with no third-party dependencies. It reads the input fixture `shift-ledger-suite.json`, **independently reimplements** the seven rules from the rule statements in `rule-check-report.json` (rather than porting the original generator), runs one baseline and seven defect injections against each of the twelve ledgers, and compares the outcome check by check with the published 96; it also recomputes the four assertions across 12 × 4 = 48 and compares them with `simulation.json`. `node protocol-check-runner.js` exits 0 when everything matches and exits 1 on any mismatch, printing the published and recomputed values for that entry. **Because it is an independent reimplementation, agreement constitutes an independent reproduction rather than a replay.** The injection definitions are documented in the script, and the R4 injection also trips R3 (blocking items still open while the smart layer is pushed to limited trial) — which matches the twelve published records whose `violations` have two elements, and can be checked one by one. **It recomputes protocol logic only; it proves nothing about field performance, safety, compliance or approval: the field rehearsal remains 0/12** [data:visual/assets/governance/protocol-check-runner.js] [metric:offline_takeover_assertion_count].
...
| Standing rounds of measurement and control | Each of the six continuous checks: frequency × number of points or scenarios | **About 856 fixed rounds a year**: HV-02 walking checks, 13 points × monthly ＝ 156; HV-03 paired observation, 12 scenarios × weekly ＝ 624; HV-05 accessibility measurement, 4 points × monthly ＝ 48; HV-06 accompanied testing, 7 scenarios × quarterly ＝ 28. HV-01 and HV-04 are full-census and scale with handover and stop events | Public space management ＋ scenario operations ＋ accessibility professionals (HV-05 is external professional service requiring quotation) |

**These three costs differ in kind, and the third had until now been left off the table.** Handover hours are **per event** — a few hours each, and they do not grow with the number of scenarios. Staffing is **per hour of opening**. Measurement rounds are **per cycle**, and carry a structural feature that has to be named: **HV-03 paired observation is not "one more look" but "one more time doing the thing"** — it requires the same task to be completed once with the smart layer on and once with it off, which doubles the workload of whatever is being observed. That is why 856 fixed rounds a year exceeds the number of handovers being measured (84–600 a year): **measurement outnumbering the thing measured is a necessary consequence of control design, not an estimating error.**

**So is that money worth spending? It is a trade this proposal accepts head-on.** Without HV-03 pairing, "switch off the smart layer and basic service does not degrade" is only a statement; with it, that statement is tested weekly and can be refuted. **856 rounds a year is the price of being refutable.** This proposal does not pretend the expense is absent, and does not book it under "adds no cost".

**The staffing line also needs restating accurately.** This section previously called it "not an expense this proposal adds" — true in establishment terms, false in fiscal reality: the staffed counter is already a target for reduction, and asking for it not to be reduced is asking that money which could have been saved keep being spent. **So the accurate formulation is not "adds no cost" but a fixed attribution of cost**: it belongs to the existing functional budget of public service, and **the arrival of a smart layer does not constitute a reason to withdraw it**. This is the most concrete price in anything this proposal claims, and the last thing that should be glossed over.

**There are therefore three increments, not one**: the three stop-only posts, the doubled workload caused by HV-03 pairing, and the external accessibility service under HV-05. None of the three should be economised — **an independent review post that can be cut is not independent review, and a control observation that can be cut is not a control.**
