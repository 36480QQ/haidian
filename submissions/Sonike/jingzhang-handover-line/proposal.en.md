...
| Figure state | On F/04 the state of that takeover point changes from "smart layer online" to "basic layer alone"; the alignment does not change, only the state label | The state encoding of the twelve takeover points |
...
**This table can be machine-checked.** `files[]` in `manifest.json` holds 83 entries, 82 of them carrying a `sha256` (`manifest.json` does not hash itself); the `rights_inventory` block records the six columns above and points each class at its `files[]` entries. **Every cell in the third-party column reads "none", which is what allows this package to make a direct grant at all** [assumption:A-FONT-001] [depth:metrics_recalculation]。
