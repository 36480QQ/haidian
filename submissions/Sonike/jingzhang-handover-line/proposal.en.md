...
The package therefore **never uses colour as the only channel**: every band of the land-use composition bar carries its code, area and share in print, and the legend gives code and name line by line, so a reader obtains every number without discriminating hue. **The limitation that does remain is stated here**: on the corridor plan the zones are separated by colour alone, and the two pairs above are genuinely indistinguishable under the corresponding deficiency. When official geometry arrives and the figures are rebuilt, those two pairs must be re-picked to ΔE00 ≥ 10 before export. All figures above can be recomputed independently from the values in `jzstyle.LAND_USE_COLORS` using Viénot 1999 simulation plus CIEDE2000 [standard:WCAG-CONTRAST] [assumption:A-CONTRAST-001].
