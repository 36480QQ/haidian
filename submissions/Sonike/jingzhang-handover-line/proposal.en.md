...
### P0 One-Page Runbook: Hour-by-Hour Inputs, Actions, Evidence IDs and Abort Conditions

The previous two sections give six cost packages, six gates and the on-site recording format, but never state how the day itself runs. This section compresses that onto one page. **Every row is derived from fields already in the package — cost packages C1–C6, verification metrics HV-01–HV-06, refusal categories R1–R7, and the four observation classes — and adds no new facts**; `E-01`–`E-11` merely number the acceptance evidence already declared, so it can be checked off item by item. They are not new evidence types.

**The precondition comes first, to prevent misreading: if any one of the six gates is unmet, this table is not entered and the intelligent layer stays off. This is a concept-level work sequence. It is not an authorised pilot, not a government arrangement, and must not be cited as a completed field result — field task status remains 0/12.**

| Window | Inputs | Role actions (types only, never named individuals) | Evidence ID | Abort on the spot if |
| --- | --- | --- | --- | --- |
| Day D−1 | Written proof for all six gates; components C1–C3 delivered | Site asset management type receives components and signs site handover; facility maintenance and on-site safety type checks stability and egress routes | E-01 site handover slip, E-02 component stability checklist (C1, C3) | Any gate lacks written proof; stability check fails |
| Day D 08:00–09:00 set-up | E-01, E-02 | Site asset management type installs the reversible boundary pieces; public service operations type puts out fixed wayfinding and the paper service pack | E-03 intelligent-layer-off task checklist (C2) | Any installation step would require groundworks or drilling |
| 09:00–10:00 pre-opening review | E-03 | Independent accessibility / public-interest review type walks the continuous "staffed window — footway — shutdown entry" route | E-04 severe-breakpoint list (C4, HV-02, HV-05) | Any severe accessibility or safety breakpoint appears |
| 10:00–12:00 first paired handover | E-03, E-04 | Handing-over type submits the minimum evidence; receiving type signs only after independent reproduction; second-review type counter-signs | E-05 handover ledger JSON: object, version, checksum, open items, acceptance decision, responsible role type (C5, HV-01) | One person signs both sides; an open item is asked to close by signature |
| 12:00–13:00 shutdown demonstration | E-05 | Facility maintenance and on-site safety type triggers the shutdown demonstrator and resets it; that piece connects to no live system | E-06 trigger, display and reset record (C3) | Baseline service is interrupted by the shutdown; reset lacks second review |
| 13:00–15:00 no-AI-equivalence paired observation | E-03, E-05 | Public service operations type handles the same matter with the intelligent layer off; the four observation classes are recorded in same-day, same-slot pairs | E-07 paired observation sheet: completion rate, duration percentiles, reachable path, post staffed (HV-03) | Any one of the four equivalence tests fails |
| 15:00–16:00 second handover and refusal drill | E-05 | Receiving type issues one acceptance, one conditional acceptance and one refusal, each classified under R1–R7 | E-08 all three records with written reason category (C5) | The more conservative state is not held after a refusal |
| 16:00–17:00 de-installation | E-01–E-08 | Site asset management type removes the components; data and records accountability type completes paper archiving and clears temporary electronic records | E-09 de-installation receipt, E-10 temporary-record deletion receipt (C6, HV-04) | Components exceed the agreed removal window; temporary records not cleared |
| Day D+1 | E-01–E-10 | Data and records accountability type publishes both positive and negative results together with the revision record | E-11 public duty-log entry (C6) | Only positive results are published |

**The de-installation deliverables are fixed at five; missing any one means the round does not count as complete**: E-09 de-installation receipt, E-10 temporary-record deletion receipt, E-11 public duty-log entry including negative results, the closure record for E-04's severe-breakpoint list, and the full set of E-08's three handover records. **E-10 is the one item that cannot be deferred** — temporary electronic records must be cleared on the day of de-installation, not left for the debrief.

**The six gates as a signable acceptance sheet.** The table above covers how the day runs; this one covers whether the day is entered at all. The six gates had until now been listed in a single sentence, without stating **who signs, what happens on failure, and where the record is filed** — only once those are set out gate by gate does it become a sheet someone can sign, rather than a promise.

| Gate | Pass test | Evidence ID | Failure determination (what happens on the spot) | Filing path | Signing role type |
| --- | --- | --- | --- | --- | --- |
| G1 Written site authorisation | Written authorisation from an entity with the power to give it, covering the actual footprint and time window | E-01 site handover slip | Day D−1 is not entered; the intelligent layer stays off, while fixed wayfinding and staffed service may be prepared independently | Paper original filed + one duty-log entry | Site asset management type |
| G2 Handing-over and receiving roles separately assigned | The two role types are assigned by **different** entities, with the second-review role assigned separately | Responsible-role-type field in E-05 handover ledger JSON | The first handover is not entered; if already under way, that round is voided and restarted | Handover ledger JSON + duty log | Handing-over, receiving and second-review types each sign once |
| G3 Insurance covers the actual form | Public liability and event cover matches the components actually on site | Policy number recorded into E-01 | Set-up is not entered; no component may be placed | Paper policy copy filed | Site asset management type |
| G4 Accessibility and safety review complete | The three-point continuous route walks through with zero severe breakpoints | E-04 severe-breakpoint list and its closure record | The intelligent layer at that point closes (following the HV-05 rule); retest immediately once fixed | Per-point list filed + closure logged in the duty log | Independent accessibility / public-interest review type |
| G5 Three-party quotes and continuing maintenance accountability | At least three reviewed quotations, with the maintenance-accountable entity type and cycle stated | Quotation and maintenance-accountability record, received alongside E-01 | Set-up is not entered; cost remains expressed only as an S-grade category, with no total to be filled in | Paper quotation record filed | Site asset management type + public service operations type |
| G6 De-installation, paper archiving and temporary-record deletion are executable | All three procedures are written and executable **before opening**, not supplied afterwards | Predefined format of E-09 de-installation receipt and E-10 deletion receipt | No window of Day D is entered — **this gate is the most expensive to fail, because it is the only one that cannot be remedied afterwards** | Pre-registered in the duty log, receipts filled in on the day of de-installation | Data and records accountability type |

**The gates and the nine windows are in series**: G1–G6 must all pass before Day D−1 is entered; failing any one stops the process outside the gate, rather than opening first and supplying the paperwork later. Every row here is sourced in the preceding text; this table only reorders it around the act of signing.

**Which metrics this table will later update, and why it updates none now.** Once a first rehearsal is complete, `field_rehearsal_task_count`, HV-01–HV-06, `handover_to_service_days` and `handover_rework_rounds` are updated together, with offline and field evidence still shown side by side and never merged into one count. Until all six gates are in place and the sequence is actually executed, these metrics remain pending, field tasks remain 0/12, and **every unimplemented item in this section stays marked "pending verification"** [metric:field_rehearsal_task_count] [depth:phasing_implementation].
...
| --- | --- | --- | --- |
| Reproduce, translate, adapt, teach or research from it (non-commercially) | **Yes, without contacting the authors** | The rights holders grant that any use complying with CC BY-NC 4.0 is already permitted | Attribute, mark changes, and keep the line "this proposal is an open co-creation suggestion and represents no government decision or approval" |
...
This table is not a fourth signal and claims precedence over no clause: **it simply reorders the precedence set out later (prohibitions → rights holders' grant → machine identifier) around the questions readers actually have.** Every row's conclusion is sourced in the two sections that follow, so nothing here requires the reader to choose between the table and the text.
