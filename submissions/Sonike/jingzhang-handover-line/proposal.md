...
**这一行不是只写在表里：触觉走廊图已随包交付** [data:assets/tactile/tactile-corridor-map.svg]。它是一张可直接用于热敏膨胀纸打印或激光雕刻的矢量图——走廊轮廓、慢行主轴与八条东西支线取自真实几何，三座交接场用**圆、方、三角**三种可触辨形状区分，十二个人工接管点用实心圆点，四处朝圣地标用菱形，**全图不依赖颜色即可辨认**，中英各一份。**凸起高度、点径与间距不写死**：须按 GB/T 15720 与现行无障碍设计规范由制版厂商核定，图内已注明这条边界——本方案给的是结构与信息层级，不是制作尺寸。SCN-05 无障碍路径副驾的无AI兜底写的正是「触觉地图与人工问路」，**现在这张地图是包内可取用的文件，不是一句承诺**。
