...
**三处重点区改以同一比例尺的放大平面表达**：区内的用地分区、连续绿廊、二十个更新类型单元的建筑占位、主轴与东西缝合支线、以及落在区内的场景节点编号，全部按提交几何绘出——**「详细设计」这四个字要由图面自己证明，不能只写在深度矩阵里**。范围仍是临时的，三张卡各自标注 `PROVISIONAL KEY AREA`。
...
**规格能不能被检验，不看「谁负责」，看「没人负责时什么必须停」。** 一份只写「某角色负责某事」的规格，在角色被指派之前无法验证——没有人可以对照它检查任何东西。而「缺岗时什么不许开」是一条**今天就能核对**的规则：它不依赖任何任命，只依赖现场此刻有没有人。八个岗位各自的这一条如下，逐字取自 [data:visual/assets/governance/role-spec.json] 的 `prohibition_when_unstaffed`：

| 角色 | 岗位类别 | 缺岗时什么不许开 | 第二复核角色 |
| --- | --- | --- | --- |
| `R-PUBLIC-SPACE` | 公共空间管理角色 | 该段不得开放智能层服务；仅保留固定导视、常亮照明与人工求助电话。 | `R-INDEPENDENT-REVIEW` |
| `R-TEST-SAFETY` | 受控测试安全责任角色 | 任何受控测试不得启动；机器专用道回到封闭或普通设备位。 | `R-INDEPENDENT-REVIEW` |
| `R-SERVICE-OPS` | 公共服务运营角色 | 不得只提供智能界面；无人工窗口时该项服务不得上线。 | `R-INDEPENDENT-REVIEW` |
| `R-COMMUNITY-CARE` | 社区服务与照护角色 | 不得以自动派单单独运行；无人工兜底时相关场景不得开放。 | `R-DISSENT` |
| `R-CULTURE-VENUE` | 文化与场地责任角色 | 不得展示来源或授权不清的对象；无人值守时智能展项停用。 | `R-INDEPENDENT-REVIEW` |
| `R-EVENT-OPS` | 年度活动运营角色 | 不得举办；已布设的临时设施须在无运营角色前撤除。 | `R-PUBLIC-SPACE` |
| `R-INDEPENDENT-REVIEW` | 独立复核角色 | 任何需双人复核的合并、发布或恢复均不得进行。 | `R-DISSENT` |
| `R-DISSENT` | 异议与被忽略人群代表角色 | 不得新增面向公众的智能场景，也不得扩大既有场景范围。 | `R-INDEPENDENT-REVIEW` |
