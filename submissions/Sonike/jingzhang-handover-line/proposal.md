...
### 「指标」这一项需要单独说清楚：不赋值不等于没有指标

**指标和目标值是两件事。** 本包的 47 项指标里 **36 项已经赋值**——场地面积、绿地率、公共空间占比、主轴长度、场景节点数、用地要素数等，全部在 EPSG:4548 中按 `formula` 与 `source_file` 可独立复算，任何人拿同一份几何都能得到同一个数。

未赋值的 **11 项**分成两类，都不是设计方能定的：

| 类别 | 指标 | 为什么保持 `unknown` |
| --- | --- | --- |
| 法定控制指标 | 容积率、建筑高度、建筑密度、总建筑规模、道路面积与道路率 | 必须由官方控规、城市设计导则、文保条件与交通市政容量共同确定；设计方赋值即是伪造法定结论 |
| 产业绩效指标 | AI创新指数、人才密度、产值规模、更新建筑规模、AI企业目标数 | 需要产业主管部门的目标与真实运营基线；在无授权、无基线的前提下赋值即是编造 |

**但"未赋值"不等于"没管理"。** 这 11 项每一项都在 `measurement-protocol.json` 里写明了十二件事：定义、数据源类型、采集角色、采样单元与范围、频率或触发条件、分母、缺失值处理、质量检查、争议复核、隐私与留存、发布门槛。三条边界写进 schema 而不是散文里——`baseline` 与 `target` 被类型约束为 `null`（不改 schema 就无法塞进数字）、缺失值处理**禁止插值与区级平均替代**、任一发布门槛不成立则该指标保持未赋值。

**发布一套测量协议不等于发布一个目标。** 测量口径是方法，目标值是主管部门的决定；本方案把方法做完，把决定留给有权做决定的人。
