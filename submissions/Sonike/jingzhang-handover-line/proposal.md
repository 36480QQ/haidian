...
“海淀有丰富创新资源”这类判断无法核验，也无法据以取舍。本包纳入一份可追溯到发布机构原页的公开统计，逐条标注统计尺度与不可空间分配，用它收敛设计问题，而不是用它设定绩效目标。[source:HAIDIAN-2025-STATISTICAL-BULLETIN]
...
北段以“模型交付前必须留下可读记录”为核心。空间形成一条公开验证街、共享测试院和临清河方向的低扰动绿色界面。**这两个名字指的是已登记要素的组合，不是另外的几何。** 公开验证街由 SCN-01 模型红队交接台、SCN-02 机器人路权沙盒、SCN-03 端侧算力能耗试验站三个场景节点串起——实测三者都落在本区的粗略范围内 [data:geometry/public_space.geojson#SCN-01]；共享测试院指本区内以「验证工坊」为原型的更新单元 [data:geometry/buildings.geojson#BLDG-017]。**此处不给院落数量，因为几何与正文表达的不是同一件事**：五个验证工坊单元中与本区相交的是 BLDG-017，其余四个分布在走廊其他段落；**二十个更新单元是抽样的类型试验单元，不是完整的建筑清单**，配置意图与抽样表达不应混着读。凡本包命名的空间对象，要么在几何中有自己的要素，要么写明它由哪些要素构成。建筑更新优先利用大开间与首层，布置模型红队交接台、机器人路权沙盒、端侧能耗试验站和标准协作室；物流与受控测试从园区内部进入，公众参观从交接线进入，二者以透明但不可穿越的安全界面分开。每次试验展示适用范围、数据来源、人工负责人、停机按钮和退出原因；缺少企业授权、道路条件和安全论证时不进行实地部署。

**任务书把本片区定位为国家级AI集聚区。本方案不声称该定位已经成立，而是给出够得着它需要先具备什么**：可被第三方复现的验证记录、对外可核验的开放接口，以及不依赖单一主体的共享设施。三者在本片区都落成了空间与规则——公开验证街是第一项，八个共享栈的接入口是第二项，共享测试院与开放研发院是第三项——而不是申报材料里的形容词 [depth:key_area_detailed_design]。

**潜力用地按「证据是否足够」分两档识别，不按权属推测。** 落在本片区粗略范围内的更新类型试验单元共六个，基底合计 63,766 ㎡ [data:geometry/buildings.geojson#BLDG-017]：四个按「保留优先的适应性再利用」处理，可先用空置时段与可移动布置验证需求；两个标注为「须逐栋核查后再定」，在结构、权属、消防与文保核查完成前不进入任何改造判断。**两档的差别不是重要程度，而是现有证据够不够支持动它。**

**「花园型创新街区」在这里是可核算的比例，不是形容词。** 蓝绿基底与本片区相交的面积占该区 **25.5%** [data:geometry/green_space.geojson]，1401 公园绿地贯穿其间 [data:geometry/land_use.geojson#LU-001]；研发院落沿绿地一侧布置可进入的连续首层，测试与物流界面背向绿地，**绿地因此同时是步行空间与安全隔离**，不需要另设围墙。

本片区承担标准制定与安全治理方向的空间需求，因此建筑、绿地与水系按同一套界面规则一体化设计：清河方向的滨水段作为“可解释的绿色空间”，把水质、径流、树荫和设备能耗以固定标识和纸本同时公示，让绿色空间本身成为AI服务的验证场而非装饰背景；清河文化——水利工程、河道治理与沿线劳动记忆——以沿岸记录牌和值班簿方式展示，不改动河道形态。对外交通结合五环路沿线的区域一体化方向提出改善需求与优先级，具体断面、匝道、跨越方式与工程可行性须由交通与市政专业依据官方资料判定，本方案不作结论 [depth:traffic_rail_slow_parking]。
...
**同一次核对把「不使用自造分类」这条纪律从用地图层补到了道路与建筑图层。** 组织方场地包除用地代码外还提供了两套词汇——`brief/site-package/enums/road_classes.json`（9 类道路等级）与 `building_types.json`（13 类建筑类型），`validate_submission.py:1225` 与 `:1231` 都会逐条校验。本包三个图层一律使用它们，与 `standard_matrix.json` 中「不使用自造分类替代」的自设要求一致：9 条道路 `road_class` 为 1 条 `greenway` + 8 条 `transit_connection`；20 个更新单元 `building_type` 为开放研发院 `ai_r_and_d`、验证工坊 `lab`、共享服务栈 `incubator`、社区协作屋 `community_service` 各 5 个 [data:geometry/roads.geojson#ROAD-001] [data:geometry/buildings.geojson#BLDG-001]。
...
四个“朝圣地标” [metric:civic_landmark_count] 不是孤立雕塑，而是可使用、可维护、可撤回的公共制度：北部“交接钟”显示模型版本、适用范围和下一次人工复核时间 [data:geometry/public_space.geojson#LANDMARK-01]；中央“公开交接桌”是一张可举行代码评审、市民质询与成果发布的长桌 [data:geometry/public_space.geojson#LANDMARK-02]；南部“百年值班簿”把铁路维护记录、开发者贡献和城市服务复盘并置 [data:geometry/public_space.geojson#LANDMARK-03]；沿线“零号接管亭”提供实体停用按钮、人工求助、纸本地图和无障碍服务 [data:geometry/public_space.geojson#LANDMARK-04]。荣誉系统不按资本或流量排名，而记录修复漏洞、维护公共代码、提出有效异议和改善无障碍的贡献。
...
实施主体按“谁受益、谁维护、谁能停用”确定，而不是按项目大小分配。但“待主管部门确定”不该是一句空话——一份交不出去的分工等于没有分工。本包把八个角色写成可交接的岗位规格 [data:visual/assets/governance/role-spec.json]，每个角色写明最低资质、能决定什么、不能决定什么、值守方式、无人值守时的兜底、启动权与停用权、需双人复核的事项及其独立第二复核角色、分歧升级路径、交班时必须移交的证据，以及缺岗时什么不得开放。
...
任务书把表达完整度定义为「以人类可直观理解的文本、图像、示意图、表格、场景卡、**视频/声音**、三维或**交互网页**形成完整成果」。本包的多模态表达含四类，文本、图像、静态网页之外还有**声音**——中英各一段三分钟音频导览 [data:assets/media/audio-guide.m4a] [data:assets/media/audio-guide-en.m4a]，讲清判据、三座交接场、最小载体「公开交接桌」与效力边界；再把网页从静态读物变成**可操作的交互网页**（见下一小节）。
...
**⚠ 标记的那一行，代码与部颁指南冲突，而冲突不在本包这一侧。** 本包七个 `land_use_code` 全部取自组织方场地包的枚举 `brief/site-package/enums/land_use_codes.json`——确定性校验 `validate_submission.py` 把该文件当作硬性白名单，任何不在表内的代码直接判 error。该枚举自称是《国土空间调查、规划、用途管制用地用海分类指南》的项目子集（`source_id: SRC-2023-MNR-LAND-USE-CLASSIFICATION`）。**逐条核对后：16 条里 15 条与指南严丝合缝，唯独 `05` 这一条对不上——指南的一级类里 `05` 是湿地，商业服务业用地是 `09`**（其下设 0901 商业用地、0902 商务金融用地、0903 娱乐用地、0904 其他商业服务业用地）[source:MNR-LAND-USE-CLASSIFICATION-GUIDE]。枚举表这一行的中文名是对的、代码是错的，是一处孤立笔误。

**核对方法写在这里，以便任何人重做一遍。** 先看枚举表内部：它同时收了 `0702 城镇社区服务设施用地`（07 居住用地的二级类）、`0802 科研用地`（08 公共管理与公共服务用地的二级类）、`1207 城镇村道路用地`（12 交通运输用地）、`1401/1402/1403 公园/防护/广场绿地`（14 绿地与开敞空间用地）——**这组二级类只可能出自一套一级类序列，而在那套序列里 05 是湿地、09 才是商业服务业用地**；若 `05 = 商业服务业`，07 就不可能是居住用地。再取指南原文代码表逐条确认，两条路径结论一致 [data:brief/site-package/enums/land_use_codes.json]。

**本包选择服从枚举表，并把冲突标出来，而不是自行改成 `09`。** 理由不是嫌麻烦：改成 `09` 会被确定性校验判为「unknown land_use_code」，四道门直接变红，方案连进入评审的资格都没有；而组织方的枚举是全场共用的口径，参赛者单方面另立一套代码，会让各家的用地统计失去可比性。该问题属于组织方场地包的数据缺陷，已按赛事规则走 Issue 反馈（open-city-ai/haidian#2977，含两条独立验证路径与建议的过渡期兼容方案），不在投稿 PR 里改动 `brief/` 或 `scripts/`。
...
**十四道里有十二道在真实使用中抓到过缺陷**——每道闸门的 `caught_real_defects` 字段逐条记着抓到的是什么、属于哪一类。三类值得指出：

- **闸门同样约束本包作者。** `G01` 记录的多数条目来自本包自己写矩阵引用时产生的错引——编造的章节名、不存在的指标键、写成简写的来源 id；**判据不因作者是谁而放宽**。
- **`G11` 记录的是组织方场地包的缺陷而非本包的**：枚举把 `05` 标为商业服务业用地，而部颁指南中 `05` 是湿地、商业服务业用地是 `09`。本包保留 `05` 并标注冲突、走 Issue，理由见前一节。
- **判据本身也会被修订。** `G02` 现在有两句：既查取值是否合法，也查**凡组织方提供了枚举的图层是否真的在用它**——只查非法值的闸门，对「整个字段都没填」是盲的。

**这套闸门的边界要说清楚，否则它自己就成了一种夸大。** 第一，它检验的是包内一致，**不证明设计正确、不证明现场可行、更不构成任何审定结论**；第二，判据可被任何人拿本包与公开场地包复算，但执行脚本不随包分发——投稿目录禁止 `.py`，工具链在仓库之外；第三，它不是持续集成，是每轮改动时按判据逐条跑一遍，**因此它的可靠性取决于是否真的跑了，而不取决于它被写下来**。
