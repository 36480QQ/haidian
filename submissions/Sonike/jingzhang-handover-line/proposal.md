...
用地方案以现行公开分类代码表达，公园绿地为 1401、科研为 0802、教育为 0804、文化为 0803、社区服务为 0702、住宅为 0701、商业服务业为 09。所有分区共同覆盖临时总体范围且不重叠；绿地既是独立用地分区，也是交接线的气候与公共舒适底板。该结果是算法生成的城市设计结构，不能倒推现状地类、权属或规划许可。设计深度由 [depth:land_use_layout] 与 [standard:MNR-LAND-USE-CLASSIFICATION-GUIDE] 控制。
...
| 用地代码与类别 | 面积 | 指标键 |
| --- | --- | --- |
| 09 商业服务业用地 | 2,133,693.511 | [metric:land_use_09_sqm] |
...
**这两块曾经挂在 `05` 下，现已按组织方修正后的枚举迁到 `09`。** 场地包最初的枚举把 `05` 标为商业服务业用地，而《国土空间调查、规划、用途管制用地用海分类指南》中 `05` 是**湿地**、商业服务业用地是 `09` [source:MNR-LAND-USE-CLASSIFICATION-GUIDE]。本包此前保留 `05` 并在正文、图例与页脚逐处标注冲突，同时把问题提给组织方（Issue #2977）；**该 Issue 已于 2026-08-16 由维护者以 PR #3017 修复**——枚举恢复 `05 = 湿地`、新增 `09` 与 0901–0904，两个一级代码均仍为合法值，并说明存量包应由投稿方在后续 PR 中自行迁移。本 PR 即执行这次迁移。

**迁移严格按本包此前登记的复算触发器执行，可逐项核对**：`land_use.geojson` 的 LU-003 与 LU-007 两个要素的 `land_use_code` 由 `05` 改为 `09`，指标键 `land_use_05_sqm` 更名为 `land_use_09_sqm`，F/02 与 F/03 图例、A0 与 A3 色标同步换码；**几何、面积与占比一位数字都没有变动**——两个要素的几何哈希与 `area_sqm_declared` 迁移前后逐条相同，合计仍为 2,133,693.511 ㎡ [data:geometry/land_use.geojson] [metric:land_use_09_sqm]。

**停在一级类 `09` 是有意的，不是漏填。** 指南在该类下设 0901–0904 四个二级类，而这两块是消费、商务与科技服务的混合业态，比例须由控规阶段依实际招商确定 [source:MOHURD-CONTROL-DETAILED-PLANNING]。没有比例依据时挑一个二级类填上，会让表看起来比实际更确定——**停在一级类并写明细分条件，是把「还没定」如实留在表上。**
...
| G11 用地代码对部颁表 | 代码须同时满足枚举与分类指南 | 7/7 一致（枚举 2026-08-16 修正后已迁 09） |
| G12 HTML 离线自足 | 不得有任何外部 http(s) 资源 | 0 处 |
| G13 送审视觉预算 | 须精确等于 18 张 | 18/18 |
| G14 首屏图不被裁切 | 首屏内图件的标题与免责声明须完整 | 已修复，中英均完整 |

**十四道里有十二道在真实使用中抓到过缺陷**——每道闸门的 `caught_real_defects` 字段逐条记着抓到的是什么。其中 `G01` 记录的多数条目来自本包自己写矩阵引用时产生的错引，`G11` 记录的则是组织方场地包的枚举缺陷——**该缺陷已由维护者于 2026-08-16 修复，本包随即完成 05→09 迁移，本闸门这次抓到的是自己「迁移只做了一半」**；`G02` 的判据本身也被修订过——只查非法值的闸门，对「整个字段都没填」是盲的。**本轮又抓到两处，都出在作者自己手上**：G01 在 1025 条引用里恰好只断掉本轮新写的那一条——为 WCAG 条目凭印象编了个不存在的章节名；G05 与本表的 G01 行则第三次被发现自陈落后于实际（154/154→1025/1025、27/27→29/29），原因每次相同：补了内容却没重跑自陈数字。**这类「讲一致性的文档自己不一致」是本包最常犯的一种错**，因此本轮起把两道闸门的重测并入提交前的固定动作，不再依赖记得去做。
