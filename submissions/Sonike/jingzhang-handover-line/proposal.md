...
**十四道里有十二道在真实使用中抓到过缺陷，逐条记着抓到了什么。** 这比闸门的数量重要——一道从没响过的闸门，和没有这道闸门，在结果上是同一件事。其中三条值得单独说：

- **G01 抓到的是本包作者当轮新写的错引。** 为新增三条合规标准补矩阵引用时，一次写错 9 处：编造的章节名、不存在的指标键 `ai_scenario_count`、写成简写的来源 id。闸门当场拦下，改成真实值后才落盘。**闸门对自己人同样生效，这一点才是它有没有用的分界线。**
