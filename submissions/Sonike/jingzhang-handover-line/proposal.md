...
**换嵌带来两处尚未关闭的缺口，在此登记。** 其一，A3 中文本第 6 页的分隔符 `·`（U+00B7）在嵌入子集里没有字形，该行 7 处全部渲染为 .notdef 方框；同一份 PDF 中 `、`（U+3001）有字形、渲染正常。范围已量清：**仅此 7 处，英文 A3 与两套 A0 各为 0**（`pdftoppm -f 6 -l 6 -r 200 drawings/a3-booklet.pdf -`）。其二，四套图纸的中文 `/ToUnicode` 部分错映——渲染正确但文本抽取错字，「产业」抽成「銳釐」、「人才」抽成「鋑㹚」，因此 **PDF 内的中文检索、复制与屏幕阅读器读取都不可靠**。该判断由 poppler、PyMuPDF 与 pypdf 三个独立引擎交叉验证一致，且核验机已安装 `poppler-data` 的 Adobe-GB1 CMap，排除「阅读器缺 CJK 资源」这一解释。两者同源于 v1.11 的字体换嵌；修复触发条件、责任角色与影响文件登记在 `assumptions.json` 的 `A-FONT-001`。**在修好之前，图纸的中文应按「可读、可打印，但不可检索」理解**——无障碍与检索需求请用 `report/proposal.html` 与 `proposal.md`，二者的中文文本是可正常抽取的。
