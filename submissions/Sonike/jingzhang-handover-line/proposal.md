...
北段以“模型交付前必须留下可读记录”为核心。空间形成一条公开验证街、共享测试院和临清河方向的低扰动绿色界面。**这两个名字指的是已登记要素的组合，不是另外的几何。** 公开验证街由 SCN-01 模型红队交接台、SCN-02 机器人路权沙盒、SCN-03 端侧算力能耗试验站三个场景节点串起——实测三者都落在本区的粗略范围内 [data:geometry/public_space.geojson#SCN-01]；共享测试院指本区内以「验证工坊」为原型的更新单元 [data:geometry/buildings.geojson#BLDG-017]。**此处原写「两组共享测试院」，与几何对不上：**五个验证工坊单元中，当前只有 BLDG-017 一个与本区相交，其余四个分布在走廊其他段落。「两组」是设计配置意图而不是几何计数，已改为不给数量——**二十个更新单元是抽样的类型试验单元，不是完整的建筑清单**，正文的配置意图与几何的抽样表达不应混着读。凡本包命名的空间对象，要么在几何中有自己的要素，要么写明它由哪些要素构成。建筑更新优先利用大开间与首层，布置模型红队交接台、机器人路权沙盒、端侧能耗试验站和标准协作室；物流与受控测试从园区内部进入，公众参观从交接线进入，二者以透明但不可穿越的安全界面分开。每次试验展示适用范围、数据来源、人工负责人、停机按钮和退出原因；缺少企业授权、道路条件和安全论证时不进行实地部署。
...
**十四道里有十二道在真实使用中抓到过缺陷，逐条记着抓到了什么**——`caught_real_defects` 字段逐条列出当时抓到的是什么、在哪一轮修的。其中三条：

- **G01 抓到的是本包作者当轮新写的错引。** 为新增三条合规标准补矩阵引用时，一次写错 9 处：编造的章节名、不存在的指标键 `ai_scenario_count`、写成简写的来源 id。闸门当场拦下，改成真实值后才落盘。
- **G11 抓到的是组织方的错误，不是本包的。** 场地包枚举把 `05` 标为「商业服务业用地」，而部颁指南中 `05` 是湿地、商业服务业用地是 `09`。本包保留 `05` 并标注冲突、走 Issue，理由见前一节。
- **G05 抓到的是为它自己写的这一节。** 加上本节后 h3 由 26 变 27，而表格里一度还写着 26/26——**一份讲一致性的文档，自陈落后于自身状态**。已同步，并由此定下一条规矩：自陈型数字必须在每轮结束时重测，不能沿用上一轮的写法。
- **G02 的判据是被这次事故改写的。** 它此前只查「有没有非法值」，于是 `road_class` 与 `building_type` 覆盖率 0/9 与 0/13 一直没被发现——**只查非法值的闸门，对「整个字段都没填」是盲的**。判据现在多了一句：凡组织方为某图层提供了枚举，该图层就必须使用它。
