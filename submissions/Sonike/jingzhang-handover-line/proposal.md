...
因此本包的做法是**不把颜色当作唯一通道**：用地构成条上每一段都印有代码、面积与占比，图例逐条给出代码与名称，读者不依赖辨色就能读出全部数值。**仍然成立的限制写在这里**：走廊平面图上的区块之间只靠颜色区分，上述两对在对应色觉类型下确实不可辨；官方几何到位、图件重制时，这两对取色须先提高到 ΔE00 ≥ 10 再出图。上述数值可用 Viénot 1999 模拟加 CIEDE2000 对 `jzstyle.LAND_USE_COLORS` 的取值独立复算 [standard:WCAG-CONTRAST] [assumption:A-CONTRAST-001]。
