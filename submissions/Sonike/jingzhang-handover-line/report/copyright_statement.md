...
构建溯源：自 v1.9 起全部载体使用同一版本印记，v1.10 已整体重刷为 `JING-ZHANG HANDOVER LINE / PACKAGE v1.10`（图纸另附页码）。此前正文、图纸 v1.6 与图件 v1.0 三套页脚并存的状态已不存在。成果时效以 manifest 的 sha256 与 changelog 为准，不以页脚数字为准。
