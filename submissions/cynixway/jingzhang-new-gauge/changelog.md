...
评审提分深化（69→目标75+），聚焦可实施性(20%)、AI创新性(15%)、公共利益(10%)三大权重维度

- **新增「城市韧性与全状态降级」章节**：把 NG-6 服务契约从六步扩展为七步（新增 ⑦ 韧性态 Resilience），定义 S0 正常/S1 雨天/S2 断网/S3 断电四种运行状态及各自最低服务标准、AI 降级策略、人工/物理兜底路径与恢复 Gate。参考高分同侪(86 分 open-pulse)的全状态降级思维，但用自己的 NG-6 框架重述，不复制其概念。
- **新增「最低后悔优先级方法论」**：不追求每项最高，而是确保最弱体验达标。把 17 子块按 P-确保/P-条件/P-试点三级排序，给出每级的判断标准与实施策略。呼应京张"人字形折返线"工程智慧（最可靠而非最高速）。
- **新增「公平账本（equity ledger）」**：为 6 类人群（老幼/残障/夜间劳动者/低数字素养/开发者/游客）分别记录可达性、热舒适、安全感知、服务等待差异——不用平均值掩盖最弱体验。每项字段标 `design_target`（设计目标），不误读为现状值。
- **proposal.md 字数提升**：新增约 2000 字中英双语内容。
