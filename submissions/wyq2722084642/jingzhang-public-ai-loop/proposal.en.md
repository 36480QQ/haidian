...
#### 5.3.1 Flagship prototype: Dazhongsi Reversible Civic Interface

This iteration develops one flagship in depth instead of spreading equal detail across all three nodes. **1:5000** asks how Dazhongsi connects to the three nodes, civic spine and transit; **1:2000** separates the ordinary public route, ground-floor interface and service route; **1:500** combines the staffed rights desk, reversible pilot bay, evidence-and-appeal strip, quiet waiting, stop point and removal route; **1:200** tests ordinary passage, AI boundary, human takeover, power isolation and full removal in section. These nominal scales structure concept review only: they do not contain official exits, road redlines, title boundaries or engineering dimensions. [data:spatial.json#dazhongsi-ordinary-route]

![Dazhongsi four-scale spatial prototype](assets/figures/dazhongsi-four-scales.en.png)

Eleven stable component IDs are governed by four operating states. **OPEN** runs paper, staffed and offline explanation only. **PILOT** adds AI solely in a rights-cleared, isolated, staffed and stoppable bay. **PAUSE** preserves logs, transfers to staff and resolves appeals after any stop trigger. **RESTORE** deletes demonstration data, removes equipment and returns the place to an empty, reviewed condition. The ordinary civic route remains open in every state; a trial may never occupy commuting or accessible passage. [metric:dazhongsi_operating_state_count]

![Dazhongsi typical section and operating states](assets/figures/dazhongsi-section-states.en.png)

This is not a site-placement conclusion. Official geometry and access, transit interface, title, fire, accessibility, temporary power, public-space operation, data and copyright all remain pending. Until written confirmation exists, on-site work is limited to paper wayfinding, staffed advice and tabletop rehearsal. `spatial.json` records concept objects, while `visual/assets/dazhongsi-operations.json` records scale, components, states, RACI, permits, sampling and handoff so drawing and operation stay linked. [metric:dazhongsi_permit_dependency_count]
...
Operations use RACI rather than an undefined multi-party promise. The public-service operator runs the ordinary route, reception and appeal; the space steward is accountable for access and the annual retain/revise/restore decision; the AI trial operator is responsible only inside the isolated bay; the site safety lead has immediate stop authority; the data-rights lead is accountable for clearance, withdrawal and deletion verification; accessibility representatives, users and transport, fire and privacy professionals are consulted at stage gates. These are roles awaiting assignment, not claims that any existing organisation has agreed. [metric:dazhongsi_raci_workstream_count]

The measurement protocol compares paper self-service, staffed service and optional AI assist. A proposed pilot threshold is at least 30 completed trials per mode, stratified by peak/off-peak, day/evening, no-smartphone use, mobility or vision need, and Chinese/English. Thirty is neither an achieved sample nor an official standard. The denominator includes every eligible entered trial, including incomplete, aborted, fallback and complaint cases; technical failures may not be removed. Measures cover task completion, comprehension, staff-takeover latency, withdrawal, route interruption and appeal closure. A blocked ordinary route, forced account or scan, undisclosed collection, failed withdrawal or unresolved safety event triggers PAUSE immediately rather than waiting for the sample target. [metric:dazhongsi_measurement_modes]
...
![Implementation route from permits to handoff](assets/figures/implementation-roadmap.en.png)

Four handoff packages close the loop: planning and design receive the four-scale atlas, component registry and missing official layers; public-service operations receive the state playbook, RACI, incident and appeal register; data and rights receive the content register, withdrawal test and deletion verification; communications receive bilingual signs, state language and an international brief. If a recipient is unassigned, its work package may not move from P1 to P2.
...
Risk controls are designed so that an AI scenario can fail without excluding anyone from public service. Provisional boundaries use low-contrast dashed styling; every service keeps no-account and human paths; no identity, face, voice or precise trajectory is required; operations include stop, appeal, correction, withdrawal and retirement. `risk.json` scores spatial dispute, policy, privacy, equity, implementation, public acceptance, technology maturity and operating cost, with professional or public review paths for every high-risk item. Only public or rights-cleared material is used, and generated figures are labelled as concepts. The largest evidence gaps remain official boundaries, key-area polygons, title, heritage, roads, utilities, fire, accessibility and operating responsibility. [data:geometry/constraints.geojson#CONSTRAINTS] [source:SOURCE-REGISTRY] [depth:risk_missing_data]
