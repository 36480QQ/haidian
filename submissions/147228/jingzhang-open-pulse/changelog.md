...
- 新增中英双语普通服务与 AI 增益等价回执，把三处重点区、14 条场景、8 项运营动作和 8 类画像接到五步停止与恢复路径。
- 新增离线 runner，4 个负向夹具回到 HOLD，固定 0 授权、0 现场数据、unknown 基线、not_run 结果和官方评分边界。
- 加强 runner 的数组长度、ID 唯一性/顺序、正向路线集合和负向 input→decision 映射校验，加入 3 个离线负向回归，避免重复或未登记夹具伪装成完整通过。
- 将正文已列出的 `brief/public-brief.md` 登记为公开任务书草案，仅用于任务上下文和参赛边界，不升级为正式空间、政策或评分依据。
- 本轮只增加审阅可读性和包内结构连接，不改变正式几何、现场指标、许可、运营授权或历史高水位。
