...
- 统一四份 PDF 的物理页面规格为横向 A3（420 × 297 mm）与横向 A0（1189 × 841 mm），补充 v2.4 版本、作者、社区展示许可和 provisional geometry 页脚。
- 去除早期二进制图册的旧版本外层页脚；所有图件仍保留“展示/QA，不构成红线、审批或实施依据”的边界。
- 补齐英文审阅稿与中文正文的 source / standard / depth / metric / data 证据交叉索引，明确两种语言的引用集合一致；英文交叉索引仍保留正式、背景和 provisional 来源的用途边界。
