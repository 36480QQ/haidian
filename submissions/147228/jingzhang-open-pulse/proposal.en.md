...
| Source-register completeness | The 67 unique source IDs in the Chinese proposal match the 67 records in package `sources.json` one-to-one; five are formal-usable, four are `background_only`, one is `provisional-only`, four are package build-toolchain records, and the rest are repository navigation, case or standard records. Every record provides a title, publisher or registering body, URL/path, access date, authority level, usable scope and disabled scope. `data/source_registry.json` remains authoritative for formal/provisional use boundaries; package registration does not upgrade provisional or background material into formal evidence. | sources.json, data/source_registry.json, visual/assets/evidence-ledger.json, visual/assets/territorial-data-baseline.json |
...
The 67 source IDs, access dates and use boundaries are recorded in `sources.json`. This is a mechanism comparison, not proof that Jing-Zhang has adopted or partnered with any of these organisations. The original six design patterns remain in `visual/assets/case-mechanism-matrix.json` as the local test layer.
