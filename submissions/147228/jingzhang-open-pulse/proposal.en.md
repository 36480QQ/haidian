...
The package uses the repository's maintained provisional geometry where official boundary polygons are not available. `geometry/site_boundary.geojson` and `geometry/key_areas.geojson` therefore remain `official_boundary=false` and `provisional_constraint`. They are suitable for design discussion, visualisation and intake-structure checking, but not for a statutory red line, approval, precise land-area claim or government commitment. The temporary geometry supports structural checking and human reading only; it does not make the package eligible for professional scoring, formal acceptance, publication or merge. Those decisions require maintainer judgment under trusted data and formal gates. When official polygons arrive, all land use, roads, buildings, green space, public space, phasing and metrics must be recalculated.
...
## v2.1 Three Future Lines: Innovation Velocity, Human Capacity, and Social Multiplier

The previous iteration made “people before models, evidence before scale, and reversible failure” into a civic loop. This iteration translates it into three questions that professionals and participants can jointly calibrate: how long an idea takes to meet the right people, space, data, test window and first real user; who can live, care, work and learn inside the belt; and how a technical test returns to an ordinary urban problem instead of stopping at a demo or a valuation. This is not a new statutory planning tier and does not replace missing organiser data. [source:AGENT-TASKBOOK] [source:NIST-HUMAN-CENTERED-AI] [source:UN-HABITAT-PEOPLE-CENTRED-SMART-CITIES]

### 1. Innovation Velocity: make innovation latency a design metric

Industry clustering does not automatically accelerate innovation. The proposal therefore splits idea-to-impact into six recordable nodes: cross-domain match, space access, minimum data permission, first real test, first real user, and first iteration. The three key areas carry trusted testing, open-source transfer and return to urban problems; spatially, the proposal favours many “collision surfaces”—shared food, public-problem desks, open workbenches, booked test windows, release rooms and failure-review walls—over one giant AI centre. These are conceptual suggestions for professional deepening, subject to official geometry, ownership, fire, data and operations review. [depth:overall_spatial_structure]

The three key-area carriers are read back separately: Zhongzhi [data:geometry/key_areas.geojson#PROV-KEY-001], AI Origin Community [data:geometry/key_areas.geojson#PROV-KEY-002], and Dazhongsi [data:geometry/key_areas.geojson#PROV-KEY-003].

To avoid claiming acceleration by slogan, the package adds `innovation-friction-simulation.js` and its JSON output. With fixed seed `147228` and 50,000 draws per scenario, it screens six-stage triangular assumptions. It is not a field measurement, project-duration forecast or causal estimate; later professional work should replace the inputs with booking records, permission logs, test windows and user receipts. Current outputs are:

| Spatial-operating archetype | Median idea→first iteration | P90 | Median reduction vs S0 | Cross-domain match | Social validation |
| --- | ---: | ---: | ---: | ---: | ---: |
| S0 fragmented corridor | 63.313 days | 68.955 days | 0 | 20.108% | 7.832% |
| S1 generic tech park | 35.558 days | 38.571 days | 43.838% | 35.154% | 18.090% |
| S2 Jing-Zhang Open Pulse | 35.096 days | 38.467 days | 44.567% | 54.826% | 35.370% |

The interpretation is deliberately bounded: under these assumptions S2 reduces process friction by about 44.567% relative to S0 while increasing cross-domain encounters and social validation. It does not promise a twofold speed-up, and the S2 model output is not written as current performance. [metric:innovation_latency_s0_median_days] [metric:innovation_latency_s1_median_days] [metric:innovation_latency_s2_median_days]; reduction is read from [metric:innovation_latency_reduction_s2_vs_s0_ratio]; cross-domain matching and social validation are read from [metric:innovation_cross_domain_match_rate_s2] [metric:innovation_social_validation_rate_s2]; model detail: [data:visual/assets/innovation-friction-simulation.json#results]

### 2. Human Capacity: care is innovation infrastructure

If an innovation belt can only accommodate people who have already succeeded, it cannot keep producing the next generation of innovation. “High-quality district” therefore cannot be reduced to premium housing, curated retail or event density. The proposal sets affordable entry, continuous care, no-account usability, night-work safety, intergenerational knowledge transfer and social-validation return as six prerequisites. They map to housing/commute surveys, carers’ time diaries, accessibility and no-app rehearsals, night-route audits, consent and knowledge withdrawal, and the problem–test–receipt chain. All six are currently `unknown`; unknown is not pass. [metric:human_capacity_gate_count] [metric:human_capacity_gate_passed_count] [data:visual/assets/human-capacity-gates.json#HC-01]

This changes spatial priorities: childcare, community health, quiet rest, night food, paper services, maintenance workstations and collaboration tables for retired engineers and residents are not “life-support outside the innovation park”. They are infrastructure that reduces talent time loss, brings ordinary people into the innovation chain and extends the life of knowledge. If an AI scene creates invisible costs for carers, older people, disabled people, night workers or maintainers, it must pause or degrade to a staffed equivalent. [data:visual/assets/human-capacity-gates.json#HC-02] [data:visual/assets/human-capacity-gates.json#HC-04]

### 3. Social Multiplier: both value chains must reach the city

Each industry test should record not only technology readiness (TRL), but also a Social Value Level (SVL) receipt: does it answer a public urban question, use minimum lawful data, pass human review, reach a real user, produce a reusable ordinary-service outcome, and leave complaint, repair, retirement and maintenance records? The current `innovation_social_validation_rate_s2` is only a sampled model parameter, not evidence of a resident problem solved; the six social-return gates still need to be established on site. [metric:innovation_social_validation_rate_s2] [data:visual/assets/human-capacity-gates.json#HC-06]

Long-term KPIs should therefore not stop at enterprise count, devices, finance or attendance. They should record “research result → real urban problem → reproducible test → public receipt → ordinary-service improvement” loops and their failure rate. A technical result enters the social-value ledger only when it leaves a public receipt, a human decision and a maintenance owner; without return, it is not counted as technology-dividend spillover. [data:visual/assets/civic-pulse-protocol.json#stage-06] [data:visual/assets/industry-validation-cases.json#case-01] [source:AGENT-TASKBOOK]

### 4. Next simulation and field boundary

The next round should jointly set the sample, time windows and minimum fields with professionals and participants. Each idea should record only anonymous stage time, whether a cross-domain collaborator was found, whether lawful minimum data permission was obtained, whether a bounded test was completed, whether a first real-user receipt was obtained, and whether the result returned to ordinary public service. Run S0/S1/S2 with the same protocol, then rerun the model. If S2 gains speed by squeezing accessibility, care, privacy or maintenance, the result fails. Once official geometry, networks, ownership, municipal data, resident baselines and operation logs arrive, the JSON, metrics, drawings and HTML must be recalculated together. [depth:metrics_recalculation] [assumption:A-HUMAN-EXPERIENCE-001]
