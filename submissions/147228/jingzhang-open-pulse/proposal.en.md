...
| Source-register completeness | The 59 unique `[source:...]` IDs in the Chinese proposal match the 59 records in package `sources.json` one-to-one; every record provides `source_type`, `usage`, and a URL/path. `data/source_registry.json` remains authoritative for formal/provisional use boundaries; package registration does not upgrade provisional material into formal evidence. | sources.json, data/source_registry.json, visual/assets/evidence-ledger.json |
