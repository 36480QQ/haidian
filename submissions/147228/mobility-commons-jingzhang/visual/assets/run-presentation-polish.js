...
function updateVisualIndex(file, lang) {
  ...
  html = html.replace(/<section[^>]*>[\s\S]*?<h2>(?:一个合成居民的完整经历。去程、受阻、返程和投诉复核|One synthetic resident journey\. Outbound, disruption, return and complaint replay)[\s\S]*?<\/section>/g, '');
  ...
  html = html.replace(/<section[^>]*>[\s\S]*?<h2>(?:校准债务与参数溯源|Calibration debt and parameter provenance|企业与居民公共决策凭证|Enterprise and resident public decision ledger)[\s\S]*?<\/section>/g, '');
