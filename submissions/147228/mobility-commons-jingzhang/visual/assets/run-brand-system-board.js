...
function updateVisualIndex(file, lang) {
  ...
  if (!html.includes('class="brand-system-board"')) html = html.replace('<section id="17" class="evidence scenario-card-board">', `${section}<section id="17" class="evidence scenario-card-board">`);
  if (!html.includes('.brand-system-board')) html = html.replace('</head>', '<style>.brand-system-board{background:#eef7f5}.brand-system-board img{background:#071A2B;border-color:#2A9D8F}.brand-system-board .micro{color:#5E7D88}</style></head>');
