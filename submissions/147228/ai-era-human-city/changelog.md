...
- 修复图 09 的中文项目族表格换行与列宽，补齐图 10 三联卡、四季卡和项目族卡的语言分栏宽度，避免审阅文字越过卡片边界。
- 按 SVG 的 1600×1000 画布重新栅格化八张双语审阅图板，并重建中英文 A3 图册与 A0 展板；源图、PNG、PDF 的版式现在保持同一宽高比。
- `run-ai-era-evidence-boards.js` 增加 SVG 画布和 PNG 像素尺寸回读，后续生成流程会直接拒绝源图与栅格图不一致的版本。
- 只改善包内表达和审阅可读性，不改变 geometry、正式 metrics、来源等级、实施边界或官方评分。
