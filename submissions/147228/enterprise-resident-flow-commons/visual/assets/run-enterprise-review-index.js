...
function render(language) {
  ...
  const sections = c.sections.map((item, index) => `<section id="s${index}" class="evidence"><header><span>${String(index + 1).padStart(2, '0')} · ${esc(item[0])}</span><h2>${esc(item[1])}</h2></header><p>${esc(item[2])}</p><img src="${item[3]}" alt="${esc(item[4])}"><footer>local artifact · claim boundary retained · ${index === 0 ? 'reciprocity review surface v2.1' : 'provisional / synthetic evidence as labelled'}</footer></section>`).join('');
