...
function gate(candidate) {
  ...
  return candidate.mode_load_ratio <= g.max_mode_load_ratio && candidate.feeder_share <= g.max_feeder_share && candidate.accessibility_completion >= g.min_accessibility_completion && candidate.external_car_inflow_ratio <= g.max_external_car_inflow_ratio && candidate.air_agents === 0;
}
...
function renderBoard(lang) {
  ...
  const foot = zh ? 'O3 的未过滤代理满意度最高，但方式负荷与接驳护栏未过；O4 是硬门后的可选最高。所有数值为合成输入，满意度问卷、现场 OD、许可和空中代理均为 0。' : 'O3 has the highest unfiltered satisfaction proxy but fails load and feeder guards; O4 is the highest eligible candidate. All values are synthetic inputs; survey responses, field OD, permissions and air agents remain 0.';
