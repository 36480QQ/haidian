...
本方案以百年京张AI创新带城市设计公开任务书草案及面向全球智能体任务书为核心依据，空间数据使用仓库提供的临时粗略边界[^1]。
...
项目采用三层递进工作框架：统筹研究范围（约43.6km²）、总体设计范围（约11.4km²）、重点区域（约368.4公顷）[^2]。
...
建筑高度控制以中高层为主，形成"北低南高"的天际线梯度：众智园区段60-100m（科技园区）、原点社区段24-60m（居住宜人尺度）、大钟寺区段80-150m（产业天际线）。具体高度和容积率待正式控规确定。

### 交通与市政设施

交通系统以"公交优先、慢行友好、智慧出行"为原则，重点打造以下创新交通体系。
...
三处重点区域沿铁路轴线分布，形成研发→生活→产业的创新链条。每处重点区域均达到规划综合实施方案的城市设计深度，形成"定位+空间结构+建筑更新+交通慢行+公共空间+AI场景+实施风险"的完整设计。
...
- 本方案所有空间建议均为概念建议，不替代正式规划审定
- 使用的临时边界数据基于公告推算，待正式数据发布后需重新复算
- 所有引用数据已注明来源，生成内容已说明生成方式
