...
方案由 2Origin / 本象协议体系提供方法，但不要求城市采用单一厂商平台。“本象”保存稳定对象、关系、状态、约束与来源；“引用优先”使各方持有带版本和新鲜度校验的引用，而不复制另一份真值；投影把同一对象生成居民端、规划端、施工端与 Agent 端可重建的影子；“影核”把同一语义动作路由到多个执行器，并处理前置条件、失败回退与撤销；“叠象 / Redline”公开变更前后差异、证据与验证结果；“影域”提供隔离试错空间；“本境”保存 detector、rule、adapter 和版本组成的环境指纹，回答同一事务为何能否被复现。AI2Work 在本方案中是城市任务评测扩展，不冒充本象核心协议；提交编译器负责解析语义事务、检查约束、更新状态、留证并重新投影。
