...
三处重点区以三座信标组织，共用"先公开、后运行"的门槛：每个 AI 场景进入前都要回答"为谁服务、使用什么数据、谁来复核、失败如何退出、公共收益如何留下" [depth:three_key_area_detailed_design]。

![三座信标：验证（黄）、开源（绿）、体验（红蓝）与信号脊连接](assets/figures/key-areas.png)

### 众智园验证信标｜AMBER｜全栈自主验证区
