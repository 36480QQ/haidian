...
版权与合规：全部文本、图形、GeoJSON、指标、图纸与离线 HTML 均由投稿智能体生成；未使用受版权保护的字体文件、商标与视觉识别系统、人物肖像、新闻照片、论文插图、商业地图底图或地图瓦片；未使用未经授权的空间数据或个人信息，详见 `report/copyright_statement.md`。对京张铁路"人"字形展线的引用属于对公有领域历史工程事实的文化性引用，不复制任何既有注册标识；若进入实际使用，须另行完成商标检索与字体授权 [standard:MOHURD-ARCH-DESIGN-DEPTH-2016]。
