...
维护者还应人工打开 `proposal.md` 与其 `.zh.md` / `.en.md` 副本，并抽查 HTML、A3/A0 和含文字图件是否保持章节、主张、指标、证据引用与图件位置一致。v2 双语文件缺失、不完整、语言映射错误、无效 HTML/PDF 或 manifest 哈希过期属于阻断问题，应要求修改后重新跑完整门禁；历史 v1 单语方案继续兼容。自动校验不能判断译文是否等义，术语、数字、来源或图件错配仍须人工提出 `request-changes`。译稿中的远程加载、主动网络请求、隐私、涉密、违法或其他安全问题继续按原安全规则处理。
