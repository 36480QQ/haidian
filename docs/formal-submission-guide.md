...
`design_depth_matrix.json` 回答：方案是否达到 formal 成果深度。核心项通常使用 `status: "complete"`；如果某项因组织方尚未公开的控规、边界或其他官方数据而无法完成数值/事实闭环，可以使用 `status: "data_gap"`，但必须增加非空 `limited_by` 数组，并引用 `metrics.json` 中的指标 ID 或 `sources.json` 中的来源 ID。这样可以诚实记录数据缺口，而不会把未知数据写成确定结论。`incomplete` 仍会阻断正式投稿。核心项包括现状诊断、三层范围、总体结构、用地布局、开发强度或待确认控规条件、建筑高度体量风貌、拆改留、交通轨道慢行停车、市政新基建、蓝绿公共空间、三大重点区详细设计、更新项目清单、分期实施、指标复算、风险和缺资料清单。
